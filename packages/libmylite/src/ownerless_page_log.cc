...
constexpr std::uint32_t k_record_flag_history_rseg_delta_payload =
    MYLITE_OWNERLESS_PAGE_LOG_RECORD_HISTORY_RSEG_DELTA;
...
struct IndexPageDeltaBaseSlot {
    ...
    std::uint32_t exact_delta_reuse_observations = 0;
    std::uint32_t exact_delta_reuse_skips = 0;
...
void note_index_delta_base_after_successful_append(
    ...
    bool exact_delta_reuse_observed,
    bool exact_delta_reuse_skipped,
    bool append_stats_enabled
);
...
bool record_is_better_for_history_rseg_recovery(
    const PageRecordHeader &candidate,
    off_t candidate_record_offset,
    const PageRecordHeader &current,
    off_t current_record_offset
...
int append_at_common(
    ...
    if (proof_only_append && page_lsn == 0U) {
        return MYLITE_OWNERLESS_PAGE_LOG_ERROR;
    }
    if (log_offset > static_cast<std::uint64_t>(std::numeric_limits<off_t>::max())) {
        return MYLITE_OWNERLESS_PAGE_LOG_ERROR;
...
int mylite_ownerless_page_log_append_session_append_native_support_proof_pair(
    ...
    mylite_ownerless_page_log_append_session *session,
    std::uint32_t space_id,
    std::uint32_t first_page_no,
    std::uint64_t first_page_lsn,
    std::uint32_t first_page_size,
    std::uint32_t second_page_no,
    std::uint64_t second_page_lsn,
    std::uint32_t second_page_size,
    std::uint64_t commit_lsn,
    ...
    const bool append_stats_enabled = page_log_append_perf_stats_are_enabled();
    PageLogAppendPerfScope total_scope(PAGE_LOG_APPEND_PERF_TOTAL_NS, append_stats_enabled);
    page_log_append_perf_add_if_enabled(append_stats_enabled, PAGE_LOG_APPEND_PERF_CALLS, 2U);
    page_log_append_perf_add_if_enabled(
        append_stats_enabled,
        PAGE_LOG_APPEND_PERF_SESSION_APPEND_CALLS,
        2U
    ...
    if (fd < 0 || session == nullptr || session->active == 0 || commit_lsn == 0U ||
        first_page_lsn == 0U || second_page_lsn == 0U || first_page_size == 0U ||
        second_page_size == 0U ||
        session->next_record_offset >
            static_cast<std::uint64_t>(std::numeric_limits<off_t>::max())) {
        return MYLITE_OWNERLESS_PAGE_LOG_ERROR;
...
int mylite_ownerless_page_log_find_latest_under_read_lock_at_with_flags(
    ...
    return mylite_ownerless_page_log_find_latest_under_read_lock_at_with_flags_and_options(
        fd,
        log_offset,
        space_id,
        page_no,
...
int mylite_ownerless_page_log_replay_stable_with_completion_at(
    ...
    std::uint64_t snapshot_end_offset = 0;
    const int snapshot_result = validate_existing_header(fd, offset);
    int replay_result = snapshot_result == MYLITE_OWNERLESS_PAGE_LOG_OK
                            ? snapshot_locked(fd, offset, &snapshot_end_offset)
                            : snapshot_result;
    if (replay_result == MYLITE_OWNERLESS_PAGE_LOG_OK &&
        snapshot_end_offset > static_cast<std::uint64_t>(std::numeric_limits<off_t>::max())) {
        replay_result = MYLITE_OWNERLESS_PAGE_LOG_ERROR;
    }
    if (replay_result == MYLITE_OWNERLESS_PAGE_LOG_OK) {
        replay_result = replay_in_snapshot(
            fd,
            offset,
            static_cast<off_t>(snapshot_end_offset),
            callback,
...
int append_record_at_locked(
    ...
    bool exact_delta_reuse_skipped = false;
    try {
        ...
        if (!proof_only_record) {
            ...
            if (fast_page_delta_encoded) {
                ...
                const bool skip_exact_reuse_probe =
                    has_page_delta_snapshot &&
                    fast_delta_decision == PageDeltaEncodeDecision::FastLimit &&
                    !rejected_delta_payload.empty() &&
                    page_delta_snapshot.exact_delta_reuse_observations >
                        page_delta_snapshot.exact_delta_reuse_skips;
                if (skip_exact_standalone_probe && !rejected_delta_payload.empty()) {
                    ...
                    rejected_delta_payload.clear();
                } else if (skip_exact_reuse_probe) {
                    exact_delta_payload_size = rejected_delta_payload.size();
                    ...
                    exact_delta_reuse_skipped = true;
                    page_log_append_perf_add_if_enabled(
                        append_stats_enabled,
                        PAGE_LOG_APPEND_PERF_DELTA_EXACT_REUSED_FAST_PAYLOAD_RECORDS,
                        1U
                    ...
                    if (index_delta_payload_beats_standalone(
                        ...
                        exact_delta_reuse_observed = true;
                    } else {
                        exact_delta_decision = PageDeltaEncodeDecision::Standalone;
                        exact_delta_rejected_standalone = true;
                        rejected_delta_payload.clear();
    ...
    if (page_delta_eligible) {
        ...
        note_index_delta_base_after_successful_append(
            ...
            exact_delta_reuse_observed,
            exact_delta_reuse_skipped,
            append_stats_enabled
        );
    }
    page_log_append_perf_add_elapsed_if_enabled(
...
int find_latest_in_snapshot_range(
    ...
    for (off_t record_offset = scan_start_offset; record_offset < snapshot_end_offset;) {
        ...
        if (!include_history_rseg_delta &&
            (record.flags & k_record_flag_history_rseg_delta_payload) != 0U) {
            record_offset = next_record_offset;
            continue;
        }
        ++scanned_visible_page_records;
        const bool better_record =
            include_history_rseg_delta
                ? record_is_better_for_history_rseg_recovery(
                      record,
                      record_offset,
                      best,
                      best_record_offset
                  )
                : record_is_better(record, record_offset, best, best_record_offset);
        if (!better_record) {
            record_offset = next_record_offset;
            continue;
        }
...
bool index_delta_base_snapshot(
    ...
    out_snapshot->exact_delta_reuse_observations = 0U;
    out_snapshot->exact_delta_reuse_skips = 0U;
    ...
    for (std::size_t attempt = 0; attempt < k_index_delta_base_probe_limit; ++attempt) {
        ...
        if (slot.log_device == log_device && slot.log_inode == log_inode &&
            ...
            out_snapshot->exact_delta_reuse_observations = slot.exact_delta_reuse_observations;
            out_snapshot->exact_delta_reuse_skips = slot.exact_delta_reuse_skips;
...
void note_index_delta_base_after_successful_append(
    ...
    if (record_delta_flag != 0U) {
        if (has_preferred_slot_index && preferred_slot_index < k_index_delta_base_slot_count) {
            ...
            if (slot.valid && slot.log_device == log_device && slot.log_inode == log_inode &&
                ...
                if (exact_delta_reuse_observed && slot.exact_delta_reuse_observations <
                                                      std::numeric_limits<std::uint32_t>::max()) {
                    ++slot.exact_delta_reuse_observations;
                }
                if (exact_delta_reuse_skipped &&
                    slot.exact_delta_reuse_skips < std::numeric_limits<std::uint32_t>::max()) {
                    ++slot.exact_delta_reuse_skips;
                }
    ...
    const auto note_standalone_base = [&](IndexPageDeltaBaseSlot &slot, bool matching_slot) {
        ...
        slot.exact_delta_reuse_observations = 0U;
        slot.exact_delta_reuse_skips = 0U;
