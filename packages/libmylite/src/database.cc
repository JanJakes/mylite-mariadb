...
enum OwnerlessDatabasePerfStatIndex : std::size_t {
    ...
    OWNERLESS_DATABASE_PERF_FOREGROUND_RECLAIM_BUDGET_SKIP_CALLS,
    OWNERLESS_DATABASE_PERF_FOREGROUND_RECLAIM_BUDGET_SKIP_ALLOWED,
    OWNERLESS_DATABASE_PERF_FOREGROUND_RECLAIM_BUDGET_SKIP_MARKER_CACHE_HITS,
...
static thread_local bool ownerless_page_read_trust_index = false;
static thread_local bool ownerless_page_read_trust_direct_hits = false;
...
struct OwnerlessInnoDBLockHookContext {
    ...
    std::atomic<std::uint64_t> *page_log_known_end_offset = nullptr;
    int checkpoint_fd = -1;
...
struct RuntimeState {
    ...
    std::atomic<bool> ownerless_runtime_starting{false};
    std::atomic<bool> ownerless_pages_visible_suppressed{false};
...
bool ownerless_page_log_known_payload_bytes(
    RuntimeState &runtime,
    std::uint64_t *out_page_log_bytes
);
void set_ownerless_page_log_known_end_offset(RuntimeState &runtime, std::uint64_t end_offset);
void note_ownerless_page_log_known_end_offset(
    std::atomic<std::uint64_t> *known_end_offset,
...
bool ownerless_runtime_has_live_peer(RuntimeState &runtime);
...
std::uint32_t ownerless_dictionary_recovery_kind_for_statement(
    mylite_db &db,
    const SqlPolicyTokens &tokens
);
...
bool ownerless_create_table_if_not_exists_recovery_statement(
    mylite_db &db,
    const SqlPolicyTokens &tokens
);
...
bool ownerless_rename_table_sources_are_views(mylite_db &db, const SqlPolicyTokens &tokens);
...
bool ownerless_create_index_if_not_exists_recovery_statement(
    mylite_db &db,
    const SqlPolicyTokens &tokens
);
bool ownerless_drop_index_if_exists_recovery_statement(
    mylite_db &db,
    const SqlPolicyTokens &tokens
);
bool ownerless_alter_table_add_index_if_not_exists_recovery_statement(
    mylite_db &db,
    const SqlPolicyTokens &tokens
);
...
bool ownerless_alter_table_column_if_exists_noop_recovery_statement(
    mylite_db &db,
    const SqlPolicyTokens &tokens
);
bool ownerless_alter_table_comment_recovery_statement(mylite_db &db, const SqlPolicyTokens &tokens);
...
bool ownerless_index_metadata_lookup(
    mylite_db &db,
...
bool ownerless_table_metadata_lookup(
    mylite_db &db,
    std::string_view schema_name,
    std::string_view table_name,
    bool *out_exists
...
void retire_ownerless_native_startup_refresh(mylite_db &db);
...
int ownerless_innodb_redo_observe_visible_hook(std::uint64_t *out_visible_lsn, void *ctx);
...
int ownerless_innodb_redo_written_leave_batch_hook(
...
bool ownerless_process_recover_dead_dictionary_owner(
    OwnerlessProcessCleanupContext &cleanup,
    std::uint32_t owner_id,
    std::uint64_t owner_generation
);
...
bool ownerless_replace_statement_allows_visible_fast_path(
    mylite_db &db,
    std::string_view sql,
    const SqlPolicyTokens &tokens
);
...
bool ownerless_replace_target_table(
    const mylite_db &db,
    const SqlPolicyTokens &tokens,
    std::string *out_schema,
    std::string *out_table
...
bool ownerless_table_has_any_triggers(
    mylite_db &db,
    std::string_view schema_name,
    std::string_view table_name
);
...
bool ownerless_table_has_auto_increment(
    mylite_db &db,
    std::string_view schema_name,
    std::string_view table_name
);
...
int mylite_step(mylite_stmt *stmt) {
    ...
    if (!stmt->executed) {
        if (!stmt->db->ownerless_rw_open) {
            const bool native_statement_retires_startup_refresh =
                stmt->ownerless_policy_tokens_valid &&
                (sql_statement_requires_write(stmt->ownerless_policy_tokens) ||
                 sql_statement_uses_locking_read(stmt->ownerless_policy_tokens));
            ...
            if (native_statement_retires_startup_refresh) {
                retire_ownerless_native_startup_refresh(*stmt->db);
            } else {
...
int exec_result_impl(
    ...
    if (!db->ownerless_rw_open) {
        const SqlPolicyTokens native_policy_tokens = collect_sql_policy_tokens(sql);
        const bool native_statement_retires_startup_refresh =
            sql_statement_requires_write(native_policy_tokens) ||
            sql_statement_uses_locking_read(native_policy_tokens);
        if (native_statement_retires_startup_refresh) {
            retire_ownerless_native_startup_refresh(*db);
        } else {
...
int prepare_impl(
    ...
    *out_stmt = nullptr;
    ...
    statement->ownerless_policy_tokens = collect_sql_policy_tokens(*statement->ownerless_sql_text);
    statement->ownerless_policy_tokens_valid = true;
    if (db->ownerless_rw_open && ownerless_prepared_write_defers_native_prepare(
                                     sql_view,
...
bool ownerless_values_statement_row_count(
    std::string_view sql,
    ...
    *out_row_count = 0U;
    ...
    std::size_t offset = 0;
    std::string_view token;
    if (!next_sql_token(sql, offset, token) || !identifier_token_equals(token, statement_keyword)) {
        return false;
    }
...
bool ownerless_insert_values_statement_row_count(std::string_view sql, std::size_t *out_row_count) {
    return ownerless_values_statement_row_count(sql, "INSERT", out_row_count);
}

bool ownerless_replace_values_statement_row_count(
    std::string_view sql,
    std::size_t *out_row_count
) {
    return ownerless_values_statement_row_count(sql, "REPLACE", out_row_count);
}

constexpr std::size_t k_ownerless_append_batch_fast_path_max_insert_values_rows = 65536U;
...
bool ownerless_replace_statement_allows_visible_fast_path(
    ...
    if (!token_equals(identifier_token_at(tokens, 0), "REPLACE")) {
        return false;
    }

    std::size_t row_count = 0U;
    if (!ownerless_replace_values_statement_row_count(sql, &row_count) || row_count == 0U) {
        return false;
    }

    for (std::size_t index = 1U; index < tokens.count; ++index) {
        const std::string_view token = tokens.values[index];
        if (token_in(token, "LOW_PRIORITY", "DELAYED", "PARTITION", "RETURNING") ||
            token_in(token, "SELECT", "WITH", "JOIN") || token_equals(token, "FROM")) {
            return false;
        }
    }
    ...
    std::string schema_name;
    std::string table_name;
    if (!ownerless_replace_target_table(db, tokens, &schema_name, &table_name)) {
        return false;
    }
    if (ownerless_table_has_referential_constraints(db, schema_name, table_name) ||
        ownerless_table_has_any_triggers(db, schema_name, table_name) ||
        ownerless_table_has_auto_increment(db, schema_name, table_name)) {
        return false;
    }
    return true;
}
...
bool ownerless_replace_target_table(
    ...
    if (out_schema == nullptr || out_table == nullptr ||
        !token_equals(identifier_token_at(tokens, 0), "REPLACE")) {
        return false;
    }
    out_schema->clear();
    out_table->clear();
    ...
    std::size_t index = 1U;
    if (index < tokens.count && token_equals(tokens.values[index], "INTO")) {
        ++index;
    }
    if (index >= tokens.count || !ownerless_table_identifier_token(tokens.values[index])) {
        return false;
...
bool ownerless_table_has_any_triggers(
    ...
    constexpr std::string_view k_any_trigger_operation = "*";
    bool has_triggers = true;
    if (ownerless_cached_table_trigger_state(
            db,
            schema_name,
            table_name,
            k_any_trigger_operation,
            &has_triggers
        )) {
        return has_triggers;
    }
...
bool ownerless_insert_target_has_auto_increment(mylite_db &db, const SqlPolicyTokens &tokens) {
    ...
    return ownerless_table_has_auto_increment(db, schema_name, table_name);
}
...
void reclaim_ownerless_page_log_after_native_checkpoint(RuntimeState &runtime) {
    ...
    if (no_live_peers && runtime.ownerless_innodb_lock_hook.redo_state != nullptr &&
        ...
        if (mylite_ownerless_redo_state_read_snapshot(
            ...
            latest_lsn = std::max(latest_lsn, redo_snapshot.latest_lsn);
            visible_lsn = std::max(visible_lsn, redo_snapshot.visible_lsn);
            visible_lsn = std::max(visible_lsn, redo_snapshot.durable_lsn);
        }
    }
    if (visible_lsn == 0U && latest_lsn == 0U) {
        static_cast<void>(clear_ownerless_native_file_op_checkpoint_without_page_log(runtime));
        return;
    }
    if (visible_lsn == 0U && !no_live_peers) {
        return;
    }
    ...
    if (!no_live_peers && runtime_has_local_write && !consumed_current_page_version_wal) {
        return;
    }
    ...
    if (no_live_peers) {
        ...
        if (visible_lsn != 0U && latest_lsn > visible_lsn && !boundary_advance_needed) {
    ...
    std::uint64_t refreshed_page_log_bytes = 0;
    static_cast<void>(ownerless_page_log_payload_bytes(runtime, &refreshed_page_log_bytes));
}
...
void set_ownerless_page_log_known_end_offset(RuntimeState &runtime, std::uint64_t end_offset) {
    runtime.ownerless_page_log_known_end_offset.store(end_offset, std::memory_order_release);
}
...
void note_ownerless_page_log_known_end_offset(
    ...
    if (known_end_offset == nullptr || end_offset == 0U) {
        return;
    }
    std::uint64_t current = known_end_offset->load(std::memory_order_relaxed);
    while (current < end_offset && !known_end_offset->compare_exchange_weak(
                                       current,
                                       end_offset,
...
void note_ownerless_page_log_known_end_offset(
    ...
    note_ownerless_page_log_known_end_offset(hook->page_log_known_end_offset, end_offset);
}
...
bool ownerless_page_log_known_payload_bytes(
    ...
    const std::uint64_t known_end_offset =
        runtime.ownerless_page_log_known_end_offset.load(std::memory_order_acquire);
    if (known_end_offset == 0U) {
        return false;
    }
    if (known_end_offset <= k_concurrency_recovery_header_size) {
        return true;
    }
    *out_page_log_bytes = known_end_offset - k_concurrency_recovery_header_size;
    return true;
}
...
bool ownerless_page_log_payload_bytes(RuntimeState &runtime, std::uint64_t *out_page_log_bytes) {
    ...
    set_ownerless_page_log_known_end_offset(
        runtime,
        wal_stat.st_size > 0 ? static_cast<std::uint64_t>(wal_stat.st_size) : 0U
    );
...
void set_ownerless_native_file_op_checkpoint_cache(RuntimeState &runtime, bool needed) {
    runtime.ownerless_native_file_op_checkpoint_cache_valid = true;
    runtime.ownerless_native_file_op_checkpoint_needed_cached = needed;
}

void invalidate_ownerless_native_file_op_checkpoint_cache(RuntimeState &runtime) {
    runtime.ownerless_native_file_op_checkpoint_cache_valid = false;
    runtime.ownerless_native_file_op_checkpoint_needed_cached = true;
}
...
bool read_ownerless_native_file_op_checkpoint_needed(RuntimeState &runtime, bool *out_needed) {
    ...
    *out_needed = true;
    bool needed = true;
    if (runtime.concurrency_checkpoint_fd < 0 || !read_concurrency_native_file_op_checkpoint_needed(
                                                     runtime.concurrency_checkpoint_fd,
                                                     &needed
                                                 )) {
        invalidate_ownerless_native_file_op_checkpoint_cache(runtime);
        return false;
    }
    set_ownerless_native_file_op_checkpoint_cache(runtime, needed);
    *out_needed = needed;
    return true;
}
...
bool ownerless_single_owner_foreground_reclaim_budget_skips(
    ...
    ownerless_database_perf_add(OWNERLESS_DATABASE_PERF_FOREGROUND_RECLAIM_BUDGET_SKIP_CALLS, 1U);
    ...
    if (runtime.ownerless_native_file_op_checkpoint_cache_valid) {
        ownerless_database_perf_add(
            OWNERLESS_DATABASE_PERF_FOREGROUND_RECLAIM_BUDGET_SKIP_MARKER_CACHE_HITS,
            1U
        );
        native_file_op_checkpoint_needed =
            runtime.ownerless_native_file_op_checkpoint_needed_cached;
    } else {
        ownerless_database_perf_add(
            OWNERLESS_DATABASE_PERF_FOREGROUND_RECLAIM_BUDGET_SKIP_MARKER_FILE_READS,
            1U
...
void maybe_reclaim_ownerless_page_log_after_statement(
    ...
    if (statement_writes) {
        g_runtime.ownerless_runtime_has_local_write.store(true, std::memory_order_relaxed);
    }
    if (ownerless_connection_is_in_explicit_transaction(db)) {
        return;
    }
    std::uint64_t page_log_bytes = 0;
    if (ownerless_page_log_known_payload_bytes(g_runtime, &page_log_bytes) &&
...
bool mark_ownerless_dictionary_state_recoverable_before_finish(mylite_db &db) {
    ...
    return mylite_ownerless_dictionary_state_mark_recoverable(
               dictionary_state,
               k_concurrency_dictionary_state_segment_size,
               owner_id,
               owner_generation,
    ...
    *out_visible_lsn = visible_lsn;
    if (latest_lsn == 0U || latest_lsn <= visible_lsn || runtime.concurrency_checkpoint_fd < 0) {
        if (force_native_checkpoint && latest_lsn != 0U && runtime.concurrency_checkpoint_fd >= 0) {
            ...
            runtime.ownerless_pages_visible_suppressed.store(true, std::memory_order_relaxed);
            static_cast<void>(mylite_ownerless_innodb_make_checkpoint());
            runtime.ownerless_pages_visible_suppressed.store(false, std::memory_order_relaxed);
            std::uint64_t refreshed_latest_lsn = 0;
            std::uint64_t refreshed_visible_lsn = 0;
            if (read_concurrency_checkpoint_lsn(
                    runtime.concurrency_checkpoint_fd,
...
bool ownerless_runtime_has_live_peer(RuntimeState &runtime) {
    ...
    std::uint64_t live_count = 0;
    return read_concurrency_process_live_count(runtime.concurrency_shm_fd, &live_count) ==
               MYLITE_OK &&
           live_count > 1U;
}
...
int open_concurrency_page_log_for_runtime(
    ...
    runtime.ownerless_innodb_lock_hook.page_log_known_end_offset =
...
int allocate_concurrency_process_slot(RuntimeState &runtime) {
    ...
    cleanup_context.checkpoint_fd = runtime.concurrency_checkpoint_fd;
...
int install_ownerless_innodb_lock_hooks(RuntimeState &runtime) {
    ...
    mylite_ownerless_innodb_lock_set_hooks(
        ...
        ownerless_innodb_redo_observe_visible_hook,
    ...
    mylite_ownerless_innodb_lock_set_redo_written_leave_batch_hook(
        ownerless_innodb_redo_written_leave_batch_hook
    );
...
int refresh_ownerless_external_pages_before_statement(
    ...
    bool no_other_active_statements = false;
    bool single_owner_epoch = false;
    ...
    refresh_stage_start = refresh_perf_start();
    {
        ...
        no_other_active_statements = g_runtime.ownerless_active_statement_count <= 1U;
        if (!single_owner_epoch) {
    ...
    bool no_external_page_version_pins = false;
    ...
    // With one active statement in a single-owner epoch, no peer can append
    // page-version WAL between the shared index lookup and page read. Direct
    // hits are safe to trust even during the local writer; absent-entry proof
    // stays narrower because active local write state can still change whether
    // a page first appears in the index.
    const bool single_owner_statement_trusts_page_index =
        page_version_reads_enabled && !explicit_transaction && single_owner_epoch &&
        no_other_active_statements && no_other_active_transactions;
    ownerless_page_read_trust_direct_hits = single_owner_statement_trusts_page_index;
    ownerless_page_read_trust_index =
        single_owner_statement_trusts_page_index && !ownerless_native_write_state_active;
    ...
    const bool current_page_version_read =
        ...
        ((no_live_explicit_transactions && no_other_live_explicit_transactions) ||
         external_page_version_pin_retains_visible_boundary || live_idle_peer_current_read);
...
std::uint32_t ownerless_dictionary_recovery_kind_for_statement(
    ...
    if (ownerless_create_schema_if_not_exists_recovery_statement(tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_CREATE_SCHEMA_IF_NOT_EXISTS;
    }
    ...
    if (ownerless_create_table_if_not_exists_recovery_statement(db, tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_CREATE_TABLE_IF_NOT_EXISTS;
    }
    if (ownerless_plain_create_table_recovery_statement(tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_CREATE_TABLE;
    }
    if (ownerless_create_table_like_recovery_statement(tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_CREATE_TABLE_LIKE;
    }
    if (ownerless_create_table_select_recovery_statement(tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_CREATE_TABLE_SELECT;
    }
    if (ownerless_create_or_replace_table_recovery_statement(tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_CREATE_OR_REPLACE_TABLE;
    }
    if (ownerless_create_or_replace_table_like_recovery_statement(tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_CREATE_OR_REPLACE_TABLE_LIKE;
    }
    ...
    if (ownerless_rename_table_recovery_statement(tokens)) {
        if (ownerless_rename_table_sources_are_views(db, tokens)) {
            return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_RENAME_VIEW;
        }
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_RENAME_TABLE;
    }
    if (ownerless_alter_view_recovery_statement(tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_ALTER_VIEW;
    }
    ...
    if (ownerless_drop_table_if_exists_recovery_statement(tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_DROP_TABLE_IF_EXISTS;
    }
    if (ownerless_drop_table_recovery_statement(tokens)) {
    ...
    if (ownerless_alter_table_column_if_exists_noop_recovery_statement(db, tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_COLUMN_IF_EXISTS_NOOP;
    }
    ...
    if (ownerless_drop_schema_if_exists_recovery_statement(tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_DROP_SCHEMA_IF_EXISTS;
    }
    if (ownerless_alter_table_comment_recovery_statement(db, tokens)) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_ALTER_TABLE_COMMENT;
    }
    ...
    const std::uint32_t compressed_key_block_recovery_kind =
        ownerless_alter_table_compressed_row_format_key_block_recovery_kind(tokens);
    if (compressed_key_block_recovery_kind != MYLITE_OWNERLESS_DICTIONARY_RECOVERY_NONE) {
        return compressed_key_block_recovery_kind;
    }
...
bool ownerless_plain_create_table_recovery_statement(const SqlPolicyTokens &tokens) {
    if (!token_equals(identifier_token_at(tokens, 0), "CREATE")) {
        return false;
    }

    bool table_keyword_seen = false;
    for (std::size_t index = 1U; !identifier_token_at(tokens, index).empty(); ++index) {
        const std::string_view token = identifier_token_at(tokens, index);
        if (token_in(token, "OR", "REPLACE", "TEMPORARY", "LIKE") ||
            token_equals(token, "SELECT")) {
            return false;
        }
        if (token_in(token, "DATABASE", "EVENT", "FUNCTION", "INDEX") ||
            token_in(token, "PROCEDURE", "SCHEMA", "SEQUENCE", "SERVER") ||
            token_in(token, "TRIGGER", "USER", "VIEW")) {
            return false;
        }
        if (token_equals(token, "TABLE")) {
...
bool ownerless_create_table_if_not_exists_recovery_statement(
    ...
    if (tokens.count < 6U || !token_equals(tokens.values[0], "CREATE") ||
        !token_equals(tokens.values[1], "TABLE") || !token_equals(tokens.values[2], "IF") ||
    ...
    for (std::size_t token_index = 1U; token_index < tokens.count; ++token_index) {
        const std::string_view token = identifier_token_at(tokens, token_index);
        if (token_in(token, "OR", "REPLACE", "TEMPORARY", "LIKE") ||
            token_equals(token, "SELECT")) {
            return false;
        }
    ...
    return ownerless_table_metadata_lookup(db, schema_name, table_name, &table_exists) &&
           table_exists;
}
...
bool ownerless_create_table_like_recovery_statement(const SqlPolicyTokens &tokens) {
    ...
    for (std::size_t index = 1U; !identifier_token_at(tokens, index).empty(); ++index) {
        const std::string_view token = identifier_token_at(tokens, index);
        if (token_in(token, "OR", "REPLACE", "TEMPORARY", "SELECT")) {
            return false;
        }
        if (token_in(token, "DATABASE", "EVENT", "FUNCTION", "INDEX") ||
            token_in(token, "PROCEDURE", "SCHEMA", "SEQUENCE", "SERVER") ||
...
bool ownerless_create_or_replace_table_recovery_statement(const SqlPolicyTokens &tokens) {
    if (!token_equals(identifier_token_at(tokens, 0), "CREATE") ||
        !token_equals(identifier_token_at(tokens, 1), "OR") ||
        !token_equals(identifier_token_at(tokens, 2), "REPLACE")) {
        return false;
    }

    bool table_keyword_seen = false;
    for (std::size_t index = 3U; !identifier_token_at(tokens, index).empty(); ++index) {
        const std::string_view token = identifier_token_at(tokens, index);
        if (token_in(token, "TEMPORARY", "LIKE", "SELECT")) {
            return false;
        }
        if (token_equals(token, "TABLE")) {
            table_keyword_seen = true;
        ...
        if (!table_keyword_seen && (token_in(token, "DATABASE", "EVENT", "FUNCTION", "INDEX") ||
                                    token_in(token, "PROCEDURE", "SCHEMA", "SEQUENCE", "SERVER") ||
                                    token_in(token, "TRIGGER", "USER", "VIEW"))) {
            return false;
        }
    }
    return table_keyword_seen;
}
...
bool ownerless_create_or_replace_table_like_recovery_statement(const SqlPolicyTokens &tokens) {
    ...
    for (std::size_t index = 3U; !identifier_token_at(tokens, index).empty(); ++index) {
        const std::string_view token = identifier_token_at(tokens, index);
        if (token_in(token, "TEMPORARY", "SELECT")) {
            return false;
        }
        if (token_equals(token, "TABLE")) {
            table_keyword_seen = true;
    ...
    table_keyword_seen = false;
    for (std::size_t index = 3U; index < tokens.count; ++index) {
        const std::string_view token = tokens.values[index];
        if (token_equals(token, "TABLE")) {
            table_keyword_seen = true;
            continue;
...
bool ownerless_create_or_replace_table_select_recovery_statement(const SqlPolicyTokens &tokens) {
    ...
    for (std::size_t index = 3U; !identifier_token_at(tokens, index).empty(); ++index) {
        const std::string_view token = identifier_token_at(tokens, index);
        if (token_in(token, "TEMPORARY", "LIKE")) {
            return false;
        }
        if (token_equals(token, "TABLE")) {
            table_keyword_seen = true;
...
bool consume_ownerless_table_identifier_parts(
    ...
    if (schema_name == nullptr || table_name == nullptr || index >= tokens.count ||
        !ownerless_table_identifier_token(tokens.values[index])) {
        return false;
    }

    std::string first = ownerless_normalized_identifier(tokens.values[index]);
    ++index;
    if (index + 1U < tokens.count && token_equals(tokens.values[index], ".") &&
        ownerless_table_identifier_token(tokens.values[index + 1U])) {
        *schema_name = std::move(first);
        *table_name = ownerless_normalized_identifier(tokens.values[index + 1U]);
        index += 2U;
    } else {
...
bool ownerless_index_metadata_lookup(
    ...
    if (out_exists == nullptr || schema_name.empty() || table_name.empty() || index_name.empty()) {
        return false;
    }
    *out_exists = false;
    ...
    const std::string escaped_schema = ownerless_escape_metadata_literal(db, schema_name);
    const std::string escaped_table = ownerless_escape_metadata_literal(db, table_name);
    const std::string escaped_index = ownerless_escape_metadata_literal(db, index_name);
    const std::string sql = "SELECT COUNT(*) FROM information_schema.statistics "
                            "WHERE table_schema = '" +
                            escaped_schema + "' AND table_name = '" + escaped_table +
                            "' AND index_name = '" + escaped_index + "'";
...
bool ownerless_column_metadata_lookup(
    ...
    if (out_exists == nullptr || schema_name.empty() || table_name.empty() || column_name.empty()) {
        return false;
    }
    *out_exists = false;
    ...
    const std::string escaped_schema = ownerless_escape_metadata_literal(db, schema_name);
    const std::string escaped_table = ownerless_escape_metadata_literal(db, table_name);
    const std::string escaped_column = ownerless_escape_metadata_literal(db, column_name);
    const std::string sql = "SELECT COUNT(*) FROM information_schema.columns "
                            "WHERE table_schema = '" +
                            escaped_schema + "' AND table_name = '" + escaped_table +
...
bool ownerless_rename_table_sources_are_views(mylite_db &db, const SqlPolicyTokens &tokens) {
    ...
    for (;;) {
        ...
        sources.emplace_back(std::move(schema_name), std::move(table_name));

        if (index >= tokens.count || !token_equals(tokens.values[index], "TO")) {
            return false;
        }
        ++index;
        if (!consume_ownerless_table_identifier(tokens, index)) {
    ...
    const ErrorSnapshot snapshot = capture_error(db);
    bool all_sources_are_views = true;
    for (const auto &source : sources) {
        const std::string escaped_schema = ownerless_escape_metadata_literal(db, source.first);
        const std::string escaped_table = ownerless_escape_metadata_literal(db, source.second);
        const std::string sql = "SELECT COUNT(*) FROM information_schema.views "
                                "WHERE table_schema = '" +
                                escaped_schema + "' AND table_name = '" + escaped_table + "'";
        ...
        if (mysql_query(&db.mysql, sql.c_str()) == 0) {
            ...
            if (result != nullptr) {
                MYSQL_ROW row = mysql_fetch_row(result);
                source_is_view =
                    row != nullptr && row[0] != nullptr && std::strtoull(row[0], nullptr, 10) == 1U;
                query_succeeded = row != nullptr && row[0] != nullptr;
                mysql_free_result(result);
            }
        }
        if (!query_succeeded || !source_is_view) {
            all_sources_are_views = false;
            break;
        }
    }
    restore_error(db, snapshot);
    return all_sources_are_views;
}
...
bool ownerless_create_index_if_not_exists_recovery_statement(
    ...
    std::string table_name;
    if (!consume_ownerless_table_identifier_parts(db, tokens, index, &schema_name, &table_name) ||
        !consume_ownerless_parenthesized_clause(tokens, index) ||
...
bool ownerless_drop_index_if_exists_recovery_statement(
    ...
    if (tokens.count < 7U || !token_equals(tokens.values[0], "DROP") ||
        !token_equals(tokens.values[1], "INDEX") || !token_equals(tokens.values[2], "IF") ||
        !token_equals(tokens.values[3], "EXISTS") ||
        !ownerless_table_identifier_token(tokens.values[4]) ||
        !token_equals(tokens.values[5], "ON")) {
        return false;
    }

    const std::string index_name = ownerless_normalized_identifier(tokens.values[4]);
    std::size_t index = 6U;
    std::string schema_name;
    std::string table_name;
    if (!consume_ownerless_table_identifier_parts(db, tokens, index, &schema_name, &table_name) ||
...
bool ownerless_alter_table_add_index_if_not_exists_recovery_statement(
    ...
    if (tokens.count < 10U || !token_equals(tokens.values[0], "ALTER") ||
        !token_equals(tokens.values[1], "TABLE")) {
        return false;
    }
...
bool ownerless_alter_table_drop_index_if_exists_recovery_statement(
    ...
    std::string table_name;
    if (!consume_ownerless_table_identifier_parts(db, tokens, index, &schema_name, &table_name) ||
        index + 4U >= tokens.count || !token_equals(tokens.values[index], "DROP") ||
...
bool ownerless_alter_table_add_column_if_not_exists_recovery_statement(
    ...
    if (tokens.count < 9U || !token_equals(tokens.values[0], "ALTER") ||
        !token_equals(tokens.values[1], "TABLE")) {
        return false;
    }
    ...
    const std::string column_name = ownerless_normalized_identifier(tokens.values[index + 3U]);
    index += 4U;
...
bool ownerless_alter_table_drop_column_if_exists_recovery_statement(
    ...
    std::string table_name;
    if (!consume_ownerless_table_identifier_parts(db, tokens, index, &schema_name, &table_name) ||
        index >= tokens.count || !token_equals(tokens.values[index], "DROP")) {
        return false;
    }
    ++index;
    if (index < tokens.count && token_equals(tokens.values[index], "COLUMN")) {
    ...
    const std::string column_name = ownerless_normalized_identifier(tokens.values[index + 2U]);
    index += 3U;
    if (!consume_ownerless_remaining_semicolons(tokens, index)) {
        return false;
    }
...
bool ownerless_alter_table_column_if_exists_noop_recovery_statement(
    ...
    if (token_equals(tokens.values[index], "MODIFY")) {
        ++index;
        if (index < tokens.count && token_equals(tokens.values[index], "COLUMN")) {
            ++index;
        }
        ...
        ++index;
        if (index + 4U >= tokens.count || !token_equals(tokens.values[index], "IF") ||
            !token_equals(tokens.values[index + 1U], "EXISTS") ||
            !ownerless_table_identifier_token(tokens.values[index + 2U]) ||
...
bool ownerless_alter_table_comment_recovery_statement(
    mylite_db &db,
    const SqlPolicyTokens &tokens
) {
    if (tokens.count < 6U || !token_equals(tokens.values[0], "ALTER") ||
    ...
    std::string table_name;
    if (!consume_ownerless_table_identifier_parts(db, tokens, index, &schema_name, &table_name) ||
        index >= tokens.count || !token_equals(tokens.values[index], "COMMENT")) {
        return false;
    }
    ++index;
    if (index < tokens.count && token_equals(tokens.values[index], "=")) {
...
bool ownerless_alter_column_set_default_recovery_statement(
    ...
    std::string table_name;
    if (!consume_ownerless_table_identifier_parts(db, tokens, index, &schema_name, &table_name) ||
        index >= tokens.count || !token_equals(tokens.values[index], "ALTER")) {
        return false;
    }
    ++index;
    if (index < tokens.count && token_equals(tokens.values[index], "COLUMN")) {
...
bool ownerless_alter_table_charset_convert_recovery_statement(const SqlPolicyTokens &tokens) {
    ...
    std::size_t index = 2U;
    if (!consume_ownerless_table_identifier(tokens, index) || index + 4U >= tokens.count ||
        !token_equals(tokens.values[index], "CONVERT") ||
        !token_equals(tokens.values[index + 1U], "TO") ||
        !token_equals(tokens.values[index + 2U], "CHARACTER") ||
        !token_equals(tokens.values[index + 3U], "SET") ||
        !ownerless_table_identifier_token(tokens.values[index + 4U])) {
        return false;
    }
    index += 5U;
    if (index < tokens.count && token_equals(tokens.values[index], "COLLATE")) {
        ++index;
        if (index >= tokens.count || !ownerless_table_identifier_token(tokens.values[index])) {
            return false;
        }
...
std::uint32_t ownerless_alter_table_compressed_row_format_key_block_recovery_kind(
    ...
    if (!ownerless_table_identifier_token(tokens.values[2]) ||
        ...
        !ownerless_table_identifier_token(tokens.values[4]) ||
        !token_equals(tokens.values[5], "ROW_FORMAT") || !token_equals(tokens.values[6], "=") ||
        !token_equals(tokens.values[7], "COMPRESSED") ||
    ...
    if (token_equals(tokens.values[10], "2")) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_ALTER_TABLE_COMPRESSED_ROW_FORMAT_KEY_BLOCK_2;
    }
    if (token_equals(tokens.values[10], "4")) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_ALTER_TABLE_COMPRESSED_ROW_FORMAT_KEY_BLOCK_4;
    }
    if (token_equals(tokens.values[10], "8")) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_ALTER_TABLE_COMPRESSED_ROW_FORMAT_KEY_BLOCK_8;
    }
    if (token_equals(tokens.values[10], "16")) {
        return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_ALTER_TABLE_COMPRESSED_ROW_FORMAT_KEY_BLOCK_16;
    }
    return MYLITE_OWNERLESS_DICTIONARY_RECOVERY_NONE;
}
...
bool ownerless_dictionary_recovery_kind_is_metadata_only(std::uint32_t recovery_kind) {
    return recovery_kind == MYLITE_OWNERLESS_DICTIONARY_RECOVERY_CREATE_VIEW ||
           ...
           recovery_kind == MYLITE_OWNERLESS_DICTIONARY_RECOVERY_COLUMN_IDEMPOTENT_ADD ||
           recovery_kind == MYLITE_OWNERLESS_DICTIONARY_RECOVERY_COLUMN_IDEMPOTENT_DROP ||
           recovery_kind == MYLITE_OWNERLESS_DICTIONARY_RECOVERY_COLUMN_IF_EXISTS_NOOP;
}
    ...
    if (ownerless_connection_is_in_explicit_transaction(db)) {
        ...
        if (statement_writes) {
            ...
            if (db.ownerless_rw_open && !db.readonly_open &&
                ...
                g_runtime.ownerless_runtime_has_peer_explicit_transaction_write.store(
                    true,
                    std::memory_order_relaxed
                );
            }
...
void retire_ownerless_native_startup_refresh(mylite_db &db) {
    if (db.ownerless_native_startup_refresh_lsn == 0U &&
        db.ownerless_pending_post_open_clean_page_refresh_lsn == 0U) {
        return;
    }

    db.ownerless_native_startup_refresh_lsn = 0;
    db.ownerless_pending_post_open_clean_page_refresh_lsn = 0;
    db.ownerless_pending_post_open_clean_page_refresh_visible_boundary = false;
    db.ownerless_pending_post_open_clean_page_refresh_current_boundary = false;
    db.ownerless_local_native_read_lsn = 0;
...
int ownerless_innodb_redo_observe_visible_hook(std::uint64_t *out_visible_lsn, void *ctx) {
    OwnerlessDatabasePerfScope perf_scope(OWNERLESS_DATABASE_PERF_REDO_OBSERVE_NS);
    ownerless_database_perf_add(OWNERLESS_DATABASE_PERF_REDO_OBSERVE_CALLS, 1U);
    if (ctx == nullptr || out_visible_lsn == nullptr) {
        return MYLITE_OWNERLESS_INNODB_LOCK_ERROR;
    }
    ...
    *out_visible_lsn = snapshot.visible_lsn;
    return MYLITE_OWNERLESS_INNODB_LOCK_OK;
}
...
int ownerless_innodb_redo_written_leave_batch_hook(
    ...
    ownerless_database_perf_add(OWNERLESS_DATABASE_PERF_REDO_WRITTEN_CALLS, 1U);
    ownerless_database_perf_add(OWNERLESS_DATABASE_PERF_REDO_LEAVE_CALLS, 1U);
    if (ctx == nullptr || ranges == nullptr || range_count == 0U ||
        range_count > MYLITE_OWNERLESS_INNODB_REDO_BATCH_MAX_RANGES) {
        return MYLITE_OWNERLESS_INNODB_LOCK_ERROR;
    }
    ...
    for (std::size_t index = 0; index < range_count; ++index) {
        ...
        redo_ranges[index].start_lsn = ranges[index].start_lsn;
        redo_ranges[index].end_lsn = ranges[index].end_lsn;
    }

    std::uint64_t advanced_latest_lsn = 0U;
    std::size_t completed_count = 0U;
    const int result = mylite_ownerless_redo_state_complete_write_and_leave_batch(
        hook->redo_state,
        hook->redo_state_size,
        hook->owner_id,
        hook->owner_generation,
...
void ownerless_innodb_pages_visible_hook(std::uint64_t visible_lsn, void *ctx) {
    ...
    if (g_runtime.ownerless_runtime_starting.load(std::memory_order_relaxed) ||
        g_runtime.ownerless_pages_visible_suppressed.load(std::memory_order_relaxed)) {
        return;
    }
    ...
    const bool other_explicit =
        !force_visible && ownerless_process_registry_has_other_live_explicit_transactions(
...
int append_ownerless_page_version(
    ...
    if (ownerless_page_log_append_batch.session.active != 0 &&
        ...
        if (external_snapshot_lineage) {
            ...
            if (append_result == MYLITE_OWNERLESS_PAGE_LOG_OK) {
                note_ownerless_page_log_known_end_offset(
                    hook,
                    ownerless_page_log_append_batch.session.next_record_offset
                );
            }
            return append_result;
        }
        const int append_result =
            mylite_ownerless_page_log_append_session_append_with_checksum_and_options(
                hook->page_log_fd,
                &ownerless_page_log_append_batch.session,
                space_id,
                page_no,
    ...
    if (external_snapshot_lineage) {
        append_result =
            mylite_ownerless_page_log_append_external_snapshot_lineage_initialized_at_with_checksum_and_options_next(
                hook->page_log_fd,
                hook->page_log_offset,
                space_id,
                page_no,
        ...
        append_result =
            mylite_ownerless_page_log_append_initialized_at_with_checksum_and_options_next(
                hook->page_log_fd,
                hook->page_log_offset,
                space_id,
                page_no,
    ...
    if (append_result == MYLITE_OWNERLESS_PAGE_LOG_OK) {
        note_ownerless_page_log_known_end_offset(hook, next_record_offset);
    }
    return append_result;
}
...
enum class OwnerlessBoundaryPageVersionState {
    Missing,
    Exists,
    Unknown,
};

OwnerlessBoundaryPageVersionState ownerless_page_log_boundary_page_version_state(
    OwnerlessInnoDBLockHookContext *hook,
    std::uint32_t space_id,
    std::uint32_t page_no,
    std::uint64_t max_commit_lsn,
    ...
    if (hook == nullptr || hook->page_index == nullptr || hook->page_index_size == 0U ||
        ...
        hook->owner_generation == 0U || max_commit_lsn == 0U || page_capacity == 0U) {
    ...
    if (index_result != MYLITE_OWNERLESS_PAGE_INDEX_SCAN_REQUIRED) {
        return OwnerlessBoundaryPageVersionState::Unknown;
    }
    ...
    std::uint64_t scan_start_offset = 0;
    ...
    std::unique_ptr<unsigned char[]> page(new (std::nothrow) unsigned char[page_capacity]);
    if (page == nullptr) {
        mylite_ownerless_page_log_end_read(hook->page_log_fd);
        return OwnerlessBoundaryPageVersionState::Unknown;
    }
    ...
    std::uint32_t page_size = 0;
    std::uint64_t found_page_lsn = 0;
    std::uint64_t found_commit_lsn = 0;
    ...
    if (scan_result == MYLITE_OWNERLESS_PAGE_LOG_OK) {
        return OwnerlessBoundaryPageVersionState::Exists;
    }
...
bool publish_ownerless_snapshot_boundary_if_needed(
    ...
    const OwnerlessBoundaryPageVersionState existing_state =
        ownerless_page_log_boundary_page_version_state(
            hook,
            space_id,
            page_no,
            oldest_pin_lsn,
    ...
    if (existing_state == OwnerlessBoundaryPageVersionState::Exists ||
        existing_state == OwnerlessBoundaryPageVersionState::Unknown) {
        return external_snapshot_pin_active;
    }
    ...
    const int append_result =
        mylite_ownerless_page_log_append_snapshot_boundary_initialized_at_next(
            hook->page_log_fd,
            hook->page_log_offset,
            space_id,
            page_no,
...
int ownerless_innodb_page_publish_hook(
    ...
    if (external_snapshot_lineage_active) {
        const bool previous_coalescing =
            ownerless_statement_allows_deferred_latest_checkpoint_coalescing;
        ownerless_statement_allows_deferred_latest_checkpoint_coalescing = false;
        ownerless_persist_redo_checkpoint(hook, visible_lsn, 0U, false);
        ownerless_statement_allows_deferred_latest_checkpoint_coalescing = previous_coalescing;
    }
    pause_for_ownerless_test_fault("page-publish-after-append");
    if (proof_only_publish) {
        ownerless_database_perf_add(
            OWNERLESS_DATABASE_PERF_PAGE_PUBLISH_INDEX_SKIPPED_NATIVE_SUPPORT,
            1U
        );
...
int ownerless_innodb_page_read_locked(
    ...
    if (index_result == MYLITE_OWNERLESS_PAGE_INDEX_OK) {
        ...
        if (read_result == MYLITE_OWNERLESS_PAGE_LOG_OK) {
            if (*out_page_lsn == index_page_lsn && *out_commit_lsn == index_commit_lsn) {
                if (ownerless_page_read_trust_direct_hits) {
                    ownerless_database_perf_add(OWNERLESS_DATABASE_PERF_PAGE_READ_INDEX_HITS, 1U);
                    const std::uint32_t visible_record_flags =
                        ownerless_innodb_page_version_flags(page_log_flags);
                    if (out_record_flags != nullptr) {
                        *out_record_flags = visible_record_flags;
                    }
                    return MYLITE_OWNERLESS_INNODB_LOCK_OK;
                }
                ...
                if (page_log_snapshot_end_offset > tail_scan_offset) {
                    std::uint64_t tail_scan_start_offset = tail_scan_offset;
                    const bool tail_negative_cache_hit =
                        ownerless_page_log_negative_cache_absence_lookup(
                            hook,
                            space_id,
                            page_no,
                            max_commit_lsn,
                            ...
                            page_log_generation,
                            page_log_snapshot_end_offset,
                            &tail_scan_start_offset
                        );
                    if (tail_negative_cache_hit) {
                        ownerless_database_perf_add(
                            OWNERLESS_DATABASE_PERF_PAGE_READ_WAL_SCAN_NEGATIVE_CACHE_HITS,
                            1U
                        );
                    ...
                    std::uint64_t tail_record_offset = 0;
                    ...
                    if (tail_result == MYLITE_OWNERLESS_PAGE_LOG_OK) {
                        ...
                        if (tail_record_is_newer) {
                            ...
                            *out_commit_lsn = tail_commit_lsn;
                            ...
                            if (tail_record_offset != 0U) {
                                static_cast<void>(mylite_ownerless_page_index_publish(
                                    hook->page_index,
                                    hook->page_index_size,
                                    hook->owner_id,
                                    hook->owner_generation,
        ...
        if (ownerless_page_read_trust_index) {
            return MYLITE_OWNERLESS_INNODB_LOCK_UNAVAILABLE;
        }
    ...
    if (result == MYLITE_OWNERLESS_PAGE_LOG_NOT_FOUND) {
        ...
        if (page_log_snapshot_available) {
            ownerless_page_log_negative_cache_store(
                hook,
                space_id,
                page_no,
...
int ownerless_process_cleanup_dead_owner_state(
    ...
    if (ownerless_process_owner_state_requires_recovery(*cleanup, owner_id, slot_generation)) {
...
bool ownerless_process_recover_dead_dictionary_owner(
    ...
    if (cleanup.checkpoint_fd < 0 || cleanup.dictionary_state == nullptr ||
        cleanup.dictionary_state_size == 0U) {
        return false;
    }
    ...
    if (native_file_op_checkpoint_needed) {
        for (const std::uint32_t recovery_kind : file_op_recovery_kinds) {
            std::uint64_t generation = 0;
            if (mylite_ownerless_dictionary_state_recover_dead_owner(
                    cleanup.dictionary_state,
                    cleanup.dictionary_state_size,
                    owner_id,
                    owner_generation,
    ...
    constexpr std::array<std::uint32_t, 21> metadata_only_recovery_kinds = {
        ...
        MYLITE_OWNERLESS_DICTIONARY_RECOVERY_RENAME_VIEW,
        ...
        MYLITE_OWNERLESS_DICTIONARY_RECOVERY_DROP_SCHEMA_IF_EXISTS,
        MYLITE_OWNERLESS_DICTIONARY_RECOVERY_INDEX_IDEMPOTENT_CREATE,
        MYLITE_OWNERLESS_DICTIONARY_RECOVERY_INDEX_IDEMPOTENT_DROP,
        MYLITE_OWNERLESS_DICTIONARY_RECOVERY_ALTER_TABLE_COMMENT,
        MYLITE_OWNERLESS_DICTIONARY_RECOVERY_ALTER_COLUMN_SET_DEFAULT,
...
int start_runtime(mylite_db &db, unsigned flags, const mylite_open_config *config) {
    ...
    try {
        ...
        if (ownerless_concurrency_runtime_needed) {
            ...
            stage_start_ns = embedded_open_perf_start_ns();
            const int process_slot_result = allocate_concurrency_process_slot(g_runtime);
            embedded_open_perf_add_elapsed(
                EMBEDDED_OPEN_PERF_START_MAP_SHARED_MEMORY_NS,
                stage_start_ns
            );
            if (process_slot_result != MYLITE_OK) {
                unmap_concurrency_shared_memory_for_runtime(g_runtime);
                concurrency_mapped = false;
                clear_runtime_state(g_runtime);
                cleanup_runtime_layout(layout);
                ...
                set_error(
                    db,
                    process_slot_result,
                    "database ownerless process registry is invalid"
                );
...
#  endif
        ...
        stage_start_ns = embedded_open_perf_start_ns();
        if (innodb_ownerless_hooks_needed) {
            g_runtime.ownerless_runtime_starting.store(true, std::memory_order_relaxed);
        }
        ...
        g_runtime.ownerless_runtime_starting.store(false, std::memory_order_relaxed);
        embedded_open_perf_add_elapsed(
            EMBEDDED_OPEN_PERF_START_MYSQL_SERVER_INIT_NS,
            stage_start_ns
        );
...
void clear_runtime_state(RuntimeState &runtime) {
    ...
    runtime.ownerless_runtime_has_peer_explicit_transaction_write.store(
        false,
        std::memory_order_relaxed
    );
    runtime.ownerless_native_file_op_checkpoint_cache_valid = false;
    ...
    runtime.ownerless_runtime_starting.store(false, std::memory_order_relaxed);
    runtime.ownerless_pages_visible_suppressed.store(false, std::memory_order_relaxed);
