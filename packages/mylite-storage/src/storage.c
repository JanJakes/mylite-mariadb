...
#define MYLITE_STORAGE_APPEND_PAGE_BUFFER_LIMIT_PAGES 256U
...
static mylite_storage_read_checkpoint_cache *read_checkpoint_cache_for(const char *filename);
...
static int copy_buffered_append_page(
    FILE *file,
    unsigned long long page_id,
    unsigned page_size,
    unsigned char *out_page
...
static mylite_storage_result read_cached_checkpoint_snapshot(mylite_storage_statement *statement) {
    ...
    mylite_storage_read_checkpoint_cache *cache = read_checkpoint_cache_for(statement->filename);
    if (cache != NULL && memcmp(header_page, cache->header_page, sizeof(cache->header_page)) == 0) {
        ...
        result = read_page_at(
            statement->file,
            cache->header.catalog_root_page,
            cache->header.page_size,
            catalog_page
        );
        if (result != MYLITE_STORAGE_OK) {
            return result;
        }
        if (memcmp(catalog_page, cache->catalog_page, cache->header.page_size) == 0) {
...
static mylite_storage_read_checkpoint_cache *read_checkpoint_cache_for(const char *filename) {
    if (!active_read_checkpoint_cache.has_snapshot ||
        ...
        strcmp(active_read_checkpoint_cache.filename, filename) != 0) {
        return NULL;
    }
...
static mylite_storage_result flush_statement_append_page_buffer(
    ...
    off_t offset = 0;
    mylite_storage_result result = page_offset_for_io(
        buffer->first_page_id,
        MYLITE_STORAGE_FORMAT_PAGE_SIZE,
        &offset
    );
    if (result != MYLITE_STORAGE_OK) {
...
static mylite_storage_result read_page_at(
    ...
    if (copy_buffered_append_page(file, page_id, page_size, out_page)) {
        return MYLITE_STORAGE_OK;
    }
...
static mylite_storage_result write_inline_update_pages(
    ...
    *out_used_fast_path = 0;
    ...
    if (header->page_count > ULLONG_MAX - 2ULL ||
        (unsigned long long)index_entry_count > ULLONG_MAX - header->page_count - 2ULL) {
        return MYLITE_STORAGE_FULL;
    }
    if (index_entry_count > SIZE_MAX - 2U) {
        return MYLITE_STORAGE_FULL;
    }

    const size_t page_count = index_entry_count + 2U;
    ...
    for (size_t i = 0U; i < index_entry_count; ++i) {
        encode_index_entry_page(
            pages + ((i + 2U) * MYLITE_STORAGE_FORMAT_PAGE_SIZE),
            state_page_id + 1ULL + (unsigned long long)i,
            table_id,
            row_page_id,
            index_entries + i
        );
    ...
    *out_next_page_id = state_page_id + 1ULL + (unsigned long long)index_entry_count;
...
static mylite_storage_result copy_exact_index_cache_entries(
    ...
    if (source->count != 0U) {
        if (source->key_size == 0U || source->count > SIZE_MAX / source->key_size ||
            source->count > SIZE_MAX / sizeof(*source->row_ids)) {
            return MYLITE_STORAGE_FULL;
        }
        keys = (unsigned char *)malloc(source->count * source->key_size);
        if (keys == NULL) {
            return MYLITE_STORAGE_NOMEM;
        }
        row_ids = (unsigned long long *)malloc(source->count * sizeof(*row_ids));
        if (row_ids == NULL) {
            free(keys);
            return MYLITE_STORAGE_NOMEM;
        }
        memcpy(keys, source->keys, source->count * source->key_size);
        memcpy(row_ids, source->row_ids, source->count * sizeof(*row_ids));
    }
...
static mylite_storage_result append_exact_index_cache_entry(
    ...
    if (cache->count == cache->capacity) {
        ...
        unsigned char *keys =
            (unsigned char *)realloc(cache->keys, next_capacity * cache->key_size);
        if (keys == NULL) {
            return MYLITE_STORAGE_NOMEM;
        }
        cache->keys = keys;
    ...
    memcpy(cache->keys + (cache->count * cache->key_size), key, cache->key_size);
    cache->row_ids[cache->count] = row_id;
    ++cache->count;
...
static mylite_storage_result ensure_exact_index_cache_buckets(
    ...
    const size_t bucket_count = exact_index_cache_bucket_count(cache->count);
    if (bucket_count == 0U || bucket_count > SIZE_MAX / sizeof(*cache->bucket_heads) ||
        cache->count > SIZE_MAX / sizeof(*cache->bucket_next)) {
        return MYLITE_STORAGE_FULL;
    }
    ...
    size_t *bucket_next = (size_t *)malloc(cache->count * sizeof(*bucket_next));
    if (bucket_next == NULL) {
        free(bucket_heads);
        return MYLITE_STORAGE_NOMEM;
    }
