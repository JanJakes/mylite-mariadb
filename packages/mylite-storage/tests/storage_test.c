...
static void test_branch_stable_update_refold(void);
...
int main(void) {
    ...
    test_branch_stable_update_refold();
...
static void test_branch_arbitrary_child_removal(void) {
    ...
    assert(access(journal_filename, F_OK) == 0);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_same_statement_split_pages + 2ULL);
...
static void test_branch_stable_update_refold(void) {
    ...
    unsigned char rows[entry_count][4] = {{0}};
    unsigned char keys[entry_count][key_size] = {{0}};
    unsigned char updated_row[4] = {0};
    unsigned char updated_key[key_size] = {0};
    unsigned long long row_ids[entry_count] = {0};
    char *root = make_temp_root();
    char *filename = path_join(root, "branch-stable-update-refold.mylite");
    char *journal_filename = journal_path(filename);
    char *transaction_journal_filename = transaction_journal_path(filename);
    mylite_storage_table_definition table_definition = {
        .size = sizeof(table_definition),
    ...
    put_test_u32_le(updated_row, 0U, 44U);
    updated_key[0] = 0U;
    mylite_storage_index_entry updated_index_entry = {
        .size = sizeof(updated_index_entry),
        .index_number = 0U,
        .key = updated_key,
        .key_size = key_size,
    };
    ...
    const unsigned long long first_leaf_page = root_page + 1ULL;
    const unsigned long long second_leaf_page = root_page + 2ULL;
    const unsigned long long before_update_pages = header.page_count;
    const unsigned long long before_update_free_list_root = header.free_list_root_page;
    assert(header.page_count == before_rebuild_pages + 3ULL);
    assert_index_root(filename, "app", "posts", 0U, root_page, entry_count);
    read_test_page(filename, root_page, page);
    assert(get_test_u32_le(page, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET) == 2U);
    read_test_page(filename, second_leaf_page, page);
    assert(get_test_u32_le(page, MYLITE_STORAGE_FORMAT_INDEX_LEAF_ENTRY_COUNT_OFFSET) == 1U);

    unsigned long long rolled_back_updated_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_update_row_with_index_entries(
            filename,
            ...
            &updated_index_entry,
            1U,
            &rolled_back_updated_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(rolled_back_updated_row_id != 0ULL);
    assert(access(journal_filename, F_OK) == 0);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_update_pages + 2ULL);
    assert(header.free_list_root_page == before_update_free_list_root);
    assert_index_root(filename, "app", "posts", 0U, root_page, entry_count);
    read_test_page(filename, root_page, page);
    assert(get_test_u32_le(page, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET) == 2U);
    const unsigned char *first_branch_cell =
    ...
    read_test_page(filename, second_leaf_page, page);
    assert(get_test_u32_le(page, MYLITE_STORAGE_FORMAT_INDEX_LEAF_ENTRY_COUNT_OFFSET) == 1U);
    assert_index_entry_lookup(filename, 0U, keys[3], key_size, MYLITE_STORAGE_NOTFOUND, 0ULL);
    assert_index_entry_lookup(
        filename,
        0U,
        updated_key,
    ...
    const unsigned char *expected_prefix_keys[] = {updated_key};
    const unsigned long long expected_prefix_row_ids[] = {rolled_back_updated_row_id};
    assert_prefix_index_entries(
        filename,
        0U,
        updated_key,
    ...
    assert_index_prefix_exists_for_index(filename, 0U, updated_key, 1U, 0ULL, 1);
    mylite_storage_index_entryset entries = {
        .size = sizeof(entries),
    };
    assert(
    ...
    assert_index_entry(&entries, 0U, rolled_back_updated_row_id, updated_key, key_size);
    assert_index_entry(&entries, 1U, row_ids[0], keys[0], key_size);
    assert_index_entry(&entries, 2U, row_ids[1], keys[1], key_size);
    assert_index_entry(&entries, 3U, row_ids[2], keys[2], key_size);
    mylite_storage_free_index_entryset(&entries);
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    statement = NULL;
    assert_file_missing(journal_filename);
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_update_pages);
    assert(header.free_list_root_page == before_update_free_list_root);
    assert_index_root(filename, "app", "posts", 0U, root_page, entry_count);
    assert_index_entry_lookup(filename, 0U, keys[3], key_size, MYLITE_STORAGE_OK, row_ids[3]);
    assert_index_entry_lookup(filename, 0U, updated_key, key_size, MYLITE_STORAGE_NOTFOUND, 0ULL);

    const pid_t statement_pid = fork();
    assert(statement_pid >= 0);
    if (statement_pid == 0) {
        mylite_storage_statement *child_statement = NULL;
        unsigned long long child_updated_row_id = 0ULL;
        if (mylite_storage_begin_statement(filename, &child_statement) != MYLITE_STORAGE_OK) {
            _exit(2);
        }
        if (mylite_storage_update_row_with_index_entries(
                ...
                &updated_index_entry,
                1U,
                &child_updated_row_id
            ) != MYLITE_STORAGE_OK) {
            _exit(3);
        }
        _exit(child_updated_row_id == 0ULL ? 4 : 0);
    }
    status = 0;
    assert(waitpid(statement_pid, &status, 0) == statement_pid);
    assert(WIFEXITED(status));
    ...
    assert(entries.entry_count == entry_count);
    assert_index_entry(&entries, 0U, updated_row_id, updated_key, key_size);
    assert_index_entry(&entries, 1U, row_ids[0], keys[0], key_size);
    assert_index_entry(&entries, 2U, row_ids[1], keys[1], key_size);
    assert_index_entry(&entries, 3U, row_ids[2], keys[2], key_size);
    mylite_storage_free_index_entryset(&entries);
...
static void test_branch_exact_entryset_spans_leaves(void) {
    ...
    static const unsigned char definition[] = {0x01U, 'f', 'r', 'm', 0x00U};
    char *root = make_temp_root();
    char *filename = path_join(root, "branch-exact-entryset-spans-leaves.mylite");
    mylite_storage_table_definition table_definition = {
        .size = sizeof(table_definition),
        .schema_name = "app",
        .table_name = "posts",
    ...
    unsigned long long *row_ids = (unsigned long long *)calloc(entry_count, sizeof(*row_ids));
    assert(row_ids != NULL);
    ...
    for (size_t i = 0U; i < entry_count; ++i) {
        ...
        put_test_u32_be(key, 0U, i < entry_capacity ? 0U : 1U);
        mylite_storage_index_entry index_entry = {
            .size = sizeof(index_entry),
            .index_number = 0U,
            .key = key,
    ...
    const unsigned long long branch_root_page = root_metadata.root_page;
    ...
    unsigned char branch_page[MYLITE_STORAGE_FORMAT_PAGE_SIZE] = {0};
    read_test_page(filename, branch_root_page, branch_page);
    const unsigned long long first_leaf_page = get_test_u64_le(
        branch_page + MYLITE_STORAGE_FORMAT_INDEX_BRANCH_PAYLOAD_OFFSET,
        MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CELL_CHILD_PAGE_ID_OFFSET
    );
    assert_index_root_page_type(
    ...
    unsigned char duplicate_key[key_size] = {0};
    put_test_u32_be(duplicate_key, 0U, 1U);
    mylite_storage_clear_thread_caches();
...
static void test_branch_prefix_entryset_spans_leaves(void) {
    ...
    static const unsigned char definition[] = {0x01U, 'f', 'r', 'm', 0x00U};
    char *root = make_temp_root();
    char *filename = path_join(root, "branch-prefix-entryset-spans-leaves.mylite");
    mylite_storage_table_definition table_definition = {
        .size = sizeof(table_definition),
        .schema_name = "app",
        .table_name = "posts",
    ...
    const size_t entry_count = entry_capacity * 3U;
    unsigned long long *row_ids = (unsigned long long *)calloc(entry_count, sizeof(*row_ids));
    unsigned char *keys = (unsigned char *)calloc(entry_count, key_size);
    assert(row_ids != NULL);
    assert(keys != NULL);
    ...
    for (size_t i = 0U; i < entry_count; ++i) {
        ...
        const unsigned suffix = (unsigned)(i < entry_capacity ? i : i - entry_capacity);
        put_test_u32_le(row, 0U, (unsigned)i + 1U);
        key[0] = i < entry_capacity ? 0x10U : 0x20U;
    ...
    static const unsigned char prefix[] = {0x20U};
    mylite_storage_clear_thread_caches();
    assert(first_leaf_page <= (unsigned long long)LONG_MAX / MYLITE_STORAGE_FORMAT_PAGE_SIZE);
    flip_file_byte(
        filename,
    ...
    assert(entries.entry_count == entry_count - entry_capacity);
    assert(entries.key_bytes == (entry_count - entry_capacity) * key_size);
    for (size_t i = entry_capacity; i < entry_count; ++i) {
        assert_index_entry(
            &entries,
            i - entry_capacity,
            row_ids[i],
...
static void test_deep_branch_navigation(void) {
    ...
    enum {
        key_size = 1024U,
        max_branch_levels = 16U,
    };

    static const unsigned char definition[] = {0x01U, 'f', 'r', 'm', 0x00U};
    char *root = make_temp_root();
    char *filename = path_join(root, "deep-branch-navigation.mylite");
    mylite_storage_table_definition table_definition = {
        .size = sizeof(table_definition),
        .schema_name = "app",
        .table_name = "posts",
    ...
    const size_t former_branch_leaf_bound =
        (MYLITE_STORAGE_FORMAT_PAGE_SIZE - MYLITE_STORAGE_FORMAT_INDEX_BRANCH_PAYLOAD_OFFSET) /
    ...
    unsigned tree_level = 0U;
    while (leaf_count <= former_branch_leaf_bound) {
        assert(tree_level + 1U < max_branch_levels);
        assert(leaf_count <= SIZE_MAX / branch_capacity);
        leaf_count *= branch_capacity;
        ++tree_level;
    }
    assert(tree_level > 1U);
    assert(leaf_count > former_branch_leaf_bound);

    size_t level_counts[max_branch_levels] = {0};
    level_counts[0] = leaf_count;
    for (unsigned level = 1U; level <= tree_level; ++level) {
        assert(level_counts[level - 1U] % branch_capacity == 0U);
        level_counts[level] = level_counts[level - 1U] / branch_capacity;
    }
    assert(level_counts[tree_level] == 1U);

    unsigned long long *level_page_ids[max_branch_levels] = {0};
    unsigned long long *level_max_row_ids[max_branch_levels] = {0};
    unsigned long long *level_entry_counts[max_branch_levels] = {0};
    unsigned char *level_max_keys[max_branch_levels] = {0};
    for (unsigned level = 0U; level <= tree_level; ++level) {
        level_page_ids[level] =
            (unsigned long long *)calloc(level_counts[level], sizeof(*level_page_ids[level]));
        level_max_row_ids[level] =
            (unsigned long long *)calloc(level_counts[level], sizeof(*level_max_row_ids[level]));
        level_entry_counts[level] =
            (unsigned long long *)calloc(level_counts[level], sizeof(*level_entry_counts[level]));
        level_max_keys[level] = (unsigned char *)calloc(level_counts[level], key_size);
        assert(level_page_ids[level] != NULL);
        assert(level_max_row_ids[level] != NULL);
        assert(level_entry_counts[level] != NULL);
        assert(level_max_keys[level] != NULL);
    }
    ...
    unsigned long long next_page_id = root_page + 1ULL;
    level_page_ids[tree_level][0] = root_page;
    for (unsigned level = tree_level; level > 0U; --level) {
        const unsigned child_level = level - 1U;
        for (size_t i = 0U; i < level_counts[child_level]; ++i) {
            level_page_ids[child_level][i] = next_page_id++;
        }
    }
    const unsigned long long tree_end_page_id = next_page_id;
    assert(leaf_count <= ULLONG_MAX / leaf_capacity);
    const unsigned long long total_entry_count =
        (unsigned long long)leaf_count * (unsigned long long)leaf_capacity;
    const unsigned long long first_synthetic_row_id = MYLITE_STORAGE_FORMAT_EMPTY_PAGE_COUNT;
    const unsigned long long last_synthetic_row_id =
        first_synthetic_row_id + total_entry_count - 1ULL;

    unsigned long long *leaf_row_ids =
        (unsigned long long *)calloc(leaf_capacity, sizeof(*leaf_row_ids));
    unsigned char *leaf_keys = (unsigned char *)calloc(leaf_capacity, key_size);
    assert(leaf_row_ids != NULL);
    assert(leaf_keys != NULL);

    unsigned long long next_row_id = first_synthetic_row_id;
    for (size_t leaf = 0U; leaf < leaf_count; ++leaf) {
        memset(leaf_keys, 0, leaf_capacity * key_size);
        for (size_t entry = 0U; entry < leaf_capacity; ++entry) {
            leaf_row_ids[entry] = next_row_id;
            put_test_u32_be(
                leaf_keys + (entry * key_size),
        ...
        unsigned char leaf_page[MYLITE_STORAGE_FORMAT_PAGE_SIZE] = {0};
        assert(
            mylite_storage_test_encode_index_leaf_page(
                leaf_page,
                level_page_ids[0][leaf],
                1ULL,
                0U,
                key_size,
                leaf_row_ids,
        ...
        write_test_page(filename, level_page_ids[0][leaf], leaf_page);
        level_max_row_ids[0][leaf] = leaf_row_ids[leaf_capacity - 1U];
        level_entry_counts[0][leaf] = (unsigned long long)leaf_capacity;
        memcpy(
            level_max_keys[0] + (leaf * key_size),
            leaf_keys + ((leaf_capacity - 1U) * key_size),
            key_size
        );
    }
    assert(next_row_id == last_synthetic_row_id + 1ULL);

    unsigned long long *child_page_ids =
        (unsigned long long *)calloc(branch_capacity, sizeof(*child_page_ids));
    unsigned long long *child_max_row_ids =
        (unsigned long long *)calloc(branch_capacity, sizeof(*child_max_row_ids));
    unsigned char *child_max_keys = (unsigned char *)calloc(branch_capacity, key_size);
    assert(child_page_ids != NULL);
    assert(child_max_row_ids != NULL);
    assert(child_max_keys != NULL);

    for (unsigned level = 1U; level <= tree_level; ++level) {
        for (size_t branch = 0U; branch < level_counts[level]; ++branch) {
            unsigned long long entry_count = 0ULL;
            const size_t child_start = branch * branch_capacity;
            for (size_t child = 0U; child < branch_capacity; ++child) {
                const size_t child_index = child_start + child;
                child_page_ids[child] = level_page_ids[level - 1U][child_index];
                child_max_row_ids[child] = level_max_row_ids[level - 1U][child_index];
                memcpy(
                    child_max_keys + (child * key_size),
                    level_max_keys[level - 1U] + (child_index * key_size),
                    key_size
                );
                entry_count += level_entry_counts[level - 1U][child_index];
            }
            ...
            write_test_page(filename, level_page_ids[level][branch], branch_page);
            level_max_row_ids[level][branch] = child_max_row_ids[branch_capacity - 1U];
            level_entry_counts[level][branch] = entry_count;
            memcpy(
                level_max_keys[level] + (branch * key_size),
                child_max_keys + ((branch_capacity - 1U) * key_size),
                key_size
            );
        }
    }

    unsigned long long page_count_after_tree = tree_end_page_id;
    if (page_count_after_tree <= last_synthetic_row_id) {
        page_count_after_tree = last_synthetic_row_id + 1ULL;
    }
    for (unsigned long long page_id = tree_end_page_id; page_id < page_count_after_tree;
         ++page_id) {
        unsigned char free_list_page[MYLITE_STORAGE_FORMAT_PAGE_SIZE] = {0};
        mylite_storage_test_encode_free_list_page(free_list_page, page_id, 0ULL, 1ULL);
        write_test_page(filename, page_id, free_list_page);
    }

    write_test_header_page_count_and_free_list_root(
        filename,
        page_count_after_tree,
        header.free_list_root_page
    );
    mylite_storage_index_root_definition index_root = {
        .size = sizeof(index_root),
    ...
    unsigned char first_key[key_size];
    unsigned char dynamic_key[key_size];
    unsigned char last_key[key_size];
    unsigned char missing_key[key_size];
    memset(first_key, 0, sizeof(first_key));
    memset(dynamic_key, 0, sizeof(dynamic_key));
    memset(last_key, 0, sizeof(last_key));
    memset(missing_key, 0, sizeof(missing_key));
    const unsigned long long dynamic_row_id =
        first_synthetic_row_id +
        ((unsigned long long)former_branch_leaf_bound * (unsigned long long)leaf_capacity);
    assert(dynamic_row_id <= last_synthetic_row_id);
    put_test_u32_be(first_key, key_size - sizeof(uint32_t), (unsigned)first_synthetic_row_id);
    put_test_u32_be(dynamic_key, key_size - sizeof(uint32_t), (unsigned)dynamic_row_id);
    put_test_u32_be(last_key, key_size - sizeof(uint32_t), (unsigned)last_synthetic_row_id);
    put_test_u32_be(missing_key, key_size - sizeof(uint32_t), (unsigned)last_synthetic_row_id + 1U);
    ...
    const unsigned char *dynamic_prefix_keys[] = {dynamic_key};
    const unsigned long long dynamic_prefix_row_ids[] = {dynamic_row_id};
    assert_prefix_index_entries(
        filename,
        0U,
        dynamic_key,
        ...
        dynamic_prefix_keys,
        sizeof(dynamic_key),
        dynamic_prefix_row_ids,
        1U
    );
    assert_index_prefix_exists_for_index(filename, 0U, dynamic_key, sizeof(dynamic_key), 0ULL, 1);
    assert_index_prefix_exists_for_index(
        filename,
        0U,
        dynamic_key,
    ...
    assert(entries.entry_count == total_entry_count);
    assert_index_entry(&entries, 0U, first_synthetic_row_id, first_key, sizeof(first_key));
    assert_index_entry(
        &entries,
        (size_t)(dynamic_row_id - first_synthetic_row_id),
        dynamic_row_id,
        dynamic_key,
        sizeof(dynamic_key)
    );
    ...
    unsigned char appended_row[8] = {0};
    unsigned char appended_key[key_size];
    unsigned long long appended_row_id = 0ULL;
    memset(appended_key, 0, sizeof(appended_key));
    put_test_u32_be(
        appended_key,
        key_size - sizeof(uint32_t),
        (unsigned)last_synthetic_row_id + 1U
    );
    mylite_storage_index_entry appended_index_entry = {
        .size = sizeof(appended_index_entry),
        .index_number = 0U,
    ...
    flip_file_byte(
        filename,
        (long)(level_page_ids[0][1] * MYLITE_STORAGE_FORMAT_PAGE_SIZE) +
            MYLITE_STORAGE_FORMAT_INDEX_LEAF_CHECKSUM_OFFSET
    );
    assert_index_entry_lookup(
        filename,
    ...
    mylite_storage_clear_thread_caches();
    for (unsigned level = 0U; level <= tree_level; ++level) {
        free(level_page_ids[level]);
        free(level_max_row_ids[level]);
        free(level_entry_counts[level]);
        free(level_max_keys[level]);
    }
    free(child_page_ids);
...
static void test_branch_page_full_root_split(void) {
    ...
    const unsigned long long left_branch_page_id = get_test_u64_le(
        left_root_cell,
        MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CELL_CHILD_PAGE_ID_OFFSET
    );
    const unsigned long long right_branch_page_id = get_test_u64_le(
        right_root_cell,
        MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CELL_CHILD_PAGE_ID_OFFSET
    );
    assert(left_branch_page_id == before_split_pages + 2ULL);
    assert(right_branch_page_id == before_split_pages + 3ULL);
    assert_index_entry_lookup(
        filename,
        0U,
        appended_key,
    ...
    unsigned char right_branch_page_bytes[MYLITE_STORAGE_FORMAT_PAGE_SIZE] = {0};
    read_test_page(filename, right_branch_page_id, right_branch_page_bytes);
    const unsigned long long right_branch_entry_count_before_second = get_test_u64_le(
        right_branch_page_bytes,
        MYLITE_STORAGE_FORMAT_INDEX_BRANCH_ENTRY_COUNT_OFFSET
    );
    assert(right_branch_entry_count_before_second != 0ULL);

    unsigned char second_appended_row[8] = {0};
    unsigned char second_appended_key[key_size] = {0};
    put_test_u32_le(second_appended_row, 0U, (unsigned)entry_count + 2U);
    put_test_u32_be(second_appended_key, key_size - sizeof(uint32_t), (unsigned)entry_count + 2U);
    mylite_storage_index_entry second_appended_index_entry = {
        .size = sizeof(second_appended_index_entry),
        .index_number = 0U,
        .key = second_appended_key,
        .key_size = sizeof(second_appended_key),
    };
    ...
    assert(
        mylite_storage_append_row_with_index_entries(
            ...
            "posts",
            second_appended_row,
            sizeof(second_appended_row),
            &second_appended_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        (unsigned long long)entry_count + 2ULL
    );
    read_test_page(filename, right_branch_page_id, right_branch_page_bytes);
    assert(
        get_test_u64_le(
            right_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_ENTRY_COUNT_OFFSET
        ) == right_branch_entry_count_before_second + 1ULL
    );
    assert_index_entry_lookup(
        filename,
        0U,
        second_appended_key,
        sizeof(second_appended_key),
        MYLITE_STORAGE_OK,
        second_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert(
        get_test_u64_le(
            right_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_ENTRY_COUNT_OFFSET
        ) == right_branch_entry_count_before_second
    );
    assert_index_entry_lookup(
        filename,
        0U,
    ...
    unsigned long long second_appended_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &second_appended_index_entry,
            1U,
            &second_appended_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(second_appended_row_id == before_split_pages + 4ULL);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_split_pages + 5ULL);
    assert_index_root(
        filename,
    ...
    const unsigned char *second_expected_prefix_keys[] = {second_appended_key};
    const unsigned long long second_expected_prefix_row_ids[] = {second_appended_row_id};
    assert_prefix_index_entries(
        filename,
        0U,
        second_appended_key,
        sizeof(second_appended_key),
        second_expected_prefix_keys,
        sizeof(second_appended_key),
        second_expected_prefix_row_ids,
        sizeof(second_expected_prefix_row_ids) / sizeof(second_expected_prefix_row_ids[0])
    );

    unsigned long long expected_level_two_page_count = before_split_pages + 5ULL;
    for (size_t leaf_entry_count = 2U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
        const unsigned key_value = (unsigned)entry_count + 1U + (unsigned)leaf_entry_count;
        unsigned char filler_row[8] = {0};
        unsigned char filler_key[key_size] = {0};
        unsigned long long filler_row_id = 0ULL;
        put_test_u32_le(filler_row, 0U, key_value);
        put_test_u32_be(filler_key, key_size - sizeof(uint32_t), key_value);
        mylite_storage_index_entry filler_index_entry = {
            .size = sizeof(filler_index_entry),
            .index_number = 0U,
            .key = filler_key,
            .key_size = sizeof(filler_key),
        };
        assert(
            mylite_storage_append_row_with_index_entries(
                filename,
        ...
        assert(filler_row_id == expected_level_two_page_count);
        ++expected_level_two_page_count;
        assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
        assert(header.page_count == expected_level_two_page_count);
        assert_index_entry_lookup(
            filename,
            0U,
            filler_key,
    ...
    read_test_page(filename, right_branch_page_id, right_branch_page_bytes);
    const unsigned long long right_branch_entry_count_before_split = get_test_u64_le(
        right_branch_page_bytes,
        MYLITE_STORAGE_FORMAT_INDEX_BRANCH_ENTRY_COUNT_OFFSET
    );
    const unsigned right_branch_child_count_before_split = get_test_u32_le(
        right_branch_page_bytes,
        MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
    );
    assert(
        right_branch_entry_count_before_split ==
        right_branch_entry_count_before_second + (unsigned long long)leaf_capacity - 1ULL
    );

    const unsigned split_key_value = (unsigned)entry_count + (unsigned)leaf_capacity + 1U;
    unsigned char split_row[8] = {0};
    unsigned char split_key[key_size] = {0};
    put_test_u32_le(split_row, 0U, split_key_value);
    put_test_u32_be(split_key, key_size - sizeof(uint32_t), split_key_value);
    mylite_storage_index_entry split_index_entry = {
        .size = sizeof(split_index_entry),
        .index_number = 0U,
        .key = split_key,
        .key_size = sizeof(split_key),
    };

    const unsigned long long before_level_two_split_pages = expected_level_two_page_count;
    statement = NULL;
    unsigned long long split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        (unsigned long long)entry_count + (unsigned long long)leaf_capacity + 1ULL
    );
    read_test_page(filename, right_branch_page_id, right_branch_page_bytes);
    assert(
        get_test_u64_le(
            right_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_ENTRY_COUNT_OFFSET
        ) == right_branch_entry_count_before_split + 1ULL
    );
    assert(
        get_test_u32_le(
            right_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == right_branch_child_count_before_split + 1U
    );
    assert_index_entry_lookup(
        filename,
        0U,
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_two_split_pages);
    read_test_page(filename, right_branch_page_id, right_branch_page_bytes);
    assert(
        get_test_u64_le(
            right_branch_page_bytes,
    ...
    assert(
        get_test_u32_le(
            right_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == right_branch_child_count_before_split
    );
    assert_index_entry_lookup(
        filename,
        0U,
    ...
    unsigned long long split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
    ...
    assert(split_row_id == before_level_two_split_pages);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_two_split_pages + 2ULL);
    assert_index_root(
        filename,
    ...
    const unsigned char *split_expected_prefix_keys[] = {split_key};
    const unsigned long long split_expected_prefix_row_ids[] = {split_row_id};
    assert_prefix_index_entries(
        filename,
        0U,
        split_key,
        sizeof(split_key),
        split_expected_prefix_keys,
        sizeof(split_key),
        split_expected_prefix_row_ids,
        sizeof(split_expected_prefix_row_ids) / sizeof(split_expected_prefix_row_ids[0])
    );

    unsigned long long expected_branch_page_count = before_level_two_split_pages + 2ULL;
    unsigned long long expected_branch_entry_count =
        (unsigned long long)entry_count + (unsigned long long)leaf_capacity + 1ULL;
    unsigned long long right_branch_entry_count_after_split =
        right_branch_entry_count_before_split + 1ULL;
    unsigned right_branch_child_count_after_split = right_branch_child_count_before_split + 1U;
    unsigned next_level_two_key_value = split_key_value + 1U;
    while (right_branch_child_count_after_split < branch_capacity) {
        for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
            append_branch_split_test_row(
                filename,
                key_size,
                next_level_two_key_value,
                expected_branch_page_count
            );
            ++expected_branch_page_count;
            ++expected_branch_entry_count;
            ++right_branch_entry_count_after_split;
            ++next_level_two_key_value;
        }
        append_branch_split_test_row(
            filename,
        ...
        expected_branch_page_count += 2ULL;
        ++expected_branch_entry_count;
        ++right_branch_entry_count_after_split;
        ++right_branch_child_count_after_split;
        ++next_level_two_key_value;
        assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
        assert(header.page_count == expected_branch_page_count);
        read_test_page(filename, right_branch_page_id, right_branch_page_bytes);
        assert(
            get_test_u64_le(
                right_branch_page_bytes,
    ...
    assert(right_branch_child_count_after_split == branch_capacity);
    assert(
        right_branch_entry_count_after_split ==
        (unsigned long long)branch_capacity * (unsigned long long)leaf_capacity
    );
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    read_test_page(filename, right_branch_page_id, right_branch_page_bytes);
    ...
    const unsigned long long before_lower_branch_split_pages = expected_branch_page_count;
    const unsigned long long before_lower_branch_split_entry_count = expected_branch_entry_count;
    const unsigned long long split_lower_branch_page_id = before_lower_branch_split_pages + 2ULL;
    const unsigned split_lower_branch_key_value = next_level_two_key_value;
    unsigned char split_lower_branch_row[8] = {0};
    unsigned char split_lower_branch_key[key_size] = {0};
    put_test_u32_le(split_lower_branch_row, 0U, split_lower_branch_key_value);
    put_test_u32_be(
        split_lower_branch_key,
        key_size - sizeof(uint32_t),
        split_lower_branch_key_value
    );
    mylite_storage_index_entry split_lower_branch_index_entry = {
        .size = sizeof(split_lower_branch_index_entry),
        .index_number = 0U,
        .key = split_lower_branch_key,
        .key_size = sizeof(split_lower_branch_key),
    };
    const unsigned split_lower_new_child_count = (unsigned)branch_capacity + 1U;
    const unsigned split_lower_left_child_count = (split_lower_new_child_count + 1U) / 2U;
    const unsigned split_lower_right_child_count =
        split_lower_new_child_count - split_lower_left_child_count;

    statement = NULL;
    unsigned long long split_lower_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            split_lower_branch_row,
            sizeof(split_lower_branch_row),
            &split_lower_branch_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_lower_branch_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 2U);
    assert(
    ...
    const unsigned char *split_lower_root_cell = root_page_bytes +
                                                 MYLITE_STORAGE_FORMAT_INDEX_BRANCH_PAYLOAD_OFFSET +
                                                 (2U * branch_cell_size);
    assert(
        get_test_u64_le(
            split_lower_root_cell,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CELL_CHILD_PAGE_ID_OFFSET
        ) == split_lower_branch_page_id
    );
    read_test_page(filename, right_branch_page_id, right_branch_page_bytes);
    assert(
        get_test_u32_le(
            right_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_lower_left_child_count
    );
    unsigned char split_lower_branch_page_bytes[MYLITE_STORAGE_FORMAT_PAGE_SIZE] = {0};
    read_test_page(filename, split_lower_branch_page_id, split_lower_branch_page_bytes);
    assert(
        get_test_u32_le(
            split_lower_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_lower_right_child_count
    );
    assert_index_entry_lookup(
        filename,
        0U,
        split_lower_branch_key,
        sizeof(split_lower_branch_key),
        MYLITE_STORAGE_OK,
        split_lower_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_lower_branch_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_lower_branch_split_entry_count
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(
        get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET) ==
    ...
    unsigned long long split_lower_branch_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &split_lower_branch_index_entry,
            1U,
            &split_lower_branch_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(split_lower_branch_row_id == before_lower_branch_split_pages);
    expected_branch_page_count += 3ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    assert(
        get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET) ==
        3U
    ...
    const unsigned char *split_lower_expected_prefix_keys[] = {split_lower_branch_key};
    const unsigned long long split_lower_expected_prefix_row_ids[] = {split_lower_branch_row_id};
    assert_prefix_index_entries(
        filename,
        0U,
        split_lower_branch_key,
        sizeof(split_lower_branch_key),
        split_lower_expected_prefix_keys,
        sizeof(split_lower_branch_key),
        split_lower_expected_prefix_row_ids,
        sizeof(split_lower_expected_prefix_row_ids) / sizeof(split_lower_expected_prefix_row_ids[0])
    );

    unsigned root_child_count_after_lower_split = 3U;
    unsigned long long active_lower_branch_page_id = split_lower_branch_page_id;
    unsigned active_lower_branch_child_count = split_lower_right_child_count;
    unsigned long long active_lower_branch_entry_count =
        ((unsigned long long)split_lower_right_child_count - 1ULL) *
            (unsigned long long)leaf_capacity +
        1ULL;
    next_level_two_key_value = split_lower_branch_key_value + 1U;
    while (root_child_count_after_lower_split < branch_capacity) {
        while (active_lower_branch_child_count < branch_capacity) {
            for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity;
                 ++leaf_entry_count) {
                append_branch_split_test_row(
                    filename,
                    key_size,
                ...
                ++expected_branch_page_count;
                ++expected_branch_entry_count;
                ++active_lower_branch_entry_count;
                ++next_level_two_key_value;
            }
            append_branch_split_test_row(
                filename,
            ...
            ++expected_branch_entry_count;
            ++active_lower_branch_entry_count;
            ++active_lower_branch_child_count;
            ++next_level_two_key_value;
        }
        for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
            append_branch_split_test_row(
        ...
        const unsigned long long next_lower_branch_page_id = expected_branch_page_count + 2ULL;
        append_branch_split_test_row(
            filename,
            key_size,
            next_level_two_key_value,
        ...
        expected_branch_page_count += 3ULL;
        ++expected_branch_entry_count;
        ++root_child_count_after_lower_split;
        ++next_level_two_key_value;
        active_lower_branch_page_id = next_lower_branch_page_id;
        active_lower_branch_child_count = split_lower_right_child_count;
        active_lower_branch_entry_count =
            ((unsigned long long)split_lower_right_child_count - 1ULL) *
                (unsigned long long)leaf_capacity +
            1ULL;
        ...
        read_test_page(filename, root_page, root_page_bytes);
        assert(
            get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 2U
        );
        assert(
            get_test_u32_le(
                root_page_bytes,
        ...
        read_test_page(filename, active_lower_branch_page_id, split_lower_branch_page_bytes);
        assert(
            get_test_u32_le(
                split_lower_branch_page_bytes,
                MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
    ...
    assert(root_child_count_after_lower_split == branch_capacity);
    assert(
        active_lower_branch_entry_count ==
        (unsigned long long)branch_capacity * (unsigned long long)leaf_capacity
    );
    ...
    const unsigned long long before_level_three_promotion_pages = expected_branch_page_count;
    const unsigned long long before_level_three_promotion_entry_count = expected_branch_entry_count;
    const unsigned long long promoted_lower_branch_page_id =
        before_level_three_promotion_pages + 2ULL;
    const unsigned long long promoted_left_branch_page_id =
        before_level_three_promotion_pages + 3ULL;
    const unsigned long long promoted_right_branch_page_id =
        before_level_three_promotion_pages + 4ULL;
    const unsigned promoted_root_child_count = (unsigned)branch_capacity + 1U;
    const unsigned promoted_left_branch_child_count = (promoted_root_child_count + 1U) / 2U;
    const unsigned promoted_right_branch_child_count =
        promoted_root_child_count - promoted_left_branch_child_count;
    const unsigned level_three_key_value = next_level_two_key_value;
    unsigned char level_three_row[8] = {0};
    unsigned char level_three_key[key_size] = {0};
    put_test_u32_le(level_three_row, 0U, level_three_key_value);
    put_test_u32_be(level_three_key, key_size - sizeof(uint32_t), level_three_key_value);
    mylite_storage_index_entry level_three_index_entry = {
        .size = sizeof(level_three_index_entry),
        .index_number = 0U,
        .key = level_three_key,
        .key_size = sizeof(level_three_key),
    };

    statement = NULL;
    unsigned long long level_three_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            ...
            "posts",
            level_three_row,
            sizeof(level_three_row),
            &level_three_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_three_promotion_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 3U);
    assert(
        get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET) ==
        2U
    );
    unsigned char promoted_branch_page_bytes[MYLITE_STORAGE_FORMAT_PAGE_SIZE] = {0};
    read_test_page(filename, promoted_right_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET
        ) == 2U
    );
    assert_index_entry_lookup(
        filename,
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_three_promotion_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_three_promotion_entry_count
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 2U);
    assert(
    ...
    unsigned long long level_three_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
    ...
    assert(level_three_row_id == before_level_three_promotion_pages);
    expected_branch_page_count += 5ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    ...
    read_test_page(filename, promoted_left_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET
    ...
    const unsigned char *level_three_expected_prefix_keys[] = {level_three_key};
    const unsigned long long level_three_expected_prefix_row_ids[] = {level_three_row_id};
    assert_prefix_index_entries(
        filename,
        0U,
        level_three_key,
        sizeof(level_three_key),
        level_three_expected_prefix_keys,
        sizeof(level_three_key),
        level_three_expected_prefix_row_ids,
        sizeof(level_three_expected_prefix_row_ids) / sizeof(level_three_expected_prefix_row_ids[0])
    );

    const unsigned long long before_level_three_fit_pages = expected_branch_page_count;
    const unsigned long long before_level_three_fit_entry_count = expected_branch_entry_count;
    const unsigned level_three_fit_key_value = level_three_key_value + 1U;
    unsigned char level_three_fit_row[8] = {0};
    unsigned char level_three_fit_key[key_size] = {0};
    put_test_u32_le(level_three_fit_row, 0U, level_three_fit_key_value);
    put_test_u32_be(level_three_fit_key, key_size - sizeof(uint32_t), level_three_fit_key_value);
    mylite_storage_index_entry level_three_fit_index_entry = {
        .size = sizeof(level_three_fit_index_entry),
        .index_number = 0U,
        .key = level_three_fit_key,
        .key_size = sizeof(level_three_fit_key),
    };

    statement = NULL;
    unsigned long long level_three_fit_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            ...
            "posts",
            level_three_fit_row,
            sizeof(level_three_fit_row),
            &level_three_fit_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_three_fit_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 3U);
    assert_index_entry_lookup(
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_three_fit_pages);
    assert_index_root(filename, "app", "posts", 0U, root_page, before_level_three_fit_entry_count);
    assert_index_entry_lookup(
        filename,
        0U,
        level_three_fit_key,
    ...
    unsigned long long level_three_fit_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_three_fit_index_entry,
            1U,
            &level_three_fit_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_three_fit_row_id == before_level_three_fit_pages);
    ++expected_branch_page_count;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_three_fit_expected_prefix_keys[] = {level_three_fit_key};
    const unsigned long long level_three_fit_expected_prefix_row_ids[] = {level_three_fit_row_id};
    assert_prefix_index_entries(
        filename,
        0U,
        level_three_fit_key,
        sizeof(level_three_fit_key),
        level_three_fit_expected_prefix_keys,
        sizeof(level_three_fit_key),
        level_three_fit_expected_prefix_row_ids,
        sizeof(level_three_fit_expected_prefix_row_ids) /
            sizeof(level_three_fit_expected_prefix_row_ids[0])
    );

    unsigned next_level_three_key_value = level_three_fit_key_value + 1U;
    for (size_t leaf_entry_count = 2U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
        append_branch_split_test_row(
            filename,
            key_size,
            next_level_three_key_value,
            expected_branch_page_count
        );
        ++expected_branch_page_count;
        ++expected_branch_entry_count;
    ...
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 3U);
    read_test_page(filename, promoted_lower_branch_page_id, split_lower_branch_page_bytes);
    assert(
        get_test_u32_le(
            split_lower_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
    ...
    const unsigned long long before_level_three_leaf_split_pages = expected_branch_page_count;
    const unsigned long long before_level_three_leaf_split_entry_count =
        expected_branch_entry_count;
    const unsigned level_three_split_key_value = next_level_three_key_value;
    unsigned char level_three_split_row[8] = {0};
    unsigned char level_three_split_key[key_size] = {0};
    put_test_u32_le(level_three_split_row, 0U, level_three_split_key_value);
    put_test_u32_be(
        level_three_split_key,
        key_size - sizeof(uint32_t),
        level_three_split_key_value
    );
    mylite_storage_index_entry level_three_split_index_entry = {
        .size = sizeof(level_three_split_index_entry),
        .index_number = 0U,
        .key = level_three_split_key,
        .key_size = sizeof(level_three_split_key),
    };

    statement = NULL;
    unsigned long long level_three_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_three_split_row,
            sizeof(level_three_split_row),
            &level_three_split_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_three_leaf_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 3U);
    read_test_page(filename, promoted_lower_branch_page_id, split_lower_branch_page_bytes);
    assert(
        get_test_u32_le(
            split_lower_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_lower_right_child_count + 1U
    );
    assert_index_entry_lookup(
        filename,
        0U,
        level_three_split_key,
        sizeof(level_three_split_key),
        MYLITE_STORAGE_OK,
        level_three_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_three_leaf_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_three_leaf_split_entry_count
    );
    read_test_page(filename, promoted_lower_branch_page_id, split_lower_branch_page_bytes);
    assert(
        get_test_u32_le(
    ...
    unsigned long long level_three_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_three_split_index_entry,
            1U,
            &level_three_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_three_split_row_id == before_level_three_leaf_split_pages);
    expected_branch_page_count += 2ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_three_split_expected_prefix_keys[] = {level_three_split_key};
    const unsigned long long level_three_split_expected_prefix_row_ids[] = {
        level_three_split_row_id,
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_three_split_key,
        sizeof(level_three_split_key),
        level_three_split_expected_prefix_keys,
        sizeof(level_three_split_key),
        level_three_split_expected_prefix_row_ids,
        sizeof(level_three_split_expected_prefix_row_ids) /
            sizeof(level_three_split_expected_prefix_row_ids[0])
    );

    unsigned level_three_lower_branch_child_count = split_lower_right_child_count + 1U;
    unsigned next_level_three_lower_key_value = level_three_split_key_value + 1U;
    while (level_three_lower_branch_child_count < branch_capacity) {
        for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
            append_branch_split_test_row(
                filename,
                key_size,
                next_level_three_lower_key_value,
                expected_branch_page_count
            );
            ++expected_branch_page_count;
            ++expected_branch_entry_count;
        ...
        expected_branch_page_count += 2ULL;
        ++expected_branch_entry_count;
        ++level_three_lower_branch_child_count;
        ++next_level_three_lower_key_value;
    }
    for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
        append_branch_split_test_row(
    ...
    assert(level_three_lower_branch_child_count == branch_capacity);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    ...
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == promoted_right_branch_child_count
    );
    read_test_page(filename, promoted_lower_branch_page_id, split_lower_branch_page_bytes);
    assert(
        get_test_u32_le(
    ...
    const unsigned long long before_level_three_lower_split_pages = expected_branch_page_count;
    const unsigned long long before_level_three_lower_split_entry_count =
        expected_branch_entry_count;
    const unsigned long long split_level_three_lower_branch_page_id =
        before_level_three_lower_split_pages + 2ULL;
    const unsigned split_level_three_lower_left_child_count = ((unsigned)branch_capacity + 2U) / 2U;
    const unsigned split_level_three_lower_right_child_count =
        (unsigned)branch_capacity + 1U - split_level_three_lower_left_child_count;
    const unsigned level_three_lower_split_key_value = next_level_three_lower_key_value;
    unsigned char level_three_lower_split_row[8] = {0};
    unsigned char level_three_lower_split_key[key_size] = {0};
    put_test_u32_le(level_three_lower_split_row, 0U, level_three_lower_split_key_value);
    put_test_u32_be(
        level_three_lower_split_key,
        key_size - sizeof(uint32_t),
        level_three_lower_split_key_value
    );
    mylite_storage_index_entry level_three_lower_split_index_entry = {
        .size = sizeof(level_three_lower_split_index_entry),
        .index_number = 0U,
        .key = level_three_lower_split_key,
        .key_size = sizeof(level_three_lower_split_key),
    };

    statement = NULL;
    unsigned long long level_three_lower_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_three_lower_split_row,
            sizeof(level_three_lower_split_row),
            &level_three_lower_split_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_three_lower_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 3U);
    read_test_page(filename, promoted_right_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == promoted_right_branch_child_count + 1U
    );
    read_test_page(filename, promoted_lower_branch_page_id, split_lower_branch_page_bytes);
    assert(
        get_test_u32_le(
            split_lower_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_three_lower_left_child_count
    );
    assert_index_entry_lookup(
        filename,
        0U,
        level_three_lower_split_key,
        sizeof(level_three_lower_split_key),
        MYLITE_STORAGE_OK,
        level_three_lower_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_three_lower_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_three_lower_split_entry_count
    );
    read_test_page(filename, promoted_right_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
    ...
    unsigned long long level_three_lower_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_three_lower_split_index_entry,
            1U,
            &level_three_lower_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_three_lower_split_row_id == before_level_three_lower_split_pages);
    expected_branch_page_count += 3ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    read_test_page(filename, split_level_three_lower_branch_page_id, split_lower_branch_page_bytes);
    assert(
        get_test_u32_le(
            split_lower_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_three_lower_right_child_count
    );
    assert_index_entry_lookup(
        filename,
        0U,
    ...
    const unsigned char *level_three_lower_split_expected_prefix_keys[] = {
        level_three_lower_split_key,
    };
    const unsigned long long level_three_lower_split_expected_prefix_row_ids[] = {
        level_three_lower_split_row_id,
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_three_lower_split_key,
        sizeof(level_three_lower_split_key),
        level_three_lower_split_expected_prefix_keys,
        sizeof(level_three_lower_split_key),
        level_three_lower_split_expected_prefix_row_ids,
        sizeof(level_three_lower_split_expected_prefix_row_ids) /
            sizeof(level_three_lower_split_expected_prefix_row_ids[0])
    );

    unsigned level_three_child_branch_child_count = promoted_right_branch_child_count + 1U;
    unsigned active_level_three_lower_child_count = split_level_three_lower_right_child_count;
    unsigned next_level_three_child_key_value = level_three_lower_split_key_value + 1U;
    while (level_three_child_branch_child_count < branch_capacity) {
        while (active_level_three_lower_child_count < branch_capacity) {
            for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity;
                 ++leaf_entry_count) {
                append_branch_split_test_row(
                    filename,
                    key_size,
                    next_level_three_child_key_value,
                    expected_branch_page_count
                );
                ++expected_branch_page_count;
                ++expected_branch_entry_count;
            ...
            expected_branch_page_count += 2ULL;
            ++expected_branch_entry_count;
            ++active_level_three_lower_child_count;
            ++next_level_three_child_key_value;
        }
        for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
            append_branch_split_test_row(
        ...
        expected_branch_page_count += 3ULL;
        ++expected_branch_entry_count;
        ++level_three_child_branch_child_count;
        active_level_three_lower_child_count = split_level_three_lower_right_child_count;
        ++next_level_three_child_key_value;
    }
    while (active_level_three_lower_child_count < branch_capacity) {
        for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
    ...
    assert(level_three_child_branch_child_count == branch_capacity);
    assert(active_level_three_lower_child_count == branch_capacity);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    ...
    const unsigned long long before_level_three_child_split_pages = expected_branch_page_count;
    const unsigned long long before_level_three_child_split_entry_count =
        expected_branch_entry_count;
    const unsigned long long split_level_three_child_lower_branch_page_id =
        before_level_three_child_split_pages + 2ULL;
    const unsigned long long split_level_three_child_branch_page_id =
        before_level_three_child_split_pages + 3ULL;
    const unsigned split_level_three_child_left_child_count = ((unsigned)branch_capacity + 2U) / 2U;
    const unsigned split_level_three_child_right_child_count =
        (unsigned)branch_capacity + 1U - split_level_three_child_left_child_count;
    const unsigned level_three_child_split_key_value = next_level_three_child_key_value;
    unsigned char level_three_child_split_row[8] = {0};
    unsigned char level_three_child_split_key[key_size] = {0};
    put_test_u32_le(level_three_child_split_row, 0U, level_three_child_split_key_value);
    put_test_u32_be(
        level_three_child_split_key,
        key_size - sizeof(uint32_t),
        level_three_child_split_key_value
    );
    mylite_storage_index_entry level_three_child_split_index_entry = {
        .size = sizeof(level_three_child_split_index_entry),
        .index_number = 0U,
        .key = level_three_child_split_key,
        .key_size = sizeof(level_three_child_split_key),
    };

    statement = NULL;
    unsigned long long level_three_child_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_three_child_split_row,
            sizeof(level_three_child_split_row),
            &level_three_child_split_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_three_child_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 3U);
    assert(
    ...
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_three_child_left_child_count
    );
    assert_index_entry_lookup(
        filename,
        0U,
        level_three_child_split_key,
        sizeof(level_three_child_split_key),
        MYLITE_STORAGE_OK,
        level_three_child_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_three_child_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_three_child_split_entry_count
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(
        get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET) ==
    ...
    unsigned long long level_three_child_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_three_child_split_index_entry,
            1U,
            &level_three_child_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_three_child_split_row_id == before_level_three_child_split_pages);
    expected_branch_page_count += 4ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    ...
    read_test_page(filename, split_level_three_child_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_three_child_right_child_count
    );
    read_test_page(
        filename,
        split_level_three_child_lower_branch_page_id,
        split_lower_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    const unsigned char *level_three_child_split_expected_prefix_keys[] = {
        level_three_child_split_key,
    };
    const unsigned long long level_three_child_split_expected_prefix_row_ids[] = {
        level_three_child_split_row_id,
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_three_child_split_key,
        sizeof(level_three_child_split_key),
        level_three_child_split_expected_prefix_keys,
        sizeof(level_three_child_split_key),
        level_three_child_split_expected_prefix_row_ids,
        sizeof(level_three_child_split_expected_prefix_row_ids) /
            sizeof(level_three_child_split_expected_prefix_row_ids[0])
    );

    unsigned level_three_root_child_count = 3U;
    unsigned active_level_three_child_branch_child_count =
        split_level_three_child_right_child_count;
    active_level_three_lower_child_count = split_level_three_lower_right_child_count;
    unsigned next_level_four_key_value = level_three_child_split_key_value + 1U;
    while (level_three_root_child_count < branch_capacity) {
        while (active_level_three_child_branch_child_count < branch_capacity) {
            while (active_level_three_lower_child_count < branch_capacity) {
                for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity;
                     ++leaf_entry_count) {
                    append_branch_split_test_row(
            ...
            expected_branch_page_count += 3ULL;
            ++expected_branch_entry_count;
            ++active_level_three_child_branch_child_count;
            active_level_three_lower_child_count = split_level_three_lower_right_child_count;
            ++next_level_four_key_value;
        }
        while (active_level_three_lower_child_count < branch_capacity) {
        ...
        expected_branch_page_count += 4ULL;
        ++expected_branch_entry_count;
        ++level_three_root_child_count;
        active_level_three_child_branch_child_count = split_level_three_child_right_child_count;
        active_level_three_lower_child_count = split_level_three_lower_right_child_count;
        ++next_level_four_key_value;
    }
    while (active_level_three_child_branch_child_count < branch_capacity) {
    ...
    assert(level_three_root_child_count == branch_capacity);
    assert(active_level_three_child_branch_child_count == branch_capacity);
    assert(active_level_three_lower_child_count == branch_capacity);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    ...
    const unsigned long long before_level_four_promotion_pages = expected_branch_page_count;
    const unsigned long long before_level_four_promotion_entry_count = expected_branch_entry_count;
    const unsigned long long promoted_level_four_left_branch_page_id =
        before_level_four_promotion_pages + 4ULL;
    const unsigned long long promoted_level_four_right_branch_page_id =
        before_level_four_promotion_pages + 5ULL;
    const unsigned level_four_key_value = next_level_four_key_value;
    unsigned char level_four_row[8] = {0};
    unsigned char level_four_key[key_size] = {0};
    put_test_u32_le(level_four_row, 0U, level_four_key_value);
    put_test_u32_be(level_four_key, key_size - sizeof(uint32_t), level_four_key_value);
    mylite_storage_index_entry level_four_index_entry = {
        .size = sizeof(level_four_index_entry),
        .index_number = 0U,
        .key = level_four_key,
        .key_size = sizeof(level_four_key),
    };

    statement = NULL;
    unsigned long long level_four_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            ...
            "posts",
            level_four_row,
            sizeof(level_four_row),
            &level_four_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_four_promotion_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    assert(
        get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET) ==
        2U
    );
    read_test_page(filename, promoted_level_four_right_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_four_promotion_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_four_promotion_entry_count
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 3U);
    assert_index_entry_lookup(
    ...
    unsigned long long level_four_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
    ...
    assert(level_four_row_id == before_level_four_promotion_pages);
    expected_branch_page_count += 6ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    ...
    read_test_page(filename, promoted_level_four_left_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET
    ...
    const unsigned char *level_four_expected_prefix_keys[] = {level_four_key};
    const unsigned long long level_four_expected_prefix_row_ids[] = {level_four_row_id};
    assert_prefix_index_entries(
        filename,
        0U,
        level_four_key,
        sizeof(level_four_key),
        level_four_expected_prefix_keys,
        sizeof(level_four_key),
        level_four_expected_prefix_row_ids,
        sizeof(level_four_expected_prefix_row_ids) / sizeof(level_four_expected_prefix_row_ids[0])
    );

    const unsigned long long before_level_four_fit_pages = expected_branch_page_count;
    const unsigned long long before_level_four_fit_entry_count = expected_branch_entry_count;
    const unsigned level_four_fit_key_value = level_four_key_value + 1U;
    unsigned char level_four_fit_row[8] = {0};
    unsigned char level_four_fit_key[key_size] = {0};
    put_test_u32_le(level_four_fit_row, 0U, level_four_fit_key_value);
    put_test_u32_be(level_four_fit_key, key_size - sizeof(uint32_t), level_four_fit_key_value);
    mylite_storage_index_entry level_four_fit_index_entry = {
        .size = sizeof(level_four_fit_index_entry),
        .index_number = 0U,
        .key = level_four_fit_key,
        .key_size = sizeof(level_four_fit_key),
    };

    statement = NULL;
    unsigned long long level_four_fit_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            ...
            "posts",
            level_four_fit_row,
            sizeof(level_four_fit_row),
            &level_four_fit_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_four_fit_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    assert(
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_four_fit_pages);
    assert_index_root(filename, "app", "posts", 0U, root_page, before_level_four_fit_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    assert_index_entry_lookup(
        filename,
    ...
    unsigned long long level_four_fit_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_four_fit_index_entry,
            1U,
            &level_four_fit_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_four_fit_row_id == before_level_four_fit_pages);
    ++expected_branch_page_count;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_four_fit_expected_prefix_keys[] = {level_four_fit_key};
    const unsigned long long level_four_fit_expected_prefix_row_ids[] = {level_four_fit_row_id};
    assert_prefix_index_entries(
        filename,
        0U,
        level_four_fit_key,
        sizeof(level_four_fit_key),
        level_four_fit_expected_prefix_keys,
        sizeof(level_four_fit_key),
        level_four_fit_expected_prefix_row_ids,
        sizeof(level_four_fit_expected_prefix_row_ids) /
            sizeof(level_four_fit_expected_prefix_row_ids[0])
    );

    unsigned next_level_four_split_key_value = level_four_fit_key_value + 1U;
    for (size_t leaf_entry_count = 2U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
        append_branch_split_test_row(
            filename,
            key_size,
            next_level_four_split_key_value,
            expected_branch_page_count
        );
        ++expected_branch_page_count;
        ++expected_branch_entry_count;
    ...
    const unsigned long long before_level_four_leaf_split_pages = expected_branch_page_count;
    const unsigned long long before_level_four_leaf_split_entry_count = expected_branch_entry_count;
    const unsigned level_four_leaf_split_key_value = next_level_four_split_key_value;
    unsigned char level_four_leaf_split_row[8] = {0};
    unsigned char level_four_leaf_split_key[key_size] = {0};
    put_test_u32_le(level_four_leaf_split_row, 0U, level_four_leaf_split_key_value);
    put_test_u32_be(
        level_four_leaf_split_key,
        key_size - sizeof(uint32_t),
        level_four_leaf_split_key_value
    );
    mylite_storage_index_entry level_four_leaf_split_index_entry = {
        .size = sizeof(level_four_leaf_split_index_entry),
        .index_number = 0U,
        .key = level_four_leaf_split_key,
        .key_size = sizeof(level_four_leaf_split_key),
    };

    statement = NULL;
    unsigned long long level_four_leaf_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_four_leaf_split_row,
            sizeof(level_four_leaf_split_row),
            &level_four_leaf_split_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_four_leaf_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    assert_index_entry_lookup(
        ...
        0U,
        level_four_leaf_split_key,
        sizeof(level_four_leaf_split_key),
        MYLITE_STORAGE_OK,
        level_four_leaf_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_four_leaf_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_four_leaf_split_entry_count
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    assert_index_entry_lookup(
    ...
    unsigned long long level_four_leaf_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_four_leaf_split_index_entry,
            1U,
            &level_four_leaf_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_four_leaf_split_row_id == before_level_four_leaf_split_pages);
    expected_branch_page_count += 2ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_four_leaf_split_expected_prefix_keys[] = {level_four_leaf_split_key};
    const unsigned long long level_four_leaf_split_expected_prefix_row_ids[] = {
        level_four_leaf_split_row_id
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_four_leaf_split_key,
        sizeof(level_four_leaf_split_key),
        level_four_leaf_split_expected_prefix_keys,
        sizeof(level_four_leaf_split_key),
        level_four_leaf_split_expected_prefix_row_ids,
        sizeof(level_four_leaf_split_expected_prefix_row_ids) /
            sizeof(level_four_leaf_split_expected_prefix_row_ids[0])
    );

    const unsigned level_four_lower_right_child_count =
        (unsigned)branch_capacity + 1U - (((unsigned)branch_capacity + 2U) / 2U);
    unsigned level_four_lower_branch_child_count = level_four_lower_right_child_count + 1U;
    unsigned next_level_four_lower_key_value = level_four_leaf_split_key_value + 1U;
    while (level_four_lower_branch_child_count < branch_capacity) {
        for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
            append_branch_split_test_row(
                filename,
                key_size,
                next_level_four_lower_key_value,
                expected_branch_page_count
            );
            ++expected_branch_page_count;
            ++expected_branch_entry_count;
        ...
        expected_branch_page_count += 2ULL;
        ++expected_branch_entry_count;
        ++level_four_lower_branch_child_count;
        ++next_level_four_lower_key_value;
    }
    for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
        append_branch_split_test_row(
    ...
    assert(level_four_lower_branch_child_count == branch_capacity);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    ...
    const unsigned long long before_level_four_lower_split_pages = expected_branch_page_count;
    const unsigned long long before_level_four_lower_split_entry_count =
        expected_branch_entry_count;
    const unsigned long long split_level_four_lower_branch_page_id =
        before_level_four_lower_split_pages + 2ULL;
    const unsigned split_level_four_lower_right_child_count =
        (unsigned)branch_capacity + 1U - (((unsigned)branch_capacity + 2U) / 2U);
    const unsigned level_four_lower_split_key_value = next_level_four_lower_key_value;
    unsigned char level_four_lower_split_row[8] = {0};
    unsigned char level_four_lower_split_key[key_size] = {0};
    put_test_u32_le(level_four_lower_split_row, 0U, level_four_lower_split_key_value);
    put_test_u32_be(
        level_four_lower_split_key,
        key_size - sizeof(uint32_t),
        level_four_lower_split_key_value
    );
    mylite_storage_index_entry level_four_lower_split_index_entry = {
        .size = sizeof(level_four_lower_split_index_entry),
        .index_number = 0U,
        .key = level_four_lower_split_key,
        .key_size = sizeof(level_four_lower_split_key),
    };

    statement = NULL;
    unsigned long long level_four_lower_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_four_lower_split_row,
            sizeof(level_four_lower_split_row),
            &level_four_lower_split_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_four_lower_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    read_test_page(filename, split_level_four_lower_branch_page_id, split_lower_branch_page_bytes);
    assert(
        get_test_u32_le(
            split_lower_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET
    ...
    assert(
        get_test_u32_le(
            split_lower_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_four_lower_right_child_count
    );
    assert_index_entry_lookup(
        filename,
        0U,
        level_four_lower_split_key,
        sizeof(level_four_lower_split_key),
        MYLITE_STORAGE_OK,
        level_four_lower_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_four_lower_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_four_lower_split_entry_count
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    assert_index_entry_lookup(
    ...
    unsigned long long level_four_lower_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_four_lower_split_index_entry,
            1U,
            &level_four_lower_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_four_lower_split_row_id == before_level_four_lower_split_pages);
    expected_branch_page_count += 3ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_four_lower_split_expected_prefix_keys[] = {
        level_four_lower_split_key
    };
    const unsigned long long level_four_lower_split_expected_prefix_row_ids[] = {
        level_four_lower_split_row_id
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_four_lower_split_key,
        sizeof(level_four_lower_split_key),
        level_four_lower_split_expected_prefix_keys,
        sizeof(level_four_lower_split_key),
        level_four_lower_split_expected_prefix_row_ids,
        sizeof(level_four_lower_split_expected_prefix_row_ids) /
            sizeof(level_four_lower_split_expected_prefix_row_ids[0])
    );

    const unsigned level_four_child_right_child_count =
        (unsigned)branch_capacity + 1U - (((unsigned)branch_capacity + 2U) / 2U);
    const unsigned level_four_root_right_child_count =
        (unsigned)branch_capacity + 1U - (((unsigned)branch_capacity + 2U) / 2U);
    unsigned level_four_child_branch_child_count = level_four_child_right_child_count + 1U;
    unsigned active_level_four_child_lower_child_count = split_level_four_lower_right_child_count;
    unsigned next_level_four_child_key_value = level_four_lower_split_key_value + 1U;
    while (level_four_child_branch_child_count < branch_capacity) {
        while (active_level_four_child_lower_child_count < branch_capacity) {
            for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity;
                 ++leaf_entry_count) {
                append_branch_split_test_row(
                    filename,
                    key_size,
                    next_level_four_child_key_value,
                    expected_branch_page_count
                );
                ++expected_branch_page_count;
                ++expected_branch_entry_count;
            ...
            expected_branch_page_count += 2ULL;
            ++expected_branch_entry_count;
            ++active_level_four_child_lower_child_count;
            ++next_level_four_child_key_value;
        }
        for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
            append_branch_split_test_row(
        ...
        expected_branch_page_count += 3ULL;
        ++expected_branch_entry_count;
        ++level_four_child_branch_child_count;
        active_level_four_child_lower_child_count = split_level_four_lower_right_child_count;
        ++next_level_four_child_key_value;
    }
    while (active_level_four_child_lower_child_count < branch_capacity) {
        for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity; ++leaf_entry_count) {
    ...
    assert(level_four_child_branch_child_count == branch_capacity);
    assert(active_level_four_child_lower_child_count == branch_capacity);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    ...
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == level_four_root_right_child_count
    );

    const unsigned long long before_level_four_child_split_pages = expected_branch_page_count;
    const unsigned long long before_level_four_child_split_entry_count =
        expected_branch_entry_count;
    const unsigned long long split_level_four_child_lower_branch_page_id =
        before_level_four_child_split_pages + 2ULL;
    const unsigned long long split_level_four_child_branch_page_id =
        before_level_four_child_split_pages + 3ULL;
    const unsigned split_level_four_child_right_child_count =
        (unsigned)branch_capacity + 1U - (((unsigned)branch_capacity + 2U) / 2U);
    const unsigned level_four_child_split_key_value = next_level_four_child_key_value;
    unsigned char level_four_child_split_row[8] = {0};
    unsigned char level_four_child_split_key[key_size] = {0};
    put_test_u32_le(level_four_child_split_row, 0U, level_four_child_split_key_value);
    put_test_u32_be(
        level_four_child_split_key,
        key_size - sizeof(uint32_t),
        level_four_child_split_key_value
    );
    mylite_storage_index_entry level_four_child_split_index_entry = {
        .size = sizeof(level_four_child_split_index_entry),
        .index_number = 0U,
        .key = level_four_child_split_key,
        .key_size = sizeof(level_four_child_split_key),
    };

    statement = NULL;
    unsigned long long level_four_child_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_four_child_split_row,
            sizeof(level_four_child_split_row),
            &level_four_child_split_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_four_child_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    read_test_page(filename, promoted_level_four_right_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == level_four_root_right_child_count + 1U
    );
    read_test_page(filename, split_level_four_child_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET
    ...
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_four_child_right_child_count
    );
    read_test_page(
        filename,
        split_level_four_child_lower_branch_page_id,
        split_lower_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    assert_index_entry_lookup(
        ...
        0U,
        level_four_child_split_key,
        sizeof(level_four_child_split_key),
        MYLITE_STORAGE_OK,
        level_four_child_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_four_child_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_four_child_split_entry_count
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    read_test_page(filename, promoted_level_four_right_branch_page_id, promoted_branch_page_bytes);
    ...
    unsigned long long level_four_child_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_four_child_split_index_entry,
            1U,
            &level_four_child_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_four_child_split_row_id == before_level_four_child_split_pages);
    expected_branch_page_count += 4ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_four_child_split_expected_prefix_keys[] = {
        level_four_child_split_key
    };
    const unsigned long long level_four_child_split_expected_prefix_row_ids[] = {
        level_four_child_split_row_id
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_four_child_split_key,
        sizeof(level_four_child_split_key),
        level_four_child_split_expected_prefix_keys,
        sizeof(level_four_child_split_key),
        level_four_child_split_expected_prefix_row_ids,
        sizeof(level_four_child_split_expected_prefix_row_ids) /
            sizeof(level_four_child_split_expected_prefix_row_ids[0])
    );

    unsigned level_four_upper_branch_child_count = level_four_root_right_child_count + 1U;
    unsigned active_level_four_upper_child_branch_child_count =
        split_level_four_child_right_child_count;
    unsigned active_level_four_upper_lower_child_count = split_level_four_lower_right_child_count;
    unsigned next_level_four_upper_key_value = level_four_child_split_key_value + 1U;
    while (level_four_upper_branch_child_count < branch_capacity) {
        while (active_level_four_upper_child_branch_child_count < branch_capacity) {
            while (active_level_four_upper_lower_child_count < branch_capacity) {
                for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity;
                     ++leaf_entry_count) {
                    append_branch_split_test_row(
                        filename,
                        key_size,
                        next_level_four_upper_key_value,
                        expected_branch_page_count
                    );
                    ++expected_branch_page_count;
                    ++expected_branch_entry_count;
                ...
                expected_branch_page_count += 2ULL;
                ++expected_branch_entry_count;
                ++active_level_four_upper_lower_child_count;
                ++next_level_four_upper_key_value;
            }
            for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity;
                 ++leaf_entry_count) {
            ...
            expected_branch_page_count += 3ULL;
            ++expected_branch_entry_count;
            ++active_level_four_upper_child_branch_child_count;
            active_level_four_upper_lower_child_count = split_level_four_lower_right_child_count;
            ++next_level_four_upper_key_value;
        }
        while (active_level_four_upper_lower_child_count < branch_capacity) {
            for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity;
        ...
        expected_branch_page_count += 4ULL;
        ++expected_branch_entry_count;
        ++level_four_upper_branch_child_count;
        active_level_four_upper_child_branch_child_count = split_level_four_child_right_child_count;
        active_level_four_upper_lower_child_count = split_level_four_lower_right_child_count;
        ++next_level_four_upper_key_value;
    }
    while (active_level_four_upper_child_branch_child_count < branch_capacity) {
    ...
    assert(level_four_upper_branch_child_count == branch_capacity);
    assert(active_level_four_upper_child_branch_child_count == branch_capacity);
    assert(active_level_four_upper_lower_child_count == branch_capacity);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    ...
    const unsigned long long before_level_four_upper_split_pages = expected_branch_page_count;
    const unsigned long long before_level_four_upper_split_entry_count =
        expected_branch_entry_count;
    const unsigned long long split_level_four_upper_lower_branch_page_id =
        before_level_four_upper_split_pages + 2ULL;
    const unsigned long long split_level_four_upper_child_branch_page_id =
        before_level_four_upper_split_pages + 3ULL;
    const unsigned long long split_level_four_upper_branch_page_id =
        before_level_four_upper_split_pages + 4ULL;
    const unsigned split_level_four_upper_right_child_count =
        (unsigned)branch_capacity + 1U - (((unsigned)branch_capacity + 2U) / 2U);
    const unsigned level_four_upper_split_key_value = next_level_four_upper_key_value;
    unsigned char level_four_upper_split_row[8] = {0};
    unsigned char level_four_upper_split_key[key_size] = {0};
    put_test_u32_le(level_four_upper_split_row, 0U, level_four_upper_split_key_value);
    put_test_u32_be(
        level_four_upper_split_key,
        key_size - sizeof(uint32_t),
        level_four_upper_split_key_value
    );
    mylite_storage_index_entry level_four_upper_split_index_entry = {
        .size = sizeof(level_four_upper_split_index_entry),
        .index_number = 0U,
        .key = level_four_upper_split_key,
        .key_size = sizeof(level_four_upper_split_key),
    };

    statement = NULL;
    unsigned long long level_four_upper_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_four_upper_split_row,
            sizeof(level_four_upper_split_row),
            &level_four_upper_split_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_four_upper_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    assert(
    ...
    read_test_page(filename, split_level_four_upper_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET
    ...
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_four_upper_right_child_count
    );
    read_test_page(
        filename,
        split_level_four_upper_child_branch_page_id,
        promoted_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    read_test_page(
        filename,
        split_level_four_upper_lower_branch_page_id,
        split_lower_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    assert_index_entry_lookup(
        ...
        0U,
        level_four_upper_split_key,
        sizeof(level_four_upper_split_key),
        MYLITE_STORAGE_OK,
        level_four_upper_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_four_upper_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_four_upper_split_entry_count
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    assert(
    ...
    unsigned long long level_four_upper_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_four_upper_split_index_entry,
            1U,
            &level_four_upper_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_four_upper_split_row_id == before_level_four_upper_split_pages);
    expected_branch_page_count += 5ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_four_upper_split_expected_prefix_keys[] = {
        level_four_upper_split_key
    };
    const unsigned long long level_four_upper_split_expected_prefix_row_ids[] = {
        level_four_upper_split_row_id
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_four_upper_split_key,
        sizeof(level_four_upper_split_key),
        level_four_upper_split_expected_prefix_keys,
        sizeof(level_four_upper_split_key),
        level_four_upper_split_expected_prefix_row_ids,
        sizeof(level_four_upper_split_expected_prefix_row_ids) /
            sizeof(level_four_upper_split_expected_prefix_row_ids[0])
    );

    unsigned level_five_root_child_count = 3U;
    unsigned active_level_five_upper_branch_child_count = split_level_four_upper_right_child_count;
    unsigned active_level_five_child_branch_child_count = split_level_four_child_right_child_count;
    unsigned active_level_five_lower_child_count = split_level_four_lower_right_child_count;
    unsigned next_level_five_key_value = level_four_upper_split_key_value + 1U;
    while (level_five_root_child_count < branch_capacity) {
        while (active_level_five_upper_branch_child_count < branch_capacity) {
            while (active_level_five_child_branch_child_count < branch_capacity) {
                while (active_level_five_lower_child_count < branch_capacity) {
                    for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity;
                         ++leaf_entry_count) {
                        append_branch_split_test_row(
                            filename,
                            key_size,
                            next_level_five_key_value,
                            expected_branch_page_count
                        );
                        ++expected_branch_page_count;
                        ++expected_branch_entry_count;
                    ...
                    expected_branch_page_count += 2ULL;
                    ++expected_branch_entry_count;
                    ++active_level_five_lower_child_count;
                    ++next_level_five_key_value;
                }
                for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity;
                     ++leaf_entry_count) {
                ...
                expected_branch_page_count += 3ULL;
                ++expected_branch_entry_count;
                ++active_level_five_child_branch_child_count;
                active_level_five_lower_child_count = split_level_four_lower_right_child_count;
                ++next_level_five_key_value;
            }
            while (active_level_five_lower_child_count < branch_capacity) {
                for (size_t leaf_entry_count = 1U; leaf_entry_count < leaf_capacity;
            ...
            expected_branch_page_count += 4ULL;
            ++expected_branch_entry_count;
            ++active_level_five_upper_branch_child_count;
            active_level_five_child_branch_child_count = split_level_four_child_right_child_count;
            active_level_five_lower_child_count = split_level_four_lower_right_child_count;
            ++next_level_five_key_value;
        }
        while (active_level_five_child_branch_child_count < branch_capacity) {
        ...
        expected_branch_page_count += 5ULL;
        ++expected_branch_entry_count;
        ++level_five_root_child_count;
        active_level_five_upper_branch_child_count = split_level_four_upper_right_child_count;
        active_level_five_child_branch_child_count = split_level_four_child_right_child_count;
        active_level_five_lower_child_count = split_level_four_lower_right_child_count;
        ++next_level_five_key_value;
    }
    ...
    assert(level_five_root_child_count == branch_capacity);
    assert(active_level_five_upper_branch_child_count == branch_capacity);
    assert(active_level_five_child_branch_child_count == branch_capacity);
    assert(active_level_five_lower_child_count == branch_capacity);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    ...
    const unsigned long long before_level_five_promotion_pages = expected_branch_page_count;
    const unsigned long long before_level_five_promotion_entry_count = expected_branch_entry_count;
    const unsigned long long split_level_five_lower_branch_page_id =
        before_level_five_promotion_pages + 2ULL;
    const unsigned long long split_level_five_child_branch_page_id =
        before_level_five_promotion_pages + 3ULL;
    const unsigned long long split_level_five_upper_branch_page_id =
        before_level_five_promotion_pages + 4ULL;
    const unsigned long long promoted_level_five_left_branch_page_id =
        before_level_five_promotion_pages + 5ULL;
    const unsigned long long promoted_level_five_right_branch_page_id =
        before_level_five_promotion_pages + 6ULL;
    const unsigned split_level_five_root_right_child_count =
        (unsigned)branch_capacity + 1U - (((unsigned)branch_capacity + 2U) / 2U);
    const unsigned level_five_promotion_key_value = next_level_five_key_value;
    unsigned char level_five_promotion_row[8] = {0};
    unsigned char level_five_promotion_key[key_size] = {0};
    put_test_u32_le(level_five_promotion_row, 0U, level_five_promotion_key_value);
    put_test_u32_be(
        level_five_promotion_key,
        key_size - sizeof(uint32_t),
        level_five_promotion_key_value
    );
    mylite_storage_index_entry level_five_promotion_index_entry = {
        .size = sizeof(level_five_promotion_index_entry),
        .index_number = 0U,
        .key = level_five_promotion_key,
        .key_size = sizeof(level_five_promotion_key),
    };

    statement = NULL;
    unsigned long long level_five_promotion_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_five_promotion_row,
            sizeof(level_five_promotion_row),
            &level_five_promotion_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_five_promotion_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 5U);
    assert(
        get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET) ==
        2U
    );
    read_test_page(filename, promoted_level_five_right_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET
    ...
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_root_right_child_count
    );
    read_test_page(filename, split_level_five_upper_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET
    ...
    read_test_page(filename, split_level_five_child_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET
    ...
    read_test_page(filename, split_level_five_lower_branch_page_id, split_lower_branch_page_bytes);
    assert(
        get_test_u32_le(
            split_lower_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
    ...
    assert_index_entry_lookup(
        ...
        0U,
        level_five_promotion_key,
        sizeof(level_five_promotion_key),
        MYLITE_STORAGE_OK,
        level_five_promotion_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_five_promotion_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_five_promotion_entry_count
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 4U);
    assert(
    ...
    unsigned long long level_five_promotion_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_five_promotion_index_entry,
            1U,
            &level_five_promotion_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_five_promotion_row_id == before_level_five_promotion_pages);
    expected_branch_page_count += 7ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    ...
    read_test_page(filename, promoted_level_five_left_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET
    ...
    const unsigned char *level_five_promotion_expected_prefix_keys[] = {level_five_promotion_key};
    const unsigned long long level_five_promotion_expected_prefix_row_ids[] = {
        level_five_promotion_row_id
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_five_promotion_key,
        sizeof(level_five_promotion_key),
        level_five_promotion_expected_prefix_keys,
        sizeof(level_five_promotion_key),
        level_five_promotion_expected_prefix_row_ids,
        sizeof(level_five_promotion_expected_prefix_row_ids) /
            sizeof(level_five_promotion_expected_prefix_row_ids[0])
    );

    const unsigned level_five_fit_key_value = level_five_promotion_key_value + 1U;
    unsigned char level_five_fit_row[8] = {0};
    unsigned char level_five_fit_key[key_size] = {0};
    put_test_u32_le(level_five_fit_row, 0U, level_five_fit_key_value);
    put_test_u32_be(level_five_fit_key, key_size - sizeof(uint32_t), level_five_fit_key_value);
    mylite_storage_index_entry level_five_fit_index_entry = {
        .size = sizeof(level_five_fit_index_entry),
        .index_number = 0U,
        .key = level_five_fit_key,
        .key_size = sizeof(level_five_fit_key),
    };

    const unsigned long long before_level_five_fit_pages = expected_branch_page_count;
    const unsigned long long before_level_five_fit_entry_count = expected_branch_entry_count;
    statement = NULL;
    unsigned long long level_five_fit_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            ...
            "posts",
            level_five_fit_row,
            sizeof(level_five_fit_row),
            &level_five_fit_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_five_fit_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 5U);
    assert(
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_five_fit_pages);
    assert_index_root(filename, "app", "posts", 0U, root_page, before_level_five_fit_entry_count);
    assert_index_entry_lookup(
        filename,
        0U,
        level_five_fit_key,
    ...
    unsigned long long level_five_fit_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_five_fit_index_entry,
            1U,
            &level_five_fit_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_five_fit_row_id == before_level_five_fit_pages);
    expected_branch_page_count += 1ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    ...
    const unsigned char *level_five_fit_expected_prefix_keys[] = {level_five_fit_key};
    const unsigned long long level_five_fit_expected_prefix_row_ids[] = {level_five_fit_row_id};
    assert_prefix_index_entries(
        filename,
        0U,
        level_five_fit_key,
        sizeof(level_five_fit_key),
        level_five_fit_expected_prefix_keys,
        sizeof(level_five_fit_key),
        level_five_fit_expected_prefix_row_ids,
        sizeof(level_five_fit_expected_prefix_row_ids) /
            sizeof(level_five_fit_expected_prefix_row_ids[0])
    );

    unsigned next_level_five_split_key_value = level_five_fit_key_value + 1U;
    for (size_t filled_leaf_entries = 2U; filled_leaf_entries < leaf_capacity;
         ++filled_leaf_entries) {
        append_branch_split_test_row(
            filename,
            key_size,
            next_level_five_split_key_value,
            expected_branch_page_count
        );
        ++expected_branch_page_count;
        ++expected_branch_entry_count;
    ...
    const unsigned long long before_level_five_leaf_split_pages = expected_branch_page_count;
    const unsigned long long before_level_five_leaf_split_entry_count = expected_branch_entry_count;
    const unsigned level_five_leaf_split_key_value = next_level_five_split_key_value;
    unsigned char level_five_leaf_split_row[8] = {0};
    unsigned char level_five_leaf_split_key[key_size] = {0};
    put_test_u32_le(level_five_leaf_split_row, 0U, level_five_leaf_split_key_value);
    put_test_u32_be(
        level_five_leaf_split_key,
        key_size - sizeof(uint32_t),
        level_five_leaf_split_key_value
    );
    mylite_storage_index_entry level_five_leaf_split_index_entry = {
        .size = sizeof(level_five_leaf_split_index_entry),
        .index_number = 0U,
        .key = level_five_leaf_split_key,
        .key_size = sizeof(level_five_leaf_split_key),
    };

    statement = NULL;
    unsigned long long level_five_leaf_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_five_leaf_split_row,
            sizeof(level_five_leaf_split_row),
            &level_five_leaf_split_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_five_leaf_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 5U);
    assert(
    ...
    assert(
        get_test_u32_le(
            split_lower_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_four_lower_right_child_count + 1U
    );
    assert_index_entry_lookup(
        filename,
        0U,
        level_five_leaf_split_key,
        sizeof(level_five_leaf_split_key),
        MYLITE_STORAGE_OK,
        level_five_leaf_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_five_leaf_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_five_leaf_split_entry_count
    );
    read_test_page(filename, split_level_five_lower_branch_page_id, split_lower_branch_page_bytes);
    assert(
        get_test_u32_le(
    ...
    unsigned long long level_five_leaf_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_five_leaf_split_index_entry,
            1U,
            &level_five_leaf_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_five_leaf_split_row_id == before_level_five_leaf_split_pages);
    expected_branch_page_count += 2ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_five_leaf_split_expected_prefix_keys[] = {level_five_leaf_split_key};
    const unsigned long long level_five_leaf_split_expected_prefix_row_ids[] = {
        level_five_leaf_split_row_id
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_five_leaf_split_key,
        sizeof(level_five_leaf_split_key),
        level_five_leaf_split_expected_prefix_keys,
        sizeof(level_five_leaf_split_key),
        level_five_leaf_split_expected_prefix_row_ids,
        sizeof(level_five_leaf_split_expected_prefix_row_ids) /
            sizeof(level_five_leaf_split_expected_prefix_row_ids[0])
    );

    unsigned level_five_lower_branch_child_count = split_level_four_lower_right_child_count + 1U;
    unsigned next_level_five_lower_key_value = level_five_leaf_split_key_value + 1U;
    while (level_five_lower_branch_child_count < branch_capacity) {
        for (size_t filled_leaf_entries = 1U; filled_leaf_entries < leaf_capacity;
             ++filled_leaf_entries) {
            append_branch_split_test_row(
                filename,
                key_size,
                next_level_five_lower_key_value,
                expected_branch_page_count
            );
            ++expected_branch_page_count;
            ++expected_branch_entry_count;
        ...
        expected_branch_page_count += 2ULL;
        ++expected_branch_entry_count;
        ++level_five_lower_branch_child_count;
        ++next_level_five_lower_key_value;
    }
    for (size_t filled_leaf_entries = 1U; filled_leaf_entries < leaf_capacity;
         ++filled_leaf_entries) {
    ...
    assert(level_five_lower_branch_child_count == branch_capacity);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    ...
    const unsigned long long before_level_five_lower_split_pages = expected_branch_page_count;
    const unsigned long long before_level_five_lower_split_entry_count =
        expected_branch_entry_count;
    const unsigned long long split_level_five_deep_lower_branch_page_id =
        before_level_five_lower_split_pages + 2ULL;
    const unsigned split_level_five_lower_left_child_count = ((unsigned)branch_capacity + 2U) / 2U;
    const unsigned split_level_five_lower_right_child_count =
        (unsigned)branch_capacity + 1U - split_level_five_lower_left_child_count;
    const unsigned split_level_five_parent_child_count =
        split_level_four_child_right_child_count + 1U;
    const unsigned level_five_lower_split_key_value = next_level_five_lower_key_value;
    unsigned char level_five_lower_split_row[8] = {0};
    unsigned char level_five_lower_split_key[key_size] = {0};
    put_test_u32_le(level_five_lower_split_row, 0U, level_five_lower_split_key_value);
    put_test_u32_be(
        level_five_lower_split_key,
        key_size - sizeof(uint32_t),
        level_five_lower_split_key_value
    );
    mylite_storage_index_entry level_five_lower_split_index_entry = {
        .size = sizeof(level_five_lower_split_index_entry),
        .index_number = 0U,
        .key = level_five_lower_split_key,
        .key_size = sizeof(level_five_lower_split_key),
    };

    statement = NULL;
    unsigned long long level_five_lower_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_five_lower_split_row,
            sizeof(level_five_lower_split_row),
            &level_five_lower_split_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_five_lower_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 5U);
    assert(
    ...
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_parent_child_count
    );
    read_test_page(filename, split_level_five_lower_branch_page_id, split_lower_branch_page_bytes);
    assert(
        get_test_u32_le(
            split_lower_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_lower_left_child_count
    );
    read_test_page(
        filename,
        split_level_five_deep_lower_branch_page_id,
        split_lower_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    assert(
        get_test_u32_le(
            split_lower_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_lower_right_child_count
    );
    assert_index_entry_lookup(
        filename,
        0U,
        level_five_lower_split_key,
        sizeof(level_five_lower_split_key),
        MYLITE_STORAGE_OK,
        level_five_lower_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_five_lower_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_five_lower_split_entry_count
    );
    read_test_page(filename, split_level_five_child_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
    ...
    unsigned long long level_five_lower_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_five_lower_split_index_entry,
            1U,
            &level_five_lower_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_five_lower_split_row_id == before_level_five_lower_split_pages);
    expected_branch_page_count += 3ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_five_lower_split_expected_prefix_keys[] = {
        level_five_lower_split_key
    };
    const unsigned long long level_five_lower_split_expected_prefix_row_ids[] = {
        level_five_lower_split_row_id
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_five_lower_split_key,
        sizeof(level_five_lower_split_key),
        level_five_lower_split_expected_prefix_keys,
        sizeof(level_five_lower_split_key),
        level_five_lower_split_expected_prefix_row_ids,
        sizeof(level_five_lower_split_expected_prefix_row_ids) /
            sizeof(level_five_lower_split_expected_prefix_row_ids[0])
    );

    unsigned level_five_child_branch_child_count = split_level_five_parent_child_count;
    unsigned active_level_five_child_lower_child_count = split_level_five_lower_right_child_count;
    unsigned next_level_five_child_key_value = level_five_lower_split_key_value + 1U;
    while (level_five_child_branch_child_count < branch_capacity) {
        while (active_level_five_child_lower_child_count < branch_capacity) {
            for (size_t filled_leaf_entries = 1U; filled_leaf_entries < leaf_capacity;
                 ++filled_leaf_entries) {
                append_branch_split_test_row(
                    filename,
                    key_size,
                    next_level_five_child_key_value,
                    expected_branch_page_count
                );
                ++expected_branch_page_count;
                ++expected_branch_entry_count;
            ...
            expected_branch_page_count += 2ULL;
            ++expected_branch_entry_count;
            ++active_level_five_child_lower_child_count;
            ++next_level_five_child_key_value;
        }
        for (size_t filled_leaf_entries = 1U; filled_leaf_entries < leaf_capacity;
             ++filled_leaf_entries) {
        ...
        expected_branch_page_count += 3ULL;
        ++expected_branch_entry_count;
        ++level_five_child_branch_child_count;
        active_level_five_child_lower_child_count = split_level_five_lower_right_child_count;
        ++next_level_five_child_key_value;
    }
    while (active_level_five_child_lower_child_count < branch_capacity) {
        for (size_t filled_leaf_entries = 1U; filled_leaf_entries < leaf_capacity;
    ...
    assert(level_five_child_branch_child_count == branch_capacity);
    assert(active_level_five_child_lower_child_count == branch_capacity);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    ...
    const unsigned long long before_level_five_child_split_pages = expected_branch_page_count;
    const unsigned long long before_level_five_child_split_entry_count =
        expected_branch_entry_count;
    const unsigned long long split_level_five_child_lower_branch_page_id =
        before_level_five_child_split_pages + 2ULL;
    const unsigned long long split_level_five_deep_child_branch_page_id =
        before_level_five_child_split_pages + 3ULL;
    const unsigned split_level_five_child_left_child_count = ((unsigned)branch_capacity + 2U) / 2U;
    const unsigned split_level_five_child_right_child_count =
        (unsigned)branch_capacity + 1U - split_level_five_child_left_child_count;
    const unsigned split_level_five_upper_child_count =
        split_level_four_upper_right_child_count + 1U;
    const unsigned level_five_child_split_key_value = next_level_five_child_key_value;
    unsigned char level_five_child_split_row[8] = {0};
    unsigned char level_five_child_split_key[key_size] = {0};
    put_test_u32_le(level_five_child_split_row, 0U, level_five_child_split_key_value);
    put_test_u32_be(
        level_five_child_split_key,
        key_size - sizeof(uint32_t),
        level_five_child_split_key_value
    );
    mylite_storage_index_entry level_five_child_split_index_entry = {
        .size = sizeof(level_five_child_split_index_entry),
        .index_number = 0U,
        .key = level_five_child_split_key,
        .key_size = sizeof(level_five_child_split_key),
    };

    statement = NULL;
    unsigned long long level_five_child_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_five_child_split_row,
            sizeof(level_five_child_split_row),
            &level_five_child_split_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_five_child_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 5U);
    read_test_page(filename, split_level_five_upper_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_upper_child_count
    );
    read_test_page(filename, split_level_five_child_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_child_left_child_count
    );
    read_test_page(
        filename,
        split_level_five_deep_child_branch_page_id,
        promoted_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_child_right_child_count
    );
    read_test_page(
        filename,
        split_level_five_child_lower_branch_page_id,
        split_lower_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    assert_index_entry_lookup(
        ...
        0U,
        level_five_child_split_key,
        sizeof(level_five_child_split_key),
        MYLITE_STORAGE_OK,
        level_five_child_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_five_child_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_five_child_split_entry_count
    );
    read_test_page(filename, split_level_five_upper_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
    ...
    unsigned long long level_five_child_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_five_child_split_index_entry,
            1U,
            &level_five_child_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_five_child_split_row_id == before_level_five_child_split_pages);
    expected_branch_page_count += 4ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_five_child_split_expected_prefix_keys[] = {
        level_five_child_split_key
    };
    const unsigned long long level_five_child_split_expected_prefix_row_ids[] = {
        level_five_child_split_row_id
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_five_child_split_key,
        sizeof(level_five_child_split_key),
        level_five_child_split_expected_prefix_keys,
        sizeof(level_five_child_split_key),
        level_five_child_split_expected_prefix_row_ids,
        sizeof(level_five_child_split_expected_prefix_row_ids) /
            sizeof(level_five_child_split_expected_prefix_row_ids[0])
    );

    unsigned level_five_upper_branch_child_count = split_level_five_upper_child_count;
    unsigned active_level_five_upper_child_branch_child_count =
        split_level_five_child_right_child_count;
    unsigned active_level_five_upper_lower_child_count = split_level_five_lower_right_child_count;
    unsigned next_level_five_upper_key_value = level_five_child_split_key_value + 1U;
    while (level_five_upper_branch_child_count < branch_capacity) {
        while (active_level_five_upper_child_branch_child_count < branch_capacity) {
            while (active_level_five_upper_lower_child_count < branch_capacity) {
                for (size_t filled_leaf_entries = 1U; filled_leaf_entries < leaf_capacity;
                     ++filled_leaf_entries) {
                    append_branch_split_test_row(
                        filename,
                        key_size,
                        next_level_five_upper_key_value,
                        expected_branch_page_count
                    );
                    ++expected_branch_page_count;
                    ++expected_branch_entry_count;
                ...
                expected_branch_page_count += 2ULL;
                ++expected_branch_entry_count;
                ++active_level_five_upper_lower_child_count;
                ++next_level_five_upper_key_value;
            }
            for (size_t filled_leaf_entries = 1U; filled_leaf_entries < leaf_capacity;
                 ++filled_leaf_entries) {
            ...
            expected_branch_page_count += 3ULL;
            ++expected_branch_entry_count;
            ++active_level_five_upper_child_branch_child_count;
            active_level_five_upper_lower_child_count = split_level_five_lower_right_child_count;
            ++next_level_five_upper_key_value;
        }
        while (active_level_five_upper_lower_child_count < branch_capacity) {
            for (size_t filled_leaf_entries = 1U; filled_leaf_entries < leaf_capacity;
        ...
        expected_branch_page_count += 4ULL;
        ++expected_branch_entry_count;
        ++level_five_upper_branch_child_count;
        active_level_five_upper_child_branch_child_count = split_level_five_child_right_child_count;
        active_level_five_upper_lower_child_count = split_level_five_lower_right_child_count;
        ++next_level_five_upper_key_value;
    }
    while (active_level_five_upper_child_branch_child_count < branch_capacity) {
    ...
    assert(level_five_upper_branch_child_count == branch_capacity);
    assert(active_level_five_upper_child_branch_child_count == branch_capacity);
    assert(active_level_five_upper_lower_child_count == branch_capacity);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    ...
    const unsigned long long before_level_five_upper_split_pages = expected_branch_page_count;
    const unsigned long long before_level_five_upper_split_entry_count =
        expected_branch_entry_count;
    const unsigned long long split_level_five_upper_lower_branch_page_id =
        before_level_five_upper_split_pages + 2ULL;
    const unsigned long long split_level_five_upper_child_branch_page_id =
        before_level_five_upper_split_pages + 3ULL;
    const unsigned long long split_level_five_deep_upper_branch_page_id =
        before_level_five_upper_split_pages + 4ULL;
    const unsigned split_level_five_upper_left_child_count = ((unsigned)branch_capacity + 2U) / 2U;
    const unsigned split_level_five_upper_right_child_count =
        (unsigned)branch_capacity + 1U - split_level_five_upper_left_child_count;
    const unsigned split_level_five_level_four_child_count =
        split_level_five_root_right_child_count + 1U;
    const unsigned level_five_upper_split_key_value = next_level_five_upper_key_value;
    unsigned char level_five_upper_split_row[8] = {0};
    unsigned char level_five_upper_split_key[key_size] = {0};
    put_test_u32_le(level_five_upper_split_row, 0U, level_five_upper_split_key_value);
    put_test_u32_be(
        level_five_upper_split_key,
        key_size - sizeof(uint32_t),
        level_five_upper_split_key_value
    );
    mylite_storage_index_entry level_five_upper_split_index_entry = {
        .size = sizeof(level_five_upper_split_index_entry),
        .index_number = 0U,
        .key = level_five_upper_split_key,
        .key_size = sizeof(level_five_upper_split_key),
    };

    statement = NULL;
    unsigned long long level_five_upper_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_five_upper_split_row,
            sizeof(level_five_upper_split_row),
            &level_five_upper_split_index_entry,
            1U,
    ...
    assert_index_root(
        ...
        0U,
        root_page,
        before_level_five_upper_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 5U);
    assert(
    ...
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_level_four_child_count
    );
    read_test_page(filename, split_level_five_upper_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_upper_left_child_count
    );
    read_test_page(
        filename,
        split_level_five_deep_upper_branch_page_id,
        promoted_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_upper_right_child_count
    );
    read_test_page(
        filename,
        split_level_five_upper_child_branch_page_id,
        promoted_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    read_test_page(
        filename,
        split_level_five_upper_lower_branch_page_id,
        split_lower_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    assert_index_entry_lookup(
        ...
        0U,
        level_five_upper_split_key,
        sizeof(level_five_upper_split_key),
        MYLITE_STORAGE_OK,
        level_five_upper_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_five_upper_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_five_upper_split_entry_count
    );
    read_test_page(filename, promoted_level_five_right_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
    ...
    unsigned long long level_five_upper_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_five_upper_split_index_entry,
            1U,
            &level_five_upper_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_five_upper_split_row_id == before_level_five_upper_split_pages);
    expected_branch_page_count += 5ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_five_upper_split_expected_prefix_keys[] = {
        level_five_upper_split_key
    };
    const unsigned long long level_five_upper_split_expected_prefix_row_ids[] = {
        level_five_upper_split_row_id
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_five_upper_split_key,
        sizeof(level_five_upper_split_key),
        level_five_upper_split_expected_prefix_keys,
        sizeof(level_five_upper_split_key),
        level_five_upper_split_expected_prefix_row_ids,
        sizeof(level_five_upper_split_expected_prefix_row_ids) /
            sizeof(level_five_upper_split_expected_prefix_row_ids[0])
    );

    unsigned level_five_level_four_branch_child_count = split_level_five_level_four_child_count;
    unsigned active_level_five_level_four_upper_child_count =
        split_level_five_upper_right_child_count;
    unsigned active_level_five_level_four_child_child_count =
        split_level_five_child_right_child_count;
    unsigned active_level_five_level_four_lower_child_count =
        split_level_five_lower_right_child_count;
    unsigned next_level_five_level_four_key_value = level_five_upper_split_key_value + 1U;
    while (level_five_level_four_branch_child_count < branch_capacity) {
        while (active_level_five_level_four_upper_child_count < branch_capacity) {
            while (active_level_five_level_four_child_child_count < branch_capacity) {
                while (active_level_five_level_four_lower_child_count < branch_capacity) {
                    for (size_t filled_leaf_entries = 1U; filled_leaf_entries < leaf_capacity;
                         ++filled_leaf_entries) {
                        append_branch_split_test_row(
                            filename,
                            key_size,
                            next_level_five_level_four_key_value,
                            expected_branch_page_count
                        );
                        ++expected_branch_page_count;
                        ++expected_branch_entry_count;
                    ...
                    expected_branch_page_count += 2ULL;
                    ++expected_branch_entry_count;
                    ++active_level_five_level_four_lower_child_count;
                    ++next_level_five_level_four_key_value;
                }
                for (size_t filled_leaf_entries = 1U; filled_leaf_entries < leaf_capacity;
                     ++filled_leaf_entries) {
                ...
                expected_branch_page_count += 3ULL;
                ++expected_branch_entry_count;
                ++active_level_five_level_four_child_child_count;
                active_level_five_level_four_lower_child_count =
                    split_level_five_lower_right_child_count;
                ++next_level_five_level_four_key_value;
            }
            ...
            expected_branch_page_count += 4ULL;
            ++expected_branch_entry_count;
            ++active_level_five_level_four_upper_child_count;
            active_level_five_level_four_child_child_count =
                split_level_five_child_right_child_count;
            active_level_five_level_four_lower_child_count =
                split_level_five_lower_right_child_count;
        ...
        expected_branch_page_count += 5ULL;
        ++expected_branch_entry_count;
        ++level_five_level_four_branch_child_count;
        active_level_five_level_four_upper_child_count = split_level_five_upper_right_child_count;
        active_level_five_level_four_child_child_count = split_level_five_child_right_child_count;
        active_level_five_level_four_lower_child_count = split_level_five_lower_right_child_count;
        ++next_level_five_level_four_key_value;
    }
    while (active_level_five_level_four_upper_child_count < branch_capacity) {
        while (active_level_five_level_four_child_child_count < branch_capacity) {
    ...
    assert(level_five_level_four_branch_child_count == branch_capacity);
    assert(active_level_five_level_four_upper_child_count == branch_capacity);
    assert(active_level_five_level_four_child_child_count == branch_capacity);
    assert(active_level_five_level_four_lower_child_count == branch_capacity);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    assert_index_root(filename, "app", "posts", 0U, root_page, expected_branch_entry_count);
    read_test_page(filename, root_page, root_page_bytes);
    ...
    const unsigned long long before_level_five_level_four_split_pages = expected_branch_page_count;
    const unsigned long long before_level_five_level_four_split_entry_count =
        expected_branch_entry_count;
    const unsigned long long split_level_five_level_four_lower_branch_page_id =
        before_level_five_level_four_split_pages + 2ULL;
    const unsigned long long split_level_five_level_four_child_branch_page_id =
        before_level_five_level_four_split_pages + 3ULL;
    const unsigned long long split_level_five_level_four_upper_branch_page_id =
        before_level_five_level_four_split_pages + 4ULL;
    const unsigned long long split_level_five_deep_level_four_branch_page_id =
        before_level_five_level_four_split_pages + 5ULL;
    const unsigned split_level_five_level_four_left_child_count =
        ((unsigned)branch_capacity + 2U) / 2U;
    const unsigned split_level_five_level_four_right_child_count =
        (unsigned)branch_capacity + 1U - split_level_five_level_four_left_child_count;
    const unsigned split_level_five_root_child_count = 3U;
    const unsigned level_five_level_four_split_key_value = next_level_five_level_four_key_value;
    unsigned char level_five_level_four_split_row[8] = {0};
    unsigned char level_five_level_four_split_key[key_size] = {0};
    put_test_u32_le(level_five_level_four_split_row, 0U, level_five_level_four_split_key_value);
    put_test_u32_be(
        level_five_level_four_split_key,
        key_size - sizeof(uint32_t),
        level_five_level_four_split_key_value
    );
    mylite_storage_index_entry level_five_level_four_split_index_entry = {
        .size = sizeof(level_five_level_four_split_index_entry),
        .index_number = 0U,
        .key = level_five_level_four_split_key,
        .key_size = sizeof(level_five_level_four_split_key),
    };

    statement = NULL;
    unsigned long long level_five_level_four_split_rolled_back_row_id = 0ULL;
    assert(mylite_storage_begin_statement(filename, &statement) == MYLITE_STORAGE_OK);
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            "posts",
            level_five_level_four_split_row,
            sizeof(level_five_level_four_split_row),
            &level_five_level_four_split_index_entry,
            1U,
    ...
    assert(access(journal_filename, F_OK) == 0);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_five_level_four_split_pages + 6ULL);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_five_level_four_split_entry_count + 1ULL
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 5U);
    assert(
        get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET) ==
        split_level_five_root_child_count
    );
    read_test_page(filename, promoted_level_five_right_branch_page_id, promoted_branch_page_bytes);
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_level_four_left_child_count
    );
    read_test_page(
        filename,
        split_level_five_deep_level_four_branch_page_id,
        promoted_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    assert(
        get_test_u32_le(
            promoted_branch_page_bytes,
            MYLITE_STORAGE_FORMAT_INDEX_BRANCH_CHILD_COUNT_OFFSET
        ) == split_level_five_level_four_right_child_count
    );
    read_test_page(
        filename,
        split_level_five_level_four_upper_branch_page_id,
        promoted_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    read_test_page(
        filename,
        split_level_five_level_four_child_branch_page_id,
        promoted_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    read_test_page(
        filename,
        split_level_five_level_four_lower_branch_page_id,
        split_lower_branch_page_bytes
    );
    assert(
        get_test_u32_le(
    ...
    assert_index_entry_lookup(
        ...
        0U,
        level_five_level_four_split_key,
        sizeof(level_five_level_four_split_key),
        MYLITE_STORAGE_OK,
        level_five_level_four_split_rolled_back_row_id
    );
    assert(mylite_storage_rollback_statement(statement) == MYLITE_STORAGE_OK);
    ...
    assert_file_size_matches_header(filename);
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == before_level_five_level_four_split_pages);
    assert_index_root(
        filename,
        "app",
        "posts",
        0U,
        root_page,
        before_level_five_level_four_split_entry_count
    );
    read_test_page(filename, root_page, root_page_bytes);
    assert(get_test_u32_le(root_page_bytes, MYLITE_STORAGE_FORMAT_INDEX_BRANCH_LEVEL_OFFSET) == 5U);
    assert(
    ...
    unsigned long long level_five_level_four_split_row_id = 0ULL;
    assert(
        mylite_storage_append_row_with_index_entries(
            filename,
            "app",
            ...
            &level_five_level_four_split_index_entry,
            1U,
            &level_five_level_four_split_row_id
        ) == MYLITE_STORAGE_OK
    );
    assert(level_five_level_four_split_row_id == before_level_five_level_four_split_pages);
    expected_branch_page_count += 6ULL;
    ++expected_branch_entry_count;
    assert(mylite_storage_open_header(filename, &header) == MYLITE_STORAGE_OK);
    assert(header.page_count == expected_branch_page_count);
    ...
    const unsigned char *level_five_level_four_split_expected_prefix_keys[] = {
        level_five_level_four_split_key
    };
    const unsigned long long level_five_level_four_split_expected_prefix_row_ids[] = {
        level_five_level_four_split_row_id
    };
    assert_prefix_index_entries(
        filename,
        ...
        level_five_level_four_split_key,
        sizeof(level_five_level_four_split_key),
        level_five_level_four_split_expected_prefix_keys,
        sizeof(level_five_level_four_split_key),
        level_five_level_four_split_expected_prefix_row_ids,
        sizeof(level_five_level_four_split_expected_prefix_row_ids) /
            sizeof(level_five_level_four_split_expected_prefix_row_ids[0])
    );
    ...
    assert(entries.entry_count == (size_t)expected_branch_entry_count);
...
static void append_branch_split_test_row(
    ...
    put_test_u32_be(key, key_size - sizeof(uint32_t), key_value);
    mylite_storage_index_entry index_entry = {
        .size = sizeof(index_entry),
        .index_number = 0U,
        .key = key,
    ...
    assert(row_id == expected_row_id);
    assert_index_entry_lookup(filename, 0U, key, key_size, MYLITE_STORAGE_OK, row_id);
}
