...
#define PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_PRINT_LIMIT 5U
#define PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_FNV_OFFSET 1469598103934665603ULL
#define PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_FNV_PRIME 1099511628211ULL
...
enum php_mylite_mysqli_libmylite_exec_result_profile_index {
    ...
    PHP_MYLITE_MYSQLI_LIBMYLITE_EXEC_RESULT_PROFILE_NATIVE_CONTROL_COMMIT_CALLS,
    PHP_MYLITE_MYSQLI_LIBMYLITE_EXEC_RESULT_PROFILE_NATIVE_CONTROL_ROLLBACK_CALLS,
...
typedef enum php_mylite_mysqli_profile_query_verb {
    ...
    PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_SHOW,
    PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_DML,
    PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_DDL,
    PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_SET,
    PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_USE,
    PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_TRANSACTION,
    PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_LOCK,
    PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_CALL,
...
typedef struct php_mylite_mysqli_profile_stats {
    ...
    struct {
        ...
        char sample[PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_SAMPLE_SIZE + 1U];
    } query_shapes[PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY];

    uint64_t query_shape_overflow_calls;
    uint64_t query_shape_overflow_ns;
    ...
    uint64_t libmylite_exec_result_native_control_commit_calls;
    uint64_t libmylite_exec_result_native_control_rollback_calls;
...
static const char *const
    php_mylite_mysqli_profile_query_verb_names[PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_COUNT] = {
        "other",
        "select",
        "show",
        "dml",
...
static void php_mylite_mysqli_profile_print_text(const char *name, const char *value);
...
static void php_mylite_mysqli_profile_record_query_shape(
    const char *sql,
    size_t sql_len,
    php_mylite_mysqli_profile_query_verb verb,
    uint64_t elapsed
);
static void php_mylite_mysqli_profile_normalize_query_shape(
    const char *sql,
    size_t sql_len,
    char *sample,
    size_t sample_size,
...
static bool php_mylite_mysqli_no_result_query_preserves_recent_result(
    const char *sql,
    size_t sql_len
);
...
static bool php_mylite_mysqli_read_sql_keyword(
    const char *sql,
    size_t sql_len,
    size_t *offset,
    const char **keyword,
...
static bool php_mylite_mysqli_sql_has_only_optional_trailing_semicolon(
    const char *sql,
    size_t sql_len,
    size_t offset
);
...
static void php_mylite_mysqli_profile_print_text(const char *name, const char *value) {
    FILE *output = php_mylite_mysqli_profile_output_file != NULL
                       ? php_mylite_mysqli_profile_output_file
                       : stderr;
    fprintf(output, "mylite_mysqli_profile_%s=%s\n", name, value != NULL ? value : "");
}
...
static void php_mylite_mysqli_profile_print_query_verb(php_mylite_mysqli_profile_query_verb verb) {
    ...
    const char *name = php_mylite_mysqli_profile_query_verb_names[verb];
    char key[96];
    snprintf(key, sizeof(key), "query_verb_%s_calls", name);
    php_mylite_mysqli_profile_print_counter(key, php_mylite_mysqli_profile.query_verb_calls[verb]);
    snprintf(key, sizeof(key), "query_verb_%s_ms_total", name);
...
static void php_mylite_mysqli_profile_print_query_shapes(void) {
    bool printed[PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY] = {false};
    unsigned tracked_count = 0;
    for (unsigned i = 0; i < PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY; ++i) {
        if (php_mylite_mysqli_profile.query_shapes[i].calls > 0U) {
            ++tracked_count;
        }
    }

    const unsigned print_count = tracked_count < PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_PRINT_LIMIT
                                     ? tracked_count
                                     : PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_PRINT_LIMIT;
    php_mylite_mysqli_profile_print_counter("query_shape_top_count", print_count);
    php_mylite_mysqli_profile_print_counter(
        "query_shape_overflow_calls",
        php_mylite_mysqli_profile.query_shape_overflow_calls
    );
    php_mylite_mysqli_profile_print_millis(
        "query_shape_overflow_ms_total",
        php_mylite_mysqli_profile.query_shape_overflow_ns
    );

    for (unsigned rank = 1; rank <= print_count; ++rank) {
        unsigned best_index = PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY;
        uint64_t best_ns = 0;
        for (unsigned i = 0; i < PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY; ++i) {
            if (printed[i] || php_mylite_mysqli_profile.query_shapes[i].calls == 0U) {
                continue;
            }
            if (best_index == PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY ||
                php_mylite_mysqli_profile.query_shapes[i].ns > best_ns) {
                best_index = i;
                best_ns = php_mylite_mysqli_profile.query_shapes[i].ns;
            }
        }
        if (best_index == PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY) {
            break;
        ...
        const php_mylite_mysqli_profile_query_verb verb =
            php_mylite_mysqli_profile.query_shapes[best_index].verb;
        const char *verb_name = verb < PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_COUNT
                                    ? php_mylite_mysqli_profile_query_verb_names[verb]
                                    : "other";
        char key[96];
        char value[64];
        snprintf(key, sizeof(key), "query_shape_%u_hash", rank);
        snprintf(
            value,
            sizeof(value),
            "%016" PRIx64,
            php_mylite_mysqli_profile.query_shapes[best_index].hash
        );
        php_mylite_mysqli_profile_print_text(key, value);
        snprintf(key, sizeof(key), "query_shape_%u_calls", rank);
        php_mylite_mysqli_profile_print_counter(
            key,
            php_mylite_mysqli_profile.query_shapes[best_index].calls
        );
        snprintf(key, sizeof(key), "query_shape_%u_ms_total", rank);
        php_mylite_mysqli_profile_print_millis(
            key,
            php_mylite_mysqli_profile.query_shapes[best_index].ns
        );
        snprintf(key, sizeof(key), "query_shape_%u_verb", rank);
        php_mylite_mysqli_profile_print_text(key, verb_name);
        snprintf(key, sizeof(key), "query_shape_%u_sample", rank);
        php_mylite_mysqli_profile_print_text(
            key,
            php_mylite_mysqli_profile.query_shapes[best_index].sample
        );
    }
}
...
static void php_mylite_mysqli_profile_record_query_shape(
    ...
    if (!php_mylite_mysqli_profile_enabled || sql == NULL || sql_len == 0U) {
        return;
    }

    char sample[PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_SAMPLE_SIZE + 1U];
    uint64_t hash = 0;
    php_mylite_mysqli_profile_normalize_query_shape(sql, sql_len, sample, sizeof(sample), &hash);

    unsigned empty_index = PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY;
    unsigned min_index = PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY;
    uint64_t min_ns = 0;
    for (unsigned i = 0; i < PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY; ++i) {
        if (php_mylite_mysqli_profile.query_shapes[i].calls == 0U) {
            if (empty_index == PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY) {
                empty_index = i;
            }
            continue;
        }
        if (php_mylite_mysqli_profile.query_shapes[i].hash == hash) {
            ++php_mylite_mysqli_profile.query_shapes[i].calls;
            php_mylite_mysqli_profile.query_shapes[i].ns += elapsed;
            return;
        }
        if (min_index == PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY ||
            php_mylite_mysqli_profile.query_shapes[i].ns < min_ns) {
            min_index = i;
            min_ns = php_mylite_mysqli_profile.query_shapes[i].ns;
        }
    }

    unsigned target_index = empty_index;
    if (target_index == PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY) {
        if (min_index == PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_CAPACITY || elapsed <= min_ns) {
            ++php_mylite_mysqli_profile.query_shape_overflow_calls;
            php_mylite_mysqli_profile.query_shape_overflow_ns += elapsed;
            return;
        }
        target_index = min_index;
        php_mylite_mysqli_profile.query_shape_overflow_calls +=
            php_mylite_mysqli_profile.query_shapes[target_index].calls;
        php_mylite_mysqli_profile.query_shape_overflow_ns +=
            php_mylite_mysqli_profile.query_shapes[target_index].ns;
    }

    php_mylite_mysqli_profile.query_shapes[target_index].hash = hash;
    php_mylite_mysqli_profile.query_shapes[target_index].calls = 1;
    php_mylite_mysqli_profile.query_shapes[target_index].ns = elapsed;
    php_mylite_mysqli_profile.query_shapes[target_index].verb = verb;
    memcpy(
        php_mylite_mysqli_profile.query_shapes[target_index].sample,
        sample,
        sizeof(php_mylite_mysqli_profile.query_shapes[target_index].sample)
    );
}

static char php_mylite_mysqli_profile_shape_upper(char ch) {
    return ch >= 'a' && ch <= 'z' ? (char)(ch - ('a' - 'A')) : ch;
}

static bool php_mylite_mysqli_profile_shape_space(char ch) {
    return ch == ' ' || ch == '\t' || ch == '\n' || ch == '\r' || ch == '\f';
}

static bool php_mylite_mysqli_profile_shape_digit(char ch) {
    return ch >= '0' && ch <= '9';
}

static uint64_t php_mylite_mysqli_profile_shape_hash(uint64_t hash, char ch) {
    return (hash ^ (unsigned char)ch) * PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_FNV_PRIME;
}

static void php_mylite_mysqli_profile_shape_emit(
    char ch,
    char *sample,
    size_t sample_size,
    size_t *sample_len,
    ...
    if (ch == '|') {
        ch = '/';
    } else if ((unsigned char)ch < 0x20U || ch == 0x7f) {
        ch = ' ';
    }
    *hash = php_mylite_mysqli_profile_shape_hash(*hash, ch);
    if (*sample_len + 1U < sample_size) {
        sample[(*sample_len)++] = ch;
        sample[*sample_len] = '\0';
    }
}
...
static void php_mylite_mysqli_profile_normalize_query_shape(
    ...
    uint64_t hash = PHP_MYLITE_MYSQLI_PROFILE_QUERY_SHAPE_FNV_OFFSET;
    size_t sample_len = 0;
    ...
    for (size_t i = 0; i < sql_len;) {
        const char ch = sql[i];
        if (php_mylite_mysqli_profile_shape_space(ch)) {
        ...
        if (pending_space && emitted) {
            php_mylite_mysqli_profile_shape_emit(' ', sample, sample_size, &sample_len, &hash);
            pending_space = false;
        }
        ...
        if (ch == '\'' || ch == '"') {
            ...
            ++i;
            while (i < sql_len) {
                if (sql[i] == '\\' && i + 1U < sql_len) {
                    i += 2U;
                    continue;
                }
                if (sql[i] == quote) {
                    ++i;
                    if (i < sql_len && sql[i] == quote) {
                        ++i;
                        continue;
                    }
                    break;
        ...
        if (php_mylite_mysqli_profile_shape_digit(ch)) {
            php_mylite_mysqli_profile_shape_emit('?', sample, sample_size, &sample_len, &hash);
            ++i;
            while (i < sql_len) {
                const char value = sql[i];
                if (php_mylite_mysqli_profile_shape_digit(value) || value == '.' || value == 'x' ||
                    value == 'X' || value == 'e' || value == 'E' || value == '+' || value == '-' ||
        ...
        php_mylite_mysqli_profile_shape_emit(
            php_mylite_mysqli_profile_shape_upper(ch),
            sample,
            sample_size,
            &sample_len,
            &hash
    ...
    while (sample_len > 0U && sample[sample_len - 1U] == ' ') {
        sample[--sample_len] = '\0';
    }
    if (out_hash != NULL) {
        *out_hash = hash;
...
static void php_mylite_mysqli_profile_read_libmylite_exec_result(void) {
    ...
    php_mylite_mysqli_profile.libmylite_exec_result_native_control_calls =
        values[PHP_MYLITE_MYSQLI_LIBMYLITE_EXEC_RESULT_PROFILE_NATIVE_CONTROL_CALLS];
    php_mylite_mysqli_profile.libmylite_exec_result_native_control_ns =
    ...
    php_mylite_mysqli_profile.libmylite_exec_result_native_control_commit_calls =
        values[PHP_MYLITE_MYSQLI_LIBMYLITE_EXEC_RESULT_PROFILE_NATIVE_CONTROL_COMMIT_CALLS];
    php_mylite_mysqli_profile.libmylite_exec_result_native_control_rollback_calls =
        values[PHP_MYLITE_MYSQLI_LIBMYLITE_EXEC_RESULT_PROFILE_NATIVE_CONTROL_ROLLBACK_CALLS];
}
...
static void php_mylite_mysqli_profile_print(void) {
    ...
    for (php_mylite_mysqli_profile_query_verb verb = PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_OTHER;
         verb < PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_COUNT;
    ...
    php_mylite_mysqli_profile_print_query_shapes();
    php_mylite_mysqli_profile_print_counter(
        "exec_result_calls",
    ...
    php_mylite_mysqli_profile_print_counter(
        "libmylite_exec_result_native_control_calls",
        php_mylite_mysqli_profile.libmylite_exec_result_native_control_calls
    );
    php_mylite_mysqli_profile_print_millis(
        "libmylite_exec_result_native_control_ms_total",
        php_mylite_mysqli_profile.libmylite_exec_result_native_control_ns
    );
    php_mylite_mysqli_profile_print_counter(
...
static int php_mylite_mysqli_profile_finish_query(
    ...
    if (php_mylite_mysqli_profile_enabled) {
        ...
        php_mylite_mysqli_profile.query_ns += elapsed;
        php_mylite_mysqli_profile_record_query_shape(sql, sql_len, verb, elapsed);
        if (verb < PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_COUNT) {
...
static int php_mylite_mysqli_query_impl(
    ...
    const bool no_result_preserves_recent_result =
        no_result_preserves_cache &&
        php_mylite_mysqli_no_result_query_preserves_recent_result(sql, sql_len);
    php_mylite_mysqli_profile_add_elapsed(
        &php_mylite_mysqli_profile.query_classify_ns,
        subphase_start
    );
    ...
    if (is_no_result_query) {
        ...
        if (no_result_preserves_cache) {
            ...
            if (!no_result_preserves_recent_result) {
                php_mylite_mysqli_clear_recent_result_sql(link);
            }
        } else {
            php_mylite_mysqli_clear_query_cache(link);
...
static php_mylite_mysqli_profile_query_verb php_mylite_mysqli_profile_classify_query_verb(
    ...
    const char *keyword = sql + keyword_start;
    if (php_mylite_mysqli_keyword_equals(keyword, keyword_len, "SELECT")) {
        return PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_SELECT;
    }
    if (php_mylite_mysqli_keyword_equals(keyword, keyword_len, "SHOW") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "DESCRIBE") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "DESC") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "EXPLAIN")) {
        return PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_SHOW;
    }
    if (php_mylite_mysqli_keyword_equals(keyword, keyword_len, "DELETE") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "INSERT") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "REPLACE") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "UPDATE")) {
        return PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_DML;
    }
    if (php_mylite_mysqli_keyword_equals(keyword, keyword_len, "ALTER") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "CREATE") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "DROP") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "TRUNCATE")) {
        return PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_DDL;
    }
    if (php_mylite_mysqli_keyword_equals(keyword, keyword_len, "SET")) {
        return PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_SET;
    }
    if (php_mylite_mysqli_keyword_equals(keyword, keyword_len, "USE")) {
        return PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_USE;
    }
    if (php_mylite_mysqli_keyword_equals(keyword, keyword_len, "BEGIN") ||
        ...
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "SAVEPOINT") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "RELEASE")) {
        return PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_TRANSACTION;
    }
    if (php_mylite_mysqli_keyword_equals(keyword, keyword_len, "LOCK") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "UNLOCK")) {
        return PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_LOCK;
    }
    if (php_mylite_mysqli_keyword_equals(keyword, keyword_len, "CALL")) {
        return PHP_MYLITE_MYSQLI_PROFILE_QUERY_VERB_CALL;
    }
...
static bool php_mylite_mysqli_no_result_query_preserves_cache(const char *sql, size_t sql_len) {
    ...
    if (!php_mylite_mysqli_read_sql_keyword(sql, sql_len, &offset, &keyword, &keyword_len)) {
        return false;
    }
    ...
    if (php_mylite_mysqli_keyword_equals(keyword, keyword_len, "BEGIN") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "COMMIT") ||
        php_mylite_mysqli_keyword_equals(keyword, keyword_len, "ROLLBACK")) {
        const char *optional_work = NULL;
        size_t optional_work_len = 0;
        size_t trailing_offset = offset;
        if (php_mylite_mysqli_read_sql_keyword(
                sql,
                sql_len,
                &trailing_offset,
                &optional_work,
            ...
            if (!php_mylite_mysqli_keyword_equals(optional_work, optional_work_len, "WORK")) {
                return false;
            }
        }
        return php_mylite_mysqli_sql_has_only_optional_trailing_semicolon(
            sql,
            sql_len,
            trailing_offset
        );
    ...
    if (php_mylite_mysqli_keyword_equals(keyword, keyword_len, "START")) {
        const char *transaction_keyword = NULL;
        size_t transaction_keyword_len = 0;
        if (!php_mylite_mysqli_read_sql_keyword(
                sql,
                sql_len,
                &offset,
                &transaction_keyword,
                &transaction_keyword_len
            ) ||
            !php_mylite_mysqli_keyword_equals(
                transaction_keyword,
                transaction_keyword_len,
                "TRANSACTION"
            )) {
        ...
        return php_mylite_mysqli_sql_has_only_optional_trailing_semicolon(sql, sql_len, offset);
    }
...
static bool php_mylite_mysqli_no_result_query_preserves_recent_result(
    ...
    return php_mylite_mysqli_keyword_equals(keyword, keyword_len, "BEGIN") ||
           php_mylite_mysqli_keyword_equals(keyword, keyword_len, "COMMIT") ||
           php_mylite_mysqli_keyword_equals(keyword, keyword_len, "ROLLBACK") ||
           php_mylite_mysqli_keyword_equals(keyword, keyword_len, "START");
}
...
static bool php_mylite_mysqli_read_sql_keyword(
    ...
    if (sql == NULL || offset == NULL || keyword == NULL || keyword_len == NULL) {
        return false;
    }
    ...
    if (*offset == keyword_start) {
        return false;
    }

    *keyword = sql + keyword_start;
