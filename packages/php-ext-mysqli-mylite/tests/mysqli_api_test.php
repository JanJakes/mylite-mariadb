...
$emptyResult = $db->query('SELECT id AS alias_id, name FROM people WHERE id = 999');
expect_true($emptyResult instanceof MyLite\MySQLiResult, 'empty SELECT did not return MySQLiResult');
expect_true($emptyResult->num_rows === 0, 'empty SELECT num_rows mismatch');
$emptyField = $emptyResult->fetch_field();
expect_true($emptyField->name === 'alias_id', 'empty SELECT field alias name mismatch');
expect_true($emptyField->orgname === 'id', 'empty SELECT field original name mismatch');
expect_true($emptyField->table === 'people', 'empty SELECT field table mismatch');
expect_true($emptyField->orgtable === 'people', 'empty SELECT field original table mismatch');
expect_true($emptyResult->fetch_assoc() === null, 'empty SELECT should have no rows');
...
$db->errno = 999;
$db->error = 'mutated';
$db->connect_errno = 998;
$db->connect_error = 'mutated-connect';
$db->affected_rows = 997;
$db->insert_id = '996';
expect_true($db->query('SELECT 1') instanceof MyLite\MySQLiResult, 'status repair query failed');
expect_true($db->errno === 0, 'successful query should repair public errno');
expect_true($db->error === '', 'successful query should repair public error');
expect_true($db->connect_errno === 0, 'successful query should repair public connect_errno');
expect_true($db->connect_error === '', 'successful query should repair public connect_error');
expect_true($db->affected_rows === 0, 'successful query should repair public affected_rows');
expect_true($db->insert_id === '0', 'successful query should repair public insert_id');
unset($stmt, $result);
