...
description: Research and design a bounded MyLite engineering slice before implementation, producing a slice spec with MariaDB source references, MySQL/MariaDB compatibility impact, architecture decisions, test strategy, and acceptance criteria. Use only for substantial start/design work, not simple docs edits, reviews, commits, pushes, or status commands.
---
...
native storage configuration, compatibility coverage, database-directory
lifecycle, build trimming, crash recovery, DDL metadata routing, wire-protocol
integration, or upstream import strategy.
...
Use MySQL behavior as compatibility evidence when a slice affects drop-in API
...
6. Write `docs/specs/<slice-slug>/specs.md` with:
   ...
   - database-directory and embedded-lifecycle implications,
   - public API or directory-layout impact,
   - native storage impact when relevant,
   - wire-protocol or integration-package impact when relevant,
   - binary-size impact when relevant,
   - license or dependency impact when relevant,
   - test and verification plan,
   - acceptance criteria,
   - risks and unresolved questions.
7. Update related architecture docs and compatibility matrices when the slice
   changes an existing design decision or compatibility claim.
...
- Single-directory, embedded-runtime, compatibility, native-storage,
  upstream-delta, and binary-size implications are explicit where relevant.
