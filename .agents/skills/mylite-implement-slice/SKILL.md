...
description: Implement a bounded MyLite engineering slice end to end after design and test expectations exist, including code, tests, docs, verification, and self-review. Use only for substantive implementation work, not simple git operations, small docs edits, generic reviews, or status requests.
---
...
upstream style. Put new first-party API and storage-engine code behind
MyLite-owned names and module boundaries.

## Workflow

1. Read `README.md`, `AGENTS.md`, `docs/architecture/engineering-standards.md`,
   and the slice spec under `docs/specs/<slice-slug>/specs.md`.
...
3. Confirm affected layers: embedded bootstrap, API, handler/storage engine,
   catalog, file format, build, tests, or docs.
4. Implement through the right layer boundaries. Do not hide product semantics
   behind wrappers that keep forbidden persistent sidecars.
...
6. Add or update tests while implementing. Cover lifecycle, errors, metadata,
   storage invariants, and regression-sensitive behavior relevant to the slice.
7. Update specs and architecture/API docs when the implementation changes the
   design.
8. Run relevant builds, tests, and static checks. Fix failures at the root.
9. Self-review the diff for single-file correctness, embedded lifetime,
   upstream delta size, binary-size impact, license/dependency impact, missing
   tests, and accidental broad changes.
