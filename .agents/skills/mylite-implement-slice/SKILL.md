...
MyLite-owned names and module boundaries. Use MySQL behavior as compatibility
evidence when validating drop-in API or application behavior.
...
3. Confirm affected layers: embedded bootstrap, API, handler/storage engine,
   storage-engine routing, catalog, file format, compatibility matrix,
   wire-protocol integration, build, tests, or docs.
...
9. Self-review the diff for drop-in compatibility, single-file correctness,
   embedded lifetime, storage-engine routing, upstream delta size, binary-size
