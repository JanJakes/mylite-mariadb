...
description: Review a completed or near-complete MyLite engineering slice as a release gate for single-file semantics, embedded API behavior, MariaDB compatibility, tests, docs, binary size, and upstream-fork hygiene. Use only for substantive slice reviews, not simple status checks or small copyedits.
---
...
1. Read `README.md`, `AGENTS.md`, `docs/architecture/engineering-standards.md`,
   the relevant slice spec, and the implementation diff. For setup reviews,
   also read `docs/specs/README.md` and the local skill files.
...
3. Check MariaDB source assumptions against the selected base ref. Treat stale
   or guessed source claims as review findings.
...
7. Check API boundaries. Public `libmylite` surfaces should not expose
   internal MariaDB handles as the primary abstraction.
8. Check tests for depth and relevance. Add missing tests when the fix is clear;
   otherwise report the gap as blocking.
9. Check license, trademark, and dependency implications when the slice changes
   public packaging, naming, linking, or imported code.
10. Check docs: slice spec, architecture docs, API docs, unsupported behavior,
    and source references.
11. Check skill boundaries and composition. `mylite-start-slice`,
...
12. Run relevant tests and checks. Fix failures when the fix is clear; otherwise
    report the blocker with the failing command and evidence.
13. Review final diff quality: focused scope, performance awareness, lean
