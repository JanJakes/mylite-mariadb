...
description: Review a completed or near-complete MyLite engineering slice as a release gate for single-directory semantics, embedded API behavior, MySQL/MariaDB compatibility, native storage behavior, tests, docs, binary size, and upstream-fork hygiene. Use only for substantive slice reviews, not simple status checks or small copyedits.
---
...
4. Check database-directory semantics. The slice must not place durable state
   outside the MyLite database directory unless the spec explicitly marks it as
   temporary non-final behavior.
...
7. Check drop-in compatibility. Supported MySQL/MariaDB API, SQL, native
   storage behavior, and protocol claims must have tests or explicit
   unsupported status.
