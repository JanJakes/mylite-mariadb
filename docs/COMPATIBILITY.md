...
| Ownerless column ALTER live recovery | 🟡&nbsp;Partial | Hook-build coverage now treats bounded real `ALTER TABLE ... DROP COLUMN`, `ALTER TABLE ... MODIFY COLUMN`, `ALTER TABLE ... CHANGE COLUMN`, and `ALTER TABLE ... RENAME COLUMN` crash boundaries as native-file-operation recoverable while another ownerless peer remains live, including exact `ALGORITHM=COPY, LOCK=EXCLUSIVE` ADD/DROP/MODIFY/CHANGE forms. The registered standalone selectors verify live recovery for dropped-column absence, modified-column width/default metadata and widened writes, changed-column old-name rejection plus widened new-name default/write behavior, renamed-column old-name rejection and new-name writes, and generated-column/CHECK dependent-expression rename behavior, while retaining the native file-operation marker until final no-live drain and preserving ownerless/native reopen plus forced `.shm` rebuild behavior. Multi-clause column ALTERs, generated-column DROP/MODIFY/CHANGE live recovery, explicit online-option variants, copy-lock rename if supported, and randomized external DDL oracle execution remain planned |
