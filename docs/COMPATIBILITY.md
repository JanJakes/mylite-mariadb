...
| Ownerless savepoint DML marker discard | 🟡&nbsp;Partial | Ownerless explicit transactions now track handle-local savepoint write state for `SAVEPOINT`, `ROLLBACK TO [SAVEPOINT]`, and `RELEASE SAVEPOINT`. A focused production selector forces post-checkpoint file-per-table DML after a savepoint, proves the process-local InnoDB file-op redo latch was set, rolls back to the savepoint, requires that latch to be discarded, commits with both dictionary and DML file-op markers still clear, and verifies the pre-savepoint row through ownerless reopen, forced `.shm` rebuild, ordinary native reopen, and a follow-up native write. This closes the bounded no-earlier-write savepoint marker classification gap; mid-rollback crash windows, concurrent-writer savepoint matrices, broader DML-origin `FILE_MODIFY` classes, DDL/file lifecycle recovery, and external randomized stress remain planned |
