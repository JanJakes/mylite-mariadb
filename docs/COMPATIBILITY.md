...
| `UPDATE` / `DELETE` / `REPLACE` | 🟡&nbsp;Partial | Full-scan and supported keyed update/delete are covered for routed base tables, including BLOB/TEXT payload rows, direct and prepared failed-statement rollback for covered unique-key update and multi-row `REPLACE` failures, and close/reopen visibility; multi-statement rollback, triggers, foreign keys, generated-column edge cases, and unsupported index classes remain planned |
