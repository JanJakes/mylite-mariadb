...
| Ownerless page-log visible-fast append batching | 🟡&nbsp;Partial | Existing single-row pure `INSERT ... VALUES` visible-fast-path ownerless statements now keep one page-log append session across the statement's ownerless mini-transactions and release it before page-log sync/page-visible LSN publication; focused SQL coverage verifies page-version records still publish, native history WAL proof remains active, page-log append session begin/end counts collapse for the successful single-row visible-fast insert, and multi-row plus unsupported upsert branches keep conservative append-session or flush behavior after a wider first attempt regressed bulk timing, while broader DML/DDL/group-commit batching remains planned |
