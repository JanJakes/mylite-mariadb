...
Ordinary `CREATE OR REPLACE TABLE` replacement and replacement-copy
`CREATE OR REPLACE TABLE ... LIKE` now also have live-peer recovery for their
native replacement boundaries. Replacement-copy CTAS, rename, truncate, drop,
...
| Ownerless table-replacement DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills representative `CREATE OR REPLACE TABLE`, `CREATE OR REPLACE TABLE ... LIKE`, and `CREATE OR REPLACE TABLE ... AS SELECT` writers after native old-table replacement or replacement-copy completion but before ownerless dictionary finish; ordinary `CREATE OR REPLACE TABLE` and replacement-copy `CREATE OR REPLACE TABLE ... LIKE` now verify live-peer cleanup, recovered replacement `.frm`/`.ibd` files, old-column and old-index absence, replacement-column/index metadata, replacement rowset semantics, post-recovery writes, ownerless/native reopen, forced `.shm` rebuild, live marker retention, and no-live marker drain, while replacement-copy CTAS still verifies live-peer cleanup remains busy until no-live recovery, CTAS copied rows, post-recovery writes, ownerless/native reopen, and forced `.shm` rebuild |
