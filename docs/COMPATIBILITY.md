...
`ALTER TABLE ... RENAME TO` prefinish crash recovery now uses metadata-only
live-peer recovery while retaining the native file-operation marker until
no-live drain. Pure multi-pair temporary rename chains now use the same
metadata-only dictionary recovery lane with native marker retention, and
...
| Ownerless temporary-table truncate tracking | 🟡&nbsp;Partial | Production ownerless SQL coverage verifies `TRUNCATE TABLE` against a same-named session-local InnoDB temporary table empties only that temporary table, keeps the temporary name tracked while a peer updates the permanent table, reveals the peer-updated permanent table only after `DROP TEMPORARY TABLE`, and preserves the durable permanent state through ownerless reopen, forced `.shm` rebuild, and ordinary native reopen; hook crash recovery during temporary-table truncate and broader temporary DDL matrices remain planned |
