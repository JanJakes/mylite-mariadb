...
| Ownerless page-version log primitive | 🟡&nbsp;Partial | A first-party fixed-record page-version log primitive can initialize a log, initialize at a payload offset, serialize appends with a byte-range lock, store real InnoDB `space_id`/`page_no` pairs including zero, read a record by WAL offset, replay committed record metadata for shared-index rebuild, read the latest version visible at a commit LSN, report missing pages and undersized buffers, and tolerate an incomplete tail record; primitive tests cover same-process and cross-process append/read behavior plus replaying record offsets into a page index; guarded SQL tests verify dirty page images are appended to `mylite-concurrency.wal`, indexed in `mylite-concurrency.shm`, replayed into a rebuilt `.shm` page index, and consumed by ownerless autocommit `SELECT` page reads for non-system InnoDB tablespace pages; the page-visible LSN advances only after dirty pages for the commit have been published and flushed through the current conservative bridge, and hook tests verify the page-version visibility LSN is scoped to the executing SQL thread; active transactions, DML/DDL, prepared execution, system tablespace pages, checkpointing, tablespace replay, and page-version retention remain on the conservative native-file bridge |
