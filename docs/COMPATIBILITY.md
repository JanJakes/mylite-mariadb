...
close, retains the peer-observed explicit DML marker and page-version WAL as
forced-rebuild evidence while broader native redo/checkpoint proof remains
unimplemented, forces `.shm` rebuild, and verifies ownerless and ordinary
native reopen preserve the updated rows. Ordinary native opens that consume
retained ownerless WAL retire the startup page-version visibility before later
native write or locking-read statements so stale retained pages cannot shadow
ordinary writes.
...
lifetime. A later no-live ownerless close after both children are reaped
retains committed DML marker/WAL evidence for forced rebuild. This is bounded durable
