...
Ownerless read/write support is complete for the admitted surface: embedded
Linux on a validated local ext4, XFS, tmpfs, or overlay filesystem, persistent
InnoDB application tables, and the DML and DDL shapes explicitly covered below.
The production hook, bounded stress, filesystem, application, and external
oracle gates pass for that boundary. Per-engine ownerless designs for
non-InnoDB durable tables, other platforms/filesystems, longer RQG-style
campaigns, and broader edge-case hardening remain future expansion rather than
release blockers for the admitted surface.
