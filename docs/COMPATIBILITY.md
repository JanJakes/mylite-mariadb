...
`INSERT ... VALUES` statements with one through sixteen row constructors, matching
the default production bulk probe shape plus bounded eight-row and sixteen-row
bulk diagnostics while keeping larger row lists and broader DML on the previous
...
`7223.47` to `10655.43` rows/s on the same local shape.
