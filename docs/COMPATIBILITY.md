...
and a next combined seed-sweep wrapper over seeds `992` through `1023` at rounds
`2`;
...
`INSERT ... VALUES` statements with one through 2048 row constructors, matching
...
2048-row bulk diagnostics while keeping larger row lists and broader DML on
the previous append-session policy.
