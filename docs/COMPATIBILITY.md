...
| Ownerless shared-memory file | 🟡&nbsp;Partial | `concurrency/mylite-concurrency.shm` is created and grown under `RECOVERY` then `SHM_RESIZE` byte-range locks, validated through `MAP_SHARED`, starts with a fixed 128-byte MyLite header bound to the database UUID, and contains fixed process-registry and wait-channel segments; dirty or rebuilding volatile state is rebuilt with an incremented recovery generation, but no lock-manager, transaction, or page-version segments are active yet |
...
| Ownerless coordination primitives | 🟡&nbsp;Partial | POSIX file-backed `MAP_SHARED` visibility, grow/remap behavior, byte-range lock conflicts, lock release on process exit, and internal mapped latch wait/wake plus timeout behavior are covered as platform evidence and summarized by an internal ownerless platform probe; no product ownerless open path uses them yet |
