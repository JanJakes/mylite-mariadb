...
skips, standalone index DDL including existence-option skips, index rename DDL
...
| Standalone `CREATE INDEX` / `DROP INDEX` | 🟡&nbsp;Partial | Route supported copy-rebuild index additions and drops through MariaDB DDL and MyLite catalog/index updates, including generated-column indexes, representative `CREATE INDEX IF NOT EXISTS` and `DROP INDEX IF EXISTS` skips, and representative default-algorithm standalone index DDL after catalog-only reopen; online DDL, unsupported index classes, SQL rollback, and foreign keys remain planned |
