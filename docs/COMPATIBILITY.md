...
`CREATE TABLE ... LIKE`, including copied secondary-index metadata. CTAS,
...
| Ownerless table-copy DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills representative `CREATE TABLE ... LIKE` and `CREATE TABLE ... SELECT` writers after native destination table creation, including CTAS row population, but before ownerless dictionary finish; `CREATE TABLE ... LIKE` recovery now verifies live-peer cleanup, recovered `.frm`/`.ibd` files, `INFORMATION_SCHEMA.TABLES` and column metadata, copied secondary-index metadata, post-recovery writes, ownerless/native reopen, forced `.shm` rebuild, live marker retention, and no-live marker drain, while CTAS still verifies live-peer cleanup remains busy until no-live recovery, CTAS copied rows, post-recovery writes, ownerless/native reopen, and forced `.shm` rebuild |
