...
| Ownerless explicit transaction visible-fast proof | 🟡&nbsp;Partial | Explicit ownerless transactions now carry a conservative per-handle visible-fast proof from eligible `INSERT ... VALUES` writes to the later `COMMIT`, allowing prepared explicit insert transactions to publish committed ownerless visibility without the later unproven-statement dirty-page flush fallback while still eliding pre-commit rollback-segment `FIL_PAGE_UNDO_LOG` native-support page-version WAL in the transaction's own rollback-segment space. The existing history-proof guard still refuses active commit history pages before undo elision can apply, and the write-history flush path inside `trx_t::write_serialisation_history()` remains measured for this explicit-transaction shape. Focused SQL coverage uses prepared inserts in one explicit transaction, verifies fast commit publication, zero conservative flush/unproven counters, transaction page publication at commit, native-support undo elision, no publish failure, remaining write-history flush pages without fallback rounds, same-handle visibility, ownerless reopen, forced-`.shm` native reopen visibility, and a savepoint-controlled transaction that deliberately stays on the conservative unproven path while preserving only the pre-savepoint row; `INSERT ... SELECT`, `UPDATE`, `DELETE`, `REPLACE`, DDL, locking-read transactions, savepoint-controlled transactions beyond this negative proof, foreign-key target inserts, broader native redo/checkpoint reconciliation, and transaction-scoped append batching remain planned |
