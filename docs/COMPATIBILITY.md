...
| Rollback | 🟡&nbsp;Partial | Explicit InnoDB transaction rollback is covered, including hook-build crash coverage that kills a full rollback after one successful native InnoDB `row_undo()` step and verifies no-live ownerless recovery preserves the original rows; MyISAM remains non-transactional |
| Savepoints | 🟡&nbsp;Partial | Explicit InnoDB savepoint rollback and release savepoint are covered through SQL transaction statements, including hook-build crash coverage that kills `ROLLBACK TO SAVEPOINT` after one successful native InnoDB `row_undo()` step and verifies no-live ownerless recovery rolls the uncommitted transaction back to the original rows |
