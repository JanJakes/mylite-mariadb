...
`INSERT ... VALUES` statements with one through 512 row constructors, matching
the default production bulk probe shape plus bounded eight-row, sixteen-row,
thirty-two-row, sixty-four-row, 128-row, 256-row, and 512-row bulk diagnostics
while keeping larger row lists and broader DML on the previous append-session
policy.
...
respectively, and focused SQL coverage now proves the 512-row positive boundary
plus a 513-row conservative boundary with no deferred latest-checkpoint
