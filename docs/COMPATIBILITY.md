...
no-live recovery; exact copy-lock ADD COLUMN now has focused live-recovery
coverage for the native table-copy rebuild boundary;
...
| Capability | MyLite status | Compatibility target |
| --- | --- | --- |
| Ownerless `ALTER TABLE ... ADD COLUMN` live recovery | 🟡&nbsp;Partial | Hook-build coverage kills plain stored-column ADD and exact `ADD COLUMN ..., ALGORITHM=COPY, LOCK=EXCLUSIVE` writers after MariaDB/InnoDB completes native metadata update or copy rebuild but before ownerless dictionary finish, then verifies live recovery while another ownerless peer remains open, native file-operation marker retention until no-live drain, recovered default-backed rows, ownerless/native reopen, and forced `.shm` rebuild; generated, `AUTO_INCREMENT`, placement, other explicit option orders, multi-action, index, constraint, and broader rebuild ADD COLUMN forms remain planned |
