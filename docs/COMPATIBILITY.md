...
The next replay covers seeds `1376` through `1407` at rounds `2` with the same
explicit FK graph retry budget, recovering transient raw MariaDB `1213` exits
for seeds `1376`, `1386`, `1389`, `1393`, `1397`, `1400`, `1401`, and `1407`
on attempt `2`, seeds `1387` and `1399` on attempt `3`, seed `1384` on
attempt `4`, and seed `1383` on attempt `8`.
...
file-operation marker until no-live drain, and metadata-only rename and
ignored/not-ignored recovery keeps that marker clear.
...
mixed ADD/DROP ALTER lists now use metadata-only live-peer recovery with clear
native file-operation markers. A focused mixed
...
is part of that boundary, but the current Linux production SQL and hook matrix
is not yet reproducibly green. Per-engine ownerless designs for non-InnoDB
durable tables, additional filesystems, longer RQG-style campaigns, and broader
edge-case hardening remain future expansion after the current lifecycle and
recovery regressions are stabilized.
...
| Ownerless FK DDL live recovery | 🟡&nbsp;Partial | Hook-build coverage now treats bounded ordinary single-clause `ALTER TABLE ... ADD [CONSTRAINT] FOREIGN KEY ... REFERENCES ...` and `ALTER TABLE ... DROP FOREIGN KEY ...`, pure comma-separated ordinary FK-only ADD and DROP lists, and FK-only mixed DROP/ADD lists as metadata-only recoverable dictionary boundaries when neither child nor referenced table has generated columns. Ordinary child/parent FK add/drop, pure multi-clause FK add/drop, and mixed FK-only drop/add crash selectors kill the writer after native metadata creation/removal but before ownerless dictionary finish, recover while another ownerless peer remains live, verify the native file-operation checkpoint-needed marker stays clear, and check recovered FK metadata/enforcement or absent-FK orphan/parent writes through ownerless/native reopen and forced `.shm` rebuild. Focused mixed FK/non-FK selectors now kill `DROP FOREIGN KEY`, simple `ADD COLUMN`, and `ADD CONSTRAINT ... FOREIGN KEY` after native success, proving retained native file-operation marker drain for the column mutation; `DROP FOREIGN KEY`, table `COMMENT`, and `ADD CONSTRAINT ... FOREIGN KEY`, proving metadata-only live recovery with the native file-operation marker clear; and `DROP FOREIGN KEY`, named CHECK drop/add, and `ADD CONSTRAINT ... FOREIGN KEY`, proving mixed FK plus CHECK metadata/enforcement recovery with the native file-operation marker retained while a peer is live and drained after final no-live recovery. Standalone table-level CHECK ADD/DROP live-peer recovery is now covered separately on the marker-retaining CHECK lane. Generated-column child and referenced-column FK add/drop crash selectors remain on the conservative native-file lane: live-peer cleanup stays busy, no-live recovery preserves generated-column values and FK state, and ownerless/native reopen plus forced `.shm` rebuild observe the recovered result. Broader mixed comma-separated FK/non-FK ALTER lists, generated-column metadata-only live recovery, broader FK crash fuzzing, and full external MariaDB/RQG FK graph execution remain planned |
...
| Ownerless index metadata crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset also kills ordinary secondary-index create/drop, online `ALTER TABLE ... ADD/DROP INDEX ..., ALGORITHM=INPLACE, LOCK=NONE`, `CREATE OR REPLACE UNIQUE INDEX`, unique-secondary `DROP INDEX`, `ALTER TABLE ... RENAME INDEX`, and `ALTER TABLE ... ALTER INDEX ... IGNORED`/`NOT IGNORED` writers after native index metadata changes but before ownerless dictionary finish; recovery verifies live-peer recovery while another ownerless peer remains open, native file-operation marker retention for physical create/drop/replacement and online add/drop until final no-live drain, metadata-only marker-clear recovery for rename and ignored/not-ignored, old/new index-name metadata, replacement unique-key metadata/enforcement, dropped unique-index metadata absence and duplicate-key release, ignored/not-ignored metadata, final forced-index reads, later writes, ownerless/native reopen, and forced `.shm` rebuild |
