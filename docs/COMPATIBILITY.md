...
| Ownerless recovery anchors | 🟡&nbsp;Partial | `concurrency/mylite-concurrency.wal` and `concurrency/mylite-concurrency.ckpt` are created under `RECOVERY` with fixed headers bound to the database UUID; guarded ownerless SQL now writes page-version records after the `.wal` header, and `.shm` rebuilds replay those records into the shared page-version index, while recovery still does not apply page records back into InnoDB tablespace files |
...
| Concurrent writers | ⚪&nbsp;Planned | Ownerless cross-process read/write concurrency requires directory-backed shared-memory coordination, native storage lock integration, page visibility, DDL invalidation, and crash recovery before support can be claimed |
...
| Guarded ownerless writer tests | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset exposes `MYLITE_OPEN_OWNERLESS_RW` for guarded cross-process SQL writer tests that bypass the directory lock and route through the ownerless process, transaction, read-view, InnoDB lock, and redo-visibility hooks; tests cover non-conflicting writers, row waits, deadlocks, committed external visibility, and a killed uncommitted writer whose state blocks peer cleanup until a no-live-process reopen rebuilds volatile coordination. This capability is unavailable in normal builds |
