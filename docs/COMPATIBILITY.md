...
no-live recovery; exact copy-lock ADD, DROP, and MODIFY COLUMN now have
focused live-recovery coverage for native table-copy rebuild boundaries;
...
| Capability | MyLite status | Compatibility target |
| --- | --- | --- |
| Ownerless column ALTER live recovery | 🟡&nbsp;Partial | Hook-build coverage kills plain stored-column ADD/DROP/MODIFY/RENAME writers and exact `ADD COLUMN`, `DROP COLUMN`, and `MODIFY COLUMN` `ALGORITHM=COPY, LOCK=EXCLUSIVE` writers after MariaDB/InnoDB completes native metadata update or copy rebuild but before ownerless dictionary finish, then verifies live recovery while another ownerless peer remains open, native file-operation marker retention until no-live drain, recovered metadata/default-backed rows, ownerless/native reopen, and forced `.shm` rebuild; generated, `AUTO_INCREMENT`, placement, `CHANGE COLUMN`, copy-lock rename if supported, other explicit option orders, multi-action, index, constraint, and broader rebuild column ALTER forms remain planned |
