...
Single-pair same-schema `RENAME TABLE` now has live-peer recovery for the
native file-move boundary. Focused non-temporary `TRUNCATE TABLE schema.table`
now has live-peer recovery for the native truncate/recreate boundary.
Cross-schema and multi-pair rename, multi-table and cross-schema drop,
broader ALTER rebuild, schema, view, trigger, and foreign-key multi-DDL
