...
| Capability | MyLite status | Compatibility target |
| --- | --- | --- |
| WordPress-shaped core tables | 🟡&nbsp;Partial | Storage-engine smoke covers a broader `wp_options`, `wp_posts`, `wp_postmeta`, `wp_users`, `wp_usermeta`, `wp_terms`, `wp_term_taxonomy`, `wp_term_relationships`, `wp_comments`, `wp_commentmeta`, and `wp_links` subset with `ENGINE=InnoDB` routing, autoincrement ids, text payloads, composite keys, secondary and prefix indexes, joins, updates, deletes, close/reopen, and sidecar gates; exact WordPress installer import, collation variants, multisite, and plugin tables remain planned |
