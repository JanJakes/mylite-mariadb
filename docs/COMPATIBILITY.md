...
resident byte/LSN mismatch still rejects the image and keeps the conservative
fallback. The focused single-owner multi-row visible-fast selector covers the
regression by preserving fast commit visibility and zero deferred-page flush
fallback for the small row-list shape.
