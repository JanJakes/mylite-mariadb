...
| Replication and binlog | ➖&nbsp;Out&nbsp;of&nbsp;scope | Server topology feature, not core library behavior; replication and binlog command families plus SQL `BINLOG` replay are rejected, `@@log_bin=0` is covered, and the default embedded archive omits the active binlog transaction/event core, SQL `BINLOG` replay source, server event writers, unsupported injector root, guarded replication execution system variables, and replication/binlog filter runtime |
