...
Ownerless read/write support is complete for the admitted surface: 64-bit
...
return `MYLITE_UNSUPPORTED_PLATFORM`. The production hook, bounded stress,
filesystem, native-platform, application, and external oracle gates pass for
that boundary. Per-engine ownerless designs for non-InnoDB durable tables,
additional filesystems, longer RQG-style campaigns, and broader edge-case
hardening remain future expansion rather than release blockers for the
admitted surface.
...
| Ownerless platform proof | 🟡&nbsp;Partial | `MYLITE_OPEN_OWNERLESS_RW` and `MYLITE_OPEN_SHARED_READONLY` are currently available only in embedded Linux builds and admit allowlisted local ext4, XFS, tmpfs, or overlay filesystems; they reject unknown, network, FUSE, and other unvalidated filesystem types before probing the prepared database directory for cross-process `MAP_SHARED` visibility, byte-range lock conflict, lock release on process exit, file grow/remap, wait/wake support, and process-identity liveness; a successful probe writes `concurrency/mylite-ownerless-platform.meta` with the database-directory device id and `process_identity=1` so later opens skip the full probe until the proof is absent, stale, or the directory is on a different filesystem; hook-only coverage forces the probe to fail and verifies ownerless opens reject the directory while ordinary read/write opens remain on the non-ownerless path; the production release gate mounts real ext4 and XFS loop filesystems plus tmpfs and overlay, then runs the two-process InnoDB write/read/reopen case on each admitted filesystem |
