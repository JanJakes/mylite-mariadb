...
native file-operation marker clear, while table-bearing `DROP DATABASE IF
...
and empty existing `IF EXISTS` variants, broader view and trigger variants, and non-rename
...
| Ownerless schema-idempotent DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills duplicate `CREATE DATABASE IF NOT EXISTS`, missing `CREATE SCHEMA IF NOT EXISTS`, missing `DROP SCHEMA IF EXISTS` no-op, and table-bearing `DROP DATABASE IF EXISTS` writers after MariaDB returns success but before ownerless dictionary finish; recovery verifies live-peer recovery with the native file-operation marker clear for duplicate create, missing create, and missing drop, and marker-retaining live recovery for table-bearing drop until final no-live drain, duplicate create preserves the original schema defaults and inherited column collation while plain duplicate create returns errno 1007, missing create exposes the created schema defaults and supports post-recovery table writes, missing drop preserves the real schema/table while keeping the missing schema metadata/directory absent, table-bearing drop keeps the removed schema/table absent, post-recovery writes succeed where the preserved or created schema remains, and ownerless/native reopen plus forced `.shm` rebuild observe the same state |
