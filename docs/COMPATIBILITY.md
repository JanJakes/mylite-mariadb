...
| Rollback | 🟡&nbsp;Partial | Explicit InnoDB transaction rollback is covered, including hook-build crash coverage that kills a full rollback after one successful native InnoDB `row_undo()` step, verifies live-peer cleanup remains busy while another ownerless process is open, and verifies no-live ownerless recovery preserves the original rows; MyISAM remains non-transactional |
