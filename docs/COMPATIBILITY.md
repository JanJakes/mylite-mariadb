...
stale retained pages cannot shadow ordinary writes.
...
published native-support `FIL_PAGE_UNDO_LOG`, proof-only native-support records
...
| Ownerless history-proof native reclaim | 🟡&nbsp;Partial | No-live ownerless reclaim now scans proof-only native-support history records before clearing DML markers or truncating page-version WAL, forces the owning native tablespace to flush through the proof page LSN, and requires native page-LSN proof for those rollback-segment/undo obligations; live-peer reclaim uses the same proof-including scan and refuses to reclaim while proof-only history obligations remain. Normal page-log replay, latest-page reads, page-index rebuild, and checkpoint callbacks still skip proof-only records, while a dedicated primitive scan covers the include-proof replay path. Focused production cutover coverage now combines an explicit-transaction `INSERT ... VALUES` write that emits ordinary user page-version payload plus proof-only rollback-segment/undo history records, then proves no-live checkpoint drain lets ordinary native InnoDB reopen after `.wal` and `.shm` removal. This closes a bounded native-history reclaim gap exposed by repeated `commit-race` corruption, but broader redo/checkpoint recovery, DDL/file lifecycle, and external randomized stress remain planned |
