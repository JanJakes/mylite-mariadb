...
transaction can still commit; accepted 1205 timeout victims prove the same
discard after explicit rollback, matching MariaDB's timeout transaction
lifetime. A later no-live ownerless close after both children are reaped now
drains committed DML marker/WAL evidence after native checkpoint proof. This is
bounded durable marker coverage for checkpointed representative DML commit,
successful rollback, and deadlock-victim rollback shapes, not a claim that
every possible DML-origin `FILE_MODIFY`, arbitrary crash, killed uncommitted
transaction, savepoint, or concurrent-writer explicit transaction path has been
