...
| Capability | MyLite status | Compatibility target |
| --- | --- | --- |
| Ownerless plain `ALTER TABLE ... ADD COLUMN` live recovery | 🟡&nbsp;Partial | Hook-build coverage kills a plain stored-column ADD writer after MariaDB/InnoDB completes native metadata update but before ownerless dictionary finish, then verifies live recovery while another ownerless peer remains open, native file-operation marker retention until no-live drain, recovered default-backed rows, ownerless/native reopen, and forced `.shm` rebuild; generated, `AUTO_INCREMENT`, placement, explicit `ALGORITHM`/`LOCK`, multi-action, index, constraint, and broader rebuild ADD COLUMN forms remain planned |
