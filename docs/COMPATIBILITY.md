...
bytes/insert, and `0.752` undo-delta records per insert were selected.
...
| Ownerless undo-log page deltas | 🟡&nbsp;Partial | Repeated `FIL_PAGE_UNDO_LOG` page-log records now use an undo-specific non-chained delta flag against a durable standalone base record when the delta is smaller than the standalone payload; primitive coverage proves byte-exact direct/latest reads, retained undo-delta checkpoint rewrite to standalone records, and post-checkpoint fallback, while the reduced production probe selected `0.752` undo-delta records per autocommit insert and cut undo-log page-log payload from `539.910` to `210.006` bytes/insert without changing the one rollback-segment plus one undo history-proof publication per insert; replacing the remaining native-support proof requirement and broader redo/checkpoint recovery remain planned |
