...
and a next combined seed-sweep wrapper over seeds `1024` through `1055` at rounds
`2`;
...
coverage for `RENAME TABLE` `FILE_RENAME`, `TRUNCATE TABLE` native
truncate/recreate, and `DROP TABLE` `FILE_DELETE` boundaries, so a killed
writer cannot lose the durable checkpoint-needed boundary before no-live
recovery for those classes.
...
`INSERT ... VALUES` statements with one through 8192 row constructors, matching
...
2048-row, plus 4096-row and 8192-row bulk diagnostics while keeping larger row lists and
broader DML on the previous append-session policy.
