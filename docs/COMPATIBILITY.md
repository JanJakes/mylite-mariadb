...
| Ownerless concurrency completion gate | Production completion requires the ordinary embedded concurrency tests, the registered ownerless SQL shards, hook-build crash/recovery and negative-proof coverage, ownerless stress, and any claimed external MariaDB replay evidence: `ctest --preset php-embedded-prod -LE compat.ownerless-cross-process-sql --output-on-failure`, `ctest --preset php-embedded-prod -R '^libmylite\.ownerless-cross-process-sql\.' --parallel 1 --output-on-failure`, `ctest --preset ownerless-test-hooks -L 'compat.crash-reopen|compat.ownerless-negative-proof|compat.platform' --output-on-failure`, `ctest --preset ownerless-stress --output-on-failure`, and the documented external replay command for the latest claimed seed window |
...
uses metadata-only live-peer recovery while retaining the native file-operation
marker until no-live drain. Pure multi-pair temporary rename chains now use the
same metadata-only dictionary recovery lane with native marker retention, and
temp-first plus permanent-first mixed temporary/permanent rename lists now prove
...
session-local and absent after crash, retain the native file-operation marker
while a peer is live, drain it after no-live recovery, and survive
