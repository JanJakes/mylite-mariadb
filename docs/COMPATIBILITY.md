...
`INSERT ... VALUES` statements with one through sixty-four row constructors,
matching the default production bulk probe shape plus bounded eight-row,
sixteen-row, thirty-two-row, and sixty-four-row bulk diagnostics while keeping
larger row lists and broader DML on the previous
append-session policy. Page-version record volume, history-proof native-support
publication, WAL sync ordering, and page-visible LSN publication are unchanged;
the optimization only reduces repeated page-log append-session begin/end work
within the capped statement. The first stats-enabled production probe for the
four-row cap kept the default four-row bulk shape at `6.100` append calls and
`6.000` page-version records per statement, while append-session begin/end
calls dropped to `50` for `50` bulk statements from the previous `250`/`250`
sample. The eight-row follow-up preserves the same one-session proof for a
...
`9856.67` to `11755.40` rows/s on the same local shape.
