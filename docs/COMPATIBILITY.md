...
| Replication and binlog | ➖&nbsp;Out&nbsp;of&nbsp;scope | Server topology feature, not core library behavior; replication and binlog command families, SQL `BINLOG` replay, GTID helper functions, and GTID state variable assignments are rejected, `@@log_bin=0` is covered, and the default embedded archive omits the active binlog transaction/event core, SQL `BINLOG` replay source, server event writers, binary-log event parser/reader runtime, replication GTID-state runtime, unsupported injector root, guarded replication execution system variables, and replication/binlog filter runtime |
