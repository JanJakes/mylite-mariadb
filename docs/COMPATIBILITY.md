...
| Ownerless cross-process SQL | `ctest --preset embedded-dev -L compat.ownerless-cross-process-sql`, registered as sixteen deterministic weighted CTest shards with flushed per-case progress diagnostics so long ownerless SQL coverage reports per-shard failures, estimated shard weights, timings, active case names, active case indexes, and `/proc` process-group/thread state on timeout; hidden per-case children run in their own process groups so timeout cleanup cannot leave orphaned descendants holding CTest output pipes open; a timed-out case can be rerun directly with `mylite_ownerless_cross_process_sql_test sql-case <index-or-name>` through the same wrapper, `sql-case-count` reports the current direct-case loop bound, and the old modulo `sql-shard <index> <count>` command remains available for comparison; the concurrent explicit-transaction commit race treats only exact MariaDB `1205`/`1213` as retryable, rolls back and reuses the same handle, retries the whole logical transaction through commit, and still requires every worker delta exactly once through ownerless/native reopen and forced shared-memory rebuild; earlier two-job and four-job modulo-shard scheduling produced load-sensitive ownerless DDL/dictionary/temporary-tablespace timeouts, while weighted-shard ownerless SQL measurement passed at two jobs locally with about half the wall time; a later smaller-shard slice split the registered weighted group from eight to sixteen shards for more granular timings without weakening the 300-second per-case watchdog; full-preset two-job scheduling still timed out when ownerless and non-ownerless tests interleaved, so CI runs non-ownerless embedded tests and ownerless SQL as separate visible production jobs; the non-ownerless step runs serially with `--output-on-failure`, while four production `embedded-prod` matrix jobs each run a disjoint four-shard regex with `--parallel 1`, after repeated GitHub Actions reruns timed out different shard cases when independent embedded runtimes shared one runner; shard 8 (`test_ownerless_text_blob_prefix_index_ddl_refreshes_peer_dictionary`) and shard 11 (`test_ownerless_foreign_key_cross_schema_child_rename_refreshes_peer_dictionary`) remain isolated within their serial matrix groups after direct-case and isolated-shard reruns proved their observed CI timeouts were load-sensitive rather than standalone failures |
| Mixed live-read/native-page safety | `mylite_ownerless_cross_process_sql_test sql-case test_four_processes_mix_ownerless_reads_and_writes`, combining three independent autocommit writers with one plain consistent reader, monotonic read totals, and exact ordinary-native plus ownerless reopen totals; this covers the transient plain-read native-page fence and no-live shared-written-redo startup proof |
| Ownerless cross-process stress | `cmake --preset ownerless-stress && cmake --build --preset ownerless-stress --target mylite_ownerless_cross_process_sql_test && ctest --preset ownerless-stress`, covering independent-table, DDL/DML, temporary-table, explicit-transaction, checksum-oracle, pseudo-random shared-table transaction, foreign-key graph, stress child-failure cleanup, active-reader pressure, expanding-page pressure, BLOB page pressure, compressed BLOB page pressure stress cases, deterministic SQL trace exporters including DDL lifecycle with same-name recreate `SPACE` identity, cross-schema rename `SPACE` identity, dropped-schema absence, and replacement-copy DDL oracles, CTAS post-create DML, active-reader pressure including AUTO_INCREMENT high-watermark final oracle, compressed row-format DDL with the `KEY_BLOCK_SIZE=1`/`2`/`4`/`8`/`16` option cycle and InnoDB zip-page metadata checks, and BLOB pressure, full deterministic trace-suite validation, seeded random transaction, explicit transaction stress with bounded `1205`/`1213` retries, DDL stress with bounded retry for pre-execution MyLite ownerless statement-lock `MYLITE_BUSY` contention plus native MariaDB `1205`/`1213` outcomes from its autocommit DDL/DML statements and read-only polls, a one-second DDL-stress statement-lock wait in the regular eight-round CTest so retries stay inside the harness deadline, plus a focused short-timeout retry selector that restores the normal statement-lock wait after the deliberately forced first miss, and FK graph trace validation, scaled check-mode validation for active-reader, compressed row-format DDL, and BLOB pressure traces, opt-in Docker-backed external MariaDB deterministic trace replay with focused `--trace` and bounded `--scale` profiles including refreshed full scale-2 deterministic replay evidence for the current 12 trace families, focused DDL lifecycle cross-schema/schema-drop replay evidence, focused seeded DDL stress replay evidence, combined random-transaction/DDL/FK seed-sweep replay evidence through seed `1407`, and the external trace-runner fake-client smoke test |
| Ownerless negative proof | `ctest --preset ownerless-test-hooks -L compat.ownerless-negative-proof` |
| Platform and filesystem probes | Linux: `ctest --preset embedded-dev -L compat.platform`; exact hook-only filesystem rejection: `ctest --preset ownerless-test-hooks -R libmylite.ownerless-platform-probe-failure`; native CI additionally builds and runs focused cross-process ownerless gates on macOS/APFS and Windows/NTFS |
| Ownerless concurrency release gate | ✅&nbsp;Covered. Final-source evidence passes the ordinary production suite (`65/65` in `139.28s`), PHP ownerless adapters (`2/2` in `8.85s`), all sixteen weighted production SQL shards (`16/16` in `474.97s`), the `270`-test hook-enabled release gate across sixteen deterministic CI shards (the preceding `269`-test monolithic baseline passed in `2659.62s`), bounded workload (`5/5` in `2287.00s`), randomized rollback/checksum/transaction/FK/cleanup (`5/5` in `1677.33s`), pressure (`4/4` in `170.39s`), mounted ext4/XFS/tmpfs/overlay coverage, explicit unsupported-filesystem rejection, native macOS/APFS and Windows/NTFS gates, focused WordPress (`7/7` with `22` assertions and `76` peer writes), the full external MariaDB trace oracle (`12/12`), and `32` seeds each for random transactions, DDL, and FK graphs. Production build policy, Release/MinSizeRel checks, formatting, clang-tidy, whitespace, and the linked bundle audit also pass. Canonical commands remain `ctest --preset php-embedded-prod -LE 'compat.ownerless-cross-process-sql|compat.ownerless-application' --output-on-failure`, `ctest --preset php-embedded-prod -L compat.ownerless-application --parallel 1 --output-on-failure`, `tools/check-ownerless-filesystems build/php-embedded-prod/packages/libmylite/mylite_ownerless_cross_process_sql_test`, `ctest --preset php-embedded-prod -R '^libmylite\.ownerless-cross-process-sql\.' --parallel 1 --output-on-failure`, `ctest --preset ownerless-hook-release-gate --parallel 1 --output-on-failure`, `ctest --preset ownerless-stress-workload-release-gate --parallel 1 --output-on-failure`, `ctest --preset ownerless-stress-randomized-release-gate --parallel 1 --output-on-failure`, `ctest --preset ownerless-stress-pressure-release-gate --parallel 1 --output-on-failure`, the documented focused WordPress command, `tools/ownerless-external-mariadb-trace-smoke --output build/ownerless-external-mariadb-trace-oracle --image mariadb:11.8.6 --full-suite --scale 2 --replay-attempts 3`, and `tools/ownerless-external-mariadb-seed-sweep --output build/ownerless-external-mariadb-oracle --image mariadb:11.8.6 --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2 --fk-graph-replay-attempts 10 --seed-range 1376:1407`. The full external trace replay uses up to three attempts because native MariaDB can choose `1205`/`1213` victims during the concurrent FK trace; each attempt still performs the complete final-state oracle. Exact native retry outcomes inside MyLite stress transactions are limited to MariaDB `1205`/`1213`; ambiguous `1180` is not retryable. CI assigns the hook release set exactly once across sixteen deterministic serial shards. |
...
and a next combined seed-sweep wrapper over seeds `1376` through `1407` at
rounds `2` with an explicit FK graph retry budget;
...
The next replay covers seeds `1152` through `1183` at rounds `2` with the same
explicit FK graph retry budget, recovering transient raw MariaDB `1213` exits
for seeds `1152`, `1155`, and `1157` on attempt `4`, seeds `1156`, `1173`,
and `1183` on attempt `3`, and seeds `1153`, `1159`, `1167`, and `1170` on
attempt `2`.
The next replay covers seeds `1184` through `1215` at rounds `2` with the same
explicit FK graph retry budget, recovering transient raw MariaDB `1213` exits
for seeds `1185` and `1203` on attempt `2`.
The next replay covers seeds `1216` through `1247` at rounds `2` with the same
explicit FK graph retry budget, recovering transient raw MariaDB `1213` exits
for seeds `1221`, `1223`, `1225`, `1228`, `1231`, `1232`, `1233`, and `1246`
on attempt `2`, seeds `1222`, `1234`, `1236`, and `1247` on attempt `3`, and
seed `1224` on attempt `4`.
The next replay covers seeds `1248` through `1279` at rounds `2` with the same
explicit FK graph retry budget, recovering transient raw MariaDB `1213` exits
for seeds `1249`, `1256`, `1262`, `1263`, `1267`, `1272`, and `1274` on
attempt `2`, seeds `1250` and `1278` on attempt `3`, and seed `1270` on
attempt `4`.
The next replay covers seeds `1280` through `1311` at rounds `2` with the same
explicit FK graph retry budget, recovering transient raw MariaDB `1213` exits
for seeds `1280` and `1285` on attempt `2`, and seeds `1286` and `1299` on
attempt `3`.
The next replay covers seeds `1312` through `1343` at rounds `2` with the same
explicit FK graph retry budget, recovering transient raw MariaDB `1213` exits
for seeds `1316`, `1323`, `1326`, and `1330` on attempt `2`, seeds `1322` and
`1332` on attempt `3`, and seed `1338` on attempt `4`.
The next replay covers seeds `1344` through `1375` at rounds `2` with the same
explicit FK graph retry budget, recovering transient raw MariaDB `1213` exits
for seeds `1344`, `1345`, `1346`, `1350`, `1353`, `1354`, `1359`, `1360`,
`1361`, `1366`, and `1374` on attempt `2`, seeds `1347`, `1356`, `1370`, and
`1372` on attempt `3`, and seed `1373` on attempt `4`.
The final stabilization replay covers seeds `1376` through `1407` at rounds `2`
with the same explicit FK graph retry budget. All `32` random-transaction,
`32` DDL, and `32` FK graph traces passed; transient raw MariaDB `1213` exits
recovered for seeds `1386`, `1390`, `1391`, `1392`, `1402`, and `1407` on
attempt `2`, and seeds `1398` and `1405` on attempt `3`.
...
plus primary-key replacement replay and compressed row-format rebuild replay
with native compressed-page evidence,
...
Focused `DROP TABLE` prefinish recovery now allows live-peer dictionary cleanup
after native file removal for explicit and implicit single-table drops plus
two-table same-schema and cross-schema drop lists.
...
Focused `ALTER TABLE schema.table FORCE` plus both exact explicit copy-lock
option orders for force-rebuild prefinish recovery now allow live-peer
dictionary cleanup after the native force rebuild. Focused same-engine
`ALTER TABLE schema.table ENGINE=InnoDB` and
`ALTER TABLE schema.table ENGINE=InnoDB, ALGORITHM=COPY, LOCK=EXCLUSIVE`
plus the reversed explicit
...
from a dynamic source table. Focused plain plus both exact explicit copy-lock option
orders for
...
boundaries plus representative `ALTER TABLE ... FORCE` including both exact
explicit copy-lock option orders and
...
boundaries plus plain and both exact explicit copy-lock option orders for
compressed key-block rebuild boundaries, so a killed writer
...
coverage now extends that live-peer recovery to non-temporary
`CREATE TABLE ... LIKE`, including copied secondary-index metadata, and to the
focused no-definition-list CTAS boundary, including populated rows.
Ordinary `CREATE OR REPLACE TABLE` replacement plus replacement-copy
`CREATE OR REPLACE TABLE ... LIKE` and `CREATE OR REPLACE TABLE ... AS SELECT`
now also have live-peer recovery for their native replacement boundaries.
Explicit schema-qualified `RENAME TABLE schema.table TO schema.table [, ...]`
...
`ALTER TABLE ... RENAME TO` forms. Focused single-pair `RENAME TABLE IF
EXISTS` native-loop crash coverage now kills after the native rename pair and
verifies MariaDB DDL-log recovery restores the original source table instead of
publishing the target. Same-schema and cross-schema non-FK multi-rename
native-loop crash coverage now kills after first, second, and final native
rename pairs and verifies MariaDB DDL-log recovery restores the original table
names, schema placement, and tablespace identities instead of completing the
swap; the same deterministic same-schema and cross-schema three-pair lists are
also covered with `RENAME TABLE IF EXISTS`. Same-schema and cross-schema
foreign-key multi-rename native-loop crash
...
same-schema longer missing-source `RENAME TABLE IF EXISTS` coverage now
interleaves three missing source pairs with two existing InnoDB renames,
proves all three skipped-source warnings, completed recovery of both moved
...
reopen, and forced `.shm` rebuild. Focused cross-schema longer missing-source
`RENAME TABLE IF EXISTS` coverage now interleaves the same three skipped source
pairs around two opposite-direction schema moves between `app` and
`app_archive`, proves all skipped-source warnings, completed recovery of both
moved targets at `dictionary-before-finish`, native-loop DDL-log rollback to
both original schema placements and `SPACE` ids after the second real native
rename hook, skipped target absence in both schema directories,
marker retention/drain, ownerless/native reopen, and forced `.shm` rebuild.
Focused same-schema foreign-key
...
retention/drain, ownerless/native reopen, and forced `.shm` rebuild. Focused
cross-schema foreign-key `RENAME TABLE IF EXISTS` missing-source coverage now
interleaves the same three skipped source pairs around real parent/child moves
from `app` into another schema, proves all three skipped-source warnings,
completed target-schema FK recovery at `dictionary-before-finish`, native-loop
DDL-log rollback to the original `app` FK state after the second real native
rename hook, skipped target absence from SQL metadata, InnoDB dictionary
metadata, and source/target schema native files, marker retention/drain,
...
observed, verifies the original source and target `.frm`/`.ibd` files and rows
remain stable while skipped targets stay absent, the native file-operation
marker stays clear, post-error writes succeed, and ownerless/native reopen
before and after forced `.shm` rebuild observe the same state; a paired
negative-proof selector arms `rename-table-after-native-file-op` and fails if
that native hook is reached for the target-conflict branch. Focused
...
`DROP VIEW`, `CREATE OR REPLACE VIEW`, `ALTER VIEW`, and focused explicit
column-list, check-option create/replacement/alter, nested check-option,
security/definer, focused view algorithm create/replacement/alter forms
(`CREATE ALGORITHM=TEMPTABLE VIEW`,
`CREATE OR REPLACE ALGORITHM=TEMPTABLE VIEW`, and
`ALTER ALGORITHM=TEMPTABLE VIEW`), and idempotent/no-op plus existing
`DROP VIEW IF EXISTS` view forms now have metadata-only live-peer recovery
without native file-operation marker evidence. Simple
`CREATE TRIGGER` and `DROP TRIGGER`, plus `CREATE OR REPLACE TRIGGER`
replacement, duplicate `CREATE TRIGGER IF NOT EXISTS`, and missing
`DROP TRIGGER IF EXISTS` no-op prefinish recovery, now use the same
metadata-only live-peer lane for `.TRG`/`.TRN` metadata with the native
file-operation marker clear. Single-table and multi-table-list
`DROP TABLE IF EXISTS` now have live-peer recovery for missing-table no-op and
existing-table removal outcomes. View-only `RENAME TABLE` now has
...
Focused combined `ALTER DATABASE ... COMMENT = 'literal' DEFAULT CHARACTER
SET/COLLATE` prefinish recovery now covers option-order `db.opt` rewrites that
change both schema comment and schema defaults in one statement while keeping
the native file-operation marker clear.
Focused `ALTER DATABASE ... DEFAULT COLLATE ... CHARSET ... COMMENT = 'literal'`
prefinish recovery now also covers a collation-first schema option rewrite
using MariaDB's `CHARSET` alias while keeping the native file-operation marker
clear.
Focused current-schema `USE schema; ALTER DATABASE DEFAULT CHARACTER
SET/COLLATE` prefinish recovery now covers schema-default `db.opt` rewrites
through MariaDB's current-database grammar while keeping the native
file-operation marker clear.
Focused current-schema `USE schema; ALTER DATABASE COMMENT = 'literal' DEFAULT
CHARACTER SET/COLLATE` prefinish recovery now covers combined option-order
`db.opt` rewrites through the same current-database grammar while keeping the
native file-operation marker clear.
Focused schema invalid-option cleanup now rejects invalid create, named alter,
charset/collation pairing, and current-schema alter option forms without
creating rejected schema directories, rewriting the existing `db.opt`, changing
schema comments/defaults, or leaving a native file-operation marker.
...
native file-operation marker clear, empty existing `DROP DATABASE IF EXISTS`
and non-idempotent empty `DROP DATABASE|SCHEMA` also have marker-clear live
recovery, and table-bearing `DROP DATABASE|SCHEMA IF EXISTS` has
marker-retaining live recovery.
Duplicate top-level `CREATE INDEX IF NOT EXISTS`, missing top-level
`DROP INDEX IF EXISTS`, duplicate `ALTER TABLE ... ADD INDEX IF NOT EXISTS`,
and missing `ALTER TABLE ... DROP INDEX IF EXISTS` no-op prefinish recovery now
uses metadata-only live-peer recovery with the native file-operation marker
clear after pre-execution index metadata proves the statement is a no-op.
Bounded real secondary-index prefinish recovery now covers top-level
secondary-index create/drop, unique-index create-or-replace/drop,
secondary-index rename, and secondary-index ignored/not-ignored metadata while a
peer remains live; physical index create/drop/replacement retains the native
file-operation marker until no-live drain, and real rename and
ignored/not-ignored mutations retain their prearmed marker until no-live drain.
...
file-operation marker clear.
Pure comma-separated ordinary foreign-key ADD/DROP ALTER lists and FK-only
mixed ADD/DROP ALTER lists now use live-peer recovery with prearmed native
file-operation markers retained until no-live drain. A focused mixed
`DROP FOREIGN KEY`, `ADD COLUMN`, and `ADD CONSTRAINT ... FOREIGN KEY` ALTER
list now uses live-peer recovery while retaining the native file-operation
marker until no-live drain. A focused mixed
`DROP FOREIGN KEY`, table `COMMENT`, and `ADD CONSTRAINT ... FOREIGN KEY`
ALTER list now also uses live-peer recovery while retaining the prearmed native
file-operation marker until no-live drain; the same lane covers a focused
...
Focused mixed `DROP FOREIGN KEY`, `ALTER INDEX ... IGNORED`, and
`ADD CONSTRAINT ... FOREIGN KEY` ALTER lists now use the existing metadata-only
secondary-index ignorability recovery lane while retaining the native
file-operation marker until final no-live drain; the covered shape proves
recovered FK enforcement, ignored-index metadata, ownerless/native reopen, and
forced `.shm` rebuild.
Focused mixed `DROP FOREIGN KEY`, primary-key replacement, and
`ADD CONSTRAINT ... FOREIGN KEY` ALTER lists now use the existing clustered
primary-key replacement recovery lane while retaining the native file-operation
marker until final no-live drain; the covered shape proves recovered FK
enforcement, replacement `PRIMARY` metadata/enforcement, ownerless/native
reopen, and forced `.shm` rebuild.
...
focused temporary-table `TRUNCATE TABLE` tracking now proves truncate empties
only the session-local temporary table while preserving its temporary identity
until `DROP TEMPORARY TABLE` reveals the peer-updated permanent table;
simple temporary-table `DROP TEMPORARY TABLE`, `RENAME TABLE`, and
`TRUNCATE TABLE`, and `ALTER TABLE ... RENAME TO` prefinish crash recovery now
uses metadata-only live-peer recovery while leaving the native file-operation
marker durable after crash and cleared by no-live drain. Pure multi-pair
temporary rename chains now use the same metadata-only dictionary recovery lane,
and temp-first plus permanent-first mixed temporary/permanent rename lists now
prove the permanent pair writes the durable native file-operation marker after
crash, retains it through a final older survivor even if user page-version WAL
was already checkpointed, and drains it after an isolated latest-generation
no-live recovery while the temporary target remains session-local and absent
after crash.
Mixed temporary/permanent `RENAME TABLE IF EXISTS` coverage now also proves
missing durable source pairs produce warnings while same-schema and
cross-schema temp-first and permanent-first real mixed lists preserve the
permanent target, keep skipped targets absent, keep the temporary target
session-local and absent after crash, write the native file-operation marker
after crash, drain it after no-live recovery, and survive
...
no-live recovery; unplaced and placed ADD COLUMN plus DROP, MODIFY, CHANGE, and
RENAME COLUMN now have both exact explicit copy-lock option orders covered for
focused live recovery of native table-copy rebuild boundaries, and charset
conversion also has both exact explicit copy-lock option orders covered;
...
transaction `COMMIT` after local writes, now consume the same native file-op
redo flag and persist a DML-specific checkpoint-needed marker when
post-checkpoint DML emits file-operation redo. The DML marker forces native
checkpoint drain but does not relax no-live user-page LSN/payload proof,
leaving the existing native file-op marker as the proof-relaxing dictionary
DDL/file-lifecycle marker. Focused SQL coverage forces a checkpoint, runs
ownerless autocommit, single-owner explicit-transaction, and idle-peer
explicit-transaction `UPDATE` commit shapes on file-per-table InnoDB tables,
observes the DML marker before close or before final peer release as
appropriate, drains the autocommit and single-owner explicit DML markers on
close, retains the peer-observed explicit DML marker and page-version WAL while
...
opener sees the committed row image, then drains the marker/WAL on close and
preserves the row through forced `.shm` rebuild plus ordinary native reopen.
A paired killed-uncommitted explicit transaction coverage forces the same
post-checkpoint file-per-table DML path before `COMMIT`, kills the writer,
keeps both native file-operation markers clear, verifies no-live ownerless
recovery sees the pre-transaction row, then confirms forced `.shm` rebuild and
ordinary native reopen do not expose the interrupted update.
Savepoint-shaped killed-uncommitted coverage now extends that proof to an
explicit transaction with one file-per-table DML write before a savepoint and
another after it, killing the writer before `ROLLBACK TO SAVEPOINT` or
`COMMIT`, keeping both native file-operation markers clear, and proving
ownerless recovery, forced `.shm` rebuild, and ordinary native reopen preserve
the pre-transaction rows.
Ordinary native opens that consume retained ownerless WAL retire the startup
page-version visibility before later native write or locking-read statements so
stale retained pages cannot shadow ordinary writes, and close checkpoints then
discards the consumed page-version WAL once current native redo covers the
native page LSNs and rollback-history headers remain structurally consistent.
Successful explicit transaction rollback after local writes is now covered as
...
`.shm` rebuild plus ordinary native reopen. Random shared-table transaction
...
proof reclaims it.
Page-write reservation deadlocks now abandon the whole SQL attempt. For
ownerless autocommit `DELETE`, `INSERT`, `REPLACE`, and `UPDATE`, MyLite retries
that whole attempt once when InnoDB proves errno `1213` came from an ownerless
external-wait or page-write coordination cycle. It does not release
transaction-owned pages and resume an already-open mini-transaction, because a
...
live and post-release checkpointing once native boundary proof is available, or
retained native-support WAL when native rollback history still requires it. The
no-live native checkpoint proof gate now rechecks every retained record, so an
older same-page WAL image is discarded only when it has its own exact native
proof or is strictly superseded by an already verified later record for the same
page; focused duplicate-page cutover coverage proves two pinned updates to the
same InnoDB row drain user-page WAL to checkpointed or native-support-only
state, then survive ordinary native reopen after forced `.shm` rebuild.
Deadlock victims after
...
native checkpoint proof. This is bounded durable marker and rollback-handoff
coverage for checkpointed representative DML commit, successful rollback,
deadlock-victim rollback, savepoint retry, and pseudo-random shared-table
explicit transaction shapes, not a claim that every possible DML-origin
`FILE_MODIFY`, native mid-rollback crash, broader same-page savepoint matrix,
DDL/file lifecycle, or concurrent-writer explicit transaction path has been
exhaustively classified. Focused concurrent-savepoint coverage now keeps one
ownerless writer open after `ROLLBACK TO SAVEPOINT`, commits independent
explicit DML from a second ownerless writer while the first writer remains
...
now snapshots transaction-deferred page images at InnoDB savepoint boundaries,
restores the target snapshot after successful native `ROLLBACK TO SAVEPOINT`,
keeps the first writer open, proves a peer writer on another row in the same
table waits on ownerless write ownership instead of independently publishing the
page, then verifies ownerless, forced-`.shm`, and native reopen preserve the
first writer's pre-savepoint row, discard the rolled-back row, and keep the
peer's later row. Same-table large-row handoff coverage now also proves a peer
updating a large row on a different page in the same table commits while the
savepoint writer remains open, leaves no stale ownerless waiter, and remains
visible while the first writer's uncommitted row stays hidden; final reopen
keeps the pre-savepoint row and peer row.
...
forced-`.shm`, and native reopen. Bounded randomized same-table savepoint
schedule coverage now runs three ownerless writers over one five-row InnoDB
table with overlapping row orders, retryable `1205`/`1213` handling,
savepoint rollbacks, full transaction rollbacks, and final ownerless/native
reopen oracles for row count, value sum, version sum, and weighted value sum.
Hook-build coverage
...
redo is authoritative, and runs a bounded native purge-settle pass before
publishing the native-current ownerless visible boundary. Focused FK graph
stress covers the previously stale same-process verifier, ordinary native
reopen, and forced `.shm` rebuild; broader native redo/checkpoint and
DDL/file-lifecycle recovery remain partial.
No-live ownerless startup also leaves InnoDB checkpoint suppression disabled
while the current native redo header is authoritative, then enables suppression
after startup reconciliation before ownerless writes can run; the focused DDL
stress statement-lock retry selector now loops without the previous concurrent
startup failure. Ownerless rollback-history publication now writes
payload-bearing native-support records on the paired fast path and per-page
fallback path, and no-live/live-peer reclaim treats rollback/undo,
`FIL_PAGE_TYPE_SYS`, and `FIL_PAGE_TYPE_TRX_SYS` native-support records as
startup-critical obligations that require exact native page proof before WAL
truncation. Final close no longer waits for purge just to force that proof; if
native file-operation, DML checkpoint, or rollback-history obligations remain,
the page-version WAL is retained and later opens/reclaim keep proving from that
durable evidence.
No-live stale-reader recovery now replays retained page-version WAL to the
latest committed checkpoint boundary, not the older reader-visible boundary,
before rebuilding `.shm` or allowing later reclaim to rely on native files.
The same FK graph reducer then exposed a separate false unique-secondary
duplicate when ownerless page refresh could run after an MTR byte mutation.
Ownerless X/SX page-latch preparation now acquires real page-write ownership
before B-tree cursor positioning, and the low-level secondary delete-mark path
prepares ownerless ownership before mutating the record byte. The fix preserves
MariaDB duplicate-key behavior without treating unexpected errno 1062 as
retryable; focused 48-round FK graph stress covers the regression.
...
finish for non-descending, descending, and composite mixed-direction
replacement keys. Recovery with a live peer proves the retained unique key on
the AUTO_INCREMENT column, replacement `PRIMARY` metadata including descending
and composite `COLLATION`/key-part order, native file-operation marker
retention/drain, and duplicate-key-consumed AUTO_INCREMENT gaps across forced
`.shm` rebuild for those deterministic ALTER shapes.
...
tablespace registration. Mutating statements are never retried after execution
begins. Immediately after an exact peer dictionary-generation refresh, parsed
autocommit `INSERT`/`REPLACE`/`UPDATE`/`DELETE` targets are instead warmed with
a zero-row native read under the existing dictionary statement lock; a `1932`
there repairs the caches before the mutation begins. Unparsed mutation shapes,
DDL, locking reads, and explicit-transaction statements do not use that
preflight path.
...
Embedded ownerless SQL CI partitions the registered sixteen weighted CTest
shards across four production `embedded-prod` matrix jobs. Each job runs four
non-overlapping shards with `--parallel 1` and records its regex and parallelism
...
`INSERT ... VALUES` statements with one through 65536 row constructors, matching
...
2048-row, plus 4096-row, 8192-row, 16384-row, 32768-row, and 65536-row bulk
diagnostics while keeping row lists above 65536 and broader DML on the previous
...
The deferred publisher now also distinguishes an evicted buffer-pool page from
a true resident-page mismatch when validating captured transaction images:
valid captured images whose page LSN is at or below the statement visible LSN
can prove the dirty page even after the buffer page leaves the pool, while a
resident mismatch still rejects the image when the resident page has the same
LSN and does not match after normal flush checksum/header initialization; a
different-LSN resident buffer page no longer suppresses a captured transaction
image whose page LSN is already covered by the transaction visible commit
boundary. The focused single-owner
multi-row visible-fast selector covers the regression by preserving fast commit
visibility and zero deferred-page flush fallback for the small row-list shape.
...
The 65536-row follow-up applies the same parser-proven policy to the next row-
list edge. Focused SQL coverage now proves a 65536-row single-owner statement
uses one append session and transaction-deferred page publication, while the
adjacent 65537-row statement stays outside append batching and deferred
latest-checkpoint coalescing. A reduced 131072-row production probe with 65536
rows per statement reported ownerless bulk at `87019.79` rows/s with an
ownerless/ordinary ratio of `0.5527`; the first statement stayed close to
ordinary at `0.9836`, while the remaining non-empty-table statement reported
`0.3637`. The same sample exposed the larger remaining single-row autocommit
...
published native-support `FIL_PAGE_UNDO_LOG`, production history proof carries
payload-bearing native-support records, and forced native-support publish
failure takes positive native history flush pages with zero accepted proof
samples.
...
The native-support proof-only WAL slice introduced explicit proof-only metadata
records instead of page-image records. The rollback-segment and undo proof
counts remain part of the same two-page proof contract, but proof-only records
carry zero payload bytes, are rejected by page-image reads, are skipped by
page-version replay and checkpoint retained-record callbacks, and still remain
subject to the existing WAL retention and checkpoint rules. A reduced
...
Ownerless production/stats-off history-proof publication then appended those
two proof-only records through one narrow paired native hook while the history
MTR still owned both page latches. The DDL-stress retry stability slice keeps
that helper covered as proof-only primitive behavior, but changes production
history proof back to payload-bearing rollback-segment and undo native-support
records with real page checksums on both the paired hook and per-page paths, so
startup and reclaim can validate exact native history state. Focused SQL
coverage proves pair calls succeed with zero exact history flush pages while the
detailed-counter proof path remains covered separately. A writer-open WAL
selector now also proves the production pair contract appends payload-bearing
native-support records and zero native-support proof-only records before
close-time reclaim can drain proven native-support state. Short
production stats-off throughput samples remained noisy, so this is tracked as a
bounded hook/append overhead reduction rather than a broad throughput-completion
claim.
...
peer-owned snapshot-sensitive pages remain unchanged. Current-owner direct-read
pins still block foreground/timer checkpoint scheduling when their boundary is
older than page-visible state, but they no longer force the hot synthesized
snapshot-boundary WAL scan path during same-runtime single-owner writes; stale
owner-generation or peer-owned pins remain conservative. This is a bounded
hot-path pruning slice, while replacing or shrinking the remaining
history-proof page publication remains a planned performance target.
Ownerless page-version reads now also trust shared page-index absence only
when the runtime proves a single-owner epoch, one active statement, no active
ownerless native write state, and no active ownerless transactions. Full
native-support page images now publish shared page-index entries, while
proof-only native-support metadata remains unindexed as non-page-image
evidence. Direct indexed hits can also skip appended-tail validation in that
single-owner one-statement epoch, including the local visible-fast writer, while
absent-index proof remains disabled during active ownerless native write state.
Retained snapshots, peer-present reads, startup-WAL readers, scan-required index
states, and same-process multi-statement work remain on the conservative WAL
scan path.
...
Ownerless read/write is implemented for the admitted surface: 64-bit
embedded Linux on validated local ext4, XFS, tmpfs, or overlay, macOS on local
APFS, and Windows on local NTFS; persistent InnoDB application tables; and the
DML and DDL shapes explicitly covered below. Remote, unknown, FUSE, and
unlisted local filesystems fail before ownerless startup with
`MYLITE_UNSUPPORTED_FILESYSTEM`; builds without a supported ownerless backend
return `MYLITE_UNSUPPORTED_PLATFORM`. Native platform/filesystem qualification
is part of that boundary. Per-engine ownerless designs for non-InnoDB durable
tables, additional filesystems, longer RQG-style campaigns, and broader
edge-case hardening remain future expansion beyond the admitted surface.
...
Ownerless history-proof pair publication introduced a page-log append-session
fast path for the two existing proof-only native-support records when the
normal append session is active. The proof-only helper writes two 64-byte
...
when the pair API is used, and remains covered by primitive tests. Production
history proof now writes payload-bearing native-support records for
startup-critical history proof on both the pair and per-page paths, while
focused history-proof SQL still proves successful pair publication in normal
builds and writer-open WAL SQL coverage verifies the pair records stay
payload-bearing rather than proof-only. A reduced stats-off 5000-row,
...
`ROLLBACK [WORK] TO [SAVEPOINT]`, and general `SET` expressions still use
MariaDB SQL parsing, and ownerless execution keeps the existing statement-lock,
page-version, and publication path. After successful transaction completion,
ownerless execution observes MariaDB's native post-command session state.
`COMMIT` or `ROLLBACK` with explicit `RELEASE`, or with
`completion_type=RELEASE`, therefore leaves subsequent SQL with MariaDB's
connection-gone diagnostic while `mylite_close()` skips a synthetic rollback
against the released session and completes normal ownerless cleanup. A focused
production profiled `^Tests_DB`
...
at `100663304` bytes versus the then-configured `100663296` bytes and matching
`FORMAT_10_8` format.
...
the then-configured `100663296` bytes.
...
A later embedded redo log size profile sets the native embedded
`innodb_log_file_size` to `16777216` bytes, preserving MariaDB redo recovery
and rebuild logic while reducing clean scan work compared with the inherited
96 MiB daemon default; the focused before/after production probe moved ordinary
`startup_innodb_recovery_start_ms_avg` from `45.391 ms` to `23.570 ms` and
ownerless from `32.755 ms` to `20.636 ms`, with zero warm-open redo rebuilds
in both samples.
...
`origin/main` (`4760d5128096e4560bc62cc19f7066bc15ff07d8`), the ownerless
branch stayed faster after the current performance CI slices. A 2026-06-22
refresh at `8d6f13986a69cd8d1126f4db2d6ef20b8d175c5a` reported two warmed
20-iteration local samples at `125.558-131.458 ms` per warm open/close versus
main at `370.065-380.739 ms`. A matching internal branch probe reported
ordinary warm open/close at `128.592 ms`, with `102.490 ms` open,
`26.098 ms` close, and active-runtime reconnect at `0.896 ms`, so the current
performance focus remains native MariaDB/InnoDB process lifecycle startup
rather than ownerless coordination or active in-process reconnect.
...
The startup page-write guard then made `mtr_t::ownerless_page_write_enter()`
itself return during InnoDB startup/recovery, even when ownerless pre-start
hooks are installed for lifecycle and recovery metadata. This prevents native
doublewrite and system-tablespace bootstrap writes from entering ownerless
page-write lock/refresh/publication paths before redo/log initialization has
finished. The guard fixes repeated fresh ownerless InnoDB bootstrap in one
process and keeps the production embedded performance probe on real timing
paths instead of aborting during probe setup; broader redo/checkpoint and
DDL/file-lifecycle recovery remain separate work.
...
`ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=1`/`2`/`4`/`8`/`16` rebuilds, while preserving the
...
close leaves that WAL for recovery unless exact native page proof already
covers the retained records; the native-successor shortcut is limited to writer
runtimes and now requires the MyLite-owned `.ibd` page at the retained record's
expected offset to carry the same space id, page number, and successor page LSN,
while reader-only consumers retain WAL when exact proof is missing.
Writer runtimes still use the normal statement, timer, and close reclaim paths.
...
gate without changing native InnoDB lock timeout semantics. The tracked
statement timeout and shared MDL conversion use 64-bit milliseconds, preserving
MariaDB's legal one-year `LONG_TIMEOUT` range instead of capping waits at
`UINT_MAX` milliseconds. Contended
...
The release-qualified ownerless read/write surface admits persistent InnoDB
...
coverage; any other valid MariaDB DDL is rejected before execution with
`MYLITE_ERROR` and `ownerless read/write mode does not support this DDL recovery
shape`. Ownerless admission also rejects existing FULLTEXT or SPATIAL indexes,
while ownerless SQL rejects creation of those special indexes. The shipped
no-vector profile rejects vector DDL before creation; profile-enabled VECTOR
ownerless admission is outside the current compatibility claim.

Ownerless coordination counters and cleanup are fail closed at exhaustion or
unlock failure. Page-observation tokens and dictionary generations saturate
instead of wrapping; dictionary generation exhaustion reports `MYLITE_IOERR`
with `ownerless dictionary generation is exhausted`, and exhaustion after
active publication quarantines the runtime. Page-log read and append-session
release statuses are consumed, with failed release identity retained for close
retry, and shutdown decisions consume bounded recovered-rollback wait results
instead of assuming the rollback drained.

## Public API

| Capability | MyLite status | Compatibility target |
| --- | --- | --- |
| Open and close a database directory | 🟡&nbsp;Partial | Implemented for read/write local directory paths with one active database directory per process, a `.mylite/` naming convention, validated format-1 metadata, an advisory directory lock, a native-storage baseline layout under the database directory, lazy ownerless coordination-file creation for ownerless/shared-readonly opens or ordinary opens that must recover retained ownerless state, ownerless startup serialization, final no-live ownerless native DDL file-operation checkpoint evidence, final no-live ownerless shutdown redo-prefix repair using MariaDB-valid checkpoint-page backups, and ordinary close-time redo-prefix repair so a fresh ordinary InnoDB database can later acquire ownerless metadata and still reopen cleanly |
| Capability reporting | 🟡&nbsp;Partial | `mylite_capabilities()` reports same-process multi-handle support in embedded builds and exposes ownerless shared read-only and read/write capability bits on 64-bit embedded Linux, macOS, and Windows builds; each ownerless open then requires an admitted local filesystem (Linux ext4/XFS/tmpfs/overlay, macOS APFS, or Windows NTFS) and successful database-directory primitive validation before runtime startup; unsupported operating systems or architectures return `MYLITE_UNSUPPORTED_PLATFORM`, and `MYLITE_CAP_SHARED_READONLY` means MariaDB server `@@read_only=ON` plus user-visible read-only SQL through ownerless coordination, not InnoDB `innodb_read_only` startup |
...
| Prepared statements | 🟡&nbsp;Partial | Reusable MariaDB prepared statements are exposed through `mylite_prepare()`, `mylite_step()`, `mylite_reset()`, and `mylite_finalize()` with 1-based parameter binding; ownerless prepared plain reads recover once from the tested stale InnoDB dictionary-cache `1932` case, and ownerless autocommit `DELETE`/`INSERT`/`REPLACE`/`UPDATE` retries once after a provenance-tagged external-wait or page-write coordination deadlock; DDL, locking reads, explicit transactions, and untagged MariaDB deadlocks do not use that DML retry path |
...
| `ENGINE=MyISAM` | 🟡&nbsp;Partial | Controlled create/alter/rename/drop/reopen and row/index coverage verifies `.frm`, `.MYD`, and `.MYI` files inside `datadir/`; ownerless read/write mode rejects explicit MyISAM tables, persistent MyISAM application tables at open time, MyISAM `LIKE`/CTAS sources including derived CTAS, and MyISAM-to-InnoDB conversion attempts until per-engine coordination is designed |
| `ENGINE=Aria` | 🟡&nbsp;Partial | Explicit Aria table creation, row persistence, and `.MAI`/`.MAD` files under `datadir/` are covered; ownerless read/write opens reject explicit Aria tables and existing persistent Aria application tables until per-engine coordination is designed |
| `ENGINE=MEMORY` | 🟡&nbsp;Partial | Explicit MEMORY table creation is covered; table definitions survive reopen while rows remain process-local and empty after reopen; ownerless read/write opens reject explicit MEMORY tables and existing persistent non-InnoDB application tables until connection-local table behavior is designed |
| Dynamic external engines | ➖&nbsp;Out&nbsp;of&nbsp;scope | Default embedded profile does not load storage-engine plugins; explicit `ENGINE=BLACKHOLE`, `ENGINE=ARCHIVE`, and `ENGINE=CSV` requests are covered as rejected where available, and ownerless read/write opens reject non-InnoDB engine requests before MariaDB execution |

## Directory Ownership

Ownerless page-log format 2 and checkpoint-stage format 3 are internal,
experimental formats. The page-log keeps immutable header metadata and both
acknowledged-end slots in separate 4 KiB failure units, binds record metadata
into its checksum, repairs only an unacknowledged sequential tail, and requires
a checksummed sibling stage before physical compaction. Stage installation
rewrites the existing WAL inode and is recoverable after every durable state
transition. An older nonempty ownerless WAL fails closed; no format migration is
claimed before ownerless mode graduates from experimental status.

| Capability | MyLite status | Target behavior |
| --- | --- | --- |
| Primary portable database directory | 🟡&nbsp;Partial | Open/create establishes and validates a MyLite-owned directory with `mylite.meta`, `mylite.lock`, `datadir/`, `tmp/`, and process-local `run/`; fresh ordinary read/write opens do not create `concurrency/`, while ownerless/shared-readonly opens and ordinary opens that find durable ownerless runtime files use `concurrency/` metadata, lock, shared-memory, WAL, and checkpoint anchors; ordinary close-time redo repair remains process-local and does not create ownerless coordination files; `.mylite/` is recommended but not enforced |
| Ownerless concurrency metadata | 🟡&nbsp;Partial | The directory owns a durable concurrency metadata file with format, MariaDB base, database UUID, concurrency generation, and exclusive-mode state, protected by a byte-range `PERSISTED_CONFIG` lock while it is created or validated; ordinary runtimes hold `mylite.lock` exclusively for their complete lifetime, while ownerless read/write and shared-read-only runtimes hold it shared, so either side of an ordinary/ownerless open race receives `MYLITE_BUSY` after the other mode wins while ownerless peers remain compatible; process slots independently distinguish ordinary-exclusive, shared-read-only, and ownerless-read/write modes and reject incompatible live-mode allocation |
| Ownerless platform proof | 🟡&nbsp;Partial | `MYLITE_OPEN_OWNERLESS_RW` and `MYLITE_OPEN_SHARED_READONLY` are available on 64-bit embedded Linux, macOS, and Windows builds and admit only local Linux ext4/XFS/tmpfs/overlay, macOS APFS, or Windows NTFS filesystems. Unknown, remote, FUSE, and other unvalidated filesystems return `MYLITE_UNSUPPORTED_FILESYSTEM` before a new database directory is created. The directory probe verifies shared-mapping visibility, byte-range lock conflict, lock close/exit behavior, file grow/remap, wait behavior, and process identity. A successful format-2 `concurrency/mylite-ownerless-platform.meta` proof binds the platform, filesystem, volume identity, and required primitive results so a later open reruns the proof after any relevant identity change. Hook coverage proves exact rejection while ordinary read/write opens remain outside the ownerless gate; native CI exercises APFS and NTFS, while the Linux filesystem release gate covers ext4, XFS, tmpfs, and overlay. |
| Ownerless native redo startup guard | 🟡&nbsp;Partial | First ownerless read/write startup after ordinary native shutdown allows native InnoDB recovery/checkpoint during `mysql_server_init()` when no ownerless page-version WAL payload, native checkpoint marker, live peer, or usable redo-header backup exists; the native redo log is then the recovery authority, while stale sidecar backups still cannot replace a current native header |
| Ownerless shared-memory file | 🟡&nbsp;Partial | `concurrency/mylite-concurrency.shm` is created and grown under `RECOVERY` then `SHM_RESIZE` byte-range locks, validated through `MAP_SHARED`, starts with a fixed 128-byte MyLite header bound to the database UUID, and contains fixed process-registry, wait-channel, MDL lock-table, transaction-registry, read-view-registry, page-version pin registry, InnoDB lock-registry, redo-visibility, page-version-index, dictionary-generation, page-write lock-registry, and AUTO_INCREMENT high-watermark registry segments; clean opens preserve those segments, while dirty, rebuilding, invalid, no-live-process stale, or incompatible-format volatile state is rebuilt with an incremented recovery generation; volatile process active/live counts are read through `MAP_SHARED` mappings so recovery decisions see peer updates; ownerless SQL opens serialize core `mysql.*` compatibility-table bootstrap through `mylite-concurrency.lock` and the full embedded native startup, connection, dictionary-generation initialization, bounded retry after partial MariaDB embedded startup cleanup and redo-prefix restore, plus final no-live ownerless native DDL file-operation or `ALTER TABLE ... AUTO_INCREMENT` checkpoint evidence and shutdown redo-header repair through separate `mylite-runtime-startup.lock`, creating `concurrency/` before taking that startup lock, clearing process-global ownerless SQL/InnoDB hooks before MariaDB embedded shutdown while preserving native hook context until `mysql_server_end()` completes and keeping the shared-file deletion guard installed until shutdown finishes, and avoiding classic `fcntl()` same-file close release hazards; hook-only coverage now forces four post-`mysql_server_init()` startup failures in each phase before successful ownerless open, ordinary native reopen after ownerless activity, and forced-`.shm` ownerless reopen, proving the enlarged bounded retry budget uses the existing cleanup and redo-prefix restore path without changing successful-open behavior; the 16,384-entry page-version index caches the newest WAL record offset per page, is replayed from `mylite-concurrency.wal` when `.shm` is rebuilt, distinguishes absent index entries from incomplete-index or older-snapshot lookups for diagnostics, keeps authoritative WAL scanning for page reads the index cannot prove, performs direct page-index reads and WAL scans under the existing page-log read guard instead of nested checkpoint read locks, classifies stats-enabled WAL-scan misses by page-key absence versus same-page-not-visible outcomes, returns page-log snapshot-boundary/external-lineage flags to the ownerless InnoDB read hook, uses process-local negative caching only after an authoritative WAL snapshot/scan has proven same-page absence instead of before the first index-miss WAL scan, and extends true no-same-page proofs across unrelated page-index generation changes with a WAL-generation/covered-offset tail cache; the index is reclaimed with live peers only when the page-version pin registry has zero active pins; active pins retain WAL until release; guarded ownerless SQL page-version reads cover direct or prepared `SELECT`/`WITH` statements at a live page-version read LSN while the page-visible LSN remains the durable recovery/checkpoint boundary, with a per-handle monotonic read watermark and a shared read pin opened before clean-page refresh; if no page-visible LSN exists yet, the baseline read pin still marks eligible plain reads as ownerless plain-read statements without exposing an external page boundary; successful direct `mylite_exec()` reads keep the handle pin after returning until a replacement read, non-read/current-read statement, error, close, or dead-owner cleanup releases it, and prepared result cursors keep the handle pin until the result is exhausted, reset, or finalized, so active result pins block single-owner statement/timer checkpoint scheduling while the same embedded runtime may still hold stale clean pages; when an eligible read retains a page-version read LSN, visible-boundary external refresh, clean-page refresh, and file-read overlays reject lower visible-boundary replacements for user data/index/blob pages while still refreshing native undo, system, allocation, and recovery pages; current live reads still accept real current ownerless page images that advance the page or are selected by current commit LSN and reject lower retained user-page boundary images after the same handle has already observed the same page at an equal-or-newer ownerless commit boundary or when the page-log record is flagged as a synthesized snapshot boundary; when a handle first observes a new ownerless process generation or advances its handle pin, clean-page refresh bypasses the single-owner skip because peer commits may already be native-checkpointed and reclaimed from WAL; repeatable-read and serializable transactions publish shared page-version pins for that read LSN on first consistent read, `START TRANSACTION WITH CONSISTENT SNAPSHOT` publishes the pin before SQL execution, and pins release on transaction end, rollback/close, or dead-owner cleanup, while transactions with local writes or locking reads avoid global refresh and clean-page refresh skips locally dirty buffer pages |
| Ownerless recovery anchors | 🟡&nbsp;Partial | `concurrency/mylite-concurrency.wal` and `concurrency/mylite-concurrency.ckpt` are created under `RECOVERY` with fixed headers bound to the database UUID; guarded ownerless SQL writes page-version records after the `.wal` header, persists latest raw redo and page-visible LSNs in `.ckpt`, treats retained page-version WAL records up to the latest committed checkpoint LSN as the no-live-process recovery authority once no live readers remain, rewinds existing native InnoDB tablespace pages selected by commit LSN first and page LSN second, preserves matching native disk pages in product no-live replay when the native and retained WAL images have the same page LSN, retains complete committed page-version WAL records after no-live-process tablespace replay until native redo/checkpoint reconciliation can prove truncation safe, skips retained page-version records for tablespaces that no longer exist during product no-live replay, replays retained page-version WAL records into the shared page-version index when `.shm` is rebuilt, and seeds rebuilt or clean runtime-attached redo-visibility state monotonically from `.ckpt`; no-live `.shm` rebuilds checkpoint retained reader-boundary WAL without replaying stale page images when their remaining state is stale read-view/page-pin evidence without native writer recovery evidence, while stale-reader rebuilds that still have user page-version records replay native tablespaces to the latest committed boundary before rebuilding shared state; focused SQL coverage verifies single-table dropped, same-schema and cross-schema same-statement multi-dropped file-per-table absence, ordinary-created file-per-table final state with secondary-index metadata/use, LIKE-copy, and CTAS-created file-per-table final states, same-name recreated and `CREATE OR REPLACE TABLE` replacement file-per-table final states, including `CREATE OR REPLACE TABLE ... LIKE` copied-shape replacement and `CREATE OR REPLACE TABLE ... AS SELECT` populated replacement, with page-0 space-id identity checks, cross-schema renamed file-per-table final state, truncated file-per-table post-truncate state, copy-style force-rebuilt, primary-key-rebuilt, and compressed row-format-rebuilt file-per-table final states, same-schema and cross-schema multi-pair rename-swap final states, and multi-table dropped-schema absence through ownerless/native reopen before and after forced `.shm` rebuild; these DDL replay final-state checks accept checkpointed or retained page-version WAL until native redo/checkpoint reconciliation can prove truncation safe; non-read-only close now advances local native LSN state when needed, and no-live close first publishes native `FILE_CHECKPOINT` evidence for completed DDL file-operation redo when it is the final live ownerless process, drains a real SQL `ALTER TABLE ... AUTO_INCREMENT` native file-op checkpoint marker after the final live peer closes, drains stale native file-op checkpoint markers even when no page-visible LSN exists, advances page-visible state to a newer raw latest LSN by publishing eligible native support/allocation/system buffer-pool pages and flushing native dirty pages, then refreshes external clean page state, forces a native InnoDB checkpoint, compacts page-version WAL records at or below the durable page-visible LSN covered by that native checkpoint, retains newer complete records, and replaces the shared page-version index before releasing checkpoint locks or leaves WAL scanning enabled as the safe fallback; this close-time reclamation path compacts user-page WAL with no live peers; with live peers, it compacts only when the proof scan has no process-local native page records after a nonblocking ownerless statement gate proves zero active page-version snapshot pins plus no active ownerless native write/recovery state; user data/index records remain retained until no-live close when an active pin or live writer requires them, while native-support proof records may already be checkpointed or reduced to native-support-only retention; active pins retain WAL until release, while page-version publish can still synthesize boundary records from a native page whose page LSN is at or below the oldest active pin so active readers and later post-release cleanup have a compact boundary image; bounded repeated same-row and distinct large-row expanding-page SQL writer pressure under a live reader are covered, an opt-in `mylite_open_config.ownerless_page_log_limit_bytes` soft cap returns `MYLITE_BUSY` for direct or prepared ownerless writes when active snapshot pins retain WAL at or above the configured byte limit, and `mylite_ownerless_pressure_status()` reports the active pin count, oldest pin LSN, raw WAL bytes, configured limit, and current throttle-reached state; thresholded ownerless write/DDL/transaction-end statement-boundary checkpoint scheduling reuses the native reclaim path for user-page WAL when no peer process is live, and live-idle coverage accepts checkpointed WAL or native-support-only retention while peers remain live and after final peer close, while user WAL remains covered by active-pin/live-writer retention tests, and the ownerless runtime scheduler can independently checkpoint retained WAL after a shared read-only snapshot pin releases while the writer remains open and idle; ordinary exclusive read/write opens enable page-version reads only when retained page-version WAL payload records exist, use a checkpoint-visible native clean-page refresh when `.ckpt` still proves a newer ownerless boundary after WAL compaction, and arm ownerless uncheckpointed file-operation recovery only when retained WAL, a native file-op checkpoint marker, or a valid saved redo-header backup proves prior ownerless redo/checkpoint suppression; that recovery path normalizes datadir-prefixed `.ibd` FILE redo names, including the observed leading-separator-stripped datadir form, before prepending the active datadir, so covered ownerless multi-writer commits and DDL policy handoffs remain visible through native exclusive reopen before and after forced `.shm` rebuild without taxing fresh ordinary opens; unsafe-hook SQL coverage now kills a writer after volatile `.shm` page-visible publication but before `.ckpt` persistence, kills after native checkpoint proof but before close-time page-log reclamation, and pauses after native checkpoint proof while a peer commits newer page-version records, proving normal reopen and forced `.shm` recreation do not depend on volatile `.shm` as the only copy of committed updates and that a resumed paused closer does not grow or invalidate already reclaimed WAL; page replay currently resolves existing tablespaces by InnoDB page-0 space id and still relies on the conservative native-file bridge for broader DDL/file lifecycle edge cases |
| Ownerless live snapshot reader-close reclaim | 🟡&nbsp;Partial | Focused SQL coverage for live snapshot pins, including synthesized native-boundary records, verifies peer page-version WAL remains retained while the repeatable-read pin is active; after release, ownerless/native reopen and forced `.shm` rebuild preserve committed rows while close-time reclaim either checkpoints retained WAL when native proof and drained rollback history allow it or leaves native-support WAL retained with native rollback-history evidence. This narrows reader-close lifecycle timing; broader active-reader pressure crash/oracle matrices, DDL/file-lifecycle recovery, native redo/checkpoint reconciliation, and external MariaDB/RQG stress remain planned |
| Ownerless no-live native checkpoint cutover proof | 🟡&nbsp;Partial | Focused SQL coverage now performs a non-empty ownerless 64-row multi-row insert, closes the final ownerless process, waits for page-version WAL compaction, asserts durable checkpoint-visible LSN advancement and volatile redo-visible state at or beyond that LSN, deletes both `mylite-concurrency.wal` and `mylite-concurrency.shm`, and verifies ordinary `MYLITE_OPEN_READWRITE` sees the expected rows while `mylite_ownerless_innodb_checkpoint_covers_lsn()` accepts the reclaimed visible LSN; duplicate same-page cutover coverage now also holds a live snapshot pin across two ownerless updates to the same InnoDB row, asserts duplicate retained user-page WAL identities, then requires no-live reclaim to drain user-page WAL to checkpointed or native-support-only state before ordinary native reopen after forced `.shm` rebuild; reclaim rechecks every retained record, so older same-page WAL records require exact native proof or an already verified later same-page successor before discard. This strengthens no-live payload-WAL reclaim evidence but does not close broader DDL/file-lifecycle recovery, native redo/checkpoint reconciliation, or external MariaDB/RQG stress gaps |
| Ownerless no-live pressure reclaim proof | 🟡&nbsp;Partial | Focused SQL coverage now lowers the durable checkpoint-visible LSN below the raw latest LSN while a repeatable-read snapshot pin retains page-version WAL, releases the final reader, requires no-live reclaim to advance the visible boundary and checkpoint the WAL, deletes both `mylite-concurrency.wal` and `mylite-concurrency.shm`, and verifies ordinary `MYLITE_OPEN_READWRITE` sees the committed row while `mylite_ownerless_innodb_checkpoint_covers_lsn()` accepts the reclaimed visible LSN. This proves the pressure-specific raw-latest-to-visible advance has a native checkpoint backing after retained WAL is removed; broader active-reader pressure crash/oracle matrices, DDL/file-lifecycle recovery, native redo/checkpoint reconciliation, and external MariaDB/RQG stress remain planned |
| Ownerless killed active-reader pressure pin | 🟡&nbsp;Partial | Focused production coverage now holds an idle live ownerless peer plus a repeatable-read snapshot reader, commits multiple writer updates that leave page-version WAL retained by that reader, verifies a configured `ownerless_page_log_limit_bytes` writer returns `MYLITE_BUSY` while the reader pin is live, kills the reader, and then verifies a configured writer can proceed while the idle peer remains live after dead-pin cleanup. Final ownerless close reclaims user page-version WAL while rollback-history native-support WAL may remain, forced `.shm` rebuild and ordinary native reopen preserve the final rows, and the read-view active count reaches zero. This narrows killed-reader pressure crash coverage; broader active-reader pressure DDL crash/oracle matrices and randomized external stress remain planned |
| Ownerless rename-create stale-reader replay | 🟡&nbsp;Partial | Focused SQL coverage updates an InnoDB file-per-table table while an older ownerless repeatable-read snapshot retains page-version WAL, renames that original table away, creates a different table at the original SQL name, writes both final tables, keeps local post-DDL `INSERT ... VALUES` on the conservative native-page bridge while the external pin is active, and verifies through ownerless/native reopen before and after forced `.shm` rebuild that no-live stale-reader recovery replays the moved table to the latest committed boundary, the moved table keeps the original `SPACE`, and the new original-name table has a distinct `SPACE`, with `.ibd` page-0 identity checks for both; final close may retain payload-bearing page-version WAL for this DDL edge until broader native redo/checkpoint reconciliation can prove truncation safe; broader DDL/file-lifecycle recovery remains partial |
| Ownerless CTAS post-create DML | 🟡&nbsp;Partial | Focused SQL coverage creates a CTAS file-per-table destination while another ownerless process pins a stale repeatable-read snapshot, verifies immediate numeric and payload `UPDATE`, `DELETE`, and `INSERT ... SELECT` statements against that CTAS table, retains page-version WAL while the reader pin is live, and verifies ownerless/native reopen before and after forced `.shm` rebuild while accepting final WAL checkpoint or native-support-only rollback-history retention; active-reader pressure coverage also blocks post-create `UPDATE`, `DELETE`, and `INSERT ... SELECT` against an existing CTAS destination while retained WAL is at the configured soft limit, verifies no side effects, then verifies CTAS DML succeeds after the pin releases; `tools/ownerless-ctas-dml-trace` emits deterministic CTAS create/update/delete/insert SQL plus repeatable snapshot reader and final oracles for external harness input, with focused Docker-backed MariaDB 11.8 scale-2 replay evidence; exhaustive CTAS DML/crash matrices and randomized external stress remain planned |
...
| MyLite-owned transient paths | 🟡&nbsp;Partial | Durable database paths use per-runtime `tmp/<runtime-id>/`, `run/<runtime-id>/`, and `mylite.lock` inside the database directory; clean close removes the current runtime's children and prunes an empty `run/` root, clean ordinary exclusive open replaces stale inactive runtime children after taking the exclusive directory lock, ownerless peers holding shared lifetime locks never delete another live runtime's `run/` or `tmp/` children, and `:memory:` uses a transient runtime directory that is removed on final close |
...
recovery bridge. When a validated backup is used for repair, MyLite restores
both the saved 12 KiB startup prefix and the recorded physical redo file size
when the current file differs within the bounded tolerance.

Ordinary exclusive read/write close now forces a native checkpoint, captures a
validated InnoDB redo startup prefix before final embedded shutdown, and
conditionally restores it after `mysql_server_end()` only when the
post-shutdown prefix no longer passes MariaDB-current checkpoint validation.
Focused production coverage creates an
...
| `CREATE TABLE ... LIKE` | 🟡&nbsp;Partial | Representative MariaDB table-definition copy behavior is covered for default-engine tables, including ownerless peer visibility from an already-open handle, hook-build live-peer crash recovery after native destination table creation before ownerless dictionary finish, hook-build live-peer crash recovery for `CREATE OR REPLACE TABLE ... LIKE` replacement-copy completion before ownerless dictionary finish, and no-live stale-reader replay for `CREATE OR REPLACE TABLE ... LIKE` preserving the copied-shape replacement tablespace final state with page-0 space-id identity checks |
| `CREATE TABLE ... SELECT` | 🟡&nbsp;Partial | Representative CTAS behavior is covered over MyLite tables, including ownerless peer visibility from an already-open handle, hook-build live-peer crash recovery after native destination table creation/population before ownerless dictionary finish, hook-build live-peer crash recovery for `CREATE OR REPLACE TABLE ... AS SELECT` replacement-copy completion before ownerless dictionary finish, and no-live stale-reader replay for `CREATE OR REPLACE TABLE ... AS SELECT` preserving the populated replacement tablespace final state with page-0 space-id identity checks |
| Schemas/databases | 🟡&nbsp;Partial | Controlled `CREATE DATABASE`, qualified table access, and `DROP DATABASE` lifecycle are covered inside `datadir/`; ownerless coverage verifies peer refresh for a schema and InnoDB table created by another process, schema default charset/collation creation and `ALTER DATABASE` refresh through MariaDB's native `db.opt` file, idempotent `CREATE SCHEMA IF NOT EXISTS` / `CREATE DATABASE IF NOT EXISTS` and `DROP SCHEMA IF EXISTS` spellings including no-op duplicate-create/default-preservation and absent-drop behavior, peer-visible schema drop, final absent-schema state through ownerless/native reopen before and after forced `.shm` rebuild, hook-build live-peer crash recovery for killed `CREATE DATABASE|SCHEMA` after native schema directory/`db.opt` creation and killed `ALTER DATABASE|SCHEMA` after native `db.opt` rewrite with the native file-operation marker clear, hook-build live-peer crash recovery for killed table-bearing `DROP DATABASE|SCHEMA` after native schema/table removal with marker retention until final no-live drain, hook-build live-peer crash recovery for killed two-table `DROP DATABASE` inside MariaDB's schema table-drop loop after early `db.opt` cleanup and one native table file operation but before schema removal is logged, hook-build live-peer crash recovery for killed empty `DROP DATABASE|SCHEMA` after native schema removal with the marker clear, hook-build live-peer crash recovery with the native file-operation marker clear for killed duplicate `CREATE DATABASE IF NOT EXISTS` preserving original defaults, killed missing `CREATE SCHEMA IF NOT EXISTS` creating schema defaults, killed missing `DROP SCHEMA IF EXISTS` preserving real schema/table state and missing-schema absence, and killed empty `DROP DATABASE IF EXISTS` preserving removed-schema absence, hook-build live-peer crash recovery for killed table-bearing `DROP DATABASE|SCHEMA IF EXISTS` with marker retention until final no-live drain, each before ownerless dictionary finish, and retained-WAL stale-reader schema-drop replay preserving dropped schema and multi-table absence through ownerless/native reopen before and after forced `.shm` rebuild; broader schema behavior remains planned |
| Ownerless schema comment DDL crash coverage | 🟡&nbsp;Partial | Hook-build live-peer recovery covers killed named `ALTER DATABASE <schema> COMMENT = 'literal'`, current-schema `USE <schema>; ALTER DATABASE COMMENT = 'literal'`, combined named `ALTER DATABASE <schema> COMMENT = 'literal' DEFAULT CHARACTER SET ... COLLATE ...`, collation-first named `ALTER DATABASE <schema> DEFAULT COLLATE ... CHARSET ... COMMENT = 'literal'`, and combined current-schema `USE <schema>; ALTER DATABASE COMMENT = 'literal' DEFAULT CHARACTER SET ... COLLATE ...` writers after MariaDB rewrites native `db.opt` but before ownerless dictionary finish, with the native file-operation marker clear, recovered `INFORMATION_SCHEMA.SCHEMATA.SCHEMA_COMMENT`, recovered or preserved schema defaults as appropriate, post-recovery table writes, ownerless/native reopen, and forced `.shm` rebuild; representative invalid-option cleanup now verifies rejected create, alter, charset/collation pairing, and current-schema comment/default option forms preserve the original schema metadata and marker-clear state; randomized external DDL stress remains planned |
| Ownerless current-schema default DDL crash coverage | 🟡&nbsp;Partial | Hook-build live-peer recovery covers killed current-schema `USE <schema>; ALTER DATABASE DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci` writers, killed combined named `ALTER DATABASE <schema> COMMENT = 'literal' DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci` writers, and killed combined current-schema `USE <schema>; ALTER DATABASE COMMENT = 'literal' DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci` writers after MariaDB rewrites native `db.opt` but before ownerless dictionary finish, with the native file-operation marker clear, recovered schema defaults, pre-alter table collation preservation, post-recovery default inheritance, ownerless/native reopen, and forced `.shm` rebuild; representative invalid-option cleanup now verifies rejected current-schema schema-option forms do not persist rejected comments/defaults; randomized external DDL stress remains planned |
...
| Primary and secondary indexes | 🟡&nbsp;Partial | Controlled MyISAM primary and secondary indexed predicates are covered, including an index added by copy-style `ALTER TABLE`; ownerless InnoDB coverage verifies peer refresh for secondary-index create/drop/rename, descending and mixed-direction key-part direction, prefix-plus-direction key-part direction/length, `VARCHAR`, utf8mb4, and TEXT/BLOB prefix key-part length, TEXT/BLOB prefix-plus-direction key-part length/direction, unique TEXT/BLOB prefix length/enforcement, unique TEXT/BLOB prefix-plus-direction length/direction/enforcement, and ignored/not-ignored metadata, unique secondary-index enforcement/drop, primary-key idempotent ADD no-op preservation including metadata-only live recovery for the no-op branch and live-peer crash recovery for replacement from `id` to `code`, descending replacement from `id` to `code DESC`, composite direction replacement from `id` to `(tenant_id ASC, code DESC)`, and AUTO_INCREMENT-preserving replacement from `id` to `code`, `code DESC`, and `(tenant_id ASC, code DESC)` while retaining a unique `id` key, with duplicate enforcement on the replacement key plus retained native file-operation marker drain after the final peer exits, no-live stale-reader replay evidence, and failed local ADD PRIMARY KEY cleanup before later peer replacement, descending primary-key replacement metadata/enforcement, composite direction primary-key replacement metadata/enforcement, AUTO_INCREMENT primary-key replacement that keeps the AUTO_INCREMENT column as a unique secondary index, AUTO_INCREMENT descending primary-key replacement metadata/allocation, and AUTO_INCREMENT composite primary-key replacement metadata/allocation |
...
| FULLTEXT, SPATIAL, and vector indexes | ⚪&nbsp;Planned | Support only where the selected native engine and embedded profile support them; ownerless read/write admission rejects databases that already contain FULLTEXT or SPATIAL indexes, ownerless SQL rejects top-level, ALTER, inline, and idempotent `FULLTEXT` and `SPATIAL` index DDL until full-text auxiliary state, spatial R-tree pages, and spatial predicate-lock coordination are designed, and vector index DDL is rejected by the shipped no-vector profile; focused pre-existing-index admission coverage proves FULLTEXT and SPATIAL rejection, while profile-enabled VECTOR ownerless admission remains outside the current compatibility claim |

## Transactions, Recovery, And Concurrency

| Capability | MyLite status | Compatibility target |
| --- | --- | --- |
| Ownerless column copy-lock table-wait negative proof | 🟡&nbsp;Partial | The hook-build SQL negative proof now arms the local ownerless table-wait callback while blocked column table-copy `ALTER TABLE` ADD, DROP, MODIFY, CHANGE, and RENAME variants run in both exact explicit `ALGORITHM=COPY, LOCK=EXCLUSIVE` and `LOCK=EXCLUSIVE, ALGORITHM=COPY` option orders, verifies they time out before the local callback, and verifies blocked metadata remains unchanged. This is negative evidence only; positive SQL-level local table-wait fault injection remains unclaimed beyond the covered external native table-wait registry path |
| Atomic commit | 🟡&nbsp;Partial | Explicit InnoDB transactions commit through native MariaDB/InnoDB hooks inside the MyLite database directory; ownerless coverage pauses multiple independent-table transactions before commit, releases the commits together, and verifies every delta is durable. A hook-only pre-serialization commit failure proves the original MariaDB error is returned, successful automatic rollback clears native transaction error state, ownerless bookkeeping is reconciled, and the same connection can read and complete a fresh transaction; a rollback-cleanup failure instead supersedes the retryable error and quarantines the connection |
| Rollback | 🟡&nbsp;Partial | Explicit InnoDB transaction rollback is covered, including hook-build crash coverage that kills a full rollback after one successful native InnoDB `row_undo()` step, verifies live-peer cleanup remains busy while another ownerless process is open, verifies no-live ownerless recovery preserves the original rows, and now repeats the native row-undo crash boundary on stored and virtual generated columns with generated-column secondary indexes plus FK `ON UPDATE CASCADE`, `ON DELETE CASCADE`, `ON DELETE SET NULL`, and trigger-audit side effects. Ownerless automatic deadlock rollback uses the same fallible coordination-aware rollback wrapper as explicit rollback; deterministic two-process deadlock coverage requires the victim connection to start, query in, and roll back a fresh transaction. A post-savepoint timeout regression keeps the pre-savepoint write hidden while the transaction is active, during native row undo, and across the plain-read refresh-to-MVCC boundary, then proves full rollback restores the original rows. Terminal rollback serialization retries the active or committed-in-memory cleanup boundary up to 64 times with exponential pauses capped at 64 milliseconds (about 3.7 seconds total); hooks force both three consecutive pre-serialization failures and exhaustion of all retry attempts, proving recoverable explicit rollback reaches zero ownerless transaction state and preserves later connection reuse while terminal exhaustion stays faulted for close recovery; MyISAM remains non-transactional |
| Savepoints | 🟡&nbsp;Partial | Explicit InnoDB savepoint rollback and release savepoint are covered through SQL transaction statements; direct and prepared ownerless coverage includes MariaDB's `ROLLBACK WORK TO name` and `ROLLBACK WORK TO SAVEPOINT name` spellings and verifies that native and shared explicit-transaction state, pre-savepoint writes, and the original repeatable-read snapshot remain active. Ownerless InnoDB transaction page images are snapshotted at savepoint boundaries and restored after successful native savepoint rollback so same-page handoff preserves pre-savepoint committed images while discarding post-savepoint images. Hook-build crash coverage kills `ROLLBACK TO SAVEPOINT` after one successful native InnoDB `row_undo()` step, verifies live-peer cleanup remains busy while another ownerless process is open, and verifies no-live ownerless recovery rolls the uncommitted transaction back to the original rows |
| Transaction release completion | 🟡&nbsp;Partial | Ownerless direct execution covers explicit `COMMIT RELEASE` and `ROLLBACK RELEASE` plus plain `COMMIT` and `ROLLBACK` under `completion_type=RELEASE`. MyLite records MariaDB's actual post-command released-session state, returns MariaDB connection-gone diagnostics for later SQL, permits `mylite_close()` without issuing a synthetic rollback on the released session, and verifies committed or rolled-back data through ordinary native reopen |
...
| Ownerless lock fault coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills a blocked writer after it enters the external ownerless record wait but before MariaDB grants the local waiting record lock, and kills a blocked writer after MariaDB grants a waited record lock and MyLite publishes that granted lock to shared state, proving live-peer cleanup remains busy and no-live reopen preserves only committed data. Primitive coverage kills a process while a table-lock wait entry is live, verifies the wait entry remains observable after process death, and proves owner cleanup removes it; embedded hook coverage directly asserts retained external table-wait snapshots dispatch through `wait_until_table` with stable transaction ID, table ID, mode, timeout, and result propagation; hook-build SQL coverage now proves a `foreign_key_checks=0` and `unique_checks=0` empty-table bulk insert waiting behind a peer ownerless `LOCK IN SHARE MODE` reader publishes a shared external native table-wait registry entry, clears it after release, supports retry insert, and remains visible through ownerless/native reopen and forced `.shm` rebuild. Hook-build SQL crash coverage kills that same native table-wait SQL waiter after the shared table-wait entry is published, verifies the dead wait remains observable, verifies live-peer cleanup remains busy while the blocking reader is still alive, verifies no-live recovery removes the dead wait after the blocker dies, confirms the interrupted insert is absent, and retries the insert through ownerless/native reopen and forced `.shm` rebuild. Hook-build SQL negative proof arms the local ownerless table-wait callback while representative DDL shapes that conflict with a remote transaction-duration `SHARED_WRITE` metadata lock time out, verifies blocked metadata remains unchanged, and fails if any tested SQL shape reaches the local callback. MariaDB-compatible instant column metadata operations are excluded because their metadata-lock mode is compatible with the held writer; they are covered by the separate ownerless instant-DDL success matrix. Positive SQL-level local table-wait fault injection remains unclaimed beyond the covered external native table-wait registry path, while ownerless `LOCK TABLES`/`UNLOCK TABLES` is explicitly rejected until SQL locked-table mode is designed |
| Ownerless writer tests | 🟡&nbsp;Partial | Normal embedded builds run `MYLITE_OPEN_OWNERLESS_RW` cross-process SQL writer tests that bypass the process-wide directory lock and route through the ownerless process, transaction, read-view, InnoDB lock, redo-visibility, page-version WAL, and checkpoint hooks; tests cover non-conflicting writers, same-page writer serialization, concurrent explicit commits on independent tables, native exclusive reopen and recovery-anchor checks after the concurrent explicit-commit race before and after forced `.shm` rebuild, row waits, gap-lock insert blocking plus post-release retry, serializable reader/writer blocking, serializable write-skew prevention for disjoint predicate-dependent updates, savepoint rollback before peer-visible commit, deadlocks across separate tables, mixed readers/writers, shared AUTO_INCREMENT assignment across concurrently opened insert workers, ownerless AUTO_INCREMENT DDL high-watermark refresh, ownerless AUTO_INCREMENT column-add rebuild refresh, ownerless AUTO_INCREMENT primary-key replacement refresh, ownerless AUTO_INCREMENT descending primary-key replacement refresh, a bounded independent-table writer/multi-object reader stress loop, committed external visibility through native refresh, direct and prepared SELECT visibility, shared read-only prepared reads and read-only rejection for direct/prepared writes, transaction first-read visibility after peer commits, transaction reads after local writes, repeatable-read and `WITH CONSISTENT SNAPSHOT` retention across peer churn, session-scoped and transaction-scoped read-committed visibility of later peer commits, shared-memory rebuild, no-live-process page-version replay with retained WAL, no-live stale-reader rebuild over retained reader-boundary WAL for single-table dropped, same-schema and cross-schema same-statement multi-dropped, ordinary-created, LIKE-copy, CTAS-created, recreated, renamed, truncated, force-rebuilt, primary-key-rebuilt, compressed row-format-rebuilt, same-schema and cross-schema multi-rename-swap file-per-table tablespaces plus multi-table schema-drop absence, live idle-peer page-log reclamation, active snapshot-pin blocking of live-peer reclamation when boundary proof is missing, native boundary synthesis for a live snapshot pin, active-reader pressure limit throttling for direct/prepared writes including prepared `INSERT ... SELECT`, representative DML/DDL write classes, AUTO_INCREMENT DDL high-watermark ALTER, variant DML/index/rename/truncate spellings, and schema/table-copy/replacement/replacement-copy/view/trigger dictionary variants, active-reader pressure diagnostics, active-pin boundary retention until release, killed snapshot-pin cleanup allowing live-peer reclamation, cross-process `ALTER TABLE` waiting, concurrent DDL table/space/index metadata allocation with online index replacement, peer-visible ownerless DDL for create, rename, truncate, post-truncate DML, large-table truncate reuse, drop, same-name recreate, idempotent table create/drop, `CREATE TABLE ... LIKE`, CTAS, standalone `CREATE INDEX`/`DROP INDEX` plus idempotent standalone, `ALTER TABLE`, and inline `CREATE TABLE` index create/duplicate-failure semantics, descending, mixed-direction, `VARCHAR`, utf8mb4, and TEXT/BLOB prefix, TEXT/BLOB prefix-plus-direction, and prefix-plus-direction key-part metadata refresh, secondary-index rename, secondary-index ignored/not-ignored metadata refresh, multi-column unique replacement/enforcement plus idempotent top-level and `ALTER TABLE` unique-index create/no-op/drop preservation, unique descending, unique prefix, unique prefix-plus-direction, utf8mb4 prefix, unique TEXT/BLOB prefix, and unique TEXT/BLOB prefix-plus-direction secondary-index DDL enforcement/drop refresh, primary-key idempotent ADD no-op preservation, primary-key, descending-primary-key, and composite direction primary-key replacement DDL refresh, foreign-key ALTER add/drop refresh, foreign-key referential actions, composite foreign-key coverage, deep foreign-key cascade-chain update/delete coverage, generated-column foreign-key coverage including virtual generated child and rejected action policy, cyclic foreign-key coverage including three-table cascade and set-null variants, same-schema foreign-key parent-table/child-table rename refresh, cross-schema foreign-key parent-table/child-table rename refresh, same-schema and cross-schema foreign-key multi-pair parent/child rename refresh, CHECK constraint ALTER add/drop enforcement, generated-column ALTER add/drop and same-kind expression-replacement refresh, table charset-conversion DDL refresh, row-format DDL refresh, table-comment DDL refresh, force-rebuild DDL refresh, column-default SET/DROP refresh, column idempotent ADD/DROP refresh, column add-modify-rename-drop ALTERs, explicit instant ADD/DROP/reorder column metadata, and instant FIRST/AFTER stored-column placement including `LOCK=DEFAULT`, `LOCK=SHARED`, and `LOCK=EXCLUSIVE` variants, column rename, and virtual generated-column add/drop including `LOCK=SHARED`/`LOCK=EXCLUSIVE` variants refresh, schema create/drop lifecycle refresh, schema default charset/collation DDL refresh plus schema-default rewrite crash recovery, and no-live absent-schema reopen checks, cross-schema table rename with `.frm`/`.ibd` movement, same-schema and cross-schema multi-pair rename-cycle tablespace swaps, and ownerless/native reopen before and after forced `.shm` rebuild, view create/query/drop, replacement/alter, idempotent create/drop, column-list, check-option, prepared check-option DML, direct/prepared non-updatable view diagnostics, invalid view dependency diagnostics, nested check-option, and security/definer metadata refresh with ownerless/native reopen before and after forced `.shm` rebuild, trigger create/fire/drop, replacement/update/delete, ordering/show-create, and idempotent create/drop metadata refresh with ownerless/native reopen before and after forced `.shm` rebuild, no-live ownerless/native exclusive reopen of the broader DDL final state before and after forced `.shm` rebuild, local DDL followed by dictionary/table flush, foreign keys, CHECK constraints, generated columns, online/in-place index alter, concurrent same-named InnoDB temporary tables, killed temporary-table peer cleanup while another temp-table peer remains live, ownerless InnoDB-only engine policy, ownerless stored-routine DDL including package specification/body and routine-execution policy rejection, ownerless sequence SQL policy rejection, ownerless table-admin SQL policy rejection, ownerless `LOCK TABLES` SQL policy rejection, ownerless `FLUSH TABLES ... WITH READ LOCK`/`FOR EXPORT` SQL policy rejection while plain ownerless `FLUSH TABLES` remains covered, ownerless table-directory SQL policy rejection, ownerless top-level, ALTER, inline, and idempotent `FULLTEXT`/`SPATIAL` index DDL policy rejection, ownerless partitioned table DDL policy rejection, and a killed uncommitted writer whose state blocks peer cleanup until a no-live-process reopen rebuilds volatile coordination. The opt-in `ownerless-stress` preset reruns the independent-table reader/writer stress case at 200 writer iterations and 400 reader polls, with deterministic independent-table stress SQL trace export covering the same per-table writer schedule and aggregate reader oracle for external harness input. It runs concurrent DDL workers with live DML readers/writers plus forced `.shm` rebuild and native exclusive reopen checks, and deterministic DDL stress SQL trace export covers the same create/alter/index/rename/truncate/drop plus DML schedule for external harness input, deterministic DDL lifecycle SQL trace export covers create/rename/truncate/force/drop/recreate final-state oracles for external harness input, runs same-name temporary-table churn across ownerless processes plus forced `.shm` rebuild and native exclusive reopen checks for the resulting permanent table, and deterministic temporary-table stress SQL trace export covers the same session-local temporary-table churn plus post-worker permanent-table oracle for external harness input, runs explicit multi-statement transaction/savepoint stress with deterministic aggregate checks, forced `.shm` rebuild checks, native exclusive reopen checks, and deterministic transaction stress SQL trace export for external harness input, runs shared-table checksum stress with mixed direct/prepared DML writers, bounded retry on ownerless statement-lock busy plus MariaDB 1205/1213 contention, deterministic sum/version/weighted-sum oracle checks, forced `.shm` rebuild checks, native exclusive reopen checks, and deterministic checksum stress SQL trace export for external harness input, runs pseudo-random shared-table transaction stress with savepoint rollback, full rollback, live aggregate-reader bounds, bounded retry on 1205/1213, deterministic final oracles, forced `.shm` rebuild checks, and native exclusive reopen checks, and runs foreign-key graph stress with concurrent ownerless workers over `CASCADE`, `SET NULL`, and `RESTRICT` edges, bounded retry on 1205/1213, tracked transaction page LSN coverage, DML current-read refresh, deterministic aggregate/referential oracles, forced `.shm` rebuild checks, and native exclusive reopen checks. Release workload and pressure presets give their bounded stress suites a 1800-second ceiling while retaining tighter explicit watchdogs for cases already proved to terminate sooner; the randomized release preset gives the 120-round transaction schedule a 3600-second ceiling for its one-second native lock waits, bounded retry scheduling, and ten-round phase handoff, and every stress case asserts final no-peer page-version WAL reclamation. The `ownerless-test-hooks` preset adds unsafe deterministic transaction-registration, page-version before-append and after-append publish/checkpoint, page-visible publish-before-checkpoint, page-visible-checkpoint, redo-reservation, redo-gap writer blocking, redo completed-write, redo latest-before-checkpoint, redo latest-after-checkpoint, native checkpoint reclamation crash/race with resumed-closer WAL non-growth after newer peer commits, primitive active-pin page-version boundary reclamation, consistent-snapshot pre-execution pin/race coverage, dictionary-DDL begin/before-finish/after-finish crash injection, unique-index replacement crash recovery, explicit schema-qualified single-pair, cross-schema, multi-pair, cross-schema multi-pair, and foreign-key parent/child multi-pair `RENAME TABLE` crash-at-dictionary-before-finish live recovery, view replacement/alter, column-list, check-option, nested check-option, and security crash recovery, and negative-proof coverage |
| Ownerless DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills explicit schema-qualified same-schema, cross-schema, same-schema multi-pair swap, cross-schema multi-pair swap, and foreign-key parent/child multi-pair `RENAME TABLE` writers after native file movement, kills a single-pair `RENAME TABLE IF EXISTS` writer after the native rename pair and verifies DDL-log rollback to the original source table, kills same-schema and cross-schema non-FK multi-rename writers after first, second, and final successful native rename pairs inside MariaDB's rename loop and verifies DDL-log rollback to the original swapped-table state, kills same-schema and cross-schema foreign-key multi-rename writers after first and later successful native rename pairs inside MariaDB's rename loop and verifies DDL-log rollback to the original FK state, standalone `CREATE INDEX` and `DROP INDEX` writers after native secondary-index metadata creation/removal, duplicate top-level `CREATE INDEX IF NOT EXISTS`, missing top-level `DROP INDEX IF EXISTS`, duplicate `ALTER TABLE ... ADD INDEX IF NOT EXISTS`, missing `ALTER TABLE ... DROP INDEX IF EXISTS`, duplicate top-level `CREATE UNIQUE INDEX IF NOT EXISTS`, and duplicate `ALTER TABLE ... ADD UNIQUE INDEX IF NOT EXISTS` no-op writers after MariaDB success, a `CREATE OR REPLACE UNIQUE INDEX` writer after native replacement metadata, an `ALTER TABLE ... DROP PRIMARY KEY, ADD PRIMARY KEY` writer after native primary-key replacement, duplicate `ALTER TABLE ... ADD PRIMARY KEY IF NOT EXISTS` no-op writer after MariaDB success, `ALTER TABLE ... ADD CONSTRAINT ... FOREIGN KEY` and `ALTER TABLE ... DROP FOREIGN KEY` writers after native foreign-key metadata creation/removal, `ALTER TABLE ... ADD CONSTRAINT ... CHECK`, `ALTER TABLE ... DROP CONSTRAINT` for CHECK constraints, plus `ALTER TABLE ... ADD COLUMN`, `ALTER TABLE ... DROP COLUMN`, `ALTER TABLE ... MODIFY COLUMN`, `ALTER TABLE ... CHANGE COLUMN`, `ALTER TABLE ... RENAME COLUMN`, duplicate `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, missing `ALTER TABLE ... DROP COLUMN IF EXISTS`, missing `ALTER TABLE ... MODIFY COLUMN IF EXISTS`, missing `ALTER TABLE ... RENAME COLUMN IF EXISTS`, missing `ALTER TABLE ... CHANGE COLUMN IF EXISTS`, missing `ALTER TABLE ... ALTER COLUMN IF EXISTS SET DEFAULT`, and missing `ALTER TABLE ... ALTER COLUMN IF EXISTS DROP DEFAULT` writers after native table-definition mutation or MariaDB no-op success, including missing rename/change/default no-ops over generated-column/CHECK expression metadata, simple `CREATE VIEW` and `DROP VIEW` writers after native view definition-file creation/removal, `CREATE OR REPLACE VIEW` and `ALTER VIEW` writers after native view definition rewrite, explicit column-list create/replace/alter writers after native alias metadata storage/rewrite, check-option create/replacement/alter writers after native check-option metadata storage/rewrite, nested check-option outer-replacement and inner-alter writers after native nested view definition rewrite, explicit definer create, invoker replacement, and definer-security ALTER view writers after native security metadata storage/rewrite, duplicate `CREATE VIEW IF NOT EXISTS` and missing `DROP VIEW IF EXISTS` no-op writers after MariaDB success, plain plus both exact explicit copy-lock option orders for `ALTER TABLE ... FORCE` writers after native table-copy rebuild, plain and both exact explicit copy-lock option orders for same-engine `ALTER TABLE ... ENGINE=InnoDB` writers after native rebuild, plain plus both exact explicit copy-lock option orders for `ALTER TABLE ... CONVERT TO CHARACTER SET` writers after native charset-conversion metadata/storage update, plain plus both exact explicit copy-lock option orders for `ALTER TABLE ... ROW_FORMAT=DYNAMIC`, `ALTER TABLE ... ROW_FORMAT=COMPACT`, `ALTER TABLE ... ROW_FORMAT=REDUNDANT`, `ALTER TABLE ... ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=1`, `ALTER TABLE ... ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=2`, `ALTER TABLE ... ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=4`, `ALTER TABLE ... ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=8`, and `ALTER TABLE ... ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=16` writers after native row-format rebuilds, an `ALTER TABLE ... COMMENT` writer after native table-comment metadata update, an `ALTER TABLE ... AUTO_INCREMENT` writer after native high-watermark persistence, an `ALTER COLUMN ... SET DEFAULT` writer after native default metadata update, a `TRUNCATE TABLE` writer after native truncate/recreate, a `DROP TABLE` writer after native file removal, a `DROP DATABASE` writer after native schema/table removal, an `ALTER DATABASE` writer after native `db.opt` rewrite, and duplicate `CREATE DATABASE IF NOT EXISTS` plus missing `DROP SCHEMA IF EXISTS` no-op writers after MariaDB success, but before ownerless dictionary finish; focused create/table-copy/replacement-copy/explicit rename-list/truncate/drop/force-rebuild/charset-conversion, dynamic, compact, redundant, and compressed-key-block row-format including both exact copy-lock option orders now verify live-peer cleanup, marker retention, and final no-live marker drain, focused standalone and field/generated CHECK ADD/DROP verify live-peer recovery on the marker-retaining lane, and focused simple/rewrite/column-list/check-option/nested-check-option/security-definer/idempotent view, simple/replacement/idempotent/ordered/definer trigger, schema idempotent/no-op, secondary-index idempotent/no-op, unique-index idempotent no-op, primary-key idempotent no-op, column-idempotent no-op, column-IF-EXISTS missing-column no-op, table-comment, and column-default metadata-only paths now verify live recovery with the native file-operation marker clear, while DDL crash classes without live-recovery proof still verify live-peer cleanup remains busy until no-live recovery. The coverage verifies recovered present/absent secondary-index metadata, forced-index reads for create, forced-index rejection for drop, recovered idempotent secondary-index no-op behavior with original key-part preservation, missing-index absence, errno 1061 for plain duplicate create, post-recovery writes, and forced-index reads from top-level and ALTER-table no-op branches, recovered unique-index idempotent no-op behavior with original unique-key preservation, attempted-key non-enforcement, errno 1061 for plain duplicate create or ALTER add, and duplicate-key enforcement, recovered replacement unique-index metadata and duplicate-key enforcement, recovered primary-key replacement metadata with duplicate-key enforcement on the replacement key and duplicate values allowed on the former key, recovered primary-key idempotent no-op behavior with original-key preservation and candidate-key non-uniqueness, recovered added foreign-key metadata with orphan-row rejection, rejected-row absence before later commit, and valid child writes, recovered dropped foreign-key metadata absence with orphan writes and parent deletes allowed, recovered generated-column FK ADD metadata/enforcement and generated-column FK DROP metadata absence with orphan writes and parent deletes allowed, recovered CHECK metadata with errno 4025 enforcement, recovered dropped-CHECK metadata absence with formerly invalid writes allowed, recovered present/absent view metadata, `.frm` file state, view query behavior, replacement/altered view column metadata, explicit column-list alias metadata and ordinal positions, stale alias rejection, recovered check-option and nested check-option metadata, updatability metadata, errno 1369 invalid-DML enforcement, old exposed-column rejection, recovered security type and non-empty definer metadata, original view-definition preservation, missing-view absence, and base-table writes, recovered added-column metadata/default values, recovered idempotent column no-op behavior with original default preservation, missing-column absence, errno 1060 for plain duplicate add, errno 1091 for plain missing drop, missing modify/rename/change/default no-op preservation with errno 1054 for plain retries, expression-table missing rename/change/default preservation with generated-column values, real defaults, and CHECK enforcement unchanged, and post-recovery writes, recovered column-default metadata and post-recovery default-backed insert behavior, recovered charset/collation metadata and retained/post-recovery rows, recovered table-comment metadata and retained/post-recovery rows, absent dropped-column metadata, recovered modified-column width/default metadata and widened-value writes, recovered changed-column old-name rejection plus widened new-name default/write behavior, recovered renamed-column metadata with old-name rejection and new-name writes, recovered schema default metadata with pre-alter table collation preservation and post-recovery default inheritance, recovered schema idempotent no-op behavior with original defaults, real schema/table preservation, missing-schema absence, and errno 1007 for plain duplicate create, recovered AUTO_INCREMENT high-watermark metadata with monotonic implicit ID allocation, with the AUTO_INCREMENT high-watermark crash selector registered as a standalone hook CTest, recovered generated-column and CHECK expression behavior after a dependent column rename, recovered force-rebuilt InnoDB table/space/index metadata, copied payload bytes, recovered dynamic, compact, redundant, and compressed row-format metadata, retained row payloads, compressed 1 KiB, 2 KiB, 4 KiB, 8 KiB, and 16 KiB ZBLOB page evidence, and post-recovery writes, verifies ownerless reopen before and after forced `.shm` rebuild, and verifies native exclusive reopen of the recovered table, column, index, constraint, view, or schema state after rebuild |
| Ownerless temporary-table truncate tracking | 🟡&nbsp;Partial | Production ownerless SQL coverage verifies `TRUNCATE TABLE` against a same-named session-local InnoDB temporary table empties only that temporary table, keeps the temporary name tracked while a peer updates the permanent table, reveals the peer-updated permanent table only after `DROP TEMPORARY TABLE`, and preserves the durable permanent state through ownerless reopen, forced `.shm` rebuild, and ordinary native reopen; hook-build coverage now classifies tracked temporary truncate as temporary metadata-only ownerless dictionary recovery, kills the writer before ownerless dictionary finish while another ownerless peer remains live, retains the native file-operation checkpoint marker until final no-live recovery, and verifies the permanent table remains writable through ownerless/native reopen and forced `.shm` rebuild; broader temporary DDL matrices remain planned |
| Ownerless column ALTER live recovery | 🟡&nbsp;Partial | Hook-build coverage now treats bounded real `ALTER TABLE ... DROP COLUMN`, `ALTER TABLE ... MODIFY COLUMN`, `ALTER TABLE ... CHANGE COLUMN`, and `ALTER TABLE ... RENAME COLUMN` crash boundaries as native-file-operation recoverable while another ownerless peer remains live, including both exact explicit copy-lock option orders for unplaced ADD, placed ADD, DROP, MODIFY, CHANGE, and RENAME COLUMN forms. The registered standalone selectors verify live recovery for placed-column order, dropped-column absence, modified-column width/default metadata and widened writes, changed-column old-name rejection plus widened new-name default/write behavior, renamed-column old-name rejection and new-name writes, generated-column/CHECK dependent-expression rename behavior, and generated stored/virtual column DROP, MODIFY, and CHANGE recovery with computed-value metadata through ownerless/native reopen plus forced `.shm` rebuild, while retaining the native file-operation marker until final no-live drain. Multi-clause column ALTERs, broader explicit online-option variants, and randomized external DDL oracle execution remain planned |
| Ownerless cross-schema multi-rename DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills explicit schema-qualified same-schema, implicit-schema same-schema, cross-schema, same-schema multi-pair swap, cross-schema multi-pair swap, foreign-key parent/child multi-pair `RENAME TABLE`, native-loop `ALTER TABLE ... RENAME TO`, and view-only `RENAME TABLE` writers before ownerless dictionary finish; covered native table rename-list shapes now verify live ownerless recovery while another peer remains open, final target metadata, absent source or temporary names, preserved or swapped InnoDB `SPACE` identities where relevant, FK metadata/enforcement where relevant, post-recovery writes, live marker retention, no-live marker drain, and ownerless/native reopen before and after forced `.shm` rebuild, while view-only rename uses metadata-only live recovery with the native file-operation marker clear. Simple tracked temporary-table `RENAME TABLE` and `ALTER TABLE ... RENAME`, plus pure multi-pair temporary-table `RENAME TABLE` chains, are covered as metadata-only dictionary recovery with native file-operation marker retention until no-live drain; temp-first, permanent-first, temporary-chain/permanent/temporary-chain, and focused same-schema plus cross-schema IF EXISTS missing-source mixed temporary/permanent rename lists are covered on the durable native file-operation marker lane, with completed-prefinish recovery retaining the permanent target and cross-schema native-loop recovery rolling the permanent target back to the original source while the temporary target remains absent through ownerless/native reopen and forced `.shm` rebuild. Single-pair `RENAME TABLE IF EXISTS` native-loop coverage proves DDL-log rollback to the original source table; same-schema and cross-schema non-FK native-loop coverage now proves first-, second-, and final-pair DDL-log rollback to the original swapped-table state, including deterministic same-schema and cross-schema `RENAME TABLE IF EXISTS` three-pair lists; same-schema and cross-schema FK native-loop crash coverage proves first- and later-pair DDL-log rollback to the original parent/child state, with the cross-schema target schema retained and moved tables absent, and the same deterministic existing-table FK lists are now covered with `RENAME TABLE IF EXISTS`; same-schema and cross-schema missing-source `IF EXISTS` no-op matrices, same-schema and cross-schema longer missing-source permutations, and same-schema plus cross-schema FK missing-source permutations are covered, and target-conflict `IF EXISTS` rename now has failed-DDL dictionary recovery plus native file-op negative proof for the errno `1050` branch; broader FK and randomized temporary/permanent or missing-source permutations remain planned |
| Ownerless table-copy DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills representative `CREATE TABLE ... LIKE` and `CREATE TABLE ... SELECT` writers after native destination table creation, including CTAS row population, but before ownerless dictionary finish; `CREATE TABLE ... LIKE` and the focused no-definition-list CTAS selector now verify live-peer cleanup, recovered `.frm`/`.ibd` files, `INFORMATION_SCHEMA.TABLES` and column metadata, copied secondary-index metadata for `LIKE`, CTAS copied rows, post-recovery writes, ownerless/native reopen, forced `.shm` rebuild, live marker retention, and no-live marker drain |
| Ownerless table-replacement DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills representative `CREATE OR REPLACE TABLE`, `CREATE OR REPLACE TABLE ... LIKE`, and `CREATE OR REPLACE TABLE ... AS SELECT` writers after native old-table replacement or replacement-copy completion but before ownerless dictionary finish; ordinary `CREATE OR REPLACE TABLE`, replacement-copy `CREATE OR REPLACE TABLE ... LIKE`, and replacement-copy `CREATE OR REPLACE TABLE ... AS SELECT` now verify live-peer cleanup, recovered replacement `.frm`/`.ibd` files, old-column and old-index absence, replacement-column/index metadata, replacement rowset semantics including copied CTAS rows, post-recovery writes, ownerless/native reopen, forced `.shm` rebuild, live marker retention, and no-live marker drain |
| Ownerless multi-drop DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset now registers same-schema and cross-schema same-statement `DROP TABLE` crash selectors as standalone CTests for visible CI timing and failure attribution; both selectors kill a writer after native table-list removal but before ownerless dictionary finish, verify live-peer recovery with both dropped `.frm`/`.ibd` file pairs absent, keep the native file-operation marker set while a peer remains live, drain it after final no-live recovery, and verify ownerless/native reopen before and after forced `.shm` rebuild. Same-schema and cross-schema inside-MariaDB-loop crash coverage also kills after the first table's native file operation but before the second table is logged or dropped, then verifies live-peer recovery with the first table absent, the second table still readable/writable, marker retention until no-live drain, and ownerless/native reopen before and after forced `.shm` rebuild. Schema-drop internal loop crash coverage kills a two-table `DROP DATABASE` after one native table file operation but before `ddl_log_drop_db()`, verifies the schema and exactly one table remain usable while the completed table drop remains absent, and checks marker retention/drain plus ownerless/native reopen before and after forced `.shm` rebuild |
| Ownerless table-idempotent DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills duplicate `CREATE TABLE IF NOT EXISTS`, missing single-table and multi-table-list `DROP TABLE IF EXISTS` no-op, and existing single-table and two-table-list `DROP TABLE IF EXISTS` writers after MariaDB returns success but before ownerless dictionary finish; recovery verifies duplicate-create metadata-only live recovery with the native file-operation marker clear after pre-execution metadata proves the target table already exists, missing drop recovers while a peer remains live with the marker clear, existing drop recovers while a peer remains live with the marker retained until final no-live drain, duplicate create preserves the original table definition and keeps plain duplicate create returning errno 1050, missing drop preserves the real table while keeping the missing table metadata/files absent, existing drop keeps the removed table metadata/files absent, post-recovery writes succeed where the preserved table remains, ownerless/native reopen plus forced `.shm` rebuild observe the same state, and the duplicate-create selector is registered as a standalone hook CTest for visible CI timing and failure attribution |
| Ownerless schema-idempotent DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills duplicate `CREATE DATABASE IF NOT EXISTS`, missing `CREATE SCHEMA IF NOT EXISTS`, missing `DROP SCHEMA IF EXISTS` no-op, empty `DROP DATABASE IF EXISTS`, and table-bearing `DROP DATABASE|SCHEMA IF EXISTS` writers after MariaDB returns success but before ownerless dictionary finish; recovery verifies live-peer recovery with the native file-operation marker clear for duplicate create, missing create, missing drop, and empty drop, and marker-retaining live recovery for table-bearing drop until final no-live drain, duplicate create preserves the original schema defaults and inherited column collation while plain duplicate create returns errno 1007, missing create exposes the created schema defaults and supports post-recovery table writes, missing drop preserves the real schema/table while keeping the missing schema metadata/directory absent, empty and table-bearing drops keep the removed schema/table absent as applicable, post-recovery writes succeed where the preserved or created schema remains, and ownerless/native reopen plus forced `.shm` rebuild observe the same state |
| Ownerless index-idempotent DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills duplicate top-level `CREATE INDEX IF NOT EXISTS` and missing top-level `DROP INDEX IF EXISTS` no-op writers after MariaDB returns success but before ownerless dictionary finish; recovery verifies live-peer metadata-only recovery with the native file-operation marker clear after pre-execution metadata proves the duplicate-create or missing-drop no-op branch, duplicate create preserves the original key part and keeps plain duplicate create returning errno 1061, missing drop preserves the real index while keeping the missing index metadata absent, post-recovery writes and forced-index reads succeed, ownerless/native reopen plus forced `.shm` rebuild observe the same state, and both selectors are registered as standalone hook CTests for visible CI timing and failure attribution |
| Ownerless ALTER index-idempotent DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills duplicate `ALTER TABLE ... ADD INDEX IF NOT EXISTS` and missing `ALTER TABLE ... DROP INDEX IF EXISTS` no-op writers after MariaDB returns success but before ownerless dictionary finish; recovery verifies live-peer metadata-only recovery with the native file-operation marker clear after pre-execution metadata proves the duplicate-add or missing-drop no-op branch, duplicate add preserves the original key part and keeps plain duplicate ALTER-add returning errno 1061, missing drop preserves the real index while keeping the missing index metadata absent, post-recovery writes and forced-index reads succeed, ownerless/native reopen plus forced `.shm` rebuild observe the same state, and both selectors are registered as standalone hook CTests for visible CI timing and failure attribution |
| Ownerless unique-index idempotent DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills duplicate top-level `CREATE UNIQUE INDEX IF NOT EXISTS` and duplicate `ALTER TABLE ... ADD UNIQUE INDEX IF NOT EXISTS` no-op writers after MariaDB returns success but before ownerless dictionary finish; recovery verifies metadata-only live recovery with the native file-operation marker clear after pre-execution metadata proves the duplicate no-op branch, duplicate create/add preserves the original unique key and keeps plain duplicate create or ALTER-add returning errno 1061, the attempted replacement key remains non-enforced, duplicate-key enforcement for the original unique key remains active, post-recovery writes and forced-index reads succeed, ownerless/native reopen plus forced `.shm` rebuild observe the same state, and both selectors are registered as standalone hook CTests for visible CI timing and failure attribution |
| Ownerless unique-index drop DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills a `DROP INDEX` writer for an active unique secondary index after native metadata removal but before ownerless dictionary finish; recovery verifies live-peer recovery while another ownerless peer remains open, native file-operation marker retention until final no-live drain, unique-index metadata absence, forced-index reads on the dropped name fail, the formerly duplicate key shape inserts successfully after recovery, and ownerless/native reopen plus forced `.shm` rebuild observe the same state |
| Ownerless primary-key DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills plain `ALTER TABLE ... DROP PRIMARY KEY, ADD PRIMARY KEY (code)`, descending `ALTER TABLE ... DROP PRIMARY KEY, ADD PRIMARY KEY (code DESC)`, composite direction `ALTER TABLE ... DROP PRIMARY KEY, ADD PRIMARY KEY (tenant_id ASC, code DESC)`, and AUTO_INCREMENT-preserving `ALTER TABLE ... DROP PRIMARY KEY, DROP INDEX old_code_key, ADD UNIQUE KEY id_key (id), ADD PRIMARY KEY (code)` writers after native clustered-key rebuild but before ownerless dictionary finish; recovery verifies live-peer recovery while another ownerless peer remains open, native file-operation marker retention until final no-live drain, recovered primary-key metadata and key-part direction, old-key absence from `PRIMARY` where applicable, retained unique AUTO_INCREMENT-column key metadata where applicable, duplicate-key enforcement on the replacement key, old-key duplicate writes where applicable, forced-index reads, AUTO_INCREMENT non-reuse after duplicate-key failure where applicable, ownerless/native reopen, and forced `.shm` rebuild |
| Ownerless view-idempotent DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills duplicate `CREATE VIEW IF NOT EXISTS`, missing `DROP VIEW IF EXISTS` no-op, and existing-view `DROP VIEW IF EXISTS` removal writers after MariaDB returns success but before ownerless dictionary finish; recovery verifies live-peer recovery with the native file-operation marker clear, duplicate create preserves the original view definition and keeps plain duplicate create returning errno 1050, missing drop preserves the real view while keeping the missing view metadata/files absent, existing drop removes the real view metadata/files while preserving the base table, post-recovery base-table writes remain visible through preserved views or base tables as appropriate, and ownerless/native reopen plus forced `.shm` rebuild observe the same state; all three selectors are registered as standalone hook CTests for visible CI timing and failure attribution |
| Ownerless primary-key idempotent DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills duplicate `ALTER TABLE ... ADD PRIMARY KEY IF NOT EXISTS` no-op writers after MariaDB returns success but before ownerless dictionary finish; recovery verifies metadata-only live recovery with the native file-operation marker clear after pre-execution metadata proves `PRIMARY` already exists, duplicate add preserves the original `PRIMARY(id)` clustered key and keeps plain duplicate primary-key add returning errno 1068, the attempted candidate `code` key remains non-unique, post-recovery writes succeed, ownerless/native reopen plus forced `.shm` rebuild observe the same state, and the selector is registered as a standalone hook CTest for visible CI timing and failure attribution |
| Ownerless column-idempotent DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills duplicate `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` and missing `ALTER TABLE ... DROP COLUMN IF EXISTS` no-op writers after MariaDB returns success but before ownerless dictionary finish; recovery verifies metadata-only live recovery with the native file-operation marker clear after pre-execution metadata proves the duplicate-add or missing-drop no-op branch, duplicate add preserves the original column and default while plain duplicate add returns errno 1060, missing drop preserves the real column while keeping the missing column absent and plain missing drop returning errno 1091, post-recovery writes succeed, ownerless/native reopen plus forced `.shm` rebuild observe the same state, and both selectors are registered as standalone hook CTests for visible CI timing and failure attribution |
| Ownerless column IF EXISTS DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills missing `ALTER TABLE ... MODIFY COLUMN IF EXISTS`, `ALTER TABLE ... RENAME COLUMN IF EXISTS`, `ALTER TABLE ... CHANGE COLUMN IF EXISTS`, `ALTER TABLE ... ALTER COLUMN IF EXISTS SET DEFAULT`, and `ALTER TABLE ... ALTER COLUMN IF EXISTS DROP DEFAULT` no-op writers after MariaDB returns success but before ownerless dictionary finish; recovery verifies metadata-only live recovery with the native file-operation marker clear for each covered missing-column no-op after pre-execution metadata proves the source column is absent; these paths preserve the original real column metadata/default, keep the missing and attempted renamed/changed columns absent, keep plain missing modify/rename/change/default retries returning errno 1054, allow post-recovery writes, and ownerless/native reopen plus forced `.shm` rebuild observe the same state; missing `RENAME COLUMN IF EXISTS`, `CHANGE COLUMN IF EXISTS`, and default-alter variants are also covered on generated-column/CHECK expression tables, proving the real column, stored and virtual generated expressions, real-column defaults, CHECK enforcement, and missing attempted names survive live recovery; all focused selectors are registered as standalone hook CTests for visible CI timing and failure attribution, while external randomized DDL oracle execution remains planned |
| Ownerless index metadata crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset also kills ordinary secondary-index create/drop, online `ALTER TABLE ... ADD/DROP INDEX ..., ALGORITHM=INPLACE, LOCK=NONE`, `CREATE OR REPLACE UNIQUE INDEX`, unique-secondary `DROP INDEX`, `ALTER TABLE ... RENAME INDEX`, and `ALTER TABLE ... ALTER INDEX ... IGNORED`/`NOT IGNORED` writers after native index metadata changes but before ownerless dictionary finish. Recovery verifies live-peer recovery while another ownerless peer remains open and retains the native dictionary checkpoint marker for physical create/drop/replacement, online add/drop, and crashed prearmed rename or ignored/not-ignored mutations until final no-live drain. Successful non-crashed metadata-only mutations clear their statement-local prearm. Coverage checks old/new index-name metadata, replacement unique-key enforcement, dropped unique-index absence and duplicate-key release, ignored/not-ignored metadata, forced-index reads, later writes, ownerless/native reopen, and forced `.shm` rebuild |
| Ownerless trigger DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills simple `CREATE TRIGGER` and `DROP TRIGGER`, `CREATE OR REPLACE TRIGGER`, ordered `CREATE TRIGGER ... PRECEDES ...`, duplicate `CREATE TRIGGER IF NOT EXISTS`, missing `DROP TRIGGER IF EXISTS`, delayed missing-dependency `CREATE TRIGGER`, explicit `CREATE DEFINER=CURRENT_USER TRIGGER`, and stored-function-body `CREATE TRIGGER` writers after native `.TRG`/`.TRN` metadata creation/removal, rewrite, no-op preservation, delayed dependency acceptance, or definer metadata storage but before ownerless dictionary finish; it verifies syntax-simple create/drop live-peer metadata-only recovery, including delayed missing-dependency and stored-function-body create triggers, plus replacement, idempotent/no-op, ordered, and explicit-definer trigger live-peer metadata-only recovery while another ownerless peer remains open with the native file-operation marker clear, recovered present/absent `INFORMATION_SCHEMA.TRIGGERS` metadata including non-empty definer metadata for the explicit-definer case, recovered trigger-file presence/absence, trigger firing after recovered create, replacement, idempotent no-op preservation, delayed dependency creation, definer recovery, stored-function trigger metadata recovery, ownerless stored-routine execution rejection before base-row mutation, and ordinary native stored-function trigger firing, recovered MariaDB 1146 failure when the missing dependency is absent, recovered `ACTION_ORDER`/firing order after ordered create, dropped-trigger non-firing after recovered drop, absent missing-trigger `.TRN` state after recovered missing drop, `SHOW CREATE TRIGGER` for recovered present triggers including explicit `DEFINER=` metadata and rejection for the dropped trigger, ownerless reopen before and after forced `.shm` rebuild, and native exclusive reopen after rebuild, with replacement, ordered, delayed missing-dependency, stored-function-body, idempotent, and explicit-definer selectors registered as standalone hook CTests for visible CI timing and failure attribution, while broader privilege/security and randomized trigger crash variants remain planned |
| Ownerless online DDL option matrix | 🟡&nbsp;Partial | Focused ownerless SQL coverage verifies already-open peer refresh, final ownerless/native reopen, and forced `.shm` rebuild for accepted ordinary secondary-index `ALGORITHM=NOCOPY, LOCK=SHARED`, `ALGORITHM=NOCOPY, LOCK=EXCLUSIVE`, `ALGORITHM=INPLACE, LOCK=NONE`, `ALGORITHM=INPLACE, LOCK=SHARED`, and `ALGORITHM=INPLACE, LOCK=EXCLUSIVE` add/drop combinations plus a unique secondary-index `ALGORITHM=INPLACE, LOCK=SHARED` add/drop pair with duplicate-key enforcement; hook-build crash coverage now also kills `ALGORITHM=INPLACE, LOCK=NONE` online secondary-index add/drop writers before ownerless dictionary finish and verifies live-peer marker retention plus final no-live drain, extending the existing `NOCOPY`, `INPLACE`, `INSTANT`, and `COPY` option coverage while broader randomized DDL oracle execution remains planned |
...
| Ownerless page-log visible-fast append batching | 🟡&nbsp;Partial | Pure `INSERT ... VALUES` visible-fast-path ownerless statements with one through 65536 streaming-proven row constructors now keep one page-log append session across the statement's ownerless mini-transactions, coalesce repeated user data/index page images through the existing transaction-deferred page publication proof only while the process registry proves a single-owner epoch, and release/sync before page-visible LSN publication; peer-present ownerless statements keep immediate page publication, and focused SQL coverage verifies page-version records still publish, native history WAL proof remains active, page-log append session begin/end counts collapse for single-row and capped multi-row visible-fast inserts, 65536-row single-owner statements use transaction-deferred image/buffer publication without snapshot-boundary immediate user-page publication, the 65537-row boundary stays outside append batching and deferred latest-checkpoint coalescing, captured transaction images remain usable when their buffer-pool page was evicted before commit but resident byte/LSN mismatches still force the conservative path, deferred redo completion now batches up to 61 ranges while preserving an active-owner entry plus two peer headroom slots in the 64-slot active-reservation table, peer-open truncate/allocation coverage keeps large single-row inserts safe, and unsupported upsert branches keep conservative flush behavior after a previous unbounded row-list attempt regressed bulk timing, while row lists above 65536, broader DML/DDL, native row/undo reductions, single-row autocommit performance, and group-commit batching remain planned |
| Ownerless active-writer visible skip | 🟡&nbsp;Partial | When InnoDB reports committed pages visible but another ownerless writer transaction is still active, MyLite now closes any deferred page-log append batch before skipping shared visible-boundary publication. No-live native checkpoint proof accepts higher-LSN native successors for retained user-page records only when the record carries external-snapshot lineage and the MyLite-owned `.ibd` file proves the successor space id, page number, and page LSN at the retained record's expected offset; plain concurrent-writer records require exact native image proof or retained WAL replay. Unsafe-hook SQL coverage pauses the `pages-visible-active-writer-skip` path for two concurrent explicit transactions, proves the first COMMIT still uses the visible-fast path, verifies user WAL is present while the peer is active, and checks ownerless/native reopen preserve both table updates. Production coverage repeats the adjacent different-table explicit transaction selector. Broader multi-table DML, DDL/file lifecycle, native redo/checkpoint reconciliation, and external MariaDB/RQG stress remain planned |
| Ownerless dirty-publish append batching | 🟡&nbsp;Partial | Ownerless dirty-scan and full buffer-pool native-support page publication now use the existing page-publish batch scope so consecutive page-version WAL records from the same hook context can flow through the page-log append session instead of repeating direct append setup per page; transaction-deferred page publication already used that scope. The change preserves page eligibility, page contents, record order, redo completion, checkpoint publication, SQL behavior, public APIs, and the page-log record format, and is scoped to append overhead in the current ownerless write-performance gap while broader native redo/checkpoint reconciliation, proof-volume reduction, DDL/file-lifecycle recovery, and group-commit batching remain planned |
...
| Ownerless explicit clean page-write release | 🟡&nbsp;Partial | Explicit ownerless transactions no longer keep clean preread/prepare page-write ownership for the transaction lifetime: after a statement already has a page-write gate, additional clean preread probes use untracked zero-timeout page-write attempts that return on conflict without publishing shared-registry waiters, statement-end cleanup releases transaction-deferred page-write records that never became dirty or image-backed, and ordinary gates remain per tablespace instead of escalating automatically to the global page-write gate, including statement-failure cleanup, but a gate is retained while dirty transaction pages or captured transaction images in that tablespace still need commit/rollback protection. Internal referential-constraint policy reads now use ownerless plain-read scope while preserving local pages, avoiding false page-write waits during supported DML metadata checks. Dirty pages and captured transaction page images still stay transaction-scoped until commit or rollback, and autocommit DDL/DML keeps conservative blocking refresh semantics. Focused production coverage now proves two explicit transactions updating `ownerless_a` and `ownerless_b` both reach their post-update barriers before either commits, `commit-race` completes without a lingering page-write waiter, the original SQL weighted shard `2/16` passes, and `CREATE OR REPLACE TABLE` replacement tablespace replay remains intact; broader multi-table DML, DDL/file lifecycle recovery, native redo/checkpoint reconciliation, and external MariaDB/RQG stress remain planned |
| Ownerless commit-race and deadlock stability | 🟡&nbsp;Partial | The concurrent explicit-transaction commit race now separates directory open/start serialization from the update/commit race and releases its commit barrier once every missing updated worker is proven to be an ownerless InnoDB-lock or page-write waiter, avoiding parent-induced deadlocks while preserving committed-delta, ownerless/native reopen, forced `.shm` rebuild, and recovery-anchor oracles. DML-only native checkpoint markers drain for autocommit and single-owner explicit writers, retain peer-explicit evidence while a peer is live, and now drain on final no-live close after native checkpoint/page-image proof; ordinary native opens can read retained WAL for startup recovery but retire that startup visibility before later native writes or locking reads. The deterministic two-row deadlock helper takes first-row ownership with `SELECT ... FOR UPDATE` before the barrier, performs both DML updates after releasing both children, and accepts MariaDB 1213 or 1205 as the single victim result while still requiring one winner and one victim; 1213 victims prove MyLite's internal deadlock rollback has discarded file-op redo, and 1205 victims prove discard after explicit rollback. Full rollback after local ownerless writes is now conservative: native InnoDB undo remains authoritative, rollback does not publish transaction-tracked user page images or force-advance the global page-visible LSN, and same-process rollback cleanup refreshes tracked pages from native storage before releasing ownerless page-write state; record/page-write/current-read refreshes consume visible LSN rather than raw latest redo, and startup visible-boundary page imports bypass ownerless page-write locks so a reader open cannot block behind a live writer's transaction-scoped page-write locks. InnoDB rollback-segment shutdown cleanup uses an ownerless-hook lifetime flag so the existing ownerless-only `TRX_UNDO_TO_PURGE` descriptor cleanup still runs after callback reset; ordinary-only lifetimes keep MariaDB's original active/prepared undo assertion. Focused production coverage repeated `deadlock-rows`, explicit-DML deadlock marker cleanup, DML marker drain/retention/rebuild selectors, `commit-race`, and the hidden-uncommitted-update direct case without timeout, mixed 305 totals, leaked uncommitted values, missing-history abort, stale ordinary-write reads, or rollback-segment assertion; broader DML origin classes, DDL/file lifecycle, and external randomized stress remain planned |
| Ownerless DML native checkpoint proof | 🟡&nbsp;Partial | No-live ownerless reclaim no longer keeps peer-explicit DML-only marker/WAL evidence solely because the committing writer was observed by a peer. After the final peer exits, the existing native checkpoint preparation path must prove InnoDB checkpoint coverage and per-page native image/absence/discard evidence before clearing the DML-only marker and checkpointing page-version WAL. Live peers still retain user page-version WAL, generic dictionary/file-op markers and autoincrement markers keep their existing stricter policy, and active-reader snapshot boundaries remain governed by page-version pin rules. Focused production selectors for autocommit DML, single-owner explicit DML, peer-explicit DML, explicit-DML deadlock cleanup, `commit-race`, and `live-reclaim` passed, as did ownerless SQL shards 1, 4, and 6 |
| Ownerless killed DML marker recovery | 🟡&nbsp;Partial | Focused Linux SQL coverage now forces the post-checkpoint file-per-table DML marker path in a child process, commits the update, exits with `_exit(0)` before `mylite_close()`, verifies the parent observes durable DML marker/WAL evidence while the child is still an unreaped zombie, and then proves the next ownerless opener sees the committed row, close-time no-live native proof drains the DML marker/WAL, forced `.shm` rebuild preserves the row, and ordinary native reopen can update the same table. A paired explicit-transaction case kills the writer before `COMMIT` after the same post-checkpoint DML path, verifies both native file-operation markers stay clear, requires no-live ownerless recovery and forced `.shm` rebuild to keep the pre-transaction row, and proves ordinary native reopen can still write the table. This covers committed and uncommitted DML process-lifecycle boundaries, while arbitrary instruction crashes, broader DML origin classes, live-peer recovery, DDL/file lifecycle recovery, and randomized external stress remain planned |
| Ownerless savepoint DML marker classification | 🟡&nbsp;Partial | Ownerless explicit transactions now track handle-local savepoint write state for `SAVEPOINT`, `ROLLBACK TO [SAVEPOINT]`, and `RELEASE SAVEPOINT`. Focused production selectors cover both successful `ROLLBACK TO` branches after checkpointed file-per-table DML: the no-earlier-write branch proves process-local InnoDB file-op redo is discarded and the later `COMMIT` leaves both file-op markers clear, while the retained-earlier-write branch proves the latch remains set, the later `COMMIT` publishes the DML-specific marker, no-live close and subsequent ownerless/ordinary native reopen keep the marker and WAL proof while native rollback history still requires them, and ownerless reopen, forced `.shm` rebuild, ordinary native reopen, and a follow-up native write preserve only the committed pre-savepoint row image. Killed-session coverage also proves an explicit transaction with writes before and after a savepoint, killed before rollback or commit, leaves both native file-op markers clear and recovers the pre-transaction rows through ownerless, forced `.shm`, and native reopen. Hook-build coverage now kills a writer after native `ROLLBACK TO SAVEPOINT` succeeds but before MyLite updates ownerless savepoint state and verifies the original row plus clear file-op markers through ownerless recovery, forced `.shm` rebuild, and ordinary native reopen. A concurrent handoff selector now keeps one ownerless writer open after rolling back post-savepoint DML, commits independent explicit DML from a second ownerless writer while the first remains live, proves the second writer's committed row is visible while the first writer's uncommitted row remains hidden, retains the DML marker/WAL while a peer is live, and verifies final ownerless reopen, forced `.shm` rebuild, ordinary native reopen, and a follow-up native write preserve only the first writer's pre-savepoint row plus the second writer's row. This closes bounded live savepoint marker classification, killed-before-savepoint-rollback, post-native/pre-state savepoint rollback, and one independent-table concurrent savepoint handoff; broader native mid-rollback crash windows beyond the focused row-undo boundary, same-page/same-table savepoint contention matrices, broader DML-origin `FILE_MODIFY` classes, DDL/file lifecycle recovery, and external randomized stress remain planned |
| Ownerless savepoint rollback live-peer crash | 🟡&nbsp;Partial | Hook-build coverage keeps a second ownerless process open while killing a writer after native `ROLLBACK TO SAVEPOINT` succeeds, after a same-table pre-savepoint write, and before MyLite updates ownerless savepoint state. A fresh ownerless read/write opener and shared-read-only opener must return `MYLITE_BUSY` while that peer is live; metadata-only and plain-read exceptions are not claimed because the live peer's private buffer pool cannot reconstruct every native undo predecessor safely. After the peer exits, no-live ownerless recovery, forced `.shm` rebuild, ordinary native reopen, and a follow-up native write preserve the original rows with both native file-operation markers clear. Native row-undo live-peer coverage now proves the same busy-then-recover lifecycle when the writer dies inside the shared InnoDB `row_undo()` path. This narrows live-peer recovery for post-native and native row-undo savepoint rollback boundaries; broader native mid-rollback substeps, longer savepoint contention matrices, broader DML origin classes, DDL/file lifecycle recovery, and external randomized stress remain planned |
| Ownerless native row-undo rollback crash coverage | 🟡&nbsp;Partial | Ownerless InnoDB rollback durably publishes each successful `row_undo()` step before the unsafe row-undo test fault can pause, so ownerless page-write flushing cannot leave a rollback page image on disk without matching native undo progress. Intermediate native undo-tail truncation is deferred to avoid waiting for history pages while transaction-owned application pages remain retained; terminal full rollback now truncates that deferred tail after restored application pages are durable and released, before native empty-commit validation. A focused 64-row, four-`VARBINARY(1024)`-column update creates a multi-page undo tail, rolls it back, reuses the ownerless connection, and reopens natively. The `ownerless-test-hooks` preset kills full-transaction rollback and `ROLLBACK TO SAVEPOINT` writers after the shared InnoDB `row_undo()` path has successfully applied and durably recorded one native row undo record but before the rollback graph completes. No-live ownerless recovery, forced `.shm` rebuild, and ordinary native reopen preserve the original pre-transaction rows, keep native DML and generic file-operation markers clear, checkpoint ownerless WAL or retain only native-support rollback-history evidence, and allow a follow-up native write. Live-peer variants hold another ownerless process open across the killed writer, require a fresh ownerless read/write opener to return `MYLITE_BUSY` while that peer remains live, reject shared read-only attachment until read/write recovery runs after peer release, then verify the same no-live recovery and reopen oracles. Generated-column row-undo crash coverage now runs at the first `rollback-after-native-row-undo` hit over stored and virtual generated columns plus generated-column secondary indexes, waits for MariaDB recovered active transactions to drain, and verifies original base values, stored generated values, virtual expression values, payloads, forced-index reads, live-peer busy behavior, forced `.shm` rebuild, ordinary native reopen, and follow-up writes. Cached rollback images are published as proof-only native-support records, so retained-WAL startup and page-index rebuilds cannot select partially rolled-back transaction images as readable user-page versions. FK/trigger row-undo crash coverage now runs update-side and delete-side selectors at the first row-undo hit for FK `ON UPDATE CASCADE`, FK `ON DELETE CASCADE`/`ON DELETE SET NULL`, and update/delete trigger-written audit rows, with both standalone no-live recovery and live-peer busy gating before final no-live recovery. This covers the first durable native row-undo boundary for full rollback, generated-column full rollback, FK/trigger update/delete side effects, and savepoint rollback; faults inside individual `row_undo_ins()`/`row_undo_mod()` substeps, broader FK/trigger side-effect matrices, XA/prepared rollback, longer savepoint contention matrices, DDL/file lifecycle recovery, and external randomized stress remain planned |
| Ownerless killed savepoint DML marker recovery | 🟡&nbsp;Partial | Focused Linux SQL coverage now kills the post-savepoint DML marker path after `ROLLBACK TO` has discarded process-local file-op redo and a later `COMMIT` has completed, but before `mylite_close()`. The parent waits for the writer to become a zombie, requires both native file-op markers to remain clear before reaping, verifies recovery close checkpoints retained non-marker WAL, and then verifies the pre-savepoint row through ownerless reopen, forced `.shm` rebuild, ordinary native reopen, and a follow-up native write. The adjacent killed-before-savepoint-rollback selector covers process death before rollback or commit, the hook-build savepoint rollback before-state selector covers process death after native rollback succeeds but before MyLite updates savepoint state, and the concurrent handoff selector covers one independent-table live-writer schedule around successful savepoint rollback. Broader crashes inside native savepoint rollback beyond the focused row-undo boundary and broader same-page/same-table concurrent-writer savepoint matrices remain planned |
...
| Ownerless startup page-write guard | 🟡&nbsp;Partial | `mtr_t::ownerless_page_write_enter()` now returns before acquiring ownerless page-write locks, refreshing page-version images, publishing snapshot boundaries, or recording page-write perf counters while InnoDB startup/recovery is still active. This covers the pre-start ownerless hook window where lifecycle/recovery hooks may already be installed before `mysql_server_init()` finishes, and regression coverage opens two fresh ownerless InnoDB databases in one process and creates InnoDB tables in both. The default production embedded performance probe now reaches its ownerless timing samples after the first fresh ownerless open/close instead of failing the next fresh bootstrap in doublewrite setup. SQL behavior, public APIs, directory layout, and WAL formats are unchanged; broader native redo/checkpoint reconciliation, DDL/file-lifecycle recovery, external MariaDB/RQG stress, and write-performance parity remain planned |
| Ownerless random transaction rollback handoff | 🟡&nbsp;Partial | Full ownerless rollback no longer publishes transaction-tracked user page images, flushes transaction page-write pages, or force-advances the ownerless page-visible LSN as if rollback were a committed boundary. Native InnoDB undo remains responsible for rollback, and ownerless cleanup still releases native and page-write locks plus SQL-layer transaction state. A focused production CTest runs the random transaction stress schedule at three rounds so full-rollback worker deltas and retry-attempt duplicates cannot regress silently, while the broader opt-in random transaction stress remains the long-form coverage. Killed-before-savepoint-rollback recovery and focused full/savepoint native row-undo rollback crashes are now covered as focused DML marker outcomes; broader rollback crash windows, mid-rollback/concurrent-writer savepoint matrices, native redo/checkpoint reconciliation, DDL/file-lifecycle recovery, and external randomized stress remain planned |
| Ownerless explicit transaction history proof | 🟡&nbsp;Partial | Explicit ownerless transactions now carry a conservative per-handle proof from eligible `INSERT ... VALUES` writes, constrained single-table `UPDATE ... SET ... WHERE ...` writes, constrained single-table `DELETE FROM ... WHERE ...` writes, and constrained single-table `REPLACE ... VALUES` writes to the later `COMMIT`, including mixed explicit transactions whose component writes all stay within those accepted classes, allowing proven prepared explicit transactions to publish committed ownerless visibility without the later unproven-statement dirty-page flush fallback, publish payload-bearing pre-commit `FIL_PAGE_UNDO_LOG` native-support page-version WAL in the transaction's own rollback-segment space so killed uncommitted writers have active undo-header recovery evidence, use the existing rollback-segment/undo history WAL proof instead of the ownerless write-history page flush in `trx_t::write_serialisation_history()`, batch COMMIT-time transaction-deferred, history-proof, native-support, and external-snapshot-lineage page-log appends for the proven COMMIT statement, and coalesce later non-durable latest-only checkpoint updates inside each proven insert statement after preserving the first update. Focused SQL coverage uses prepared inserts, prepared simple and ordered/limited updates, prepared simple and ordered/limited deletes, prepared simple replacements, and a prepared mixed insert/update/delete/replace transaction in explicit transactions and verifies fast commit publication, zero conservative flush/unproven counters, zero write-history ownerless flush pages, positive rollback-segment and undo history-proof page publication matching sample counters, transaction page publication at commit, proven-path page-log append evidence, active undo native-support publication for insert transactions, deferred latest-checkpoint coalescing for insert transactions, no publish failure, same-handle visibility, ownerless reopen, forced-`.shm` native reopen visibility, a savepoint-controlled transaction that deliberately stays on the conservative unproven path while preserving only the pre-savepoint row and not coalescing after disqualification, subquery update/delete and `REPLACE ... SELECT` statements that deliberately stay on the conservative unproven path, a mixed transaction with one unproven subquery update that stays conservative, trigger-bearing update/delete/replace targets that preserve trigger side effects while staying conservative, and auto-increment replacement targets that stay conservative; `INSERT ... SELECT`, `REPLACE ... SELECT`, broader `UPDATE` shapes beyond ordered/limited single-table forms, broader `DELETE`/`REPLACE` shapes beyond ordered/limited single-table deletes, DDL, locking-read transactions, savepoint-controlled transactions beyond this negative proof, foreign-key target or parent DML, auto-increment replacement semantics, and broader native redo/checkpoint reconciliation remain planned |
| Ownerless history-proof publication harness | 🟡&nbsp;Partial | Focused production SQL coverage now asserts the controlled single-owner insert fast path has zero exact native history flush pages/fallback rounds, accepted rollback-segment proof samples matching published rollback-segment proof pages, accepted undo proof samples matching published undo proof pages, rollback-segment proof pages accounting for published native-support `FIL_PAGE_TYPE_SYS`, undo proof pages accounting for published native-support `FIL_PAGE_UNDO_LOG`, and positive page-index publication for ordinary payload records while rollback-history proof stays out of live user-page read selection. Additional writer-open SQL coverage keeps page-publish stats disabled, proves normal production history-proof pair calls all succeed with zero exact flush fallback, observes newly appended payload-bearing native-support WAL records before close-time reclaim, and verifies zero native-support proof-only records before ownerless/native reopen. The unsafe ownerless hook build can force native-support page-version publication failure and proves that `trx_t::write_serialisation_history()` rejects the incomplete proof, accepts zero proof samples, takes positive native history flush pages, reports conservative COMMIT visibility because of publish failure, and still preserves same-handle, ownerless reopen, and forced-`.shm` native reopen visibility. This is a guardrail for future history-proof shrink/elision work, not a production speedup or a claim that the current proof pages are optional |
| Ownerless page-index trusted reads | 🟡&nbsp;Partial | Full native-support page images now publish live shared page-index entries while proof-only native-support metadata remains unindexed and unreadable as a page image. In a continuous single-owner epoch with one active statement and no active ownerless transactions, direct page-index hits can skip appended-tail validation, including during the local visible-fast writer; when active ownerless native write state is also absent, page-version reads trust absent index entries instead of repeatedly scanning the page-version WAL tail for pages the shared index can prove absent. Peer-present reads, retained snapshots, startup-WAL readers, scan-required index states, and same-process multi-statement work keep the conservative WAL scan proof. The visible-fast selector is back in the single-digit-second range on the local production build, while focused hidden-uncommitted, native-support, history-proof, commit-race, deadlock, marker-discard, and page-index crash selectors passed; broader redo/checkpoint, DDL/file lifecycle, and external randomized stress remain planned |
| Ownerless native-support proof-only WAL | 🟡&nbsp;Partial | Ownerless native-support proof-only page-log metadata remains supported as primitive WAL machinery: records carry page identity, page LSN, commit LSN, and native-support state with zero page payload; primitive coverage proves proof-only append validation, latest/read rejection, replay skipping, checkpoint retained-callback skipping, and the session-scoped proof-pair helper while preserving append/session record counts. Production rollback-history publication keeps exact payload-bearing native-support records where startup and reclaim need byte proof, while cached transaction rollback images are flagged proof-only so they cannot be replayed as user-page payload. Focused writer-open WAL SQL coverage asserts production history-proof records are native-support records rather than ordinary readable user records, and generated-column rollback crash coverage proves proof-only rollback images do not expose partially rolled-back rows. Broader native redo/checkpoint reconciliation and DDL/file-lifecycle recovery remain planned |
| Ownerless history-proof native reclaim | 🟡&nbsp;Partial | No-live ownerless reclaim now scans both proof-only native-support history metadata and payload-bearing startup-critical native-support records before clearing DML markers or truncating page-version WAL. Startup-critical records include rollback/undo pages whose payload is `FIL_PAGE_UNDO_LOG`, `FIL_PAGE_TYPE_SYS`, or `FIL_PAGE_TYPE_TRX_SYS`; reclaim forces the owning native tablespace to flush through the record page LSN and requires exact native page proof for those obligations. Proof-only metadata has no exact page bytes, so it fails closed and remains retained rather than proving truncation. Live-peer reclaim uses the same proof-including scan and refuses to reclaim while startup-critical history obligations remain. Reclaim also treats in-memory rollback history as a hard retention obligation, defers startup retained-payload reclaim until retained WAL has rebuilt the shared page index, and clears DML checkpoint markers only after native checkpoint, redo coverage, and rollback-history header proof are consistent. Normal latest-page reads still reject proof-only records as page images, while payload-bearing rollback-history records remain available to retained-WAL startup/replay. Focused production cutover coverage combines explicit-transaction `INSERT ... VALUES` writes with ordinary user page-version payload plus rollback-segment/undo history evidence, then accepts either the fully proven native-only cutover after `.wal`/`.shm` removal or the fast retained-WAL path when native file-operation, DML checkpoint, or rollback-history evidence still requires the WAL; both paths verify ordinary native or ownerless reopen without a shutdown-time purge wait. The DDL statement-lock retry loop covers the startup-storm regression that exposed insufficient history retention. This closes a bounded native-history reclaim gap, but broader redo/checkpoint recovery, DDL/file lifecycle, and external randomized stress remain planned |
| Ownerless proof-only readable WAL scan | 🟡&nbsp;Partial | Ownerless open/close retained-payload decisions now scan complete page-log records and ignore valid proof-only metadata records, instead of treating any bytes after the initialized page-log header as readable page-version payload. Uncheckpointed-record detection remains file-size based, mixed proof-only plus non-proof logs still enable ordinary native page-log read handling, incomplete tail bytes after proof-only records do not count as readable payload evidence, and scan errors fail closed by treating readable records as present. Primitive coverage exercises empty, proof-only-only, proof-only plus corrupt-tail, and mixed proof/payload logs. This optimizes proof-only-only startup/shutdown classification; it does not shrink payload-bearing native-support records or close broader redo/checkpoint and DDL/file-lifecycle gaps |
...
| Ownerless native redo startup and transaction images | 🟡&nbsp;Partial | No-live ownerless startup with retained ownerless checkpoint state enables a temporary startup-only InnoDB external-LSN advance gate during `mysql_server_init()`: external user-page visibility remains capped at the persisted ownerless visible boundary, while startup native-support reads can use the checkpoint latest LSN so rollback-segment and active undo-header evidence newer than the user-visible boundary can be restored before recovered rollback. Startup now separates the current native redo header as the startup source from the stronger claim that native state is reconciled enough to skip hooks, so pending native file-op, DML, or peer-explicit DML markers keep ownerless hooks installed for post-start checkpoint refresh while still preserving the current native redo header instead of restoring a stale sidecar backup. Captured transaction image validation now accepts same-LSN resident pages only when normalized flush checksum/header bytes match the captured image, and different-LSN resident buffer pages no longer suppress bounded committed transaction images whose page LSN is covered by the transaction visible commit boundary. Shared-memory recovery and ordinary startup now distinguish retained user page-version records from native-support-only WAL: committed user page records force visible tablespace replay/page-log reads, while nonempty native-support-only WAL remains durable recovery evidence instead of user payload. Ordinary exclusive startup with retained native-support-only ownerless evidence arms the uncheckpointed file-operation recovery bridge and relative redo-path normalization so recovered ownerless DDL resolves file-per-table redo paths under the active MyLite datadir. Product tablespace replay preserves same-LSN native pages only for snapshot-boundary records; final no-live shutdown replays retained visible user tablespace images after `mysql_server_end()` before checkpointing WAL at the ownerless visible LSN, while native-support-only WAL can be checkpointed after native redo covers native pages and rollback-history headers are consistent. Forced no-live native checkpoint proof stores ownerless latest separately while clamping visible to MariaDB's exact native checkpoint LSN instead of the checkpoint-record gap before WAL-empty native-authoritative startup. Retained-WAL ownerless shutdown now only lowers MariaDB from the configured crash-style fast shutdown to slow checkpoint shutdown when native rollback history is already empty; retained WAL with rollback-history obligations keeps the configured fast shutdown and relies on the retained WAL/recovery proof instead of waiting indefinitely in embedded native shutdown. Peer-explicit ownerless DML has a separate durable marker: final ownerless close keeps user WAL when this marker is pending, still forces a native InnoDB checkpoint so ordinary startup can begin safely, and a later no-live startup/reclaim path clears it only after exact native proof instead of a broad multi-tablespace heuristic. Hook-build crash coverage verifies the record-lock-grant and foreign-key dictionary crash selectors reopen successfully through ownerless and native recovery after retained-WAL/native-obligation classes; production commit-race coverage repeated the WAL-empty native reopen path 20 times after the exact-checkpoint clamp, and mixed history-proof cutover coverage proves ordinary native reopen both for the native-only `.wal`/`.shm` deletion path and for the retained-WAL native-obligation path; external randomized stress and additional edge-case matrices remain hardening work |
| Ownerless active-reader native purge deferral | 🟡&nbsp;Partial | InnoDB purge wake/coordinator paths now defer physical purge when ownerless read-view hooks are installed, cloning the directory-owned oldest read view as a native purge boundary even when the broader lock/page hook set is not the deciding signal. Focused active-reader pressure coverage asserts the repeatable-read reader process slot and read-view slot remain published while peer writers repeatedly open, update, and close, and verifies the reader does not raise `DB_MISSING_HISTORY`, retained page-version WAL stays present during the pin, and ownerless/native reopen plus forced `.shm` rebuild observe the final committed state after release. Broader external MariaDB/RQG active-reader stress remains planned |
| Ownerless expanding-page pressure | 🟡&nbsp;Partial | Ownerless active-reader pressure now covers distinct large-row updates across an expanding data-page set while a repeatable-read snapshot pin remains live, with retained user page-version WAL during the pin, post-release user-WAL reclamation while rollback-history native-support WAL may remain, and ownerless/native reopen after forced `.shm` rebuild once the reader releases; no-live close-time reclamation also has deterministic coverage for a raw-latest/page-visible checkpoint gap during retained-WAL pressure; opt-in `ownerless_page_log_limit_bytes` write throttling covers direct/prepared writes including prepared `INSERT ... SELECT`, representative DML/DDL write classes, variant DML/upsert/index/rename/truncate spellings, DML modifier spellings, column ALTER variants, CHECK and FOREIGN KEY constraint DDL variants, storage/rebuild ALTER variants, AUTO_INCREMENT DDL high-watermark ALTER, generated-column DDL/index variants, generated-column FK DDL variants, and schema/table-copy/replacement/replacement-copy/view/trigger dictionary variants at the first user-visible WAL pressure limit, `mylite_ownerless_pressure_status()` exposes the current active-pin/WAL throttle state, thresholded ownerless write/DDL/transaction-end statement-boundary checkpoint scheduling is covered when no peer process is live; live-idle coverage accepts checkpointed WAL or native-support-only retention while a peer is live and after the peer closes, while live writer and active-pin coverage retain user WAL, timer-driven checkpoint scheduling is covered after shared read-only snapshot release without another writer SQL statement and now proves an active prepared result cursor keeps retained WAL from being reclaimed until finalized, and dead pressure-writer cleanup coverage kills a writer after pressure-rejected direct DML, representative table/rename DDL, FK ADD/DROP, replacement-copy table DDL, view replacement, trigger replacement, and prepared DML with unclosed handles, then proves a later writer cleans stale process state, remains throttled while the real reader pin is live, leaves those DML/DDL families unchanged under pressure, succeeds after reader release while an idle peer remains live, and finishes with WAL checkpointed or reduced to native-support-only rollback-history evidence; deterministic active-reader pressure SQL trace export now provides external harness input with replacement-copy DDL and AUTO_INCREMENT high-watermark ALTER oracles plus bounded raw-client retry for MariaDB `1020`/`1205`/`1213` and SQLSTATE `40001` contention, plus Docker-backed MariaDB 11.8 scale-3 replay evidence, while full external MariaDB/RQG oracle stress remains planned |
| Ownerless pressure DDL variants | 🟡&nbsp;Partial | The active-reader pressure write-policy selectors now split the throttled and post-release phases so CI reports smaller failure domains while preserving the same retained-WAL setup. They throttle column `ALTER TABLE ... MODIFY COLUMN`, `CHANGE COLUMN`, `DROP COLUMN`, `RENAME COLUMN`, `ALTER COLUMN ... SET DEFAULT`, and `ALTER COLUMN ... DROP DEFAULT`, CHECK constraint add/drop, FOREIGN KEY add/drop, charset conversion, `ALTER TABLE ... FORCE`, ordinary row-format rebuild, compressed `ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=1`/`2`/`4`/`8`/`16` rebuilds, `ALTER TABLE ... AUTO_INCREMENT` high-watermark DDL, generated-column ALTER, generated-column secondary-index create/drop, stored generated-column child FK ADD, and stored generated-column referenced-FK DROP, plus `ALTER DATABASE`, duplicate `CREATE TABLE IF NOT EXISTS`, missing and real `DROP TABLE IF EXISTS`, `CREATE OR REPLACE TABLE ... LIKE`, `CREATE OR REPLACE TABLE ... AS SELECT`, `CREATE OR REPLACE VIEW`, `ALTER VIEW`, `CREATE OR REPLACE TRIGGER`, duplicate `CREATE TRIGGER IF NOT EXISTS`, and missing and real `DROP TRIGGER IF EXISTS` while retained WAL is at the configured ownerless pressure limit, verify those blocked variants leave column metadata/defaults, CHECK and FK metadata, charset/collation, row-format metadata, compressed payload/ZBLOB state for each covered key-block size, AUTO_INCREMENT high-watermark state, generated-column, generated-index, and generated-column FK metadata, schema defaults, table state, replacement-copy target metadata, view projection, trigger bodies, and trigger presence unchanged, also verify representative unsupported ownerless table-admin, `LOCK TABLES`, flush read-lock/export, host-file export/import, event/scheduler SQL including prepared event DDL/metadata, top-level sequence DDL/value SQL, `DISCARD TABLESPACE`, partitioned-table DDL, and rejected storage-option SQL keep explicit policy errors instead of pressure busy under the same retained-WAL limit, and verify the same statement families succeed after reader release with final ownerless/native reopen before and after forced `.shm` rebuild |
| Ownerless BLOB page pressure | 🟡&nbsp;Partial | Ownerless active-reader pressure now covers `ROW_FORMAT=DYNAMIC` off-page `LONGBLOB` payload updates that create native InnoDB BLOB page types, while a repeatable-read snapshot pin continues to read the original BLOB aggregates; coverage verifies retained user page-version WAL during the pin, post-release user-WAL reclamation while rollback-history native-support WAL may remain, ownerless/native reopen before and after forced `.shm` rebuild for the final BLOB aggregates, and a bounded 12 KiB / 24 KiB / 48 KiB / 96 KiB / 192 KiB long-value size matrix, while exhaustive long-value limits, broader row-format, encryption, crash, and external oracle matrices remain planned |
| Ownerless compressed BLOB page pressure | 🟡&nbsp;Partial | Ownerless active-reader pressure now covers `ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=8` off-page `LONGBLOB` payload updates created through prepared binary bindings that produce native InnoDB `ZBLOB`/`ZBLOB2` page types, while a repeatable-read snapshot pin continues to read the original compressed BLOB aggregates; coverage verifies retained user page-version WAL during the pin, post-release user-WAL reclamation while rollback-history native-support WAL may remain, ownerless/native reopen before and after forced `.shm` rebuild for final aggregates, a bounded 12 KiB / 24 KiB / 48 KiB / 96 KiB / 192 KiB compressed long-value size matrix, and a bounded compressed `KEY_BLOCK_SIZE=1` / `2` / `4` / `8` / `16` matrix, while broader compressed DDL option combinations, encryption, crash, and external oracle matrices remain planned |
| Ownerless compressed row-format DDL refresh | 🟡&nbsp;Partial | Already-open ownerless peers now handle `ALTER TABLE ... ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=8` table-copy rebuilds by observing the dictionary-generation change before page-version read selection, refreshing page-0 space-header flags with the rebuilt physical page size, evicting clean external pages, and staying in conservative native-read mode so stale pre-rebuild `(space_id, page_no)` page-version records are not overlaid onto the rebuilt compressed table; if the generation change is first noticed inside statement refresh, the same statement now revokes page-version-read eligibility before native execution; focused coverage verifies prepared BLOB writes after `KEY_BLOCK_SIZE=1`/`2`/`4`/`8`/`16` peer rebuilds, compressed row-format metadata, `ZBLOB`/`ZBLOB2` page evidence, post-peer parent refresh calls with zero `refresh_page_version_reads_enabled`, ownerless/native reopen, and forced `.shm` rebuild; `tools/ownerless-compressed-row-format-trace` now emits deterministic external-harness SQL for bounded `ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=1`/`2`/`4`/`8`/`16` rebuild cycles with retry-aware repeatable-reader polling, per-round InnoDB `ZIP_PAGE_SIZE` metadata checks, and final metadata/aggregate oracles, with focused scale-2 Docker-backed MariaDB replay evidence for the widened key-block cycle plus existing full 12-family scale-2 replay evidence for the trace family, hook-build crash coverage covers focused compressed row-format and `KEY_BLOCK_SIZE=1`/`2`/`4`/`8`/`16` dictionary-boundary rebuilds, and hook-build marker coverage proves plain plus both exact explicit copy-lock option orders for the `KEY_BLOCK_SIZE=1`/`2`/`4`/`8`/`16` crash windows leave the native file-op checkpoint-needed marker durable before recovery drains it, focused `KEY_BLOCK_SIZE=1`/`2`/`4`/`8`/`16` crash coverage now verifies live-peer dictionary recovery for both exact explicit copy-lock option orders with marker retention until final no-live drain, and rebuild-generation-aware page-version invalidation and external MariaDB/RQG stress remain planned |
