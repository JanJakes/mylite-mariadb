...
| Ownerless shared-memory file | 🟡&nbsp;Partial | `concurrency/mylite-concurrency.shm` is created and grown under a byte-range `SHM_RESIZE` lock, starts with a fixed 128-byte MyLite header bound to the database UUID, and is never shrunk by open; stale or invalid header bytes are rebuilt, but no runtime coordination segments are stored there yet |
