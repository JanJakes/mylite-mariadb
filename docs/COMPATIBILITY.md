...
| Ownerless shared-memory file | 🟡&nbsp;Partial | `concurrency/mylite-concurrency.shm` is created and grown under a byte-range `SHM_RESIZE` lock and is never shrunk by open; no runtime state is stored there yet |
