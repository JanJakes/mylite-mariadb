...
| Ownerless explicit transaction undo WAL elision | 🟡&nbsp;Partial | Explicit ownerless transactions now elide pre-commit rollback-segment `FIL_PAGE_UNDO_LOG` native-support page-version WAL in the transaction's own rollback-segment space; the existing history-proof guard still refuses active commit history pages before this elision can apply, while the current prepared explicit-transaction shape stays on the conservative commit flush fallback instead of visible-fast publication. Focused SQL coverage uses prepared inserts in one explicit transaction, verifies native-support undo elision, verifies the flush fallback has no publish failure, and checks same-handle, ownerless reopen, and forced-`.shm` native reopen visibility, while broader native redo/checkpoint reconciliation and transaction-scoped append batching remain planned |
