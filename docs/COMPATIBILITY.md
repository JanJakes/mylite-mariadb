...
| Multiple readers | 🟡&nbsp;Partial | Ownerless read/write opens can read committed InnoDB updates from peer processes through page-version visibility; dedicated shared read-only opens remain planned |
| Concurrent writers | 🟡&nbsp;Partial | Ownerless cross-process read/write opens coordinate InnoDB row/table locks, transaction visibility, redo visibility, and committed page-version reads through directory-owned files; broader DDL, retained-record checkpoint replay, and long-running stress remain planned |
...
| Ownerless writer tests | 🟡&nbsp;Partial | Normal embedded builds run `MYLITE_OPEN_OWNERLESS_RW` cross-process SQL writer tests that bypass the process-wide directory lock and route through the ownerless process, transaction, read-view, InnoDB lock, redo-visibility, page-version WAL, and checkpoint hooks; tests cover non-conflicting writers, row waits, deadlocks, committed external visibility, shared-memory rebuild, page-version checkpoints, and a killed uncommitted writer whose state blocks peer cleanup until a no-live-process reopen rebuilds volatile coordination. The `ownerless-test-hooks` preset adds unsafe deterministic fault injection and negative-proof coverage |
