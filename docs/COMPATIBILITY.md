...
cross-schema and schema-drop internal multi-drop crash points,
...
| Ownerless multi-drop DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset now registers same-schema and cross-schema same-statement `DROP TABLE` crash selectors as standalone CTests for visible CI timing and failure attribution; both selectors kill a writer after native table-list removal but before ownerless dictionary finish, verify live-peer recovery with both dropped `.frm`/`.ibd` file pairs absent, keep the native file-operation marker set while a peer remains live, drain it after final no-live recovery, and verify ownerless/native reopen before and after forced `.shm` rebuild. Same-schema inside-MariaDB-loop crash coverage also kills after the first table's native file operation but before the second table is logged or dropped, then verifies live-peer recovery with the first table absent, the second table still readable/writable, marker retention until no-live drain, and ownerless/native reopen before and after forced `.shm` rebuild; cross-schema and schema-drop internal loop crash points remain planned |
