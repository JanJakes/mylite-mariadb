...
| Schemas/databases | 🟡&nbsp;Partial | Direct and prepared `CREATE DATABASE` / `CREATE SCHEMA` store catalog namespace records with default character set, collation, and comment options, file-backed reopen rehydrates transient runtime directories from the catalog so `USE`, `SHOW DATABASES`, information-schema schema options, and table resolution work without durable datadir directories, and direct and prepared `DROP DATABASE` / `DROP SCHEMA` remove covered catalog metadata; final SQL-layer database hooks remain planned |
