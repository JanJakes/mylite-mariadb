...
live-peer recovery while keeping the native file-operation marker clear. Pure
multi-pair temporary rename chains now use the same metadata-only recovery lane,
and one representative mixed temporary/permanent rename list now proves the
permanent pair keeps the durable native file-operation marker until no-live
