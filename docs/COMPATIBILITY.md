...
| No explicit engine | 🟡&nbsp;Partial | `CREATE TABLE` routes to MyLite, stores requested engine `DEFAULT` with effective engine `MYLITE`, supports keyless row insert, full scans, update/delete, copy `ALTER` rebuilds, BLOB/TEXT payloads, and the narrow single-column autoincrement key insert path, and updates catalog metadata on `DROP TABLE` and simple `RENAME TABLE`; general indexes remain unsupported |
| `ENGINE=MYLITE` | 🟡&nbsp;Partial | Explicit MyLite DDL stores MariaDB table-definition metadata in the `.mylite` catalog, rediscovers it after reopen, supports keyless row insert, full scans, update/delete, copy `ALTER` rebuilds, BLOB/TEXT payloads, and the narrow single-column autoincrement key insert path, and updates catalog metadata on `DROP TABLE` and simple `RENAME TABLE`; general keyed writes remain unsupported |
| `ENGINE=InnoDB` | 🟡&nbsp;Partial | DDL routes to MyLite and records requested `InnoDB`; keyless row insert, update/delete, copy `ALTER` rebuilds, BLOB/TEXT payloads, single-column autoincrement primary-key insert, full scans, `DROP TABLE`, and simple `RENAME TABLE` are covered, while InnoDB transactions, foreign keys, general indexes, and tablespaces remain unsupported |
| `ENGINE=MyISAM` | 🟡&nbsp;Partial | DDL routes to MyLite and records requested `MyISAM`; keyless row insert, update/delete, copy `ALTER` rebuilds, BLOB/TEXT payloads, full scans, `DROP TABLE`, and simple `RENAME TABLE` are covered, while MyISAM data and index files are never durable application storage |
| `ENGINE=Aria` | 🟡&nbsp;Partial | DDL routes to MyLite and records requested `Aria`; keyless row insert, update/delete, copy `ALTER` rebuilds, BLOB/TEXT payloads, full scans, `DROP TABLE`, and simple `RENAME TABLE` are covered, while Aria data, index, and log files are never durable application storage |
...
| `ALTER TABLE` | 🟡&nbsp;Partial | Copy rebuilds are covered for supported MyLite-routed keyless table shapes, including column add/drop/rename, BLOB/TEXT payload rows, requested-engine metadata, and close/reopen visibility; `LOCK=NONE`, in-place algorithms, indexes, rollback, crash recovery, and foreign keys remain planned |
