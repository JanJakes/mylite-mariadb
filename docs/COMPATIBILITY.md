...
| `CREATE TABLE ... SELECT` | 🟡&nbsp;Partial | Successful supported CTAS creates MyLite catalog metadata and inserts SELECT result rows through the normal handler write path, including no-engine and explicit `ENGINE=InnoDB` targets, BLOB/TEXT payloads, autoincrement state, supported indexes, close/reopen visibility, and sidecar gates; failed-statement cleanup, temporary CTAS, `OR REPLACE`, `IGNORE` / `REPLACE`, unsupported source objects, foreign keys, partitions, and SQL rollback remain planned |
