...
| Ownerless schema comment DDL crash coverage | 🟡&nbsp;Partial | Hook-build live-peer recovery covers killed named `ALTER DATABASE <schema> COMMENT = 'literal'` and current-schema `USE <schema>; ALTER DATABASE COMMENT = 'literal'` writers after MariaDB rewrites native `db.opt` but before ownerless dictionary finish, with the native file-operation marker clear, recovered `INFORMATION_SCHEMA.SCHEMATA.SCHEMA_COMMENT`, preserved schema defaults, post-recovery table writes, ownerless/native reopen, and forced `.shm` rebuild; broader schema option ordering, invalid-option cleanup, and randomized external DDL stress remain planned |
