...
| Ownerless FK DDL live recovery | 🟡&nbsp;Partial | Hook-build coverage now treats bounded ordinary single-clause `ALTER TABLE ... ADD [CONSTRAINT] FOREIGN KEY ... REFERENCES ...` and `ALTER TABLE ... DROP FOREIGN KEY ...` as metadata-only recoverable dictionary boundaries when neither child nor referenced table has generated columns. Ordinary child/parent FK add/drop crash selectors kill the writer after native metadata creation/removal but before ownerless dictionary finish, recover while another ownerless peer remains live, verify the native file-operation checkpoint-needed marker stays clear, and check recovered FK metadata/enforcement or absent-FK orphan/parent writes through ownerless/native reopen and forced `.shm` rebuild. Generated-column child and referenced-column FK add/drop crash selectors remain on the conservative native-file lane: live-peer cleanup stays busy, no-live recovery preserves generated-column values and FK state, and ownerless/native reopen plus forced `.shm` rebuild observe the recovered result. Comma-separated FK multi-clause ALTER, generated-column metadata-only live recovery, broader FK crash fuzzing, and full external MariaDB/RQG FK graph execution remain planned |
