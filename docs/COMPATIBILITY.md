...
`ALTER TABLE schema.table ROW_FORMAT=COMPACT` prefinish recovery now allows
live-peer dictionary cleanup after a native compact row-format copy rebuild
from a dynamic source table. Focused
`ALTER TABLE schema.table ROW_FORMAT=REDUNDANT` prefinish recovery now allows
...
boundaries plus compact `ALTER TABLE ... ROW_FORMAT=COMPACT` rebuild
boundaries plus redundant `ALTER TABLE ... ROW_FORMAT=REDUNDANT` rebuild
