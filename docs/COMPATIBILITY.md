...
`INSERT ... VALUES` statements with one through thirty-two row constructors,
matching the default production bulk probe shape plus bounded eight-row,
sixteen-row, and thirty-two-row bulk diagnostics while keeping larger row lists
and broader DML on the previous
...
`8345.99` to `9307.03` rows/s on the same local shape.
