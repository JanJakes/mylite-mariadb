...
`INSERT ... VALUES` statements with one through eight row constructors, matching
the default production bulk probe shape and the next bounded eight-row bulk
diagnostic while keeping larger row lists and broader DML on the previous
...
deferred latest-checkpoint coalesces per statement. Larger native commit and
history-proof work remain separate performance targets.
