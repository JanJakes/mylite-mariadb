...
| Ownerless explicit transaction publish attribution | 🟡&nbsp;Partial | The production embedded performance probe now records compact stats-enabled summaries for prepared single-row inserts inside one explicit ownerless transaction, including page versions per insert and transaction, native-support undo/trx-system publication, page-log append session counts and timing, page-write commit-log timing, final commit-visibility timing, transaction-deferred page publication, undo-report MTR cost, undo-cache reuse, write-history flush cost, and ownerless-minus-ordinary row-insert/undo deltas; this is diagnostic coverage only, does not hold the page-log append lock across user transactions, and leaves the remaining explicit transaction optimization work planned |
