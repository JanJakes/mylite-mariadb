...
prefinish recovery now allows live-peer dictionary cleanup after the native
dynamic row-format copy rebuild; compressed/key-block row-format and other
...
Cross-schema and multi-pair rename, drop, compressed/key-block row-format
rebuild, schema, view, trigger, and foreign-key multi-DDL live-peer recovery
