...
| Ownerless concurrency completion gate | Production completion requires the ordinary embedded concurrency tests, the registered ownerless SQL shards, hook-build crash/recovery, negative-proof, and InnoDB-lock coverage, ownerless stress, and any claimed external MariaDB replay evidence: `ctest --preset php-embedded-prod -LE compat.ownerless-cross-process-sql --output-on-failure`, `ctest --preset php-embedded-prod -R '^libmylite\.ownerless-cross-process-sql\.' --parallel 1 --output-on-failure`, `ctest --preset ownerless-test-hooks -L 'compat.crash-reopen|compat.ownerless-negative-proof|compat.ownerless-innodb-lock' --output-on-failure`, `ctest --preset ownerless-stress --output-on-failure`, and the documented external replay command for the latest claimed seed window |
...
peer's later row. Same-table large-row
...
shards through the production `php-embedded-prod` preset with `--parallel 1`,
...
Ownerless read/write support is complete for the supported persistent InnoDB
application-table surface in this branch. Future work tracks per-engine
ownerless designs for non-InnoDB durable tables plus broader randomized,
external-oracle, and edge-case matrix hardening.
...
Ownerless read/write concurrency is supported for persistent InnoDB
application tables. Persistent non-InnoDB application tables are rejected at
...
| Ownerless savepoint rollback live-peer crash | 🟡&nbsp;Partial | Hook-build coverage keeps a second ownerless process open while killing a writer after native `ROLLBACK TO SAVEPOINT` succeeds, after a same-table pre-savepoint write, and before MyLite updates ownerless savepoint state. A fresh ownerless read/write opener must return `MYLITE_BUSY` while that peer is live, proving no-live cleanup is not attempted under live-peer ambiguity. After the peer exits, no-live ownerless recovery, forced `.shm` rebuild, ordinary native reopen, and a follow-up native write preserve the original rows with both native file-operation markers clear. Native row-undo live-peer coverage now proves the same busy-then-recover lifecycle when the writer dies inside the shared InnoDB `row_undo()` path. This narrows live-peer recovery for post-native and native row-undo savepoint rollback boundaries; broader native mid-rollback substeps, longer savepoint contention matrices, broader DML origin classes, DDL/file lifecycle recovery, and external randomized stress remain planned |
