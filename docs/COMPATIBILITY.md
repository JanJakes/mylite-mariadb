...
Ordinary `CREATE OR REPLACE TABLE` replacement now also has live-peer recovery
for the native replacement boundary. Replacement-copy, rename, truncate, drop,
...
| `CREATE TABLE ... LIKE` | 🟡&nbsp;Partial | Representative MariaDB table-definition copy behavior is covered for default-engine tables, including ownerless peer visibility from an already-open handle, hook-build live-peer crash recovery after native destination table creation before ownerless dictionary finish, hook-build no-live crash recovery for `CREATE OR REPLACE TABLE ... LIKE` replacement-copy completion before ownerless dictionary finish, and no-live stale-reader replay for `CREATE OR REPLACE TABLE ... LIKE` preserving the copied-shape replacement tablespace final state with page-0 space-id identity checks |
...
| Ownerless table-replacement DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills representative `CREATE OR REPLACE TABLE`, `CREATE OR REPLACE TABLE ... LIKE`, and `CREATE OR REPLACE TABLE ... AS SELECT` writers after native old-table replacement or replacement-copy completion but before ownerless dictionary finish; ordinary `CREATE OR REPLACE TABLE` now verifies live-peer cleanup, recovered replacement `.frm`/`.ibd` files, old-column and old-index absence, new-column and new-index metadata, empty replacement rowset, post-recovery writes, ownerless/native reopen, forced `.shm` rebuild, live marker retention, and no-live marker drain, while replacement-copy LIKE/CTAS still verifies live-peer cleanup remains busy until no-live recovery, copied secondary-index metadata for LIKE, CTAS copied rows, post-recovery writes, ownerless/native reopen, and forced `.shm` rebuild |
