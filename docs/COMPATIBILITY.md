...
| Ownerless native row-undo rollback crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills full-transaction rollback and `ROLLBACK TO SAVEPOINT` writers after the shared InnoDB `row_undo()` path has successfully applied one native row undo record but before the rollback graph completes. No-live ownerless recovery, forced `.shm` rebuild, and ordinary native reopen preserve the original pre-transaction rows, keep native DML and generic file-operation markers clear, checkpoint ownerless WAL, and allow a follow-up native write. Live-peer variants now hold another ownerless process open across the killed writer, require a fresh ownerless read/write opener to return `MYLITE_BUSY` while that peer remains live, reject shared read-only attachment until read/write recovery runs after peer release, then verify the same no-live recovery and reopen oracles. Generated-column row-undo crash coverage now repeats the full-transaction boundary over stored and virtual generated columns plus generated-column secondary indexes, verifying original base values, stored generated values, virtual expression values, payloads, forced-index reads, live-peer busy behavior, forced `.shm` rebuild, ordinary native reopen, and follow-up writes. FK/trigger row-undo crash coverage now repeats the full-transaction boundary over FK `ON UPDATE CASCADE` child-row changes and a later deterministic delete-side boundary, reached by skipping three `rollback-after-native-row-undo` hits, for FK `ON DELETE CASCADE`/`ON DELETE SET NULL` child-row changes plus update/delete trigger-written audit rows. The delete-side boundary now has both standalone no-live recovery and live-peer busy gating before final no-live recovery, with forced `.shm` rebuild, ordinary native reopen, and follow-up FK/trigger native writes. This covers one deterministic native row-undo boundary for full rollback, generated-column full rollback, FK/trigger full rollback, delete-side FK/trigger full rollback, and savepoint rollback; arbitrary row-undo substeps, earlier FK-delete undo hits, broader FK/trigger side-effect matrices, XA/prepared rollback, longer savepoint contention matrices, DDL/file lifecycle recovery, and external randomized stress remain planned |
