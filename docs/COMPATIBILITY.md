...
`ALTER TABLE ... RENAME TO` forms. Same-schema foreign-key multi-rename
native-loop crash coverage now kills after the first native rename, verifies
MariaDB DDL-log recovery restores the original parent/child names and FK
enforcement, and retains the native file-operation marker until final no-live
drain. Focused
