...
| Ownerless cross-schema multi-rename DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills explicit schema-qualified same-schema, implicit-schema same-schema, cross-schema, same-schema multi-pair swap, cross-schema multi-pair swap, foreign-key parent/child multi-pair `RENAME TABLE`, native-loop `ALTER TABLE ... RENAME TO`, and view-only `RENAME TABLE` writers before ownerless dictionary finish; covered native table rename-list shapes now verify live ownerless recovery while another peer remains open, final target metadata, absent source or temporary names, preserved or swapped InnoDB `SPACE` identities where relevant, FK metadata/enforcement where relevant, post-recovery writes, live marker retention, no-live marker drain, and ownerless/native reopen before and after forced `.shm` rebuild, while view-only rename uses metadata-only live recovery with the native file-operation marker clear. `IF EXISTS` and temporary-table crash rename variants remain planned |
