...
| Ownerless page-version log primitive | 🟡&nbsp;Partial | A first-party fixed-record page-version log primitive can initialize a log, serialize appends with a byte-range lock, store real InnoDB `space_id`/`page_no` pairs including zero, read the latest version visible at a commit LSN, report missing pages and undersized buffers, and tolerate an incomplete tail record; primitive tests cover same-process and cross-process append/read behavior, but production ownerless read/write still does not consult this log from InnoDB page IO or checkpoint it into tablespace files |
