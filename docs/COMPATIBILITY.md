...
| --- | --- | --- |
| Primary portable database directory | 🟡&nbsp;Partial | Open/create establishes and validates a MyLite-owned directory with `mylite.meta`, `mylite.lock`, `datadir/`, `tmp/`, process-local `run/`, `concurrency/mylite-concurrency.meta`, `concurrency/mylite-concurrency.lock`, and `concurrency/mylite-concurrency.shm`; `.mylite/` is recommended but not enforced |
...
| Ownerless shared-memory file | 🟡&nbsp;Partial | `concurrency/mylite-concurrency.shm` is created and grown under a byte-range `SHM_RESIZE` lock, validated through `MAP_SHARED`, starts with a fixed 128-byte MyLite header bound to the database UUID, and contains a fixed process-registry segment that records the active exclusive runtime process; dirty or rebuilding volatile state is rebuilt with an incremented recovery generation, but no wait, lock-manager, transaction, or page-version segments are active yet |
