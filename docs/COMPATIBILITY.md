...
file-operation marker clear. Single-table `DROP TABLE IF EXISTS` now has
live-peer recovery for both missing-table no-op and existing-table removal
outcomes. Representative `CREATE DATABASE|SCHEMA`, `ALTER DATABASE|SCHEMA`,
and table-bearing or empty `DROP DATABASE|SCHEMA` now have live-peer recovery
for schema directory/`db.opt` creation, rewrite, or removal.
...
`DROP TEMPORARY TABLE`, multi-table
...
| Ownerless table-idempotent DDL crash coverage | 🟡&nbsp;Partial | The `ownerless-test-hooks` preset kills duplicate `CREATE TABLE IF NOT EXISTS`, missing `DROP TABLE IF EXISTS` no-op, and existing single-table `DROP TABLE IF EXISTS` writers after MariaDB returns success but before ownerless dictionary finish; recovery verifies duplicate-create live-peer cleanup remains busy until no-live recovery, missing drop recovers while a peer remains live with the native file-operation marker clear, existing drop recovers while a peer remains live with the marker retained until final no-live drain, duplicate create preserves the original table definition and keeps plain duplicate create returning errno 1050, missing drop preserves the real table while keeping the missing table metadata/files absent, existing drop keeps the removed table metadata/files absent, post-recovery writes succeed where the preserved table remains, and ownerless/native reopen plus forced `.shm` rebuild observe the same state |
