...
non-CHECK primary/unique constraint DDL,
...
| Index add/drop DDL | 🟡&nbsp;Partial | Route supported copy-rebuild index additions and drops through MariaDB DDL and MyLite catalog/index updates, including generated-column indexes, named primary/unique constraint syntax in initial DDL, representative `ALTER TABLE ... ADD CONSTRAINT ... UNIQUE`, unique-key `DROP CONSTRAINT`, standalone `CREATE INDEX IF NOT EXISTS`, ALTER-table `ADD INDEX IF NOT EXISTS`, and `DROP INDEX IF EXISTS` skips, and representative default-algorithm standalone index DDL after catalog-only reopen; online DDL, unsupported index classes, broader constraint matrices, SQL rollback, and foreign keys remain planned |
