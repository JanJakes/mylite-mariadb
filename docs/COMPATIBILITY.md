...
| Ownerless shared-memory file | 🟡&nbsp;Partial | `concurrency/mylite-concurrency.shm` is created and grown under a byte-range `SHM_RESIZE` lock, starts with a fixed 128-byte MyLite header bound to the database UUID, and contains a fixed process-registry segment that records the active exclusive runtime process; stale or invalid header bytes are rebuilt, but no wait, lock-manager, transaction, or page-version segments are active yet |
