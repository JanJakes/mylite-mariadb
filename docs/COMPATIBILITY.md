...
| Ownerless coordination primitives | 🟡&nbsp;Partial | POSIX file-backed `MAP_SHARED` visibility, grow/remap behavior, byte-range lock conflicts, lock release on process exit, internal mapped latch wait/wake plus timeout behavior, and an internal cross-process exclusive lock-table primitive are covered as platform evidence; no product ownerless open path uses them yet |
