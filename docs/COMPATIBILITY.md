...
CHECK constraint enforcement, generated column coverage, initial application-schema smoke, and
...
| `ENGINE=InnoDB` | 🟡&nbsp;Partial | DDL routes to MyLite and records requested `InnoDB`; row insert, update/delete, truncate, copy `ALTER` rebuilds, `CREATE TABLE ... LIKE`, successful supported `CREATE TABLE ... SELECT`, BLOB/TEXT payloads, autoincrement, supported primary/unique/secondary indexes, bounded BLOB/TEXT prefix indexes, basic CHECK constraints, basic unindexed generated columns, full scans, `DROP TABLE`, and simple `RENAME TABLE` are covered, while InnoDB transactions, foreign-key enforcement, generated-column indexes, full BLOB/TEXT indexes, and tablespaces remain unsupported; foreign-key DDL is rejected explicitly |
...
| Primary and secondary indexes | 🟡&nbsp;Partial | Supported primary and secondary keys use durable append-only MyLite index-entry pages plus ordered in-memory handler cursors, including standalone copy-rebuild index DDL, bounded BLOB/TEXT prefix indexes, and logical truncate filtering; B-tree navigation, oversized/full BLOB/TEXT indexes, FULLTEXT, SPATIAL, generated/expression indexes, and compaction remain planned, and generated-column indexes reject before catalog publication |
