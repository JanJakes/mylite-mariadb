...
outcomes. Representative `CREATE DATABASE`, `ALTER DATABASE`, and table-bearing
`DROP DATABASE` now have live-peer recovery for schema directory/`db.opt`
creation, rewrite, or removal. `CREATE DATABASE|SCHEMA IF NOT EXISTS`
...
also has marker-clear live recovery, and table-bearing `DROP DATABASE IF
EXISTS` has marker-retaining live recovery.
`DROP TEMPORARY TABLE`, multi-table
`DROP TABLE IF EXISTS` lists,
inside-MariaDB-loop multi-drop crash points, view-only rename forms,
partition/FK truncate variants, broader ALTER rebuild, schema lifecycle synonym
and existing-drop synonym variants, broader view and trigger variants, and non-rename
foreign-key multi-DDL live-peer recovery
