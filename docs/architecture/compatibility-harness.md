...
| `routed-ddl-dml` | `storage-smoke-dev` | `compat-routed-ddl-dml` | Routed direct/prepared schema namespaces, table and index DDL, indexed table/index rename, `CREATE TABLE ... LIKE`, `CREATE TABLE ... SELECT` including duplicate modes, OR REPLACE, truncate, and DML smoke |
