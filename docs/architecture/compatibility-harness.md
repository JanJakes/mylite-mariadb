...
unsupported inventory currently records 2,955 upstream MTR files that are
...
Performance Schema, server plugin, native encryption, partition, Oracle SQL
mode, Sequence, log-table, or profile-specific result surfaces. This is a scale
measurement, not compatibility proof for unrun tests.
