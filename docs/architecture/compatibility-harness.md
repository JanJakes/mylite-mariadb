...
| `transaction` | `storage-smoke-dev` | `compat-transaction` | Direct row-DML transaction commit/rollback, supported session autocommit mode, read-only/read-write and completion-type transaction SET controls, direct and prepared savepoint rollback/release, active transaction crash recovery, and unsupported transaction-control policy |
