...
| `sql-comparison` | `embedded-dev` | `compat-sql-comparison` | MariaDB baseline SQL API comparison, including direct and prepared expression result sets |
...
The `sql-comparison` group compares representative direct execution, direct and
prepared expression result sets, prepared statement, metadata, and warning
behavior against a raw MariaDB embedded runtime. MariaDB's MTR remains the
long-term source for broad upstream compatibility cases. MyLite should not run MTR
