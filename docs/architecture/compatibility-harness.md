...
| `transaction` | `storage-smoke-dev` | `compat-transaction` | Direct row-DML transaction commit/rollback, supported session autocommit mode, no-op read-write/no-chain transaction SET controls, direct and prepared savepoint rollback/release, active transaction crash recovery, and unsupported transaction-control policy |
