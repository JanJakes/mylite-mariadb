...
default/version behavior, and static global variable metadata, plus selected
`my_print_defaults`, `mysqltest`, and `perror` support-tool behavior.
...
files plus 27 MyLite-owned MTR files; accepted coverage is 415 upstream baseline
tests, 8 MyLite profile tests, and 19 MyLite storage-routed tests. The known
unsupported inventory currently records 4,613 upstream MTR files that are
...
console-codepage probes, exact partition DDL/metadata probes, information-schema
...
engine metadata for unsupported native engines,
...
plugin installation, routine metadata, stored-function rollback/account
runtime leftovers, administrative FLUSH/FTWRL behavior,
table-maintenance/key-cache administration, online/in-place ALTER probes,
legacy `.frm` metadata, or profile-specific result surfaces, including plugin
inventory/origin probes,
main `--big-test` probes, big processlist preallocation, upstream-disabled
MEMORY placeholders, zlib compression, DES/SFORMAT, `INSERT DELAYED`, static
SHOW, packaging-file checks, and daemon status counters, plus
generated/virtual-column probes that exercise disabled JSON_TABLE,
...
restart, and persistent-stats leftovers, generated/virtual optional-engine and
SQL HANDLER probes, virtual-column upgrade sidecars, native MyISAM key/repair
