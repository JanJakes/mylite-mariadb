...
| `server-surface` | `storage-smoke-dev` | `compat-server-surface` | Unsupported server, SQL HELP, SQL file-I/O, server utility function, Oracle SQL mode, XML SQL function, GIS SQL function, SFORMAT SQL function, and non-table object surface policy |
