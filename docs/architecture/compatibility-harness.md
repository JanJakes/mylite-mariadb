...
| `server-surface` | `storage-smoke-dev` | `compat-server-surface` | Unsupported server, binlog/replication, SQL HELP, SELECT PROCEDURE, SQL file-I/O, table-maintenance/key-cache administration, server utility function, Oracle SQL mode, XML SQL function, GIS SQL function, SFORMAT SQL function, stored-program runtime, dynamic UDF runtime, and non-table object surface policy |
