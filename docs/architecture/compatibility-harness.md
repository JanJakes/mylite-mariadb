...
| `server-surface` | `storage-smoke-dev` | `compat-server-surface` | Unsupported server, binlog/replication, SQL HANDLER, SQL sequence values, virtual sequence storage engine, SQL HELP, SELECT PROCEDURE, SQL file-I/O, table-maintenance/key-cache administration, user-statistics, statement profiling, optimizer trace, static SHOW information, foreign-server metadata, external backup SQL, query cache administration, server utility function, Oracle SQL mode, XML SQL function, GIS SQL function, SFORMAT SQL function, JSON schema validation function, JSON table function, dynamic column function, stored-program runtime, dynamic UDF runtime, and non-table object surface policy |
