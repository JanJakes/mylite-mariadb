...
metadata, selected MTR-profile view and trigger DDL/runtime behavior, DDL
constraint/index metadata, selected Aria ALTER/index-upgrade behavior, selected
lock-table DDL behavior, deprecated server syntax rejection, `ORDER BY`,
optimizer-cost metadata, selected EXPLAIN plan output,
