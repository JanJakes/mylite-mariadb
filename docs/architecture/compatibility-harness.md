...
| `application-schema` | `storage-smoke-dev` | `compat-application-schema` | WordPress-shaped core schema, WordPress installer DDL and seed fixtures including deterministic default options and role payloads, WordPress multisite schema/seed fixtures, BuddyPress component schema/seed fixtures, Laravel default ORM schema fixtures, collation restart matrix, and prefix-index smoke |
