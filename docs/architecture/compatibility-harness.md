...
| `transaction` | `storage-smoke-dev` | `compat-transaction` | Direct row-DML transaction commit/rollback, supported session autocommit mode, direct and prepared savepoint rollback/release, active transaction crash recovery, and unsupported transaction-control policy |
