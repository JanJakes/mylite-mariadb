...
- MyLite is MariaDB-derived. Use MariaDB source and official MariaDB
  documentation for the selected base line as the primary authority.
...
- MySQL behavior may inform historical compatibility questions, but it is not
  the normative target for MyLite.
...
- MyLite is GPL-2.0-only because it modifies MariaDB Server.
- SQLite-like API semantics do not imply SQLite-like licensing. Public docs and
  packaging must not suggest permissive embedding rights.
- MariaDB and MySQL trademark obligations are separate from GPL obligations.
  Public packaging should include no-affiliation notices and review project
  naming before release.
...
- Preserve a path to useful in-process write concurrency where practical.
- Do not assume SQLite-like or cross-process concurrency until it is
...
- Substantial work should have a slice spec under `docs/specs/<slice>/`.
- Pre-code implementation work should account for
  `docs/architecture/pre-implementation-checklist.md`.
...
- Keep workflow docs and local skills aligned: when a new required slice exists
  in the checklist, the roadmap and skill guidance should name the same slice.
