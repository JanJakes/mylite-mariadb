...
profile reports `8` full-page checksum calls, `127,063` zero-tail checksum
calls, `0` index-leaf page clears, and `0` encoded index leaf max-cell reads.
The same run sampled `73.184 us/op` and reports `2` raw entry order builds plus
...
`decode_maintained_index_root_page`, while verification contributes `7,078`
zero-tail `row` calls through `decode_row_page_metadata` after uncached
full-row reads began materializing rows during the validated row-page scan.
Index-leaf
...
current prepared-insert smoke profile reports `21,031` buffer-limit
`index-leaf` refreshes, `1` statement-commit `index-leaf` refresh, `2`
statement-commit `index-branch` refreshes, and `66,144`
...
current prepared-insert profile reports `14` full buffer-limit leaf victims,
`118` victims with `1-15` free slots, and `20,899` victims with `16+` free
...
reports `15,389` buffer-limit leaf victims as `never-replaced`, `3,647` as
`replaced-once`, and `1,995` as `replaced-multiple`, showing that the remaining
...
the same profile reports `11,961` never-replaced victims in the `75-99%` fill
band, `7` never-replaced full leaves, and no buffer-limit victims below
...
free-slot pressure victims are mostly first-admitted leaves: `94` were
...
prepared-insert smoke profile reports `16,793` incoming leaves in the
...
free slots and `21,031` incoming leaves with `16+` free slots after
...
`7,430` with `64-127`, and `1,978` with `128+`, showing the bounded policy
...
reports all `21,031` buffer-limit pressure admissions under
...
The current prepared-insert smoke profile reports all `21,031` pressure
...
merge. The current prepared-insert smoke profile attributes all `21,031`
...
prepared-insert smoke profile reports `3,827` dirty `index-leaf`
`future-current-header-direct-write` rows, `31,312`
`future-current-header-near-full-direct-write` rows, `18,120`
`future-current-header-16-31-direct-write` rows, `4,218`
`future-current-header-replaced-broad-victim-direct-write` rows, `2,343`
`future-current-header-dense-broad-victim-direct-write` rows, `2,147`
`future-current-header-equal-broad-victim-direct-write` rows, `2,058`
`future-current-header-equal-dense-victim-direct-write` rows, `2,119`
`future-current-header-wider-victim-direct-write` rows, `21,031`
`future-current-header-partial-leaf` fallback rows, `4,455` dirty
`index-leaf` rows blocked as `parent-not-full`, and no `missing-undo` leaf
rows. That keeps the current policy bounded to full, near-full, `16-31`,
already-replaced broad-victim, dense/equal victim, and strictly wider-victim
future-current leaves; broader fallback leaves and branch entries continue to
...
reports `3,827` full future-current leaf direct writes, `31,312` near-full
future-current leaf direct writes with `1-15` free slots, `18,120`
`16-31` future-current leaf direct writes, `2,343` dense-victim direct-written
leaves with `64-127` free slots, and `2,119` wider-victim direct-written
leaves. The remaining `21,031`
...
coalescing. The same profile reports `30,758` parent-resident leaves, mostly
...
`11,623` with `32-63` free slots, `7,430` with `64-127`, and `1,978` with
`128+`. Parent-resident guard rows also remain mostly broad partial leaves,
with `30,337` rows in the coarse `16+` bucket, so any broader residual
direct-write policy still needs a bounded threshold and benchmark proof before
it bypasses parent dirty-buffer coalescing.
...
Full, near-full, and `16-31` free-slot future-current index leaves can now
publish directly during dirty-buffer merge when the parent buffer is full,
while append-buffer-resident pages, parent-resident pages, branches, pages
past the parent current header, and partial leaves with `32+` free slots use
...
victims. That storage-smoke sample reported a `76.832 us/op` prepared
...
same prepared-insert smoke profile reports `1,152` lifecycle starts from the
...
prepared-insert smoke profile reports `6,634` residual rejected below-tail
candidate admissions, `6,627` buffer-limit flushes, `7` discards, and `0`
clears, so those residual candidate admissions also reconcile exactly. The
same sample reported a `78.120 us/op` prepared insert step.
Dense broad-victim direct write adds one more bounded future-current
publication predicate without changing the replaced-broad lifecycle counters:
when a `64-127` free-slot below-tail incoming leaf would evict a
checksum-dirty `32-63` resident victim, merge direct-writes the incoming page
and keeps the denser victim buffered. That prepared-insert smoke profile
reports `2,773`
`future-current-header-dense-broad-victim-direct-write` dirty `index-leaf`
rows, reducing dirty leaf pressure admissions to `28,551`, increasing direct
dirty leaf merge writes to `59,392`, and lowering index-leaf dirty refreshes
to `87,944` without reintroducing the rejected broad below-tail regression.
...
dirty leaf merge writes to `61,711`, and lowering index-leaf dirty refreshes
to `87,911`. Residual rejected below-tail candidate admissions fall to
...
buffer.
Equal dense-victim direct write covers the matching `32-63` residual case:
...
equal-density victim buffered. That prepared-insert smoke profile
reports `2,542`
...
Wider-victim direct write covers the next bounded residual case: when a
`32-63` or `64-127` free-slot below-tail incoming leaf would evict a
checksum-dirty resident victim with a strictly wider free-slot band, merge
direct-writes the incoming page and keeps the wider-capacity victim buffered.
The current prepared-insert smoke profile reports `2,119`
`future-current-header-wider-victim-direct-write` dirty `index-leaf` rows,
...
host load.
The same merge guard facts now carry the incoming dirty-buffer entry's page
family into the guard outcome and future-page relation recorders. Those
test-hook recorders keep fallback classification for direct tests, while the
prepared-insert hot path avoids parsing the same child page family twice. The
...
Dirty checksum refresh counters now record the known page family from the
typed refresh branch instead of reparsing the page header in test-hook builds.
The parser-based helper remains covered as fallback for future callers, while
dirty checksum selection and checksum bytes stay unchanged. The prepared-insert
dirty-refresh rows stayed unchanged (`87,178` dirty-page-flush refreshes,
`6,849` append-buffer-flush refreshes, `6` append-buffer-copy refreshes, and
`94,033` total dirty refreshes), as did `8` full-page checksum calls,
...
claim.
Zeroed index-leaf checksum attribution now reuses the index-leaf family written
by `encode_zeroed_index_leaf_page` before computing the leaf checksum. Generic
checksum callers keep parser-backed attribution, and encoded leaf checksum
bytes stay unchanged. The prepared-insert call-site rows stayed `25,572`
...
snapshot pages. Structural counters stayed unchanged (`8` full-page checksum
calls, `227,063` zero-tail checksum calls, `677` maintained-root decodes,
`94,033` dirty refreshes, `21,031` pressure admissions, `66,144` merge direct
writes, `87,176` index-leaf dirty refreshes, `31,938` pressure-context builds,
...
so this is recorded as profiling-path simplification rather than a timing
claim.
The remaining typed checksum call sites now also use caller-proved page-family
facts in test-hook builds: journal header encoding, index-entry run decoding,
branch page encoding, row-page encoding, header encoding/decoding, and catalog
validation. Ambiguous page detection and generic checksum callers keep
parser-backed attribution. The prepared-insert checksum call-site table stayed
unchanged for those rows (`1` journal full-page call, `12` index-entry
zero-tail calls, `2` branch zero-tail calls from
`encode_index_branch_page_from_leaf_run`, `365` row zero-tail calls, `1`
header zero-tail call, `1` header full-page call, and `1` catalog full-page
call), and structural counters stayed unchanged (`8` full-page checksum calls,
`227,063` zero-tail checksum calls, `94,033` dirty refreshes, `21,031`
pressure admissions, `66,144` merge direct writes, `87,176` index-leaf dirty
refreshes, `677` maintained-root decodes, `31,938` pressure-context builds,
and `19,053` planned stores). The sampled storage-smoke prepared insert step
...
timing claim.
Test-hook dirty-buffer entries now cache derived resident-entry leaf occupancy
facts when admitted or replaced. Merge guard, flush, rank, fallback-origin,
pressure-victim, and broad-victim direct-write accounting reuse those cached
facts for resident entries, while standalone incoming page buffers and
manually constructed test entries keep parser fallback paths. Dirty-buffer
publication, checksum bytes, journal protection, recovery validation,
maintained-root planning, and non-test builds remain unchanged. The
...
step was `76.886 us/op` under variable host load, so this is recorded as a
resident-entry accounting simplification rather than a timing claim.
...
callers without writer facts, and non-test builds keep the existing writer API.
The prepared-insert publication and merge direct-write rows stayed unchanged
(`66,144` checksum-dirty `index-leaf` merge direct writes, `21,031`
buffer-limit `index-leaf` publications, `1` statement-commit `index-leaf`,
and `2` statement-commit `index-branch`), as did `8` full-page checksum calls,
`227,063` zero-tail checksum calls, `677` maintained-root decodes, `87,176`
...
leaf fast-path work. The current prepared-insert smoke profile reports `4,793`
append-only leaf replacements and `29,755` interior single-entry insert leaf
...
class-specific fast-path output reports matching `4,793` append and `29,755`
insert fast replacements, reconciling exactly with the `34,548` leaf growth
fast replacement total. Test-hook replacement recording computes that leaf
change class once per incoming leaf replacement and fans the result out to both
replacement and merge-fallback counters, so the measured path avoids duplicate
classification while preserving eviction, flush, rollback, checksum, and
...
Maintained-root dirty-buffer replacement now has one-cell insert and overflow
tail-mark fast paths for the root page images already proved by
maintained-root planning and protected by the write journal. The insert helper
verifies fixed root metadata, entry count, used-bytes growth, unchanged
flags/overflow-tail bytes, unchanged prefix/suffix cells, and unchanged unused
tail before updating the resident dirty-buffer page with one cell move and
copying only the entry-count, used-bytes, and checksum fields. The overflow
helper verifies that a single-page root only adds `HAS_OVERFLOW_TAIL`, writes a
new forward tail page id, clears the checksum slot, and leaves the rest of the
page unchanged before copying only those header fields. Planning decodes,
journal validation, durable checksum publication, and fallback full-page
replacement behavior are unchanged. The current prepared-insert smoke profile
reports `666` maintained-root insert fast replacements and `2` maintained-root
overflow fast replacements, covering all `668` checksum-dirty `index-root`
...
Uncached full-row reads now materialize live rows during the row-page scan
instead of first collecting live row ids and then rereading the same packed
physical pages through rowset materialization. The scan callback preserves row
order, row ids, row-payload cache seeding, and durable live-row-id cache
storage while reusing the already validated row-page metadata. The current
prepared-insert smoke profile still keeps protected maintained-root decodes at
`5` and full-page checksum calls at `8`, while lowering
`decode_row_page_metadata` zero-tail calls from `14,086` to `7,078` and total
zero-tail checksum calls from `134,071` to `127,063`.
...
The writer keeps dirty-page undo capture, dirty-buffer buffering,
checksum-dirty direct fallback, active leaf-cache publication, and stale
dirty-buffer discard, while skipping the generic branch-cache publication
probe. The current prepared-insert smoke profile keeps `8` full-page checksum
calls, `227,063` zero-tail checksum calls, `677` maintained-root decodes, and
`0` branch/leaf writer decodes; dirty leaf pressure admissions remain
`21,031`, all from `insert_branch_index_leaf_entry`, and dirty leaf
replacements remain `34,548`, all from the same writer. The timing sample ran
under unrelated host load, so this slice is recorded as source-path
simplification rather than a wall-clock claim.
Test-hook merge fallback attribution now derives parent leaf page-id rank and
tail distance from one parent dirty-buffer scan. The benchmark output and
storage behavior stay the same, but prepared-insert counter builds avoid the
duplicate scan for the `21,031` `future-current-header-partial-leaf` fallback
rows. The current smoke profile keeps `8` full-page checksum calls, `227,063`
...
planned stores; the sampled storage-smoke prepared insert step was
`71.769 us/op`.
Dirty-buffer entries also carry cached index-leaf key size and used-byte
bounds so dirty-buffer publication can refresh checksums for valid cached index
leaves without rerunning the generic leaf metadata parser. The generic checksum
refresh remains the fallback for entries without valid cached leaf facts and
for non-leaf pages; append-buffer, copy-for-read, journal, durable-read, and
maintained-root validation paths are unchanged. The prepared-insert profile
reported `87,176` cached index-leaf refreshes and kept the same structural
counters (`8` full-page checksum calls, `127,063` zero-tail checksum calls,
...
facts after their existing single-insert or overflow-tail byte proof succeeds.
`store_dirty_page_in_buffer()` treats those entries as current and skips the
generic page-type refresh for those two maintained-root fast paths, while
non-fast root replacements, journal validation, recovery validation, durable
reads, planning protected-page validation, and checksum publication are
unchanged. The comparable prepared-insert profile reported `1,336`
...
checksum calls, `5` protected maintained-root decodes, `21,031` dirty leaf
pressure admissions, `66,144` merge direct writes, `87,176` index-leaf dirty
refreshes, `31,938` pressure-context builds, `19,053` planned stores,
`13,004` cached victim free-slot reads, `542,656` cached pressure page-type
probes, `34,548` cached leaf-growth fact publications, and `130,311` cached
branch fast replacement fact publications); the sampled storage-smoke prepared
insert step was `77.282 us/op`.
...
candidate leaf's shape and entry count. Selected-leaf branch insert planning
uses the same metadata probe before falling back to a full page read, avoiding
cached page copies when the planner only needs entry count and key shape.
