# Ownerless External Seed 576-607 Replay
...
`255`, and Docker-backed combined seed-sweep replay through seed `575` for the
...
`256` through `319`, and records Docker-backed MariaDB replay evidence for seeds
`576` through `607` at two rounds across the random transaction, DDL stress,
...
- The replay windows from `224` through `575` used
...
- Add a production CTest command-plan check for seeds `256` through `319` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `576` through `607` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `576`
  through `607` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `256:319` check-mode seed sweep at two rounds.
- Run the direct `576:607` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `576:607` replay artifacts for manifest, final
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-256-319-r2-check --random-tx-rounds 2
  --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 256:319 --check` passed and
  ended with `ownerless_external_mariadb_seed_sweep_check=ok`.
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-576-607-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 576:607 --check` passed and
  wrote a check-mode manifest for seeds `576` through `607`.
- Docker-backed replay artifacts under
  `/tmp/mylite-ownerless-external-seed-sweep-576-607-r2-attempt10-replay`
  ...
  `seed_count=32`, and seeds `576` through `607`.
...
- FK graph replay recovered ordinary raw MariaDB deadlock exits under the
  whole-seed retry budget: seeds `581`, `586`, and `605` passed attempt `4`;
  seeds `576`, `584`, `595`, `599`, and `604` passed attempt `3`; seeds `578`,
  `579`, `582`, `587`, `589`, `590`, and `606` passed attempt `2`. Failed
  attempts showed `ERROR 1213 (40001)` from raw MariaDB clients.
- `ctest --preset prod -R
  ...
  --output-on-failure` passed 6/6 tests in 3.98 seconds.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `256` through `319` at two rounds.
- The direct check-mode command validates seeds `576` through `607` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `576` through `607` at two
  rounds.
