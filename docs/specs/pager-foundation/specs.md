...
migrates typed storage page access for row pages, row-state pages,
autoincrement pages, BLOB payload pages, free-list pages, append-only
index-entry pages, and index leaf pages through it. The pager currently
delegates to the existing fixed-page read/write helpers, so file bytes, journal
selection, active append-buffer reads, and header-view semantics are unchanged.
