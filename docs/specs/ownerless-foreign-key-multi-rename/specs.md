...
- Cross-schema foreign-key multi-rename.
...
not claim complete FK rename graph coverage, cross-schema FK multi-rename, or
crash/error recovery inside the statement.
...
- This proves one same-schema multi-pair parent/child FK rename shape.
  Cross-schema FK multi-rename, generated-column foreign keys, cyclic/deep
  cascade chains, and crash/error injection during FK rename remain follow-up
  DDL/recovery work.
