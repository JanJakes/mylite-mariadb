...
- This is focused compressed BLOB pressure coverage, not a full compressed
  ...
  matrix. The bounded compressed payload-size matrix is covered separately by
  `ownerless-compressed-blob-page-size-matrix`, the bounded key-block matrix is
  covered separately by `ownerless-compressed-blob-key-block-matrix`, and
