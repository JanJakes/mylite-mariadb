# Ownerless External Seed 320-351 Replay
...
`63`, and Docker-backed combined seed-sweep replay through seed `319` for the
...
This slice records Docker-backed MariaDB replay for seeds `320` through `351`
...
- The previous replay windows from `224` through `319` used
...
- Run one dependency-free command-plan check for seeds `320` through `351` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Run one Docker-backed MariaDB 11.8 seed sweep for seeds `320` through `351`
...
- Run the direct `320:351` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Run the Docker-backed `320:351` replay at two rounds with
  `--fk-graph-replay-attempts 10`.
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-320-351-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 320:351 --check`
  passed and ended with `ownerless_external_mariadb_seed_sweep_check=ok`.
- `tools/ownerless-external-mariadb-seed-sweep --output
  /tmp/mylite-ownerless-external-seed-sweep-320-351-r2-attempt10-replay
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 320:351` passed through a
...
- FK graph replay recovered ordinary raw MariaDB deadlock exits under the
  whole-seed retry budget: seed `344` failed attempts `1` and `2` with
  `ERROR 1213 (40001)` and passed attempt `3`; seed `348` failed attempt `1`
  with `ERROR 1213 (40001)` and passed attempt `2`.

## Acceptance Criteria

- The direct check-mode command validates seeds `320` through `351` at two
...
- Docker-backed MariaDB 11.8 replay passes random transaction, DDL stress, and
  FK graph seeded suites for seeds `320` through `351` at two rounds.
