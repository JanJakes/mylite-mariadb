...
with ordinary non-transaction assignments. Later transaction-control slices add
duplicate supported session autocommit assignments where the final value wins.
Later transaction-variable slices accept supported transaction variable `SET`
forms, including duplicate supported assignments. Savepoints, global
autocommit changes, global transaction variables, transaction modifiers, XA,
and transactional DDL remain rejected.
...
coverage extends the same checkpoint evidence to representative direct,
prepared, and select-source failed `REPLACE` row-DML, failed OR REPLACE
replacement paths, and representative failed multi-table DROP/RENAME paths.
