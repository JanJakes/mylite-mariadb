...
inside the process. It is also the prerequisite for storage-engine registration
smoke tests that need to ask MariaDB what engines are visible.
...
- Do not claim durable SQL state in the `.mylite` file.
- Do not enable existing durable MariaDB engines as application storage.
...
until MyLite catalog and storage exist. Any MariaDB runtime files remain
bootstrap debt and must be removed on final close.
