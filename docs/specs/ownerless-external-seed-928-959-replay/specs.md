# Ownerless External Seed 928-959 Replay
...
`927`, and Docker-backed combined seed-sweep replay through seed `927` for the
...
`928` through `959`, and records Docker-backed MariaDB replay evidence for the
...
- The replay windows from `224` through `927` used
...
- Add a production CTest command-plan check for seeds `928` through `959` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `928` through `959` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `928`
  through `959` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `928:959` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `928:959` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-928-959-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 928:959 --check` passed with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed replay through
  `/tmp/mylite-ownerless-external-seed-sweep-928-959-r2-attempt10-replay`
...
- FK graph whole-seed retries recovered transient raw MariaDB `1213` exits for
  seed `957` on attempt `4`, seed `939` on attempt `3`, and seeds `932`,
  `934`, `946`, and `956` on attempt `2`.

## Acceptance Criteria

- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `928` through `959` at two rounds.
- The direct check-mode command validates seeds `928` through `959` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `928` through `959` at two
  rounds.
