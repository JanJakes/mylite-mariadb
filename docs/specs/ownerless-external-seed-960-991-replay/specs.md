# Ownerless External Seed 960-991 Replay
...
`959`, and Docker-backed combined seed-sweep replay through seed `959` for the
...
`960` through `991`, and records Docker-backed MariaDB replay evidence for the
...
- The replay windows from `224` through `959` used
...
- Add a production CTest command-plan check for seeds `960` through `991` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `960` through `991` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `960`
  through `991` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `960:991` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `960:991` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-960-991-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 960:991 --check` passed with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- `ctest --preset prod -R
  '^tools\.ownerless-external-mariadb-seed-sweep-next-check$'
  --output-on-failure` passed.
- Docker-backed replay through
  `/tmp/mylite-ownerless-external-seed-sweep-960-991-r2-attempt10-replay`
...
- FK graph whole-seed retries recovered transient raw MariaDB `1213` exits for
  seed `968` on attempt `5`, seeds `963`, `969`, and `985` on attempt `4`,
  seeds `965`, `972`, `976`, `979`, and `984` on attempt `3`, and seeds
  `975`, `978`, `981`, and `982` on attempt `2`.

## Acceptance Criteria

- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `960` through `991` at two rounds.
- The direct check-mode command validates seeds `960` through `991` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `960` through `991` at two
  rounds.
