# Ownerless External Seed 1312-1343 Replay
...
`1311`, and Docker-backed combined seed-sweep replay through seed `1311` for
...
`1312` through `1343`, and records Docker-backed MariaDB replay evidence for
...
- Replay windows from `224` through `1311` used
...
- Add a production CTest command-plan check for seeds `1312` through `1343` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `1312` through `1343`
  at two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `1312`
  through `1343` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `1312:1343` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `1312:1343` replay artifacts for manifest,
...
- Direct command-plan check passed for `1312:1343` at rounds `2` with
  `--fk-graph-replay-attempts 10`, ending with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed replay passed for `1312:1343` at rounds `2` with
  `--fk-graph-replay-attempts 10`, ending with
...
- FK graph retry inspection found transient raw MariaDB `1213` exits for seed
  `1316` recovered on attempt `2`, seed `1322` recovered on attempt `3`, seed
  `1323` recovered on attempt `2`, seed `1326` recovered on attempt `2`, seed
  `1330` recovered on attempt `2`, seed `1332` recovered on attempt `3`, and
  seed `1338` recovered on attempt `4`.
- The previous pushed `Cover row-undo live-peer crashes` GitHub Actions run
  completed successfully while this slice was in progress.

## Acceptance Criteria

- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `1312` through `1343` at two rounds.
- The direct check-mode command validates seeds `1312` through `1343` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `1312` through `1343` at two
  rounds.
