# Ownerless External Seed 768-799 Replay
...
`767`, and Docker-backed combined seed-sweep replay through seed `767` for the
...
`768` through `799`, and records Docker-backed MariaDB replay evidence for the
same seeds at two rounds across the random transaction, DDL stress, and
...
- The replay windows from `224` through `767` used
...
- Add a production CTest command-plan check for seeds `768` through `799` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `768` through `799` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `768`
  through `799` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `768:799` check-mode seed sweep at two rounds.
- Run the direct `768:799` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `768:799` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-768-799-r2-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --seed-range 768:799 --check` passed with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-768-799-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 768:799 --check` passed with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed MariaDB 11.8 replay passed:
  `tools/ownerless-external-mariadb-seed-sweep --output
  /tmp/mylite-ownerless-external-seed-sweep-768-799-r2-attempt10-replay
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 768:799`.
- The replay manifest records `image=mariadb:11.8`, `mode=replay`,
  `random_tx_rounds=2`, `ddl_rounds=2`, `fk_graph_rounds=2`,
  `suite_fk_graph_replay_attempts=10`, and seeds `768` through `799`.
- The replay produced `96` `expected.out` files and `96` `expected.err`
  files. All `expected.out` files contained `ok`, and all `expected.err`
  files were empty.
- FK graph whole-seed retries recovered transient raw MariaDB `1213` exits for
  seed `778` on attempt `4`, seeds `774`, `780`, and `788` on attempt `3`,
  and seeds `769`, `777`, `785`, `787`, and `794` on attempt `2`.
- `cmake --preset prod && ctest --preset prod -R
  ...
  --output-on-failure` passed 6/6 tests in 7.66 seconds.

## Acceptance Criteria

- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `768` through `799` at two rounds.
- The direct check-mode command validates seeds `768` through `799` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `768` through `799` at two
  rounds.
