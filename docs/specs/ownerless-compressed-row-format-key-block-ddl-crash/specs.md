...
refreshes after another ownerless process rebuilds InnoDB tables with
`ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=4` and
`ROW_FORMAT=COMPRESSED KEY_BLOCK_SIZE=16`, and verifies 4 KiB plus 16 KiB
compressed BLOB page evidence after reopen. The crash matrix needs matching
kill-at-boundary coverage after MariaDB/InnoDB completes those native
compressed rebuilds but before MyLite publishes ownerless dictionary finish.

This slice adds crash-boundary evidence for those focused key-block rebuild
paths.
...
- `mariadb/storage/innobase/handler/ha_innodb.cc:11257` through
  ...
  key-block values including `4` and `16` and validates file-per-table
  requirements.
...
- initialize an ownerless database and create
  `app.ownerless_compressed_row_format_kb4` or
  `app.ownerless_compressed_row_format_kb16` as an InnoDB table with
  `ROW_FORMAT=DYNAMIC`,
...
- verify retained prepared BLOB rows and aggregate values, then insert another
  prepared BLOB row,
- verify the final compressed metadata, row data, and native ZBLOB page
  evidence at the requested key-block size survive ownerless reopen, native
...
- Run the 16 KiB focused selector:
  `build/ownerless-test-hooks/packages/libmylite/mylite_ownerless_cross_process_sql_test dictionary-compressed-row-format-key-block-16-crash`.
...
- Native 4 KiB and 16 KiB compressed ZBLOB page evidence remains present after
  recovery.
