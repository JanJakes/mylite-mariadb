# Ownerless External Seed 1152-1183 Replay
...
`1151`, and Docker-backed combined seed-sweep replay through seed `1151` for
...
`1152` through `1183`, and records Docker-backed MariaDB replay evidence for
...
- Replay windows from `224` through `1151` used
...
- Add a production CTest command-plan check for seeds `1152` through `1183` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `1152` through `1183`
  at two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `1152`
  through `1183` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `1152:1183` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `1152:1183` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-1152-1183-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 1152:1183 --check` passed with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed replay through
  `/tmp/mylite-ownerless-external-seed-sweep-1152-1183-r2-attempt10-replay`
...
- FK graph whole-seed retries recovered transient raw MariaDB `1213` exits for
  seeds `1152`, `1155`, and `1157` on attempt `4`, seeds `1156`, `1173`, and
  `1183` on attempt `3`, plus seeds `1153`, `1159`, `1167`, and `1170` on
  attempt `2`.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `1152` through `1183` at two rounds.
- The direct check-mode command validates seeds `1152` through `1183` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `1152` through `1183` at two
  rounds.
- The combined wrapper manifest records the FK graph replay-attempt budget.
- All `expected.out` oracle outputs contain `ok`, and every `expected.err` file
  is empty.
...
- The attempted explicit-transaction DML file-op marker slice showed that
  consuming native file-op evidence at transaction end can destabilize
  `commit-race`; that path needs a separate design before implementation.
