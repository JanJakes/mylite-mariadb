# Ownerless External Seed 1024-1055 Replay
...
`1023`, and Docker-backed combined seed-sweep replay through seed `1023` for
...
`1024` through `1055`, and records Docker-backed MariaDB replay evidence for
...
- Replay windows from `224` through `1023` used
...
- Add a production CTest command-plan check for seeds `1024` through `1055` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `1024` through `1055`
  at two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `1024`
  through `1055` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `1024:1055` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `1024:1055` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-1024-1055-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 1024:1055 --check` passed with
...
- Docker-backed replay through
  `/tmp/mylite-ownerless-external-seed-sweep-1024-1055-r2-attempt10-replay`
...
- FK graph whole-seed retries recovered transient raw MariaDB `1213` exits for
  seed `1026` on attempt `3`, seeds `1029`, `1031`, `1042`, `1043`, `1053`,
  and `1055` on attempt `2`, seed `1052` on attempt `7`, and seed `1054` on
  attempt `3`.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `1024` through `1055` at two rounds.
- The direct check-mode command validates seeds `1024` through `1055` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `1024` through `1055` at two
  rounds.
