# Ownerless External Seed 32-63 Replay
...
seed `31`, Docker-backed replay through seed `31`, and focused full-suite
trace replay. The remaining external-oracle gap is still broader randomized
...
This slice records Docker-backed MariaDB replay for seeds `32` through `63` at
...
- Run one Docker-backed MariaDB 11.8 seed sweep for seeds `32` through `63` at
  two rounds.
...
- Adding a new SQL generator, RQG runner, SQLancer runner, shrinker, or default
  Docker dependency.
- Adding a new default CTest entry for a replay command that depends on Docker.
...
tools/ownerless-external-mariadb-seed-sweep \
  --output /tmp/mylite-ownerless-external-seed-sweep-32-63-r2-replay \
  --random-tx-rounds 2 \
  --ddl-rounds 2 \
  --fk-graph-rounds 2 \
  --seed-range 32:63
...
- Run the direct `32:63` check-mode seed sweep at two rounds.
- Run the Docker-backed `32:63` replay at two rounds.
...
- The direct check-mode command validates seeds `32` through `63` at two rounds
  for all three suites.
- Docker-backed MariaDB 11.8 replay passes random transaction, DDL stress, and
  FK graph seeded suites for seeds `32` through `63` at two rounds.
...
```text
tools/ownerless-external-mariadb-seed-sweep --output build/manual-external-seed-sweep-32-63-r2-check --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 32:63 --check
seed=32
...
seed=63
check=ok
...
```text
tools/ownerless-external-mariadb-seed-sweep --output /tmp/mylite-ownerless-external-seed-sweep-32-63-r2-replay --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 32:63
...
`fk_graph_rounds=2`, `seed_count=32`, and seeds `32` through `63`. All
random transaction, DDL stress, and FK graph final `expected.out` files ended
in `ok`, and all final `expected.err` files were empty. FK graph seeds `34`,
`35`, `36`, and `37` recovered on attempt `2` after raw external MariaDB
`1213` deadlock exits, matching the suite's bounded whole-seed retry model.
