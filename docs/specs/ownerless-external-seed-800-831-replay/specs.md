# Ownerless External Seed 800-831 Replay
...
`799`, and Docker-backed combined seed-sweep replay through seed `799` for the
...
`800` through `831`, and records Docker-backed MariaDB replay evidence for the
...
- The replay windows from `224` through `799` used
...
- Add a production CTest command-plan check for seeds `800` through `831` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `800` through `831` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `800`
  through `831` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `800:831` check-mode seed sweep at two rounds.
- Run the direct `800:831` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `800:831` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-800-831-r2-check --random-tx-rounds 2
  --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 800:831 --check` passed
  with `ownerless_external_mariadb_seed_sweep_check=ok`.
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-800-831-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 800:831 --check` passed with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed replay through `/tmp/mylite-ownerless-external-seed-sweep-800-831-r2-attempt10-replay`
  recorded `image=mariadb:11.8`, `mode=replay`, `seed_count=32`, per-suite
  rounds `2`, and `suite_fk_graph_replay_attempts=10`.
- Replay artifact inspection found `96` `expected.out` oracle files and `96`
  `expected.err` files. Every `expected.out` contained `ok`, and every
  `expected.err` was empty.
- FK graph whole-seed retries recovered transient raw MariaDB `1213` exits for
  seeds `801` and `803` on attempt `4`, seeds `811` and `829` on attempt `3`,
  and seeds `802`, `806`, `823`, `826`, and `830` on attempt `2`.
...
- `ctest --preset prod -R
  ...
  --output-on-failure` passed 6/6 in 8.96 seconds.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `800` through `831` at two rounds.
- The direct check-mode command validates seeds `800` through `831` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `800` through `831` at two
  rounds.
