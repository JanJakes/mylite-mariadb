# Ownerless External Seed 608-639 Replay
...
`319`, and Docker-backed combined seed-sweep replay through seed `607` for the
...
`320` through `383`, and records Docker-backed MariaDB replay evidence for seeds
`608` through `639` at two rounds across the random transaction, DDL stress,
...
- The replay windows from `224` through `607` used
...
- Add a production CTest command-plan check for seeds `320` through `383` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `608` through `639` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `608`
  through `639` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `320:383` check-mode seed sweep at two rounds.
- Run the direct `608:639` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `608:639` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-320-383-r2-check --random-tx-rounds 2
  --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 320:383 --check` passed and
  ended with `ownerless_external_mariadb_seed_sweep_check=ok`.
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-608-639-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 608:639 --check` passed and
  wrote a check-mode manifest for seeds `608` through `639`.
- Docker-backed replay artifacts under
  `/tmp/mylite-ownerless-external-seed-sweep-608-639-r2-attempt10-replay`
  ...
  `seed_count=32`, and seeds `608` through `639`.
...
- FK graph replay recovered ordinary raw MariaDB deadlock exits under the
  whole-seed retry budget: seeds `609`, `613`, and `621` passed attempt `3`;
  seeds `608`, `616`, `618`, `626`, `633`, and `634` passed attempt `2`.
  Failed attempts showed `ERROR 1213 (40001)` from raw MariaDB clients.
...
- `ctest --preset prod -R
  ...
  --output-on-failure` passed 6/6 tests in 5.60 seconds.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `320` through `383` at two rounds.
- The direct check-mode command validates seeds `608` through `639` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `608` through `639` at two
  rounds.
