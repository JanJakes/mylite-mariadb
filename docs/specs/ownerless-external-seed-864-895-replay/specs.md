# Ownerless External Seed 864-895 Replay
...
`863`, and Docker-backed combined seed-sweep replay through seed `863` for the
...
`864` through `895`, and records Docker-backed MariaDB replay evidence for the
...
- The replay windows from `224` through `863` used
...
- Add a production CTest command-plan check for seeds `864` through `895` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `864` through `895` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `864`
  through `895` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `864:895` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `864:895` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-864-895-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 864:895 --check` passed with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed replay through
  `/tmp/mylite-ownerless-external-seed-sweep-864-895-r2-attempt10-replay`
...
- FK graph whole-seed retries recovered transient raw MariaDB `1213` exits for
  seed `887` on attempt `5`, seed `872` on attempt `3`, and seeds `865`,
  `870`, `878`, `879`, `880`, `886`, and `891` on attempt `2`.

## Acceptance Criteria

- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `864` through `895` at two rounds.
- The direct check-mode command validates seeds `864` through `895` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `864` through `895` at two
  rounds.
