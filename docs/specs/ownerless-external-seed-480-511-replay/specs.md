# Ownerless External Seed 480-511 Replay
...
`63`, and Docker-backed combined seed-sweep replay through seed `479` for the
...
`64` through `127`, and records Docker-backed MariaDB replay evidence for seeds
`480` through `511` at two rounds across the random transaction, DDL stress, and
...
- The replay windows from `224` through `479` used
...
- Add a production CTest command-plan check for seeds `64` through `127` at two
  rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `480` through `511` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `480`
  through `511` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `64:127` check-mode seed sweep at two rounds.
- Run the direct `480:511` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `480:511` replay artifacts for manifest, final
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-64-127-r2-check --random-tx-rounds 2
  --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 64:127 --check` passed and
  ended with `ownerless_external_mariadb_seed_sweep_check=ok`.
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-480-511-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 480:511 --check` passed and
  wrote a check-mode manifest for seeds `480` through `511`.
- Docker-backed replay artifacts under
  `/tmp/mylite-ownerless-external-seed-sweep-480-511-r2-attempt10-replay`
...
- FK graph replay recovered ordinary raw MariaDB deadlock exits under the
  whole-seed retry budget: seed `481` passed attempt `2`; seed `483` passed
  attempt `3`; seed `486` passed attempt `2`; seed `490` passed attempt `5`;
  seed `492` passed attempt `2`; seed `493` passed attempt `4`; seed `500`
  passed attempt `4`; seed `503` passed attempt `2`; seed `505` passed attempt
  `3`; seeds `507`, `509`, and `510` passed attempt `2`; and seed `511` passed
  attempt `3`. Failed attempts showed `ERROR 1213 (40001)` from raw MariaDB
  clients.
- `ctest --preset prod -R
  ...
  --output-on-failure` passed 6/6 tests in 3.54 seconds.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `64` through `127` at two rounds.
- The direct check-mode command validates seeds `480` through `511` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `480` through `511` at two
  rounds.
