...
- crash coverage for DROP CHECK, field-level CHECK constraints,
  generated-column CHECK expressions, partitioned tables,
  `check_constraint_checks=OFF`, and concurrent CHECK DDL conflicts,
...
- This is deterministic CHECK ADD crash coverage, not the full CHECK DDL crash
  matrix.
- DROP CHECK, field-level CHECK, generated-expression CHECK, and randomized
  CHECK DDL crash variants remain separate candidate slices.
