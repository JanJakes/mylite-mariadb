# Ownerless External Seed 672-703 Replay
...
`447`, and Docker-backed combined seed-sweep replay through seed `671` for the
...
`448` through `511`, and records Docker-backed MariaDB replay evidence for seeds
`672` through `703` at two rounds across the random transaction, DDL stress,
...
- The replay windows from `224` through `671` used
...
- Add a production CTest command-plan check for seeds `448` through `511` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `672` through `703` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `672`
  through `703` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `448:511` check-mode seed sweep at two rounds.
- Run the direct `672:703` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `672:703` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-448-511-r2-check --random-tx-rounds 2
  --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 448:511 --check` passed and
  ended with `ownerless_external_mariadb_seed_sweep_check=ok`.
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-672-703-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 672:703 --check` passed and
  wrote a check-mode manifest for seeds `672` through `703`.
- Docker-backed replay artifacts under
  `/tmp/mylite-ownerless-external-seed-sweep-672-703-r2-attempt10-replay`
  ...
  `seed_count=32`, and seeds `672` through `703`.
...
- FK graph replay recovered ordinary raw MariaDB deadlock exits under the
  whole-seed retry budget: seed `701` passed attempt `6`, seed `689` passed
  attempt `5`, seed `678` passed attempt `4`, seeds `693`, `698`, `699`,
  `700`, and `702` passed attempt `3`, and seeds `672`, `692`, `694`, and
  `703` passed attempt `2`. Failed attempts showed `ERROR 1213 (40001)` from
  raw MariaDB clients.
...
- `ctest --preset prod -R
  ...
  --output-on-failure` passed 6/6 tests in 6.48 seconds.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `448` through `511` at two rounds.
- The direct check-mode command validates seeds `672` through `703` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `672` through `703` at two
  rounds.
