# Ownerless External Seed 1120-1151 Replay
...
`1119`, and Docker-backed combined seed-sweep replay through seed `1119` for
...
`1120` through `1151`, and records Docker-backed MariaDB replay evidence for
...
- Replay windows from `224` through `1119` used
...
- Add a production CTest command-plan check for seeds `1120` through `1151` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `1120` through `1151`
  at two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `1120`
  through `1151` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `1120:1151` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `1120:1151` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-1120-1151-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 1120:1151 --check` passed with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed replay through
  `/tmp/mylite-ownerless-external-seed-sweep-1120-1151-r2-attempt10-replay`
...
- FK graph whole-seed retries recovered transient raw MariaDB `1213` exits for
  seeds `1120`, `1121`, and `1130` on attempt `3`, plus seeds `1123`, `1128`,
  `1133`, `1136`, `1150`, and `1151` on attempt `2`.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `1120` through `1151` at two rounds.
- The direct check-mode command validates seeds `1120` through `1151` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `1120` through `1151` at two
  rounds.
