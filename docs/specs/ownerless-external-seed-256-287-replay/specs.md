# Ownerless External Seed 256-287 Replay
...
`63`, Docker-backed combined seed-sweep replay through seed `255`, and focused
...
This slice records Docker-backed MariaDB replay for seeds `256` through `287`
...
- Run one dependency-free command-plan check for seeds `256` through `287` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Run one Docker-backed MariaDB 11.8 seed sweep for seeds `256` through `287`
...
- Run the direct `256:287` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Run the Docker-backed `256:287` replay at two rounds with
  `--fk-graph-replay-attempts 10`.
...
```text
tools/ownerless-external-mariadb-seed-sweep --output build/manual-external-seed-sweep-256-287-r2-attempt10-check --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2 --fk-graph-replay-attempts 10 --seed-range 256:287 --check
seed=256
...
seed=287
check=ok
...
Docker-backed MariaDB 11.8 replay passed the same `256` through `287` window:

```text
tools/ownerless-external-mariadb-seed-sweep --output /tmp/mylite-ownerless-external-seed-sweep-256-287-r2-attempt10-replay --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2 --fk-graph-replay-attempts 10 --seed-range 256:287
...
and seeds `256` through `287`. All `96` random transaction, DDL stress, and
FK graph final `expected.out` files ended in `ok`, and all `96` final
`expected.err` files were empty. FK graph seeds `260`, `267`, `277`, and `280`
recovered after raw external MariaDB `1213` / SQLSTATE `40001` deadlock exits;
final attempts were `4`, `2`, `2`, and `2`, respectively.

## Acceptance Criteria

- The direct check-mode command validates seeds `256` through `287` at two
...
- Docker-backed MariaDB 11.8 replay passes random transaction, DDL stress, and
  FK graph seeded suites for seeds `256` through `287` at two rounds.
