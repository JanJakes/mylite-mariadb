# Ownerless External Seed 1344-1375 Replay
...
`1343`, and Docker-backed combined seed-sweep replay through seed `1343` for
...
`1344` through `1375`, and records Docker-backed MariaDB replay evidence for
...
- Replay windows from `224` through `1343` used
...
- Add a production CTest command-plan check for seeds `1344` through `1375` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `1344` through `1375`
  at two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `1344`
  through `1375` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `1344:1375` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `1344:1375` replay artifacts for manifest,
...
- Direct command-plan check passed for `1344:1375` at rounds `2` with
  `--fk-graph-replay-attempts 10`, ending with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed replay passed for `1344:1375` at rounds `2` with
  `--fk-graph-replay-attempts 10`, ending with
...
- FK graph retry inspection found transient raw MariaDB `1213` exits for seeds
  `1344`, `1345`, `1346`, `1350`, `1353`, `1354`, `1359`, `1360`, `1361`,
  `1366`, and `1374` recovered on attempt `2`; seeds `1347`, `1356`, `1370`,
  and `1372` recovered on attempt `3`; and seed `1373` recovered on attempt
  `4`.
- The previous pushed `Harden ownerless pressure and SQL timing` GitHub
  Actions run completed successfully before this slice started.

## Acceptance Criteria

- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `1344` through `1375` at two rounds.
- The direct check-mode command validates seeds `1344` through `1375` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `1344` through `1375` at two
  rounds.
