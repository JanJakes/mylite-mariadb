...
- Do not claim exhaustive affected-row behavior for all SQL modes, duplicate
  modes, or engine-specific edge cases.
...
insert/update/delete effects against the MariaDB embedded baseline. This
strengthens the SQL execution API compatibility claim without claiming broad
MTR-scale coverage.
...
3. Run direct multi-row `INSERT`, `UPDATE`, and `DELETE`.
4. Run prepared `INSERT`, `UPDATE`, and `DELETE`.
...
1. Extend `mylite_embedded_comparison_test` with direct and prepared statement
   effect observations.
2. Run the `sql-comparison` compatibility harness group.
3. Run embedded preset tests, format checks, tidy, shell checks, and
   `git diff --check`.
