# Ownerless External Seed 128-159 Replay
...
`31`, Docker-backed combined seed-sweep replay through seed `127`, and focused
...
This slice records Docker-backed MariaDB replay for seeds `128` through `159`
...
- Run one dependency-free command-plan check for seeds `128` through `159` at
  two rounds.
- Run one Docker-backed MariaDB 11.8 seed sweep for seeds `128` through `159`
  at two rounds.
...
tools/ownerless-external-mariadb-seed-sweep \
  --output /tmp/mylite-ownerless-external-seed-sweep-128-159-r2-replay \
  --random-tx-rounds 2 \
  --ddl-rounds 2 \
  --fk-graph-rounds 2 \
  --seed-range 128:159
...
- Run the direct `128:159` check-mode seed sweep at two rounds.
- Run the Docker-backed `128:159` replay at two rounds.
...
```text
tools/ownerless-external-mariadb-seed-sweep --output build/manual-external-seed-sweep-128-159-r2-check --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 128:159 --check
seed=128
...
seed=159
check=ok
...
```text
tools/ownerless-external-mariadb-seed-sweep --output /tmp/mylite-ownerless-external-seed-sweep-128-159-r2-replay --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 128:159
...
`fk_graph_rounds=2`, `seed_count=32`, and seeds `128` through `159`. All
...
seeds `129` and `132` recovered on attempt `2` after raw external MariaDB
`1213` deadlock exits, matching the suite's bounded whole-seed retry model.

## Acceptance Criteria

- The direct check-mode command validates seeds `128` through `159` at two
  rounds for all three suites.
- Docker-backed MariaDB 11.8 replay passes random transaction, DDL stress, and
  FK graph seeded suites for seeds `128` through `159` at two rounds.
