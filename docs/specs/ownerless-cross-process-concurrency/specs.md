...
1. Coordinate table ID and space ID allocation across processes.
   ...
   but before ownerless dictionary finish, then verifies metadata-only live
   recovery with the native file-operation marker clear after pre-execution
   metadata proves `PRIMARY` already exists, recovered `PRIMARY(id)`
   preservation, `PRIMARY(code)` absence, duplicate-id rejection,
   duplicate-code allowance, ownerless/native reopen, and forced `.shm`
   rebuild. Descending
   ...
   after pre-execution metadata proves the source column is absent; all paths
   verify original column/default preservation, missing and attempted
   renamed/changed column absence, plain retry errno 1054, post-recovery
   writes, ownerless/native reopen, and forced `.shm` rebuild. Focused
   ...
   verifies peer-visible source absence
   ...
   before ownerless dictionary finish, then recovers them while a peer remains
   live, keeps the native file-operation marker clear, and verifies recovered
   inner/outer metadata, nested valid DML, errno 1369 for invalid inner or
   outer predicate violations, base-table writes, and ownerless/native reopen
   before and after forced `.shm` rebuild.
   ...
   before ownerless dictionary finish, then verifies metadata-only live recovery
   with the native file-operation marker clear, the original primary key remains
   enforced, and the attempted candidate key remains non-unique through
   ownerless/native reopen before and after forced `.shm` rebuild.
   ...
   FK add/drop lists, FK-only mixed drop/add lists, and a focused mixed
   FK/non-FK ADD COLUMN list,
...
5. Add broad DDL compatibility tests.
   ...
   with live-peer marker-clear recovery, recovered default metadata,
   post-recovery InnoDB table writes, ownerless/native reopen, and forced
   `.shm` rebuild checks.
   Hook-build schema-alter crash coverage adds a killed
   `ALTER DATABASE ... DEFAULT CHARACTER SET/COLLATE` writer after native
   `db.opt` rewrite but before ownerless dictionary finish, with live-peer
   marker-clear recovery, recovered default metadata, pre-alter table collation
   preservation, post-recovery default inheritance, ownerless/native reopen,
   ...
   finish and verifies metadata-only live recovery with the native
   file-operation marker clear, the recovered no-op preserves `PRIMARY(id)`,
   and `code` remains non-unique through ownerless/native reopen before and
   after forced `.shm` rebuild. A
   ...
   after pre-execution metadata proves the source column is absent; all paths
   verify original column/default preservation, missing and attempted
   renamed/changed column absence, MariaDB 1054 retry errno, post-recovery
   writes, ownerless/native reopen, and forced `.shm` rebuild. Focused
   expression
...
6. Extend the current bounded multi-object reader/writer stress into
   ...
   exits.
   The `ownerless-external-seed-1280-1311-replay` follow-up advances the next
   dependency-free seed-sweep CTest window to seeds `1280` through `1311` and
   records Docker-backed replay for the same window at rounds `2`, using the
   same explicit FK graph whole-seed retry budget of `10` so seeds `1280` and
   `1285` can recover on attempt `2`, and seeds `1286` and `1299` can recover
   on attempt `3` after transient raw MariaDB `1213` deadlock exits.
   The `ownerless-external-seed-1312-1343-replay` follow-up advances the next
   dependency-free seed-sweep CTest window to seeds `1312` through `1343` and
   records Docker-backed replay for the same window at rounds `2`, using the
   same explicit FK graph whole-seed retry budget of `10` so seeds `1316`,
   `1323`, `1326`, and `1330` can recover on attempt `2`, seeds `1322` and
   `1332` can recover on attempt `3`, and seed `1338` can recover on attempt
   ...
   for key-block sizes `1`, `2`, `4`, `8`, and `16`.
   ...
   seed-sweep replay windows through seed `1343`; the `16` through `31`,
   ...
   `1248` through `1279`, `1280` through `1311`, and `1312` through `1343`
   replays also cover FK graph seeds at rounds `2`. FK
   graph now
   participates in the
   dependency-free combined seed-sweep command-plan windows through seed `1343`,
   ...
   `1280` through `1311`, and `1312` through `1343`
...
- crash/fault injection:
  ...
  - after missing `ALTER TABLE ... MODIFY COLUMN IF EXISTS`,
    ...
    recovery with the native file-operation marker clear after pre-execution
    metadata proves the source column is absent, and preserved original
...
metadata mutation, plus a focused mixed FK/non-FK named CHECK drop/add list
...
- Long readers can starve checkpoint progress, as in SQLite WAL. A configured
  ...
  The FK/trigger row-undo follow-up repeats the full-transaction boundary on
  FK `ON UPDATE CASCADE` child-row changes and trigger-written audit rows,
  ...
  retention/drain, forced `.shm` rebuild, and ordinary native reopen.
  The cross-schema foreign-key missing-source IF EXISTS follow-up interleaves
  the same three skipped missing sources around real parent/child moves from
  `app` into a target schema. It proves all skipped-source warnings, completed
  target-schema FK metadata and enforcement at `dictionary-before-finish`,
  native-loop rollback to the original `app` FK state after the second real
  native rename hook, skipped-target absence from SQL metadata, InnoDB
  dictionary metadata, and source/target schema native files, marker
  retention/drain, forced `.shm` rebuild, and ordinary native reopen.
  ...
  stays clear.
  The nested check-option follow-up promotes the outer replacement and inner
  ...
  marker stays clear.
  The view algorithm follow-up promotes focused
  `ALTER ALGORITHM=TEMPTABLE VIEW`, `CREATE ALGORITHM=TEMPTABLE VIEW`, and
  `CREATE OR REPLACE ALGORITHM=TEMPTABLE VIEW` crash selectors to the same
  lane, proving persisted `INFORMATION_SCHEMA.VIEWS.ALGORITHM` metadata while
  a peer remains live and the native file-operation marker stays clear.
