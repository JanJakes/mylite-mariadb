# Ownerless External Seed 992-1023 Replay
...
`991`, and Docker-backed combined seed-sweep replay through seed `991` for the
...
`992` through `1023`, and records Docker-backed MariaDB replay evidence for the
...
- The replay windows from `224` through `991` used
...
- Add a production CTest command-plan check for seeds `992` through `1023` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `992` through `1023` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `992`
  through `1023` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `992:1023` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `992:1023` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-992-1023-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 992:1023 --check` passed with
...
- Docker-backed replay through
  `/tmp/mylite-ownerless-external-seed-sweep-992-1023-r2-attempt10-replay`
...
- FK graph whole-seed retries recovered transient raw MariaDB `1213` exits for
  seeds `992`, `993`, `994`, `997`, `999`, `1004`, `1011`, `1015`, `1020`,
  and `1022` on attempt `2`; no seed needed a third attempt.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `992` through `1023` at two rounds.
- The direct check-mode command validates seeds `992` through `1023` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `992` through `1023` at two
  rounds.
