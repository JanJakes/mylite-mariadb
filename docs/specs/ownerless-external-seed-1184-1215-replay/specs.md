# Ownerless External Seed 1184-1215 Replay
...
`1183`, and Docker-backed combined seed-sweep replay through seed `1183` for
...
`1184` through `1215`, and records Docker-backed MariaDB replay evidence for
...
- Replay windows from `224` through `1183` used
...
- Add a production CTest command-plan check for seeds `1184` through `1215` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `1184` through `1215`
  at two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `1184`
  through `1215` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `1184:1215` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `1184:1215` replay artifacts for manifest,
...
- Direct command-plan check passed for `1184:1215` at rounds `2` with
  `--fk-graph-replay-attempts 10`, ending with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed replay passed for `1184:1215` at rounds `2` with
  `--fk-graph-replay-attempts 10`, ending with
...
- FK graph retry inspection found transient raw MariaDB `1213` exits for seeds
  `1185` and `1203`; both recovered on attempt `2`.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `1184` through `1215` at two rounds.
- The direct check-mode command validates seeds `1184` through `1215` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `1184` through `1215` at two
  rounds.
