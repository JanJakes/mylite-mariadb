# Ownerless External Seed 896-927 Replay
...
`895`, and Docker-backed combined seed-sweep replay through seed `895` for the
...
`896` through `927`, and records Docker-backed MariaDB replay evidence for the
...
- The replay windows from `224` through `895` used
...
- Add a production CTest command-plan check for seeds `896` through `927` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `896` through `927` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `896`
  through `927` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `896:927` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `896:927` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-896-927-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 896:927 --check` passed with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed replay through
  `/tmp/mylite-ownerless-external-seed-sweep-896-927-r2-attempt10-replay`
...
- FK graph whole-seed retries recovered transient raw MariaDB `1213` exits for
  seed `905` on attempt `5`, seeds `904`, `910`, and `918` on attempt `3`, and
  seeds `899` and `908` on attempt `2`.

## Acceptance Criteria

- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `896` through `927` at two rounds.
- The direct check-mode command validates seeds `896` through `927` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `896` through `927` at two
  rounds.
