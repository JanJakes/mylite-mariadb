...
- Charset-width edge cases, full algorithm/lock option matrix, crash recovery
  during unique prefix-direction index DDL, and external randomized DDL
  oracles. Unique TEXT/BLOB prefix-plus-direction indexes are covered
  separately by
...
prefix-plus-direction secondary-index option. It does not claim charset-width,
online-option matrix, crash-recovery, or external-oracle coverage for unique
prefix-plus-direction indexes. Unique TEXT/BLOB prefix-plus-direction indexes
are covered by a separate focused slice.
...
- Charset-width edge cases, algorithm/lock matrices, and crash recovery during
  index DDL remain planned. Unique TEXT/BLOB prefix-plus-direction indexes are
