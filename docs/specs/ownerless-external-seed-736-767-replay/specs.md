# Ownerless External Seed 736-767 Replay
...
`575`, and Docker-backed combined seed-sweep replay through seed `735` for the
...
`576` through `639`, and records Docker-backed MariaDB replay evidence for seeds
`736` through `767` at two rounds across the random transaction, DDL stress,
...
- The replay windows from `224` through `735` used
...
- Add a production CTest command-plan check for seeds `576` through `639` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `736` through `767` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `736`
  through `767` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `576:639` check-mode seed sweep at two rounds.
- Run the direct `736:767` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `736:767` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-576-639-r2-check --random-tx-rounds 2
  --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 576:639 --check` passed and
  ended with `ownerless_external_mariadb_seed_sweep_check=ok`.
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-736-767-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 736:767 --check` passed and
  ended with `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed replay artifacts under
  `/tmp/mylite-ownerless-external-seed-sweep-736-767-r2-attempt10-replay`
  ...
  `seed_count=32`, and seeds `736` through `767`.
...
- FK graph replay recovered ordinary raw MariaDB deadlock exits under the
  whole-seed retry budget: seeds `737`, `738`, `755`, and `764` passed attempt
  `4`, seeds `742`, `745`, and `747` passed attempt `3`, and seeds `736`,
  `744`, `746`, `750`, `757`, and `765` passed attempt `2`. Failed attempts
  showed `ERROR 1213 (40001)` from raw MariaDB clients.
- `cmake --preset php-embedded-prod` and `cmake --preset prod` refreshed the
  production CTest build trees.
- `ctest --preset php-embedded-prod -R
  ...
  --output-on-failure` passed 6/6 tests in 3.74 seconds.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `576` through `639` at two rounds.
- The direct check-mode command validates seeds `736` through `767` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `736` through `767` at two
  rounds.
