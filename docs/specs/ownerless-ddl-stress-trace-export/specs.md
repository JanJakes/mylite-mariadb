...
`tools/ownerless-ddl-stress-trace` accepts `--output DIR`, `--rounds N`,
`--seed N`, and `--check`. Seed `0` is the original deterministic product
schedule; nonzero seeds preserve the same worker and DDL lifecycle shape while
varying row/default values and DML increments. It writes:
...
- The smoke test verifies representative DDL, DML, reader, and final oracle
  fields are emitted.
- Seed `0` preserves the original oracle shape, while nonzero seeds can be
  generated by `tools/ownerless-ddl-stress-seed-suite`.
