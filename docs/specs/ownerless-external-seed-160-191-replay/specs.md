# Ownerless External Seed 160-191 Replay
...
`31`, Docker-backed combined seed-sweep replay through seed `159`, and focused
...
This slice records Docker-backed MariaDB replay for seeds `160` through `191`
...
- Run one dependency-free command-plan check for seeds `160` through `191` at
  two rounds.
- Run one Docker-backed MariaDB 11.8 seed sweep for seeds `160` through `191`
  at two rounds.
...
tools/ownerless-external-mariadb-seed-sweep \
  --output /tmp/mylite-ownerless-external-seed-sweep-160-191-r2-replay \
  --random-tx-rounds 2 \
  --ddl-rounds 2 \
  --fk-graph-rounds 2 \
  --seed-range 160:191
...
- Run the direct `160:191` check-mode seed sweep at two rounds.
- Run the Docker-backed `160:191` replay at two rounds.
...
```text
tools/ownerless-external-mariadb-seed-sweep --output build/manual-external-seed-sweep-160-191-r2-check --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 160:191 --check
seed=160
...
seed=191
check=ok
...
```text
tools/ownerless-external-mariadb-seed-sweep --output /tmp/mylite-ownerless-external-seed-sweep-160-191-r2-replay --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 160:191
...
`fk_graph_rounds=2`, `seed_count=32`, and seeds `160` through `191`. All
...
seeds `160`, `162`, `164`, `168`, `170`, and `176` recovered on attempt `2`
after raw external MariaDB `1213` deadlock exits, matching the suite's bounded
whole-seed retry model.

## Acceptance Criteria

- The direct check-mode command validates seeds `160` through `191` at two
  rounds for all three suites.
- Docker-backed MariaDB 11.8 replay passes random transaction, DDL stress, and
  FK graph seeded suites for seeds `160` through `191` at two rounds.
