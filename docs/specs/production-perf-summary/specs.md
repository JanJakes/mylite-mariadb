...
The start-transaction fast path adds an embedded helper for exact non-ownerless
`START TRANSACTION` and exposes
`libmylite_exec_result_native_control_start_transaction_calls` in the PHP mysqli
profile and WordPress timing summary. Option-bearing starts such as
`START TRANSACTION READ ONLY` and `START TRANSACTION WITH CONSISTENT SNAPSHOT`
remain on the MariaDB SQL parser path because they require
`lex->start_transaction_opt`. A focused production WordPress `^Tests_DB` sample
after the reduced helper passed 651 tests with 3 skips and reported
...
Later ownerless slices continued to move correctness evidence forward without
changing the production WordPress timing contract. The 2026-06-19 production
CI run for `a751fdcf` passed after the external MariaDB replay evidence
reached seeds `608` through `639`. Its split WordPress timing artifact reported
the build separately from PHPUnit: `mylite_mariadb_embedded_seconds=406`,
`mylite_php_build_seconds=14`, and `build-php` total `422s`. The primary
PHPUnit test-only shards totaled about `646.5s` shell real time excluding the
diagnostic database profile step; the diagnostic profile step added about
`7.4s`. The non-isolated shards remained the dominant test-suite cost
(`168.469s`, `156.103s`, and `257.967s` shell real in the three shards), while
the database profile attributed only `2854.901 ms` to mysqli query execution
across `4010` calls. The perf probe reported stock PHP process startup
`19.739 ms`, MyLite-extension startup `26.912 ms`, process plus connect/close
`131.138 ms`, in-process connect/close `99.742 ms`, active-runtime reconnect
`2.034 ms`, `SELECT 1` `1545.96 ops/s`, transactional inserts
`1473.28 ops/s`, point selects `1394.40 ops/s`, prepared autocommit inserts
`1182.64 ops/s`, and direct autocommit inserts `1162.55 ops/s`. This confirms
that current CI needs continued ownerless write-path work, but it does not
show a WordPress PHPUnit suite regression versus the preceding split CI runs.

To make that evidence visible without manual summing, CI now appends
`timing-rollup` rows to the same WordPress timing table before publishing the
artifact. The rollup is generated by `tools/wordpress-phpunit-timing-rollup`
and covered by `tools.wordpress-phpunit-timing-rollup`; it separates primary
PHPUnit test shards from the diagnostic `phpunit-db-profile` probe, splits
database, process-isolated, and non-isolated PHPUnit classes, preserves the
production build totals, carries through the process and engine perf-probe
metrics, and now rolls up process-isolated child counts, child runtime,
child-script runtime, outer-minus-script runtime, parent lock-release,
reconnect, and baseline-restore timing. The child averages are recomputed from
summed seconds and summed child counts rather than by summing per-shard
averages. The workflow still runs the WordPress harness with production build
...
slice.
That top-tail fanout passed in CI run `27913693745`. It validated the new
REST, block, media, comment, and XML-RPC shards, but setup drifted from `67s`
to `73s`, and the estimated WordPress workflow critical path moved from `144s`
...
- CI WordPress timing rollup rows aggregate process-isolated child counts,
