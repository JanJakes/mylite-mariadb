# Ownerless External Seed 704-767 Check
...
command-plan gate now covers seeds `640` through `703`; the final
already-replayed contiguous window, seeds `704` through `767`, is not yet the
persistent default gate.

The next bounded slice is to advance that always-available CTest gate to seeds
`704` through `767`, catching the default command-plan coverage up to the
current replay-backed external seed frontier without adding Docker to default
CI or claiming that long-running randomized RQG is complete.
...
- `docs/specs/ownerless-external-seed-704-735-replay/specs.md` records
  Docker-backed MariaDB 11.8 replay evidence for seeds `704` through `735`.
- `docs/specs/ownerless-external-seed-736-767-replay/specs.md` records
  Docker-backed MariaDB 11.8 replay evidence for seeds `736` through `767`.
- `docs/specs/ownerless-external-seed-640-703-check/specs.md` advanced the
  persistent dependency-free next-check gate through seeds `640` through `703`,
  leaving `704` through `767` as the next replay-backed command-plan window.
...
- Change `tools.ownerless-external-mariadb-seed-sweep-next-check` to validate
  seeds `704` through `767` at two rounds across random transaction, DDL
  stress, and FK graph generated suites.
- Run the direct dependency-free seed-sweep check for seeds `704` through
  `767`.
...
- Running new Docker-backed MariaDB replay in this slice; the `704` through
  `767` replay artifacts are already recorded by the prior replay specs.
...
This keeps default CI independent of Docker while catching the persistent check
up to the last currently recorded Docker-backed contiguous replay window. The
check validates tool argument expansion, per-suite command planning, seed
enumeration, and manifest metadata for all three generated suites.
...
- Run the direct `704:767` check-mode seed sweep at two rounds.
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-704-767-r2-check --random-tx-rounds 2
  --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 704:767 --check` passed and
...
- `ctest --preset prod -R
  ...
  --output-on-failure` passed 6/6 tests in 4.51 seconds.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `704` through `767` at two rounds.
- The direct check-mode command validates seeds `704` through `767` at two
...
- With the persistent gate caught up to the current replay frontier, the next
  external-stress work should either add new replay windows beyond `767` or
  introduce a broader randomized MariaDB/RQG runner.
