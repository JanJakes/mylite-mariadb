# Ownerless External Seed 64-95 Replay
...
through seed `31`, Docker-backed replay through seed `63`, and focused
...
This slice records Docker-backed MariaDB replay for seeds `64` through `95` at
...
- Run one dependency-free command-plan check for seeds `64` through `95` at two
  rounds.
- Run one Docker-backed MariaDB 11.8 seed sweep for seeds `64` through `95` at
  two rounds.
...
tools/ownerless-external-mariadb-seed-sweep \
  --output /tmp/mylite-ownerless-external-seed-sweep-64-95-r2-replay \
  --random-tx-rounds 2 \
  --ddl-rounds 2 \
  --fk-graph-rounds 2 \
  --seed-range 64:95
...
- Run the direct `64:95` check-mode seed sweep at two rounds.
- Run the Docker-backed `64:95` replay at two rounds.
...
```text
tools/ownerless-external-mariadb-seed-sweep --output build/manual-external-seed-sweep-64-95-r2-check --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 64:95 --check
seed=64
...
seed=95
check=ok
...
```text
tools/ownerless-external-mariadb-seed-sweep --output /tmp/mylite-ownerless-external-seed-sweep-64-95-r2-replay --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 64:95
...
`fk_graph_rounds=2`, `seed_count=32`, and seeds `64` through `95`. All
random transaction, DDL stress, and FK graph final `expected.out` files ended
in `ok`, and all final `expected.err` files were empty. FK graph seeds `64`,
`81`, and `85` recovered on attempt `2` after raw external MariaDB `1213`
deadlock exits, matching the suite's bounded whole-seed retry model.

## Acceptance Criteria

- The direct check-mode command validates seeds `64` through `95` at two rounds
  for all three suites.
- Docker-backed MariaDB 11.8 replay passes random transaction, DDL stress, and
  FK graph seeded suites for seeds `64` through `95` at two rounds.
