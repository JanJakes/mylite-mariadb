# Ownerless Sixty-Four-Row Visible-Fast Batch

## Problem Statement

The bounded eight-row, sixteen-row, and thirty-two-row visible-fast append-
batch slices showed that parser-proven `INSERT ... VALUES` row lists can keep
one page-log append session for the full SQL statement while preserving page-
version publication, page-visible LSN ordering, and native history WAL proof.
A fresh 64-row production probe at current head showed the same fixed-work gap
at the next bounded row-list shape: visible-fast commit publication already
succeeds, but the thirty-two-row append-batch cap falls back to per-mini-
transaction append sessions and disables deferred latest-checkpoint coalescing.

The pre-slice 64-row probe on 2026-06-18 used `640` rows, `10` SQL statements,
and ownerless page-publish stats. It reported `669` page-log appends,
`651` append-session begin/end calls, visible-fast commit at `1.000` per
...
- `packages/libmylite/src/database.cc`
  ...
  eligibility from append-batch eligibility. The 64-row baseline proves
...
thirty-two to sixty-four. Keep every existing eligibility guard:
...
- direct and prepared ownerless `INSERT ... VALUES` statements with one through
  sixty-four parser-proven row constructors;
- page-log append-session lifetime and latest-only checkpoint coalescing for
  that bounded shape;
- focused SQL coverage for a 64-row visible-fast statement.

Out of scope:

- row lists above sixty-four;
...
- deferred latest-checkpoint coalescing becomes active for the 64-row shape.

## Test And Verification Plan

- Extend `single-owner-multi-row-insert-visible-fast-path` to assert a 64-row
...
- Run a 64-row production performance probe before and after the cap change.
- Run hook/stress coverage for the focused selector, production-build guards,
  format checks, and `git diff --check`.

## Acceptance Criteria

- Sixty-four-row pure ownerless `INSERT ... VALUES` statements use one page-log
...
- Production probe evidence shows reduced 64-row append-session churn or this
  slice is not kept.

## Implementation Evidence

The implementation raises
`k_ownerless_append_batch_fast_path_max_insert_values_rows` from `32` to `64`.
...
64-row `INSERT ... VALUES` case using the same `(id, value)` row-list shape as
...
- Pre-slice 64-row probe:
  `MYLITE_PERF_INSERT_ITERATIONS=640`,
  `MYLITE_PERF_BULK_INSERT_ROWS_PER_STATEMENT=64`, and ownerless page-publish
  stats enabled reported `669` page-log appends, `651` append-session begin
  ...
  at `0.000`, and ownerless 64-row bulk throughput at `9856.67` rows/s.
- Post-slice 64-row probe with the same shape kept `669` page-log appends,
  reduced append-session begin/end calls to `10`, enabled `1280` deferred
  latest-checkpoint coalesces (`128.000` per statement), kept visible-fast
  commit at `1.000` per statement and conservative flush at `0.000`, reduced
  page-write publish total from `17.148 ms` to `14.809 ms`, reduced page-write
  commit-log total from `35.506 ms` to `24.965 ms`, and reported ownerless
  64-row bulk throughput at `11755.40` rows/s. The ordinary comparison moved
  from `74812.47` to `89645.55` rows/s, so the ownerless ratio stayed roughly
  flat at `0.1318` versus `0.1311` despite higher ownerless throughput.

## Risks And Follow-Up

The cap is still a lock-hold-time tradeoff. A previous unbounded row-list
attempt regressed bulk timing, so this slice deliberately stops at sixty-four
