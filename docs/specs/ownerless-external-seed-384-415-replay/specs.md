# Ownerless External Seed 384-415 Replay
...
`63`, and Docker-backed combined seed-sweep replay through seed `383` for the
...
This slice records Docker-backed MariaDB replay for seeds `384` through `415`
...
- The previous replay windows from `224` through `383` used
...
- Run one dependency-free command-plan check for seeds `384` through `415` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Run one Docker-backed MariaDB 11.8 seed sweep for seeds `384` through `415`
...
- Run the direct `384:415` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Run the Docker-backed `384:415` replay at two rounds with
  `--fk-graph-replay-attempts 10`.
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-384-415-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 384:415 --check`
  passed and ended with `ownerless_external_mariadb_seed_sweep_check=ok`.
- `tools/ownerless-external-mariadb-seed-sweep --output
  /tmp/mylite-ownerless-external-seed-sweep-384-415-r2-attempt10-replay
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 384:415` passed through a
...
- FK graph replay recovered ordinary raw MariaDB deadlock exits under the
  whole-seed retry budget: seed `403` failed attempts `1` and `2` with
  `ERROR 1213 (40001)` and passed attempt `3`; seed `405` failed attempt `1`
  with `ERROR 1213 (40001)` and passed attempt `2`; seed `406` failed attempts
  `1` through `4` with `ERROR 1213 (40001)` and passed attempt `5`; seed `408`
  failed attempts `1` and `2` with `ERROR 1213 (40001)` and passed attempt
  `3`; seed `413` failed attempt `1` with `ERROR 1213 (40001)` and passed
  attempt `2`.
- `ctest --preset prod -R
...
- The direct check-mode command validates seeds `384` through `415` at two
...
- Docker-backed MariaDB 11.8 replay passes random transaction, DDL stress, and
  FK graph seeded suites for seeds `384` through `415` at two rounds.
