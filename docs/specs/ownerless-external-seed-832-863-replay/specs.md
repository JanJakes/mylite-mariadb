# Ownerless External Seed 832-863 Replay
...
`831`, and Docker-backed combined seed-sweep replay through seed `831` for the
...
`832` through `863`, and records Docker-backed MariaDB replay evidence for the
...
- The replay windows from `224` through `831` used
...
- Add a production CTest command-plan check for seeds `832` through `863` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `832` through `863` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `832`
  through `863` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `832:863` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `832:863` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-832-863-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 832:863 --check` passed with
  `ownerless_external_mariadb_seed_sweep_check=ok`.
- Docker-backed replay through `/tmp/mylite-ownerless-external-seed-sweep-832-863-r2-attempt10-replay`
...
- FK graph whole-seed retries recovered transient raw MariaDB `1213` exits for
  seed `849` on attempt `4`, seeds `834` and `852` on attempt `3`, and seeds
...
- `ctest --preset prod -R
  ...
  --output-on-failure` passed 6/6 in 4.64 seconds.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `832` through `863` at two rounds.
- The direct check-mode command validates seeds `832` through `863` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `832` through `863` at two
  rounds.
