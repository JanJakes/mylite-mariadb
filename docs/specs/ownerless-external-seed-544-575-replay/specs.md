# Ownerless External Seed 544-575 Replay
...
`191`, and Docker-backed combined seed-sweep replay through seed `543` for the
...
`192` through `255`, and records Docker-backed MariaDB replay evidence for seeds
`544` through `575` at two rounds across the random transaction, DDL stress, and
...
- The replay windows from `224` through `543` used
...
- Add a production CTest command-plan check for seeds `192` through `255` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `544` through `575` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `544`
  through `575` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `192:255` check-mode seed sweep at two rounds.
- Run the direct `544:575` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `544:575` replay artifacts for manifest, final
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-192-255-r2-check --random-tx-rounds 2
  --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 192:255 --check` passed and
  ended with `ownerless_external_mariadb_seed_sweep_check=ok`.
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-544-575-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 544:575 --check` passed and
  wrote a check-mode manifest for seeds `544` through `575`.
- Docker-backed replay artifacts under
  `/tmp/mylite-ownerless-external-seed-sweep-544-575-r2-attempt10-replay`
  ...
  `seed_count=32`, and seeds `544` through `575`.
- Replay artifact inspection found `96` final `expected.out` files and `96`
  final `expected.err` files, split evenly across the three suites. Every final
  `expected.out` contained an `ok` marker, and every final `expected.err` was
  empty.
- FK graph replay recovered ordinary raw MariaDB deadlock exits under the
  whole-seed retry budget: seed `545` passed attempt `5`; seed `546` passed
  attempt `2`; seed `548` passed attempt `4`; seed `549` passed attempt `3`;
  seed `557` passed attempt `3`; seeds `560`, `562`, `564`, and `575` passed
  attempt `2`. Failed attempts showed `ERROR 1213 (40001)` from raw MariaDB
  clients.
- `ctest --preset prod -R
  ...
  --output-on-failure` passed 6/6 tests in 3.88 seconds.
...
- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `192` through `255` at two rounds.
- The direct check-mode command validates seeds `544` through `575` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `544` through `575` at two
  rounds.
