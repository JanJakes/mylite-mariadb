# Ownerless External Seed 640-671 Replay
...
`383`, and Docker-backed combined seed-sweep replay through seed `639` for the
...
`384` through `447`, and records Docker-backed MariaDB replay evidence for seeds
`640` through `671` at two rounds across the random transaction, DDL stress,
...
- The replay windows from `224` through `639` used
...
- Add a production CTest command-plan check for seeds `384` through `447` at
  two rounds across all three seeded suites.
- Run one dependency-free command-plan check for seeds `640` through `671` at
  two rounds with `--fk-graph-replay-attempts 10`.
- Record one Docker-backed MariaDB 11.8 seed-sweep replay for seeds `640`
  through `671` at two rounds with `--fk-graph-replay-attempts 10`.
...
- Run the direct `384:447` check-mode seed sweep at two rounds.
- Run the direct `640:671` check-mode seed sweep at two rounds with
  `--fk-graph-replay-attempts 10`.
- Inspect Docker-backed `640:671` replay artifacts for manifest,
...
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-384-447-r2-check --random-tx-rounds 2
  --ddl-rounds 2 --fk-graph-rounds 2 --seed-range 384:447 --check` passed and
  ended with `ownerless_external_mariadb_seed_sweep_check=ok`.
- `tools/ownerless-external-mariadb-seed-sweep --output
  build/manual-external-seed-sweep-640-671-r2-attempt10-check
  --random-tx-rounds 2 --ddl-rounds 2 --fk-graph-rounds 2
  --fk-graph-replay-attempts 10 --seed-range 640:671 --check` passed and
  wrote a check-mode manifest for seeds `640` through `671`.
- Docker-backed replay artifacts under
  `/tmp/mylite-ownerless-external-seed-sweep-640-671-r2-attempt10-replay`
  ...
  `seed_count=32`, and seeds `640` through `671`.
...
- FK graph replay recovered ordinary raw MariaDB deadlock exits under the
  whole-seed retry budget: seed `657` passed attempt `4`, seed `654` passed
  attempt `3`, and seeds `641`, `644`, `647`, `649`, `652`, `655`, `667`,
  `669`, and `670` passed attempt `2`. Failed attempts showed
...
- `ctest --preset prod -R
  ...
  --output-on-failure` passed 6/6 tests in 4.08 seconds.
- `tools/check-ci-production-builds` passed.
- `cmake --build --preset format-check-prod` passed.
- `git diff --check` passed before staging, and `git diff --cached --check`
  passed after staging the new spec.

## Acceptance Criteria

- Production CTest includes a dependency-free combined seed-sweep check for
  seeds `384` through `447` at two rounds.
- The direct check-mode command validates seeds `640` through `671` at two
...
- Docker-backed MariaDB 11.8 replay artifacts prove random transaction, DDL
  stress, and FK graph seeded suites for seeds `640` through `671` at two
  rounds.
