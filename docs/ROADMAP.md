...
| 19 | Ownerless cross-process concurrency | ✅&nbsp;Done | The release-qualified admitted surface is 64-bit embedded Linux on validated local ext4, XFS, tmpfs, or overlay, macOS on local APFS, and Windows on local NTFS; persistent InnoDB application tables; and the explicitly classified DML and DDL shapes. Unsupported platforms and filesystems return stable explicit errors before ownerless startup. Native platform/filesystem qualification and the complete Linux SQL, hook, workload, randomized, and pressure release gates cover the claim. Non-InnoDB durable tables, unclassified DDL, special indexes, unlisted or remote filesystems, and documented server/global surfaces continue to fail closed; broader engine/filesystem support is future expansion rather than unfinished work in this slice. |
