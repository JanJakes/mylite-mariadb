...
| 11 | Row and index storage | 🟡&nbsp;In&nbsp;progress | Table insert, CTAS row population, full-scan, update/delete, truncate, copy rebuild, NULL values, BLOB/TEXT overflow payloads, autoincrement state, supported primary/unique/secondary index entries, bounded BLOB/TEXT prefix indexes, standalone supported index DDL, basic CHECK enforcement, basic unindexed generated columns, and explicit foreign-key DDL rejection are implemented; foreign-key enforcement, broader constraint coverage, generated-column indexes, oversized/full BLOB/TEXT indexes, compaction, and transactional index maintenance remain. |
