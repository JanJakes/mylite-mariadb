...
| 19 | Ownerless cross-process concurrency | ✅&nbsp;Done | The completed admitted surface is embedded Linux on validated local ext4, XFS, tmpfs, or overlay filesystems, persistent InnoDB application tables, and the explicitly classified DML and DDL shapes. The production SQL, hook, workload, randomized, pressure, filesystem, WordPress, PHP, and external MariaDB release gates pass. Non-InnoDB durable tables, unclassified DDL, special indexes, unsupported platforms/filesystems, and documented server/global surfaces continue to fail closed; broader engine/platform support is future work rather than part of this slice. |
