...
| 5 | SQL execution API | 🟡&nbsp;In&nbsp;progress | Add direct execution, prepared statements, bindings, columns, warnings, affected rows, and insert ids. Direct execution, first prepared-statement typed value access, and warning enumeration are implemented; rich metadata, streaming values, and broader comparison coverage remain. |
