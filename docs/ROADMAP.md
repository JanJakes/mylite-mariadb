...
| 14 | Transactions and recovery | 🟡&nbsp;In&nbsp;progress | Rollback-journal atomic publication and recovery are implemented for current append-only storage mutations, covered failed file-backed statements restore a statement-start header/catalog checkpoint, and explicit SQL transaction-control statements are rejected until the handler is transaction-aware; multi-statement rollback, savepoints, WAL/checkpoints, and transaction-aware engine hooks remain. |
