...
| 8 | MyLite metadata DDL and discovery | 🟡&nbsp;In&nbsp;progress | Store routed table definitions in the catalog and discover them without durable `.frm` sidecars. Create/discovery, `DROP TABLE`, simple `RENAME TABLE`, copy `ALTER` rebuilds, supported keyed rebuilds, and standalone supported index DDL are implemented; broader schema objects remain. |
