...
| 19 | Ownerless cross-process concurrency | 🟡&nbsp;In&nbsp;progress | The implemented admitted surface is 64-bit embedded Linux on validated local ext4, XFS, tmpfs, or overlay, macOS on local APFS, and Windows on local NTFS; persistent InnoDB application tables; and the explicitly classified DML and DDL shapes. Unsupported platforms and filesystems return stable explicit errors before ownerless startup. Native platform/filesystem qualification is covered, but the full Linux SQL and hook release matrix still needs reproducible stabilization before this project is complete. Non-InnoDB durable tables, unclassified DDL, special indexes, unlisted or remote filesystems, and documented server/global surfaces continue to fail closed; broader engine/filesystem support is future work rather than part of the current claim. |
...
The mmap-backed `tc.log` transaction coordinator is omitted after policy
