...
| 18 | Server-surface policy | 🟡&nbsp;In&nbsp;progress | Runtime defaults disable networking, grants, binlog, events, and host plugin discovery; representative server SQL plus SQL host-file import/export and server utility function rejection smoke is implemented; the default profile compiles out dynamic plugins, Performance Schema, socket auth, feedback, thread-pool info, host-file SQL I/O, and unsupported server utility functions. |
| 19 | Size profile hardening | 🟡&nbsp;In&nbsp;progress | Archive and linked-runtime size reporting is implemented, server-surface trims are measured, and the embedded profile now omits unsupported `LOAD DATA` / `LOAD XML` execution, SQL host-file I/O bodies, and low-value server utility SQL functions; deeper daemon-only and low-value optional component trims remain. |
