...
deprecated rename-database diagnostics while capping current accepted upstream
MTR coverage at 200 of 5,901 imported
...
`(key, row_id)` fences for exact-key child selection; branch-assisted prefix
lookup, split/merge, and multi-page branch maintenance remain pending. SQL copy-rebuild DDL now
