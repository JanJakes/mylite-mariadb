...
| 8 | MyLite metadata DDL and discovery | 🟡&nbsp;In&nbsp;progress | Store routed metadata in the catalog and discover it without durable MariaDB sidecars. Direct and prepared schema namespaces, create/discovery, `CREATE TABLE ... LIKE`, successful supported `CREATE TABLE ... SELECT`, `DROP TABLE`, simple `RENAME TABLE`, copy `ALTER` rebuilds, supported keyed rebuilds, and standalone supported index DDL are implemented; views, triggers, routines, schema options, and final SQL-layer database hooks remain. |
