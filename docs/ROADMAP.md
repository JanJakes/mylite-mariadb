...
| 19 | Ownerless cross-process concurrency | ✅&nbsp;Done | Replace exclusive cross-process directory ownership with directory-backed mmap coordination, durable recovery logs, shared metadata/transaction/lock state, and tested multi-process InnoDB application-table read/write semantics without an owner process. Non-InnoDB durable application tables are an explicit unsupported ownerless surface and remain available only in exclusive mode until a per-engine design exists. |
