...
| 10 | Engine routing policy | ✅&nbsp;Done | Record requested engine vs. effective MyLite engine and route omitted/default, `InnoDB`, `MyISAM`, and `Aria` metadata where safe; `BLACKHOLE` routes to MyLite metadata with row-discard behavior, while `MEMORY` / `HEAP` volatile row semantics remain planned. |
...
| 18 | Server-surface policy | 🟡&nbsp;In&nbsp;progress | Runtime defaults disable networking, grants, binlog, events, and host plugin discovery; representative server SQL plus SQL host-file import/export rejection smoke is implemented; the default profile compiles out dynamic plugins, Performance Schema, socket auth, feedback, and thread-pool info. |
| 19 | Size profile hardening | 🟡&nbsp;In&nbsp;progress | Archive and linked-runtime size reporting is implemented, the first server-surface trim is measured, and the embedded profile now omits the unsupported `LOAD DATA` / `LOAD XML` execution object; deeper daemon-only and low-value optional component trims remain. |
