...
| 10 | Engine routing policy | ✅&nbsp;Done | Record requested engine vs. effective MyLite engine and route omitted/default metadata where safe; active file-backed sessions resolve `InnoDB`, `MyISAM`, `Aria`, `BLACKHOLE`, `MEMORY`, and `HEAP` directly to MyLite, including under `NO_ENGINE_SUBSTITUTION`; `BLACKHOLE` routes to MyLite metadata with row-discard behavior, `MEMORY` / `HEAP` route to durable metadata with runtime-volatile row semantics, and unsupported explicit `ENGINE` table options reject before catalog publication. |
