...
storage-smoke profile reports `3,681` full-page calls, `243,497` zero-tail
calls, and a `76.625 us/op` prepared insert step; all `2,129` `index-root`
...
Maintained-root decode call-site counters now split those `2,129` root decodes
...
packed-admission validation helper has been removed; maintained-root planning
remains the pre-write validation gate.
