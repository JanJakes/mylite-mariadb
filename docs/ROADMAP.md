...
| 19 | Ownerless cross-process concurrency | ✅&nbsp;Done | The completed admitted surface is 64-bit embedded Linux on validated local ext4, XFS, tmpfs, or overlay, macOS on local APFS, and Windows on local NTFS; persistent InnoDB application tables; and the explicitly classified DML and DDL shapes. Unsupported platforms and filesystems now return stable explicit errors before ownerless startup. The production SQL, hook, workload, randomized, pressure, filesystem, native-platform, WordPress, PHP, and external MariaDB release gates pass. Non-InnoDB durable tables, unclassified DDL, special indexes, unlisted or remote filesystems, and documented server/global surfaces continue to fail closed; broader engine/filesystem support is future work rather than part of this slice. |
