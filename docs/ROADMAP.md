...
| 8 | MyLite metadata DDL and discovery | 🟡&nbsp;In&nbsp;progress | Store routed metadata in the catalog and discover it without durable MariaDB sidecars. Direct and prepared schema namespaces with basic schema options, create/discovery, `CREATE TABLE ... LIKE`, successful supported `CREATE TABLE ... SELECT`, `DROP TABLE`, simple `RENAME TABLE`, copy `ALTER` rebuilds, supported keyed rebuilds, and standalone supported index DDL are implemented; views, triggers, routines, and final SQL-layer database hooks remain. |
...
| 16 | Compatibility harness | 🟡&nbsp;In&nbsp;progress | Group existing public API, storage, recovery, locking, embedded lifecycle, SQL API comparison, sidecar, and routed SQL tests by compatibility surface; MTR-scale comparison and broader application suites remain. |
