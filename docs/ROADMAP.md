...
| 19 | Size profile hardening | 🟡&nbsp;In&nbsp;progress | Archive and linked-runtime size reporting is implemented, the first server-surface trim is measured, and the embedded profile now omits unsupported `LOAD DATA` / `LOAD XML` execution plus SQL host-file I/O bodies; deeper daemon-only and low-value optional component trims remain. |
