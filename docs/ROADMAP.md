...
| 11 | Row and index storage | 🟡&nbsp;In&nbsp;progress | Keyless table insert and full-scan storage is implemented for non-BLOB rows, including NULL values; the narrow single-column autoincrement key path is implemented; primary/secondary indexes, general uniqueness, BLOB/TEXT overflow, update/delete, and copy `ALTER` rebuilds remain. |
