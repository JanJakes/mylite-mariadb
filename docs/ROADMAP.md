...
| 5 | SQL execution API | 🟡&nbsp;In&nbsp;progress | Add direct execution, prepared statements, bindings, columns, warnings, affected rows, and insert ids. Direct execution including statement-effect coverage, prepared-statement typed values, column metadata, warning enumeration including selected failed paths, current-row large-value reads, and representative MariaDB baseline comparison are implemented; an initial opt-in MTR smoke runner exists under the compatibility-harness work, while MTR-scale comparison remains. |
...
| 8 | MyLite metadata DDL and discovery | 🟡&nbsp;In&nbsp;progress | Store routed metadata in the catalog and discover it without durable MariaDB sidecars. Direct and prepared schema namespaces with basic schema options, SQL-layer catalog discovery after reopen, create/discovery, `CREATE TABLE ... LIKE`, successful supported `CREATE TABLE ... SELECT` including generated-source projections, `DROP TABLE`, simple and indexed `RENAME TABLE`, copy `ALTER` rebuilds including catalog-only reopened CHECK drops, generated-column add/modify/drop, and representative default-algorithm reopened column/index/autoincrement ALTER, supported keyed rebuilds, standalone supported index DDL, SQL-level supported index rename, basic CHECK and generated-column metadata persistence, named table-level CHECK add/drop ALTER, explicit non-table object rejection, explicit partition DDL rejection, and explicit foreign-key DDL rejection are implemented; catalog-backed views, triggers, routines, remaining filesystem-free schema DDL, partition metadata/routing, foreign-key metadata/enforcement, broader constraint DDL, and SQL rollback remain. |
...
| 11 | Row and index storage | 🟡&nbsp;In&nbsp;progress | Table insert, CTAS row population including generated-source projections, full-scan, update/delete, truncate, copy rebuild, NULL values, BLOB/TEXT overflow payloads, generated BLOB/TEXT values, autoincrement state including `ALTER TABLE ... AUTO_INCREMENT`, supported primary/unique/secondary index entries, index preservation across table rename, bounded BLOB/TEXT prefix indexes, bounded generated BLOB/TEXT prefix indexes, standalone supported index DDL, SQL-level supported index rename, basic CHECK enforcement, basic generated columns with copy ALTER add/modify/drop, ordinary generated-column secondary/unique index and generated-index DDL coverage, explicit unsupported FULLTEXT/SPATIAL index rejection, explicit partition DDL rejection, and explicit foreign-key DDL rejection are implemented; partition-aware row/index maintenance, foreign-key enforcement, broader constraint coverage, generated primary keys, expression/hidden generated indexes, generated target CTAS definitions, oversized/full BLOB/TEXT indexes, FULLTEXT/SPATIAL access paths, compaction, and transactional index maintenance remain. |
