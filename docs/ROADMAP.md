...
covers selected engine-alias DDL/DML routing, requested-engine
`SHOW CREATE TABLE` metadata, sidecar absence, and routed `InnoDB` rollback,
commit, savepoint, and representative foreign-key behavior against a static
MyLite storage-engine build with a primary `.mylite` file. It
