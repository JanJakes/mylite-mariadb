/*
  MyLite embedded profile stub for MariaDB query cache runtime.
...
extern "C" const uchar *query_cache_query_get_key(const void *, size_t *length,
                                                  my_bool)
{
  if (length)
    *length= 0;
...
extern "C" const uchar *query_cache_table_get_key(const void *, size_t *length,
                                                  my_bool)
{
  if (length)
    *length= 0;
...
extern "C" void mysql_query_cache_invalidate4(THD *, const char *, unsigned,
                                              int)
{
}

Query_cache::Query_cache(size_t, size_t min_allocation_unit_arg,
                         size_t min_result_data_size_arg,
                         uint def_query_hash_size_arg,
                         uint def_table_hash_size_arg)
  :query_cache_size(0), query_cache_limit(0), free_memory(0),
   queries_in_cache(0), hits(0), inserts(0), refused(0),
   free_memory_blocks(0), total_blocks(0), lowmem_prunes(0),
#ifndef DBUG_OFF
   m_cache_lock_thread_id(0),
#endif
   m_requests_in_progress(0), m_cache_lock_status(UNLOCKED),
   m_cache_status(DISABLED), additional_data_size(0), cache(NULL),
   first_block(NULL), queries_blocks(NULL), tables_blocks(NULL), bins(NULL),
   steps(NULL), min_allocation_unit(min_allocation_unit_arg),
   min_result_data_size(min_result_data_size_arg),
   def_query_hash_size(def_query_hash_size_arg),
   def_table_hash_size(def_table_hash_size_arg), mem_bin_num(0),
   mem_bin_steps(0), initialized(false)
{
}
...
size_t Query_cache::set_min_res_unit(size_t)
{
...
{
  if (thd)
    thd->query_cache_tls.first_query_block= NULL;
}
